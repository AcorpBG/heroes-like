...
- that same live harness now also proves the battle-side routed save-resume path by saving from `BattleShell.gd` into the selected manual slot, returning through `AppRouter.gd` to `MainMenu.gd`, loading that slot through the shipped save browser, and verifying the router lands back on `BattleShell.gd` with the same encounter, battle-context, round, distance, and roster signature before the run continues
...
- the same dormant harness now includes a `boot_to_campaign_full_arc` flow for authored campaigns, currently proving the full Reedfall arc from the shipped campaign browser through routed overworld, town, battle, outcome, save/resume, next-chapter, finale-complete, and completed-browser replay/default-action semantics without writing progression state directly
- validation hooks remain narrow read-or-trigger seams on shipped shells, and deterministic town-assault combat seeds now derive from scenario, launch mode, day, and town placement so repeatable live validation can replay routed town fights without tying combat outcomes to runtime session ids
