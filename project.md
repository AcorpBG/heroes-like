...
- Each faction has one gameplay town template and one construction tree. Retired town IDs are compatibility aliases, not alternate town definitions; existing placement IDs, source-map records and scenario targets retain their identity. Shared building lines use one active stage, with ancestry satisfying prerequisites without stacking obsolete effects. Tiers 1–5 choose one of two dwelling branches; all seven tiers have Veteran upgrades. Per-faction main/secondary resources, hero training claims, artifact shop stock and legacy town migration remain data-driven and save-persistent.
...
- Player/controller identity and team membership are independent of faction content. Repeated-faction players must not share mutable economy, ownership, commander instances or AI state. Native player/team assignments remain authoritative through package adoption and saves; legacy ownership interpretation needs explicit compatibility coverage.
- Autoloads are for cross-cutting services, not hiding gameplay rules.
- Audio manifests declare whether a music cue is a complete stereo composition or a synchronized layered arrangement. A full mix plays alone; legacy layers remain a compatibility path and must never be synthesized over a full mix. Audio variation and notification state are presentation-only and must not consume simulation RNG or enter saves. Production sources and generation/edit provenance remain separate from exported WAV/OGG assets.
