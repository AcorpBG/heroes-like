...
def prepare():
    ...
    refs = []
    for name, filename, rect, anchor in [('near_contact','input_blue.png',[0,0,887,887],[492,867]),('far_contact','middle_blue.png',[887,0,1774,887],[1287,867])]:
    ...
    graph = {
        ...
        '7':node('RandomNoise',noise_seed=2026092412),
        ...
        '14':node('SaveVideo',video=['13',0],filename_prefix='dustjack_move_h3_blue_v2/original',**{'format':'mp4','format.codec':'h264'}),
        '15':node('SaveImage',images=['11',0],filename_prefix='dustjack_move_h3_blue_v2/frames/decoded'),
...
def submit():
    ...
    for key,filename in [('4','input_blue.png'),('16','middle_blue.png')]:
...
def process():
    ...
    write(OUT/'matte.json',dict(frames=124,fps=24,rgba_sha256=hashes,background_frames=meta,recipe='Uniform deep-blue corner check; blue dominance alpha unmix and RGB despill; background distance floor12; alpha>=8 components retained when containing alpha>=128 pixels. No geometry editing or alignment.'))


def review():
    target=ROOT/'.artifacts/dustjack_move_h3/blue_v2';target.mkdir(parents=True,exist_ok=True)
    for part in range(2):
...
def build():
    ...
    target=ROOT/'.artifacts/dustjack_move_h3/blue_v2';target.mkdir(parents=True,exist_ok=True);write(target/'combined_handoff.json',dict(schema_version=1,units=[candidate]))
