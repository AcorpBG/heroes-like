"""Rebuild original Beacon Lectors H3 takes and pending candidates."""
import argparse
import hashlib
import json
import sys
...
def submit(out,c):
 ...
 write(out/'workflow_api.json',g);start=time.time();r=request(URL,'/prompt',dict(prompt=g,client_id='beacon_lectors_h3'))
...
def key(rgb):
 ...
 # This lector contains red, ivory, brown, steel and brass but no green
 ...
 # keeping pole tips, lantern edges, limbs and fallen equipment attached
...
def process(out,c):
 ...
 with av.open(str(out/'original_lossless.mkv')) as video:
  for i,frame in enumerate(video.decode(video=0)):
   ...
   if detail['mode']!='flat_green_chroma_unmix':raise ValueError('Backdrop changed to a color unsafe for red/ivory cloak, steel armor and brown/brass bell-staff and lantern; reject this interval')
   p=out/'matte'/f'rgba_{i:03}.png';im.save(p);hashes.append(sha(p));details.append(detail)
 assert len(hashes)==124
 write(out/'matte.json',dict(frames=124,fps=24,rgba_sha256=hashes,background_frames=details,recipe='Measured uniform green plate absent from Beacon Lector palette; corner spread<=10, chroma separation>=80; color-specific alpha unmix and edge-only despill; alpha>=8 regions retained when containing alpha>=128 pixels. Original geometry unchanged.'))

def review(out,c):
 target=ROOT/'.artifacts/beacon_lectors_h3'/out.name;target.mkdir(parents=True,exist_ok=True)
 for part in range(2):
