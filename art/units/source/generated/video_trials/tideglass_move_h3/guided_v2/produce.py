...
def prepare():
    ...
    graph = {
        ...
        '7':node('RandomNoise',noise_seed=2026092429),
        ...
        '14':node('SaveVideo',video=['13',0],filename_prefix='tideglass_move_h3/guided_v2/original',**{'format':'mp4','format.codec':'h264'}),
        '15':node('SaveImage',images=['11',0],filename_prefix='tideglass_move_h3/guided_v2/frames/decoded'),
    }
...
def review():
    target=ROOT/'.artifacts/tideglass_move_h3/guided_v2';target.mkdir(parents=True,exist_ok=True)
    for part in range(2):
...
def build():
    ...
    target=ROOT/'.artifacts/tideglass_move_h3/guided_v2';target.mkdir(parents=True,exist_ok=True);write(target/'combined_handoff.json',dict(schema_version=1,units=[candidate]))
