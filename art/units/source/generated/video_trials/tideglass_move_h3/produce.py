...
def prepare():
    ...
    recipe = json.loads((ROOT/'art/animation/source/fluid/unit_veilmourn_dreamwake_tideglass_oracles/reviewed_handoff.json').read_bytes())['units'][0]
    ...
    graph = {
        ...
        '7':node('RandomNoise',noise_seed=2026092428),
        ...
        '14':node('SaveVideo',video=['13',0],filename_prefix='tideglass_move_h3/original',**{'format':'mp4','format.codec':'h264'}),
        '15':node('SaveImage',images=['11',0],filename_prefix='tideglass_move_h3/frames/decoded'),
    }
...
def review():
    target=ROOT/'.artifacts/tideglass_move_h3';target.mkdir(parents=True,exist_ok=True)
    for part in range(2):
...
def build():
    ...
    entry=dict(unit_id=UNIT,reference_height=256,source_facing='right',frames=frames,alpha_noise_cutoff=0,clips={'move':dict(indices=list(range(len(frames))),frame_msec=selection['frame_msec'],loop=True,static_frame=0)},source_scale_reason='Original accepted ready painting uses .62 runtime scale; input.9 and extraction.62/.9 with fixed anchor480,475 preserve body and staff size. No per-frame size normalization.',provenance=provenance,visual_review=dict(status='pending',notes=selection['review_note']))
    ...
    target=ROOT/'.artifacts/tideglass_move_h3';target.mkdir(parents=True,exist_ok=True);write(target/'combined_handoff.json',dict(schema_version=1,units=[candidate]))
