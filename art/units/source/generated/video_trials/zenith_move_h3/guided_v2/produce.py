...
def prepare():
    ...
    contacts = [dict(name=name,clip='move',source='art/units/source/generated/video_trials/zenith_move_h3/key_guides/'+name+'.png',rects=[[0,0,1093,1438]],anchor=anchor,scale=.158,alpha_noise_cutoff=8) for name,anchor in [('near_contact_v1',[570,1404]),('far_contact_v1',[587,1414])]]
    refs = []
    for original,filename in zip(contacts,['input_green.png','far_green.png']):
        frame = original.copy()
        original_scale = frame['scale']
        assert original_scale==.158
    ...
    graph = {
        ...
        '16':node('LoadImage',image='zenith_far.png'),
        '17':node('MiniMaxH3AddGuide',positive=['5',0],latent=['5',1],frame_idx=62,vae=['3',0],image=['16',0]),
        '6':node('BasicGuider',model=['1',0],conditioning=['17',0]),
        '7':node('RandomNoise',noise_seed=2026092425),
        ...
        '14':node('SaveVideo',video=['13',0],filename_prefix='zenith_move_h3/guided_v2/original',**{'format':'mp4','format.codec':'h264'}),
        '15':node('SaveImage',images=['11',0],filename_prefix='zenith_move_h3/guided_v2/frames/decoded'),
    }
...
def verify():
    ...
    assert graph['17']['inputs']['frame_idx']==62 and graph['17']['inputs']['positive']==['5',0]
...
def review():
    target=ROOT/'.artifacts/zenith_move_h3/guided_v2';target.mkdir(parents=True,exist_ok=True)
    for part in range(2):
...
def build():
    ...
    target=ROOT/'.artifacts/zenith_move_h3/guided_v2';target.mkdir(parents=True,exist_ok=True);write(target/'combined_handoff.json',dict(schema_version=1,units=[candidate]))
