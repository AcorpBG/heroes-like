"""Rebuild original Citadel Pikeward H3 takes and pending animation candidates."""
import argparse
import hashlib
import json
import sys
...
def prepare(out,c):
 ...
 graph={
  ...
  '14':node('SaveVideo',video=['13',0],filename_prefix=f'citadel_pikeward_h3/{out.name}/original',**{'format':'mp4','format.codec':'h264'}),
  '15':node('SaveImage',images=['11',0],filename_prefix=f'citadel_pikeward_h3/{out.name}/frames/decoded')}
 for i in range(len(refs)):graph[str(30+i)]=node('LoadImage',image=f'citadel_pikeward_{out.name}_guide_{i}.png')
 previous='5'
...
def submit(out,c):
 ...
 for i in range(len(c['references'])):
  name=f'citadel_pikeward_{out.name}_guide_{i}.png';boundary='----CitadelPikewardH3'
 ...
 write(out/'workflow_api.json',g);start=time.time();r=request(URL,'/prompt',dict(prompt=g,client_id='citadel_pikeward_h3'))
...
def key(rgb):
 ...
 # This guard contains red, steel, brown, teal and ivory, with neither saturated
 # green nor magenta material. Measure the actual uniform source plate; never
...
def process(out,c):
 ...
 with av.open(str(out/'original_lossless.mkv')) as video:
  for i,frame in enumerate(video.decode(video=0)):
   im,detail=key(frame.to_image().convert('RGB'));p=out/'matte'/f'rgba_{i:03}.png';im.save(p);hashes.append(sha(p));details.append(detail)
...
def review(out,c):
 target=ROOT/'.artifacts/citadel_pikeward_h3'/out.name;target.mkdir(parents=True,exist_ok=True)
 for part in range(2):
