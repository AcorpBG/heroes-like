...
def prepare():
    ...
    graph = {
        ...
        '7':node('RandomNoise',noise_seed=2026092426),
        ...
        '14':node('SaveVideo',video=['13',0],filename_prefix='rimebell_move_h3/original',**{'format':'mp4','format.codec':'h264'}),
        '15':node('SaveImage',images=['11',0],filename_prefix='rimebell_move_h3/frames/decoded'),
    }
...
def review():
    target=ROOT/'.artifacts/rimebell_move_h3';target.mkdir(parents=True,exist_ok=True)
    for part in range(2):
        ...
        for j,i in enumerate(range(part*62,(part+1)*62)):
            ...
            im=Image.open(OUT/'matte'/f'rgba_{i:03}.png').crop((220,0,850,544));im.thumbnail((174,178),Image.Resampling.LANCZOS);sheet.paste(im,(x+(180-im.width)//2,y+20),im);draw.text((x+5,y+4),str(i),fill=(160,110,65))
...
def build():
    ...
    target=ROOT/'.artifacts/rimebell_move_h3';target.mkdir(parents=True,exist_ok=True);write(target/'combined_handoff.json',dict(schema_version=1,units=[candidate]))
