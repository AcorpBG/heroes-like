"""Original-guide H3 four-fin flight for Rimebell Skyrakers; publication is explicit."""
import argparse
import hashlib
import json
import sys
...
UNIT = 'unit_neutral_rimebell_skyrakers'
URL = 'http://127.0.0.1:8189'
ANCHOR = [480, 425]
INPUT_SCALE = .8
RUNTIME_SCALE = .5 / INPUT_SCALE
PROMPT = 'Locked orthographic three-quarter LEFT-facing fantasy strategy sprite camera. Animate this exact original Rimebell Skyraker flying IN PLACE through two complete natural wingbeat cycles, returning to the reference wing pose. Preserve its ice-white and cobalt-blue feather armor, swept narrow beaked crest, shadowed face, luminous blue chest and ONE long tapering ice-vapor tail trailing to IMAGE RIGHT. Exactly FOUR feather-fins: two large upper wings attached at shoulders and two smaller lower feather-fins attached along flanks, with exactly TWO separate forward curled rake talons beneath the chest. The upper pair lift with bent leading joints, spread feather tips, sweep down to push air, fold slightly during recovery and lift again; the near and far wings remain on their own sides and never merge or become extra wings. Lower pair counterbalance the upper wingbeats with smaller distinct delayed strokes, retaining clear layered feather edges. Both curled talons stay tucked under the chest, gently flex with the wing strokes, never becoming walking legs, hands or extra claws. Tail flows as a coherent tapered icy ribbon with modest delayed curl, staying attached to the same rear body; no new particles, snow, glowing floor or separate smoke clouds. Keep face, beak, crest and chest volume anatomically unchanged. Body core remains centered at original position and size, with only slight natural pitch; movement comes from articulated wings/talons/tail, not whole-sprite bobbing. Remain three-quarter LEFT-facing throughout, no turning to front, right or back, no travel, landing, attacks, camera pan, zoom or perspective changes. All four complete fins and feather tips, crest, talons and complete tail remain safely inside frame. Background is perfectly uniform PURE GREEN RGB(0,255,0), no ground, shadow, gradients, text, motion blur or scenery.'
...
def prepare():
    ...
    recipe = json.loads((ROOT/'art/animation/source/fluid/unit_neutral_rimebell_skyrakers/reviewed_handoff.json').read_bytes())['units'][0]
    ...
    graph = {
        ...
        '4':node('LoadImage',image='rimebell_flight_input.png'),
        ...
        '7':node('RandomNoise',noise_seed=2026092427),
        ...
        '14':node('SaveVideo',video=['13',0],filename_prefix='rimebell_move_h3/framed_v2/original',**{'format':'mp4','format.codec':'h264'}),
        '15':node('SaveImage',images=['11',0],filename_prefix='rimebell_move_h3/framed_v2/frames/decoded'),
    }
...
def submit():
    ...
    for key,filename in [('4','input_green.png')]:
        boundary='----RimebellFlightH3'
        data=(f'--{boundary}\r\nContent-Disposition: form-data; name="image"; filename="rimebell_flight_{filename}"\r\nContent-Type: image/png\r\n\r\n'.encode()+(OUT/filename).read_bytes()+f'\r\n--{boundary}--\r\n'.encode())
    ...
    write(OUT/'workflow_api.json',graph)
    started=time.time();r=request(URL,'/prompt',dict(prompt=graph,client_id='rimebell_flight_h3'))
...
def review():
    target=ROOT/'.artifacts/rimebell_move_h3/framed_v2';target.mkdir(parents=True,exist_ok=True)
    for part in range(2):
        ...
        for j,i in enumerate(range(part*62,(part+1)*62)):
            ...
            im=Image.open(OUT/'matte'/f'rgba_{i:03}.png').crop((110,0,850,544));im.thumbnail((174,178),Image.Resampling.LANCZOS);sheet.paste(im,(x+(180-im.width)//2,y+20),im);draw.text((x+5,y+4),str(i),fill=(160,110,65))
...
def build():
    ...
    entry=dict(unit_id=UNIT,reference_height=256,source_facing='left',frames=frames,alpha_noise_cutoff=0,clips={'move':dict(indices=list(range(len(frames))),frame_msec=selection['frame_msec'],loop=True,static_frame=0)},source_scale_reason='Original accepted attack-ready painting uses .5 runtime scale; input.8 and extraction.625 with fixed anchor480,425 preserve the body and original hover clearance. No per-pose size normalization.',provenance=provenance,visual_review=dict(status='pending',notes=selection['review_note']))
    ...
    target=ROOT/'.artifacts/rimebell_move_h3/framed_v2';target.mkdir(parents=True,exist_ok=True);write(target/'combined_handoff.json',dict(schema_version=1,units=[candidate]))
