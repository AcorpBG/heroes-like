...
def prepare():
    ...
    contacts = [next(f for f in originals if f['name']=='ranged_00'),next(f for f in originals if f['name']=='ranged_02')]
    refs = []
    ...
    graph = {
        ...
        '7':node('RandomNoise',noise_seed=2026092440),
        ...
        '14':node('SaveVideo',video=['13',0],filename_prefix='millstone_h3/ranged_v1/original',**{'format':'mp4','format.codec':'h264'}),
        '15':node('SaveImage',images=['11',0],filename_prefix='millstone_h3/ranged_v1/frames/decoded'),
    }
...
def review():
    target=ROOT/'.artifacts/millstone_h3/ranged_v1';target.mkdir(parents=True,exist_ok=True)
    for part in range(2):
...
def build():
    ...
    target=ROOT/'.artifacts/millstone_h3/ranged_v1';target.mkdir(parents=True,exist_ok=True);write(target/'combined_handoff.json',dict(schema_version=1,units=[candidate]))
