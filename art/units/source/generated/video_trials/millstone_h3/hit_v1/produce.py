...
INPUT_SCALE = .95
RUNTIME_SCALE = .6
PROMPT = 'Locked orthographic fantasy strategy sprite camera. Keep this exact adult female Millstone Slinger: brown hair in small bun, green cloth headband and neck scarf, off-white rolled-sleeve tunic, loose moss-green trousers, brown lace-up leather boots, brown forearm bracers and apron, belt pouches of gray stones, diagonal leather bandolier with gray stones. Face and body remain three-quarter RIGHT-facing. Exactly two arms, two hands and two legs. ONE plain brown leather sling with two flexible cords and one small leather cup belongs only to her image-left hand; no bow, staff, shield, sword or extra limbs. All straps and pouches remain attached. One brief non-graphic impact recoil and recovery, not an attack or death. Start ready, chest and shoulders jerk backward once, face grimaces, neck/head follow and both elbows react independently as in middle guide. Knees flex to absorb impact, both boots remain planted on same ground, pelvis stays registered. Sling stays coiled in its original image-left fist and never lengthens or flies away. Recover shoulders, neck, elbows and knees naturally to original ready stance and settle. No repeated hits, jumping, collapse, blood, magic or effects. Fixed body scale and ground reference, no camera motion, pan, zoom or perspective change. Entire head, both hands, all sling cords and both boots stay within frame. Perfectly flat PURE MAGENTA RGB(255,0,255) background, no floor, shadows, gradients, text, scenery, motion blur or translucent ghosting.'
...
def prepare():
    ...
    graph = {
        ...
        '17':node('MiniMaxH3AddGuide',positive=['5',0],latent=['5',1],frame_idx=24,vae=['3',0],image=['16',0]),
        '6':node('BasicGuider',model=['1',0],conditioning=['17',0]),
        '7':node('RandomNoise',noise_seed=2026092439),
        ...
        '14':node('SaveVideo',video=['13',0],filename_prefix='millstone_h3/hit_v1/original',**{'format':'mp4','format.codec':'h264'}),
        '15':node('SaveImage',images=['11',0],filename_prefix='millstone_h3/hit_v1/frames/decoded'),
    }
...
def review():
    target=ROOT/'.artifacts/millstone_h3/hit_v1';target.mkdir(parents=True,exist_ok=True)
    for part in range(2):
...
def build():
    ...
    target=ROOT/'.artifacts/millstone_h3/hit_v1';target.mkdir(parents=True,exist_ok=True);write(target/'combined_handoff.json',dict(schema_version=1,units=[candidate]))
