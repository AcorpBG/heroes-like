...
INPUT_SCALE = .95
RUNTIME_SCALE = .6
PROMPT = 'Locked orthographic fantasy strategy sprite camera. Keep this exact adult female Millstone Slinger: brown hair in small bun, green cloth headband and neck scarf, off-white rolled-sleeve tunic, loose moss-green trousers, brown lace-up leather boots, brown forearm bracers and apron, belt pouches of gray stones, diagonal leather bandolier with gray stones. Face and body remain three-quarter RIGHT-facing. Exactly two arms, two hands and two legs. ONE plain brown leather sling with two flexible cords and one small leather cup belongs only to her image-left hand; no bow, staff, shield, sword or extra limbs. All straps and pouches remain attached. One COMPACT physical sling throw and complete recovery toward IMAGE RIGHT. Windup by26, release38, keep arm extended and open sling SHORT through46, gather sling and ready62, then stand still until end. Never spin sling repeatedly. The whole sling is SMALL, leather cup remains PALM-SIZED, cords FOREARM-LENGTH at ALL times. No giant pouch, no oversized whip or elasticity.  Start sling low, free hand briefly positions a stone inside leather cup (stone is concealed). Sling hand raises elbow behind shoulder, winds up with sling hanging just behind raised hand near shoulder as in frame26 guide, then sweeps through ONE COMPACT forward arc into frame38 release guide; free arm aims right. Torso and shoulder rotate into a strong forward release: sling hand releases ONLY the release cord while retaining retention cord, cup opens, single loose end follows through; then wrist and elbow lower and collect both ends back into original ready grip. Keep sling attached to original hand throughout, each cord no longer than ONE FOREARM, keep cup within ONE FOREARM of throwing hand throughout, consistent short cord length and single cup; no duplicate sling or floating circular ring. Distinct windup, release and recovery. Never stretch sling, never whip it beyond her immediate arm reach, never send cup/cord offscreen. No visible flying projectile or stone trail because game renders projectile separately. Both feet planted with natural weight transfer. No magic, effects, beams, extra weapons or repeated throws. Fixed body scale and ground reference, no camera motion, pan, zoom or perspective change. Entire head, both hands, all sling cords and both boots stay within frame. Perfectly flat PURE MAGENTA RGB(255,0,255) background, no floor, shadows, gradients, text, scenery, motion blur or translucent ghosting.'
...
def prepare():
    ...
    contacts = [next(f for f in originals if f['name']=='ranged_00'),next(f for f in originals if f['name']=='ranged_01'),dict(name='release_contact',clip='ranged',source='art/units/source/generated/video_trials/millstone_h3/key_guides/release_v1.png',rects=[[0,0,1247,1261]],anchor=[560,1231],scale=.18,alpha_noise_cutoff=8)]
    refs = []
    ...
    graph = {
        ...
        '19':node('MiniMaxH3AddGuide',positive=['17',0],latent=['5',1],frame_idx=38,vae=['3',0],image=['18',0]),
        '20':node('MiniMaxH3AddGuide',positive=['19',0],latent=['5',1],frame_idx=46,vae=['3',0],image=['18',0]),
        '21':node('MiniMaxH3AddGuide',positive=['20',0],latent=['5',1],frame_idx=62,vae=['3',0],image=['4',0]),
        '22':node('MiniMaxH3AddGuide',positive=['21',0],latent=['5',1],frame_idx=86,vae=['3',0],image=['4',0]),
        '6':node('BasicGuider',model=['1',0],conditioning=['22',0]),
        '7':node('RandomNoise',noise_seed=2026092442),
        ...
        '14':node('SaveVideo',video=['13',0],filename_prefix='millstone_h3/ranged_guided_v3/original',**{'format':'mp4','format.codec':'h264'}),
        '15':node('SaveImage',images=['11',0],filename_prefix='millstone_h3/ranged_guided_v3/frames/decoded'),
    }
...
def verify():
    ...
    assert graph['6']['inputs']['conditioning']==['22',0] and graph['19']['inputs']['frame_idx']==38
...
def review():
    target=ROOT/'.artifacts/millstone_h3/ranged_guided_v3';target.mkdir(parents=True,exist_ok=True)
    for part in range(2):
...
def build():
    ...
    target=ROOT/'.artifacts/millstone_h3/ranged_guided_v3';target.mkdir(parents=True,exist_ok=True);write(target/'combined_handoff.json',dict(schema_version=1,units=[candidate]))
