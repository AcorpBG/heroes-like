...
def prepare():
    ...
    graph = {
        ...
        '17':node('MiniMaxH3AddGuide',positive=['5',0],latent=['5',1],frame_idx=34,vae=['3',0],image=['16',0]),
        ...
        '7':node('RandomNoise',noise_seed=2026092441),
        ...
        '14':node('SaveVideo',video=['13',0],filename_prefix='millstone_h3/ranged_guided_v2/original',**{'format':'mp4','format.codec':'h264'}),
        '15':node('SaveImage',images=['11',0],filename_prefix='millstone_h3/ranged_guided_v2/frames/decoded'),
    }
...
def verify():
    ...
    assert graph['6']['inputs']['conditioning']==['19',0] and graph['19']['inputs']['frame_idx']==54
...
def review():
    target=ROOT/'.artifacts/millstone_h3/ranged_guided_v2';target.mkdir(parents=True,exist_ok=True)
    for part in range(2):
...
def build():
    ...
    target=ROOT/'.artifacts/millstone_h3/ranged_guided_v2';target.mkdir(parents=True,exist_ok=True);write(target/'combined_handoff.json',dict(schema_version=1,units=[candidate]))
