...
INPUT_SCALE = .95
RUNTIME_SCALE = .6
PROMPT = 'One adult female fantasy slinger makes ONE compact overarm sling throw to the right, then recovers. Preserve the exact reference woman, clothing, equipment, body proportions and right-facing three-quarter view. Raise the small loaded sling from her image-left hand into the short taut anticipation guide at26, hold through34. Her throwing shoulder rotates forward and elbow extends, releasing at38 to the open-cup guide42. The single leather cup stays palm-sized and the two thin leather cords remain forearm-length. One cord releases from her fingers and hangs loose, the retention cord stays attached. Her other hand balances low. Do not draw a flying stone; the game handles the projectile separately. Lower the same sling and recover to the original ready position by72. Only one short casting stroke, with no repeated twirls or huge loops. Two arms, two legs, both boots registered. Fixed camera, scale and ground. Crisp opaque sprite edges. Background remains one stationary solid magenta color identical to the guides throughout.'
...
def prepare():
    ...
    graph = {
        ...
        '19':node('MiniMaxH3AddGuide',positive=['17',0],latent=['5',1],frame_idx=42,vae=['3',0],image=['18',0]),
        '20':node('MiniMaxH3AddGuide',positive=['19',0],latent=['5',1],frame_idx=50,vae=['3',0],image=['18',0]),
        '21':node('MiniMaxH3AddGuide',positive=['20',0],latent=['5',1],frame_idx=72,vae=['3',0],image=['4',0]),
        '22':node('MiniMaxH3AddGuide',positive=['21',0],latent=['5',1],frame_idx=96,vae=['3',0],image=['4',0]),
        '23':node('MiniMaxH3AddGuide',positive=['22',0],latent=['5',1],frame_idx=34,vae=['3',0],image=['16',0]),
        '24':node('MiniMaxH3AddGuide',positive=['23',0],latent=['5',1],frame_idx=38,vae=['3',0],image=['18',0]),
        '6':node('BasicGuider',model=['1',0],conditioning=['24',0]),
        '7':node('RandomNoise',noise_seed=2026092447),
        ...
        '14':node('SaveVideo',video=['13',0],filename_prefix='millstone_h3/ranged_compact_v4/original',**{'format':'mp4','format.codec':'h264'}),
        '15':node('SaveImage',images=['11',0],filename_prefix='millstone_h3/ranged_compact_v4/frames/decoded'),
    }
...
def verify():
    ...
    assert graph['6']['inputs']['conditioning']==['24',0] and graph['19']['inputs']['frame_idx']==42
...
def review():
    target=ROOT/'.artifacts/millstone_h3/ranged_compact_v4';target.mkdir(parents=True,exist_ok=True)
    for part in range(2):
...
def build():
    ...
    target=ROOT/'.artifacts/millstone_h3/ranged_compact_v4';target.mkdir(parents=True,exist_ok=True);write(target/'combined_handoff.json',dict(schema_version=1,units=[candidate]))
