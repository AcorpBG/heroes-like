...
INPUT_SCALE = .95
RUNTIME_SCALE = .6
PROMPT = 'One adult female fantasy slinger performs a brief controlled backward flinch and regains her stance. Starting with bent knees and hands raised, shoulders pull back and elbows spread at frame32, then knees and torso absorb the movement and both hands recover by frame58. Her leather sling stays loosely coiled inside the same image-left fist; she never opens that fist. It does not extend or swing. Exactly one single character alone, with only her original clothing and small coiled sling, during the entire action. No attacker, no incoming weapon, no projectile, no streak or impact effect. Preserve face, bun, green headband and scarf, ivory tunic, green trousers, boots, pouches and gray stones. Both boots remain registered. Fixed three-quarter right-facing camera and scale. Two arms and two legs. Clean crisp opaque sprite contours. The entire background is one stationary solid magenta matte, unchanged throughout, identical to guide images. No camera or scene changes.'
...
def prepare():
    ...
    contacts = [recipe['frames'][0].copy(),next(f for f in originals if f['name']=='hit_01')]
    refs = []
    ...
    graph = {
        ...
        '4':node('LoadImage',image='millstone_hit_input.png'),
        ...
        '17':node('MiniMaxH3AddGuide',positive=['5',0],latent=['5',1],frame_idx=32,vae=['3',0],image=['16',0]),
        '18':node('MiniMaxH3AddGuide',positive=['17',0],latent=['5',1],frame_idx=58,vae=['3',0],image=['4',0]),
        '19':node('MiniMaxH3AddGuide',positive=['18',0],latent=['5',1],frame_idx=88,vae=['3',0],image=['4',0]),
        '6':node('BasicGuider',model=['1',0],conditioning=['19',0]),
        '7':node('RandomNoise',noise_seed=2026092445),
        ...
        '14':node('SaveVideo',video=['13',0],filename_prefix='millstone_h3/hit_guided_v2/original',**{'format':'mp4','format.codec':'h264'}),
        '15':node('SaveImage',images=['11',0],filename_prefix='millstone_h3/hit_guided_v2/frames/decoded'),
    }
...
def submit():
    ...
    for key,filename in [('4','input_magenta.png'),('16','peak_magenta.png')]:
        ...
        data=(f'--{boundary}\r\nContent-Disposition: form-data; name="image"; filename="millstone_hit_{filename}"\r\nContent-Type: image/png\r\n\r\n'.encode()+(OUT/filename).read_bytes()+f'\r\n--{boundary}--\r\n'.encode())
    ...
    write(OUT/'workflow_api.json',graph)
    started=time.time();r=request(URL,'/prompt',dict(prompt=graph,client_id='millstone_hit_h3'))
...
def review():
    target=ROOT/'.artifacts/millstone_h3/hit_guided_v2';target.mkdir(parents=True,exist_ok=True)
    for part in range(2):
...
def build():
    ...
    frames=[dict(name=f'hit_h3_{i:03}',clip='hit',source=(OUT/'matte'/f'rgba_{i:03}.png').relative_to(ROOT).as_posix(),rects=[[0,0,960,544]],anchor=ANCHOR,scale=RUNTIME_SCALE,alpha_noise_cutoff=0,video_frame=i,video_time_seconds=i/24) for i in indices]
    ...
    target=ROOT/'.artifacts/millstone_h3/hit_guided_v2';target.mkdir(parents=True,exist_ok=True);write(target/'combined_handoff.json',dict(schema_version=1,units=[candidate]))
