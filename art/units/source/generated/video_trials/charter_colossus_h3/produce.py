"""Rebuild original Charter Colossus H3 takes and pending candidates."""
import argparse
import hashlib
import json
import sys
...
def prepare(out,c):
 ...
 graph={
  ...
  '14':node('SaveVideo',video=['13',0],filename_prefix=f'charter_colossus_h3/{out.name}/original',**{'format':'mp4','format.codec':'h264'}),
  '15':node('SaveImage',images=['11',0],filename_prefix=f'charter_colossus_h3/{out.name}/frames/decoded')}
 if c.get('tiled_decode'):
  graph['11']=node('VAEDecodeTiled',samples=['10',0],vae=['3',0],**c['tiled_decode'])
 for i in range(len(refs)):graph[str(30+i)]=node('LoadImage',image=f'charter_colossus_{out.name}_guide_{i}.png')
 previous='5'
...
def submit(out,c):
 ...
 for i in range(len(c['references'])):
  name=f'charter_colossus_{out.name}_guide_{i}.png'
  if c.get('reuse_uploaded_guides'):
   ...
   assert not item.get('subfolder') and name.startswith('charter_colossus_') and '/' not in name and '\\' not in name
 ...
 write(out/'workflow_api.json',g);start=time.time();r=request(URL,'/prompt',dict(prompt=g,client_id='charter_colossus_h3'))
...
def key(rgb):
 ...
 # This colossus contains ivory stone, dark iron, bronze and scarlet but no green
...
def process(out,c):
 ...
 with av.open(str(out/'original_lossless.mkv')) as video:
  for i,frame in enumerate(video.decode(video=0)):
   ...
   if detail['mode']!='flat_green_chroma_unmix':raise ValueError('Backdrop changed to a color unsafe for ivory armor, dark iron, bronze joints and red cloth; reject this interval')
   p=out/'matte'/f'rgba_{i:03}.png';im.save(p);hashes.append(sha(p));details.append(detail)
 assert len(hashes)==124
 write(out/'matte.json',dict(frames=124,fps=24,rgba_sha256=hashes,background_frames=details,recipe='Measured uniform green plate absent from Charter Colossus palette; corner spread<=10, chroma separation>=80; color-specific alpha unmix and edge-only despill; alpha>=8 regions retained when containing alpha>=128 pixels. Original geometry unchanged.'))

def review(out,c):
 target=ROOT/'.artifacts/charter_colossus_h3'/out.name;target.mkdir(parents=True,exist_ok=True)
 for part in range(2):
  ...
  for j,i in enumerate(range(part*62,(part+1)*62)):
   ...
   im=Image.open(out/'matte'/f'rgba_{i:03}.png').crop((280,40,750,525));im.thumbnail((196,195),Image.Resampling.LANCZOS);sheet.paste(im,(x+(200-im.width)//2,y+22),im);d.text((x+5,y+4),str(i),fill=(160,110,65))
...
def assemble():
 ...
 unit['preserved_accepted_clips']=[];unit['visual_review']=dict(status='pending',notes='Selected Charter Colossus H3 actions; native review required. Seven dedicated actions; identity matches original idle painting.')
