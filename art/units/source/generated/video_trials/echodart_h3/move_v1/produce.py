"""Original-guide H3 articulated idle for Echodart Casts; publication is explicit."""
import argparse
import hashlib
import json
import sys
...
UNIT = 'unit_neutral_echodart_casts'
URL = 'http://127.0.0.1:8189'
ANCHOR = [480, 475]
INPUT_SCALE = .9
RUNTIME_SCALE = .4 / INPUT_SCALE
PROMPT = 'This exact adult female Echodart crossbow scout: dark brown high ponytail, blue crystal hair clasp, gray scarf and trailing torn gray cloth, dark leather armor and boots with bronze trim and small cyan-blue crystals, belt pouches and cyan bolts at the hip. Exactly two arms, two hands and two legs. Keep one rigid short bronze-and-dark-wood crossbow with its cyan front tip, two rigid limbs and string. Both original hand grips stay attached: one hand supports the front stock and the other holds the rear grip. Preserve the original three-quarter LEFT-facing view, face, proportions and equipment. Walk briskly IN PLACE through three full reciprocal cycles. Near and far legs alternately lead toward LEFT, bend past each other, plant heels and push off toes. Both boot toes point left. Near-leg contact at0/41/82/123, opposite-leg contact halfway between. Keep the crossbow held securely across the lower chest in both hands while elbows absorb the stride; shoulders counter-turn modestly without changing the camera view. Ponytail and cloth trail gently. No shooting or reloading. Anatomical root stays centered for engine-driven travel. Fixed scale, locked orthographic camera, no camera movement or world travel. Entire crossbow, ponytail, hands and boots remain inside the frame. Clean opaque sprite, no motion blur or ghosting. Background is one solid stationary green color, exactly like the reference, throughout.'
...
def prepare():
    ...
    recipe = json.loads((ROOT/'art/animation/source/fluid/unit_neutral_echodart_casts/reviewed_handoff.json').read_bytes())['units'][0]
    contacts = [dict(name='move_original_contact',clip='move',source='art/units/source/generated/fluid_animation/batch_d/unit_neutral_echodart_casts/move_v1.png',rects=[[1330, 0, 1773, 445]],anchor=[1551.5, 438],scale=.4,alpha_noise_cutoff=8)]
    refs = []
    ...
    graph = {
        ...
        '4':node('LoadImage',image='echodart_move_input.png'),
        ...
        '16':node('MiniMaxH3AddGuide',positive=['5',0],latent=['5',1],frame_idx=41,vae=['3',0],image=['4',0]),
        '17':node('MiniMaxH3AddGuide',positive=['16',0],latent=['5',1],frame_idx=82,vae=['3',0],image=['4',0]),
        '6':node('BasicGuider',model=['1',0],conditioning=['17',0]),
        '7':node('RandomNoise',noise_seed=2026092450),
        ...
        '14':node('SaveVideo',video=['13',0],filename_prefix='echodart_h3/move_v1/original',**{'format':'mp4','format.codec':'h264'}),
        '15':node('SaveImage',images=['11',0],filename_prefix='echodart_h3/move_v1/frames/decoded'),
    }
...
def submit():
    ...
    for key,filename in [('4','input_green.png')]:
        boundary='----EchodartIdleH3'
        data=(f'--{boundary}\r\nContent-Disposition: form-data; name="image"; filename="echodart_move_{filename}"\r\nContent-Type: image/png\r\n\r\n'.encode()+(OUT/filename).read_bytes()+f'\r\n--{boundary}--\r\n'.encode())
    ...
    write(OUT/'workflow_api.json',graph)
    started=time.time();r=request(URL,'/prompt',dict(prompt=graph,client_id='echodart_move_h3'))
...
def review():
    target=ROOT/'.artifacts/echodart_h3/move_v1';target.mkdir(parents=True,exist_ok=True)
    for part in range(2):
...
def build():
    ...
    entry=dict(unit_id=UNIT,reference_height=256,source_facing='left',frames=frames,alpha_noise_cutoff=0,clips={'move':dict(indices=list(range(len(frames))),frame_msec=selection['frame_msec'],loop=True,static_frame=0)},source_scale_reason='Original Echodart painting uses .4 runtime scale; input.9 and extraction.4/.9 with fixed anchor480,475 preserve body and equipment scale. No per-frame size normalization.',provenance=provenance,visual_review=dict(status='pending',notes=selection['review_note']))
    ...
    target=ROOT/'.artifacts/echodart_h3/move_v1';target.mkdir(parents=True,exist_ok=True);write(target/'combined_handoff.json',dict(schema_version=1,units=[candidate]))
