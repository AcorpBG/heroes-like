...
def prepare():
    ...
    contacts = [dict(name='idle_original_contact',clip='idle',source='art/units/source/generated/fluid_animation/batch_d/unit_neutral_echodart_casts/idle_v1.png',rects=[[0, 0, 443, 445]],anchor=[221, 438],scale=.4,alpha_noise_cutoff=8)]
    refs = []
    ...
    graph = {
        ...
        '4':node('LoadImage',image='echodart_idle_input.png'),
        ...
        '7':node('RandomNoise',noise_seed=2026092449),
        ...
        '14':node('SaveVideo',video=['13',0],filename_prefix='echodart_h3/idle_v1/original',**{'format':'mp4','format.codec':'h264'}),
        '15':node('SaveImage',images=['11',0],filename_prefix='echodart_h3/idle_v1/frames/decoded'),
    }
...
def submit():
    ...
    for key,filename in [('4','input_green.png')]:
        ...
        data=(f'--{boundary}\r\nContent-Disposition: form-data; name="image"; filename="echodart_idle_{filename}"\r\nContent-Type: image/png\r\n\r\n'.encode()+(OUT/filename).read_bytes()+f'\r\n--{boundary}--\r\n'.encode())
    ...
    write(OUT/'workflow_api.json',graph)
    started=time.time();r=request(URL,'/prompt',dict(prompt=graph,client_id='echodart_idle_h3'))
...
def review():
    target=ROOT/'.artifacts/echodart_h3/idle_v1';target.mkdir(parents=True,exist_ok=True)
    for part in range(2):
...
def build():
    ...
    target=ROOT/'.artifacts/echodart_h3/idle_v1';target.mkdir(parents=True,exist_ok=True);write(target/'combined_handoff.json',dict(schema_version=1,units=[candidate]))
