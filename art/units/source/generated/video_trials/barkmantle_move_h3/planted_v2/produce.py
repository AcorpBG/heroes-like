"""Original-guide H3 reciprocal walk for Barkmantle; publication is explicit."""
import argparse
import hashlib
import json
import sys
...
UNIT = 'unit_thornwake_barkmantle_rams'
URL = 'http://127.0.0.1:8189'
ANCHOR = [480, 465]
INPUT_SCALE = .4
RUNTIME_SCALE = .214 / INPUT_SCALE
PROMPT = '''Locked orthographic three-quarter RIGHT-facing fantasy strategy game sprite camera. The supplied original pictures show ONE identical heavy quadruped Barkmantle Ram at phases of an IN-PLACE short walking cycle. Preserve its barrel torso, ivory birch plates, amber sap nodules, moss, red vine bindings, patterned ribbons and rigid horned wooden head exactly. EXACTLY FOUR legs with four root paws attached to their original joints. Keep every paw at its original size. Legs bend and lift with short controlled reach, NEVER grow longer, sprawl toward the camera or push below the support plane. Ground contacts stay at the same horizontal foot lanes shown by the supplied PLANTED reference; do not reach toward the bottom image edge. Start with the near-front/far-hind diagonal pair raised as in the first picture. Lower that pair into the supplied all-four-paws PLANTED picture at one quarter. Transfer weight and lift the near-hind/far-front pair into the opposite diagonal picture at one half. Lower that pair into the same PLANTED picture at three quarters. Then lift the original near-front/far-hind pair back to the first picture. Smooth original articulated motion BETWEEN these poses: clear bent elbow/knee passing, small lifted-paw clearance, planting and weight transfer. Do not hold stationary at the guide poses. One complete heavy walking cycle; no attacks or running. Torso stays centered, rigid and constant size. The two far legs remain anatomically behind the torso; no additional or merged legs. Small natural shoulder/hip weight transfer only. Head and horns remain fixed in shape; ribbons settle gently. No sideways travel, camera movement, zoom, perspective changes, forward lunges, oversized paws, limb stretching, whole-body sliding, glow, particles or extra objects. Entire beast remains within the canvas. Every background pixel stays perfectly SOLID FLAT CYAN RGB(0,255,255), without shadow, floor, ground, scenery, gradients or text.'''
...
def prepare():
    ...
    refs = []
    for name, filename in [('walk_near_diagonal','input_cyan.png'),('walk_far_diagonal','middle_cyan.png'),('idle_0','planted_cyan.png')]:
        ...
        frame['scale'] = original_scale / RUNTIME_SCALE if name=='idle_0' else INPUT_SCALE
        if frame['scale'] > 1:
            subject,(x,y) = source_pose(dict(frame,scale=1))
            factor=frame['scale'];subject=subject.resize((round(subject.width*factor),round(subject.height*factor)),Image.Resampling.LANCZOS);x,y=round(x*factor),round(y*factor)
        else:
            subject,(x,y) = source_pose(frame)
        transparent = Image.new('RGBA',(960,544))
        transparent.alpha_composite(subject,(ANCHOR[0]+x,ANCHOR[1]+y))
        transparent.save(OUT/filename.replace('_cyan','_rgba'))
        canvas = Image.new('RGBA',(960,544),(0,255,255,255))
    ...
    graph = {
        ...
        '4':node('LoadImage',image='barkmantle_walk_input.png'),
        ...
        '7':node('RandomNoise',noise_seed=2026092409),
        ...
        '14':node('SaveVideo',video=['13',0],filename_prefix='barkmantle_move_h3/planted_v2/original',**{'format':'mp4','format.codec':'h264'}),
        '15':node('SaveImage',images=['11',0],filename_prefix='barkmantle_move_h3/planted_v2/frames/decoded'),
        '16':node('LoadImage',image='barkmantle_walk_middle.png'),
        '17':node('MiniMaxH3AddGuide',positive=['5',0],latent=['5',1],vae=['3',0],frame_idx=61,image=['16',0]),
        '18':node('LoadImage',image='barkmantle_planted.png'),
        '19':node('MiniMaxH3AddGuide',positive=['17',0],latent=['5',1],vae=['3',0],frame_idx=30,image=['18',0]),
        '20':node('MiniMaxH3AddGuide',positive=['19',0],latent=['5',1],vae=['3',0],frame_idx=92,image=['18',0]),
    }
...
def verify():
    ...
    assert graph['20']['inputs']==dict(positive=['19',0],latent=['5',1],vae=['3',0],frame_idx=92,image=['18',0])
...
def submit():
    ...
    for key,filename in [('4','input_cyan.png'),('16','middle_cyan.png'),('18','planted_cyan.png')]:
        boundary='----BarkmantleWalkH3'
        data=(f'--{boundary}\r\nContent-Disposition: form-data; name="image"; filename="barkmantle_walk_{filename}"\r\nContent-Type: image/png\r\n\r\n'.encode()+(OUT/filename).read_bytes()+f'\r\n--{boundary}--\r\n'.encode())
    ...
    write(OUT/'workflow_api.json',graph)
    started=time.time();r=request(URL,'/prompt',dict(prompt=graph,client_id='barkmantle_walk_h3'))
...
def process():
    ...
    with av.open(str(OUT/'original_lossless.mkv')) as video:
        for i,frame in enumerate(video.decode(video=0)):
            ...
            if np.max(np.linalg.norm(corners-bg,axis=1))>10 or min(bg[1],bg[2])-bg[0]<150:raise ValueError(f'Frame {i}: unsafe cyan plate')
            alpha=np.clip(1-(np.minimum(a[:,:,1],a[:,:,2])-a[:,:,0])/(min(bg[1],bg[2])-bg[0]),0,1)
            rgb=np.clip((a-(1-alpha[:,:,None])*bg)/np.maximum(alpha[:,:,None],.001),0,255)
            spill=np.maximum(0,np.minimum(rgb[:,:,1],rgb[:,:,2])-rgb[:,:,0]);rgb[:,:,1]-=spill;rgb[:,:,2]-=spill
    ...
    write(OUT/'matte.json',dict(frames=124,fps=24,rgba_sha256=hashes,recipe='Uniform cyan corner check; cyan alpha unmix and RGB despill; alpha>=8 components retained if containing alpha>=128. Preserve separate ornaments; no largest-component masking, geometry editing or per-frame alignment.'))


def review():
    target=ROOT/'.artifacts/barkmantle_move_h3/planted_v2';target.mkdir(parents=True,exist_ok=True)
    for part in range(2):
        ...
        for j,i in enumerate(range(part*62,(part+1)*62)):
            ...
            im=Image.open(OUT/'matte'/f'rgba_{i:03}.png').crop((145,20,805,544));im.thumbnail((174,178),Image.Resampling.LANCZOS);sheet.paste(im,(x+(180-im.width)//2,y+20),im);draw.text((x+5,y+4),str(i),fill=(160,110,65))
...
def build():
    ...
    target=ROOT/'.artifacts/barkmantle_move_h3/planted_v2';target.mkdir(parents=True,exist_ok=True);write(target/'combined_handoff.json',dict(schema_version=1,units=[candidate]))
