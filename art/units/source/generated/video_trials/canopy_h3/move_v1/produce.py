"""Preserved Canopy Rammer H3 take; see selection/rejection and root README."""
import argparse
import hashlib
import json
import sys
...
UNIT = 'unit_thornwake_canopy_rammers'
URL = 'http://127.0.0.1:8189'
ANCHOR = [480, 475]
INPUT_SCALE = .95
RUNTIME_SCALE = .6
PROMPT = 'This exact massive bark-and-root forest golem walks heavily IN PLACE through three full reciprocal stride cycles. Preserve its two huge wooden arms and fists, two thick root legs with spread woody toes, rhino-like branching horn crown, amber seams, vines, moss and white lichen plates. Original three-quarter RIGHT-facing view, stable face and fixed body scale. Near and far root legs alternately swing forward to the RIGHT, plant, bear the body weight, push off and pass. Root feet lift clear of one another during passing. Near leg leads at0/41/82/123; far leg leads halfway between. Arms swing modestly in opposition while horns stay rigid to the head and vines follow gently. Heavy grounded steps, not a hop. Anatomical root stays centered for engine-driven movement. Locked orthographic camera, no camera movement or world travel. Exactly two legs and two arms; no new branches or equipment. Entire creature and horns remain in frame. Clean opaque painted sprite, no motion blur. Background remains a single stationary solid magenta matte color identical to the guide throughout.'
...
def prepare():
    ...
    recipe = json.loads((ROOT/'art/animation/source/fluid/unit_thornwake_canopy_rammers/reviewed_handoff.json').read_bytes())['units'][0]
    originals=json.loads((ROOT/'art/units/source/generated/fluid_animation/batch_b/unit_thornwake_canopy_rammers/handoff.json').read_bytes())['units'][0]['frames']
    contacts = [dict(name='move_contact',clip='move',source='art/units/source/generated/fluid_animation/batch_b/unit_thornwake_canopy_rammers/move_v1.png',rects=[[96,0,465,373]],anchor=[270,355],scale=.57,alpha_noise_cutoff=8)]
    refs = []
    for original,filename in zip(contacts,['input_magenta.png']):
        frame = original.copy()
        frame.setdefault('alpha_noise_cutoff',8)
        original_scale = frame['scale']
        assert original_scale in [.55,.57]
    ...
    graph = {
        ...
        '4':node('LoadImage',image='canopy_move_input.png'),
        ...
        '17':node('MiniMaxH3AddGuide',positive=['5',0],latent=['5',1],frame_idx=41,vae=['3',0],image=['4',0]),
        '18':node('MiniMaxH3AddGuide',positive=['17',0],latent=['5',1],frame_idx=82,vae=['3',0],image=['4',0]),
        '6':node('BasicGuider',model=['1',0],conditioning=['18',0]),
        '7':node('RandomNoise',noise_seed=2026092452),
        ...
        '14':node('SaveVideo',video=['13',0],filename_prefix='canopy_h3/move_v1/original',**{'format':'mp4','format.codec':'h264'}),
        '15':node('SaveImage',images=['11',0],filename_prefix='canopy_h3/move_v1/frames/decoded'),
    }
...
def verify():
    ...
    assert graph['6']['inputs']['conditioning']==['18',0]
...
def submit():
    ...
    for key,filename in [('4','input_magenta.png')]:
        boundary='----CanopyH3'
        data=(f'--{boundary}\r\nContent-Disposition: form-data; name="image"; filename="canopy_move_{filename}"\r\nContent-Type: image/png\r\n\r\n'.encode()+(OUT/filename).read_bytes()+f'\r\n--{boundary}--\r\n'.encode())
    ...
    write(OUT/'workflow_api.json',graph)
    started=time.time();r=request(URL,'/prompt',dict(prompt=graph,client_id='canopy_move_h3'))
...
def key(rgb):
 ...
 # keeping thin horn tips, moss strands, root toes and fists attached
...
def review():
    target=ROOT/'.artifacts/canopy_h3/move_v1';target.mkdir(parents=True,exist_ok=True)
    for part in range(2):
...
def build():
    ...
    entry=dict(unit_id=UNIT,reference_height=256,source_facing='right',frames=frames,alpha_noise_cutoff=0,clips={'move':dict(indices=list(range(len(frames))),frame_msec=selection['frame_msec'],loop=True,static_frame=0)},source_scale_reason='Original Canopy source scale .55 for support and .57 for walking, calibrated to the existing approximately192px standing silhouette. Input scale is original scale/.6; extraction fixed .6 with anatomical ground anchor480,475. No per-frame normalization.',provenance=provenance,visual_review=dict(status='pending',notes=selection['review_note']))

    if 'contact_frame' in selection: entry['clips']['move']['contact_frame']=selection['contact_frame']
    if 'projectile_travel_msec' in selection: entry['clips']['move']['projectile_travel_msec']=selection['projectile_travel_msec']
    ...
    target=ROOT/'.artifacts/canopy_h3/move_v1';target.mkdir(parents=True,exist_ok=True);write(target/'combined_handoff.json',dict(schema_version=1,units=[candidate]))
