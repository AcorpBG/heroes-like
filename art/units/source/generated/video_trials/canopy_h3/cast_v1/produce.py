...
INPUT_SCALE = .95
RUNTIME_SCALE = .6
PROMPT = 'This exact massive bark-and-root forest golem makes a physical rally salute. Preserve its two huge wooden arms and fists, two thick root legs with spread woody toes, rhino-like branching horn crown, amber seams, vines, moss and white lichen plates. In the original three-quarter RIGHT-facing view, raise the far image-right fist across to the upper chest, briefly press it there and dip the horned head, then lower that fist back to ready. Near image-left fist stays low. Both root feet stay firmly planted. Raise through frames12-42, hold chest salute42-58, return by86. This is a physical loyalty gesture, not a spell or attack. Stable face, branch count, horn lengths, massive proportions and scale. Locked orthographic camera, no camera movement. Full figure and every horn remain inside frame. Clean crisp opaque painted sprite, no motion blur. The background is one stationary solid magenta matte color identical to the guides for the entire video.'
...
def prepare():
    ...
    contacts = [next(f for f in originals if f['name']=='cast_v1_00'),next(f for f in originals if f['name']=='cast_v1_02')]
    refs = []
    ...
    graph = {
        ...
        '4':node('LoadImage',image='canopy_cast_input.png'),
        ...
        '16':node('LoadImage',image='canopy_peak.png'),
        ...
        '7':node('RandomNoise',noise_seed=2026092451),
        ...
        '14':node('SaveVideo',video=['13',0],filename_prefix='canopy_h3/cast_v1/original',**{'format':'mp4','format.codec':'h264'}),
        '15':node('SaveImage',images=['11',0],filename_prefix='canopy_h3/cast_v1/frames/decoded'),
    }
...
def submit():
    ...
    for key,filename in [('4','input_magenta.png'),('16','peak_magenta.png')]:
        boundary='----CanopyH3'
        data=(f'--{boundary}\r\nContent-Disposition: form-data; name="image"; filename="canopy_cast_{filename}"\r\nContent-Type: image/png\r\n\r\n'.encode()+(OUT/filename).read_bytes()+f'\r\n--{boundary}--\r\n'.encode())
    ...
    write(OUT/'workflow_api.json',graph)
    started=time.time();r=request(URL,'/prompt',dict(prompt=graph,client_id='canopy_cast_h3'))
...
def review():
    target=ROOT/'.artifacts/canopy_h3/cast_v1';target.mkdir(parents=True,exist_ok=True)
    for part in range(2):
...
def build():
    ...
    target=ROOT/'.artifacts/canopy_h3/cast_v1';target.mkdir(parents=True,exist_ok=True);write(target/'combined_handoff.json',dict(schema_version=1,units=[candidate]))
