...
INPUT_SCALE = .95
RUNTIME_SCALE = .65
PROMPT = 'This exact adult bearded male infantry soldier: short dark brown hair, cream scarf, rust-red quilted tunic, steel shoulder plates, navy trousers, brown gloves and boots, leather belt pouches. One rigid rectangular navy wooden shield with brass rim, vertical brass stripe and round boss stays on the far image-right forearm. The near image-left hand grips the same short wooden spear with one steel leaf-shaped head. Exactly two arms, hands and legs; no duplicate weapons. Original three-quarter RIGHT-facing orthographic view and body proportions. Perform one continuous defeat and collapse. From ready, shoulders slump and both knees buckle; descend onto knees by42. Then lean and roll onto the shield side toward RIGHT, lowering head and torso until lying on the ground by86. The shield falls flat beside and beneath the head; the spear hand lowers the same rigid short spear onto the ground, steel tip pointing LEFT. End fully motionless on the side in the reference corpse pose. Never rise again. Both legs fold naturally, no extra limbs, no floating or upright discarded equipment. Preserve face, clothing and scale through the fall. Locked camera; grounded local fall with no camera movement. Entire body and equipment stay in frame. No gore, particles or motion blur. Single solid stationary magenta background identical to the guides throughout.'
...
def prepare():
    ...
    contacts = [next(f for f in originals if f['name']=='cast_00'),next(f for f in originals if f['name']=='death_03'),next(f for f in originals if f['name']=='death_07')]
    seeds={'cast_00':[315,200],'cast_02':[315,570],'death_03':[790,710],'death_07':[770,1400],'move_00':[335,150]}
    for frame in contacts:
        seed=seeds[frame['name']]
        frame['rects']=body_rectangles(ROOT/frame['source'],seed,8)
        if frame['name'] in ['cast_00','cast_02','move_00']:
            frame['anchor']=[frame['anchor'][0],{'cast_00':377,'cast_02':765,'move_00':334}[frame['name']]]
        frame['crop_recipe']=dict(tool='tools/refine_fluid_frame_crops.py',seed=seed,cutoff=8,reason='Isolate original character and connected equipment; exclude neighboring sheet fragments without repainting.')
    refs = []
    for original,filename in zip(contacts,['input_magenta.png','peak_magenta.png','corpse_magenta.png']):
        frame = original.copy()
    ...
    graph = {
        ...
        '4':node('LoadImage',image='milestone_death_input.png'),
        '5':node('MiniMaxH3ImageToVideo',clip=['2',0],vae=['3',0],prompt=PROMPT,width=960,height=544,length=124,first_frame=['4',0],last_frame=['20',0]),
        ...
        '20':node('LoadImage',image='milestone_corpse.png'),
        '19':node('MiniMaxH3AddGuide',positive=['17',0],latent=['5',1],frame_idx=86,vae=['3',0],image=['20',0]),
        '6':node('BasicGuider',model=['1',0],conditioning=['19',0]),
        '7':node('RandomNoise',noise_seed=2026092455),
        ...
        '14':node('SaveVideo',video=['13',0],filename_prefix='milestone_h3/death_v1/original',**{'format':'mp4','format.codec':'h264'}),
        '15':node('SaveImage',images=['11',0],filename_prefix='milestone_h3/death_v1/frames/decoded'),
    }
...
def verify():
    ...
    assert graph['5']['inputs']['first_frame']==['4',0] and graph['5']['inputs']['last_frame']==['20',0]
...
def submit():
    ...
    for key,filename in [('4','input_magenta.png'),('16','peak_magenta.png'),('20','corpse_magenta.png')]:
        boundary='----MilestoneH3'
        data=(f'--{boundary}\r\nContent-Disposition: form-data; name="image"; filename="milestone_death_{filename}"\r\nContent-Type: image/png\r\n\r\n'.encode()+(OUT/filename).read_bytes()+f'\r\n--{boundary}--\r\n'.encode())
    ...
    write(OUT/'workflow_api.json',graph)
    started=time.time();r=request(URL,'/prompt',dict(prompt=graph,client_id='milestone_death_h3'))
...
def review():
    target=ROOT/'.artifacts/milestone_h3/death_v1';target.mkdir(parents=True,exist_ok=True)
    for part in range(2):
...
def build():
    ...
    if 'contact_frame' in selection: entry['clips']['death']['contact_frame']=selection['contact_frame']
    if 'projectile_travel_msec' in selection: entry['clips']['death']['projectile_travel_msec']=selection['projectile_travel_msec']
    ...
    target=ROOT/'.artifacts/milestone_h3/death_v1';target.mkdir(parents=True,exist_ok=True);write(target/'combined_handoff.json',dict(schema_version=1,units=[candidate]))
