...
INPUT_SCALE = .95
RUNTIME_SCALE = .65
PROMPT = 'This exact adult bearded male infantry soldier: short dark brown hair, cream scarf, rust-red quilted tunic, steel shoulder plates, navy trousers, brown gloves and boots, leather belt pouches. One rigid rectangular navy wooden shield with brass rim, vertical brass stripe and round boss stays on the far image-right forearm. The near image-left hand grips the same short wooden spear with one steel leaf-shaped head. Exactly two arms, hands and legs; no duplicate weapons. Original three-quarter RIGHT-facing orthographic view and body proportions. Walk briskly IN PLACE through three complete reciprocal gait cycles. Near and far legs alternately advance toward RIGHT, plant heel, bear weight, pass and push off toes. Near leg leads at0/41/82/123 and opposite leg leads halfway between. Preserve two distinct boots during passing. Hold the shield securely and the spear low angled back toward image-left; both retain fixed shape and original length while arms absorb the stride. Modest shoulder counterturn, stable face. Anatomical root remains centered for engine-driven travel. Locked camera, fixed body scale. Full shield and spear remain in frame. Clean crisp opaque painted sprite, no blur. Background stays one stationary solid magenta matte color identical to the guide.'
...
def prepare():
    ...
    contacts = [next(f for f in originals if f['name']=='move_00')]
    ...
    graph = {
        ...
        '4':node('LoadImage',image='milestone_move_input.png'),
        ...
        '7':node('RandomNoise',noise_seed=2026092454),
        ...
        '14':node('SaveVideo',video=['13',0],filename_prefix='milestone_h3/move_v1/original',**{'format':'mp4','format.codec':'h264'}),
        '15':node('SaveImage',images=['11',0],filename_prefix='milestone_h3/move_v1/frames/decoded'),
    }
...
def submit():
    ...
    for key,filename in [('4','input_magenta.png')]:
        ...
        data=(f'--{boundary}\r\nContent-Disposition: form-data; name="image"; filename="milestone_move_{filename}"\r\nContent-Type: image/png\r\n\r\n'.encode()+(OUT/filename).read_bytes()+f'\r\n--{boundary}--\r\n'.encode())
    ...
    write(OUT/'workflow_api.json',graph)
    started=time.time();r=request(URL,'/prompt',dict(prompt=graph,client_id='milestone_move_h3'))
...
def review():
    target=ROOT/'.artifacts/milestone_h3/move_v1';target.mkdir(parents=True,exist_ok=True)
    for part in range(2):
...
def build():
    ...
    target=ROOT/'.artifacts/milestone_h3/move_v1';target.mkdir(parents=True,exist_ok=True);write(target/'combined_handoff.json',dict(schema_version=1,units=[candidate]))
