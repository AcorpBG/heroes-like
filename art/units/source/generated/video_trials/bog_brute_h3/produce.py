"""Rebuild original Bog Brute H3 takes and pending candidates."""
import argparse
import hashlib
import json
import sys
...
def prepare(out,c):
 ...
 graph={
  ...
  '14':node('SaveVideo',video=['13',0],filename_prefix=f'bog_brute_h3/{out.name}/original',**{'format':'mp4','format.codec':'h264'}),
  '15':node('SaveImage',images=['11',0],filename_prefix=f'bog_brute_h3/{out.name}/frames/decoded')}
 for i in range(len(refs)):graph[str(30+i)]=node('LoadImage',image=f'bog_brute_{out.name}_guide_{i}.png')
 previous='5'
...
def submit(out,c):
 ...
 for i in range(len(c['references'])):
  name=f'bog_brute_{out.name}_guide_{i}.png';boundary='----BogBruteH3'
 ...
 write(out/'workflow_api.json',g);start=time.time();r=request(URL,'/prompt',dict(prompt=g,client_id='bog_brute_h3'))
...
def key(rgb):
 ...
 # This brute contains green, brown, brass and steel but no magenta
 ...
 # keeping club edges, hanging hook/chain, finger grips and boot soles attached
...
def process(out,c):
 ...
 write(out/'matte.json',dict(frames=124,fps=24,rgba_sha256=hashes,background_frames=details,recipe='Measured uniform magenta plate absent from brute palette; corner spread<=10, chroma separation>=80; color-specific alpha unmix and edge-only despill; alpha>=8 regions retained when containing alpha>=128 pixels. Original geometry unchanged.'))

def review(out,c):
 target=ROOT/'.artifacts/bog_brute_h3'/out.name;target.mkdir(parents=True,exist_ok=True)
 for part in range(2):
