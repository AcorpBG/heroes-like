"""Rebuild original Blackbranch Cutthroat H3 takes and pending candidates."""
import argparse
import hashlib
import json
import sys
...
def prepare(out,c):
 ...
 for i,original in enumerate(c['references']):
  ...
  rgba.save(out/f'guide_{i}_rgba.png');back=Image.new('RGBA',rgba.size,tuple(c.get('key_rgb',[255,0,255]))+(255,));back.alpha_composite(rgba);back.convert('RGB').save(out/f'guide_{i}_magenta.png')
  refs.append(dict(source_frame=f,source_sha256=sha(ROOT/f['source']),input_file=f'guide_{i}_magenta.png',input_sha256=sha(out/f'guide_{i}_magenta.png'),runtime_source_scale=original['scale']))
 ...
 graph={
  ...
  '14':node('SaveVideo',video=['13',0],filename_prefix=f'blackbranch_cutthroat_h3/{out.name}/original',**{'format':'mp4','format.codec':'h264'}),
  '15':node('SaveImage',images=['11',0],filename_prefix=f'blackbranch_cutthroat_h3/{out.name}/frames/decoded')}
 for i in range(len(refs)):graph[str(30+i)]=node('LoadImage',image=f'blackbranch_cutthroat_{out.name}_guide_{i}.png')
 previous='5'
...
def submit(out,c):
 ...
 for i in range(len(c['references'])):
  name=f'blackbranch_cutthroat_{out.name}_guide_{i}.png';boundary='----BlackbranchCutthroatH3'
  data=(f'--{boundary}\r\nContent-Disposition: form-data; name="image"; filename="{name}"\r\nContent-Type: image/png\r\n\r\n'.encode()+(out/f'guide_{i}_magenta.png').read_bytes()+f'\r\n--{boundary}--\r\n'.encode())
 ...
 write(out/'workflow_api.json',g);start=time.time();r=request(URL,'/prompt',dict(prompt=g,client_id='blackbranch_cutthroat_h3'))
...
def key(rgb):
 ...
 # This cutthroat contains green, brown, brass and steel but no magenta
 # material. Use a magenta plate to preserve green clothing. Measure the actual uniform source plate; never
...
def process(out,c):
 ...
 with av.open(str(out/'original_lossless.mkv')) as video:
  for i,frame in enumerate(video.decode(video=0)):
   ...
   if detail['mode']!='flat_magenta_chroma_unmix':raise ValueError('Backdrop changed to a color unsafe for green clothing; reject this interval')
   p=out/'matte'/f'rgba_{i:03}.png';im.save(p);hashes.append(sha(p));details.append(detail)
 assert len(hashes)==124
 write(out/'matte.json',dict(frames=124,fps=24,rgba_sha256=hashes,background_frames=details,recipe='Measured uniform green or magenta plate absent from guard palette; corner spread<=10, chroma separation>=80; color-specific alpha unmix and edge-only despill; alpha>=8 regions retained when containing alpha>=128 pixels. Original geometry unchanged.'))

def review(out,c):
 target=ROOT/'.artifacts/blackbranch_cutthroat_h3'/out.name;target.mkdir(parents=True,exist_ok=True)
 for part in range(2):
...
def assemble():
 ...
 unit['preserved_accepted_clips']=['idle'];unit['visual_review']=dict(status='pending',notes='Six selected H3 clips; native review required. Preserve original accepted articulated idle.')
