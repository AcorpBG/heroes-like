"""Rebuild original River Guard H3 takes and pending animation candidates."""
import argparse
import hashlib
import json
import sys
...
def prepare(out,c):
 ...
 graph={
  ...
  '14':node('SaveVideo',video=['13',0],filename_prefix=f'river_guard_h3/{out.name}/original',**{'format':'mp4','format.codec':'h264'}),
  '15':node('SaveImage',images=['11',0],filename_prefix=f'river_guard_h3/{out.name}/frames/decoded')}
 for i in range(len(refs)):graph[str(30+i)]=node('LoadImage',image=f'river_guard_{out.name}_guide_{i}.png')
 previous='5'
...
def submit(out,c):
 ...
 for i in range(len(c['references'])):
  name=f'river_guard_{out.name}_guide_{i}.png';boundary='----RiverGuardH3'
 ...
 write(out/'workflow_api.json',g);start=time.time();r=request(URL,'/prompt',dict(prompt=g,client_id='river_guard_h3'))
...
def key(rgb):
 ...
 # This guard contains red, gold, brown, blue and ivory, with neither saturated
...
def review(out,c):
 target=ROOT/'.artifacts/river_guard_h3'/out.name;target.mkdir(parents=True,exist_ok=True)
 for part in range(2):
