"""Original-guide H3 reciprocal walk for Emberpack Lobbers; publication is explicit."""
import argparse
import hashlib
import json
import sys
...
UNIT = 'unit_neutral_emberpack_lobbers'
URL = 'http://127.0.0.1:8189'
ANCHOR = [480, 475]
INPUT_SCALE = .45
RUNTIME_SCALE = .195 / INPUT_SCALE
PROMPT = 'Locked orthographic three-quarter RIGHT-facing fantasy strategy sprite camera. Animate the exact Emberpack Lobber man from the reference walking IN PLACE for two complete reciprocal left-right cycles and returning to the reference stance. Preserve his hooded face, perforated brass forehead plate, dark hood and leather coat, orange scarf and split orange coat tails, dark trousers, belts, brass belt cases, wrapped forearms and tall brown boots. Exactly two human arms, two human legs. Keep the ONE rigid strapped wicker-and-brass ember basket on his back with its glowing coals and attached brass side flask. No added backpack tanks, floating props or changes in basket shape. Hand at IMAGE LEFT continuously grasps the gathered straps of the ONE sling; its ember-filled pouch hangs outside the left leg at knee height, clear of boots and floor. Strap remains connected to the same grip and pouch, never duplicates, vanishes, lengthens or crosses between legs. Only gentle small outward sway; no throwing, firing, winding or release. Free hand at IMAGE RIGHT remains empty beside body, gently counter-swings with gait. Preserve contained warm ember glow in basket and sling pouch, without added smoke, flame jets, particles or projectile. Near boot reaches forward heel-first while far boot trails; near foot plants and supports body while far knee flexes and far boot passes beneath hips to the opposite forward contact. Far foot plants and accepts weight while near boot trails, passes, then reaches forward again. Both legs each show forward contact, load, toe-off and passing; never repeat the same leading leg. Modest deliberate stride, knees below waist, no hopping, skating, foot stretching or limb swaps. Coat tails gently lag behind the alternating steps. Torso stays centered at constant original size and this corrected RIGHT-facing reference orientation; forward steps go toward IMAGE RIGHT, never IMAGE LEFT. Chest, hips, knees and feet retain that direction. Maintain original three-quarter facing, restrained shoulder counter-rotation only; no camera travel, zoom, turns to front or profile or body translation. Entire hood, basket, flask, both boots and sling pouch stay inside frame. Background remains perfectly uniform PURE GREEN RGB(0,255,0), with no floor, ground shadow, scenery, gradient, text, motion blur or other creatures.'
...
def prepare():
    ...
    contacts = [dict(name='right_contact_v2',clip='move',source='art/units/source/generated/video_trials/emberpack_move_h3/key_guides/right_contact_v2.png',rects=[[0,0,1536,1024]],anchor=[750,954],scale=.195,alpha_noise_cutoff=8)]
    refs = []
    for original,filename in zip(contacts,['input_green.png']):
        frame = original.copy()
        original_scale = frame['scale']
        assert original_scale==.195
    ...
    graph = {
        ...
        '4':node('LoadImage',image='emberpack_walk_input.png'),
        ...
        '7':node('RandomNoise',noise_seed=2026092423),
        ...
        '14':node('SaveVideo',video=['13',0],filename_prefix='emberpack_move_h3/facing_v2/original',**{'format':'mp4','format.codec':'h264'}),
        '15':node('SaveImage',images=['11',0],filename_prefix='emberpack_move_h3/facing_v2/frames/decoded'),
    }
...
def submit():
    ...
    for key,filename in [('4','input_green.png')]:
        boundary='----EmberpackWalkH3'
        data=(f'--{boundary}\r\nContent-Disposition: form-data; name="image"; filename="emberpack_walk_{filename}"\r\nContent-Type: image/png\r\n\r\n'.encode()+(OUT/filename).read_bytes()+f'\r\n--{boundary}--\r\n'.encode())
    ...
    write(OUT/'workflow_api.json',graph)
    started=time.time();r=request(URL,'/prompt',dict(prompt=graph,client_id='emberpack_walk_h3'))
...
def review():
    target=ROOT/'.artifacts/emberpack_move_h3/facing_v2';target.mkdir(parents=True,exist_ok=True)
    for part in range(2):
...
def build():
    ...
    entry=dict(unit_id=UNIT,reference_height=256,source_facing='right',frames=frames,alpha_noise_cutoff=0,clips={'move':dict(indices=list(range(len(frames))),frame_msec=selection['frame_msec'],loop=True,static_frame=0)},source_scale_reason='Repaired contact uses .195 runtime scale to match original approximately180px body; video .45 input, extraction .195/.45, fixed anchor480,475. No per-pose normalization.',provenance=provenance,visual_review=dict(status='pending',notes=selection['review_note']))
    ...
    target=ROOT/'.artifacts/emberpack_move_h3/facing_v2';target.mkdir(parents=True,exist_ok=True);write(target/'combined_handoff.json',dict(schema_version=1,units=[candidate]))
