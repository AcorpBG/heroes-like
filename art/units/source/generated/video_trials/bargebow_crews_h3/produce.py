"""Rebuild original Bargebow Crews H3 takes and pending candidates."""
import argparse
import hashlib
import json
import sys
...
def prepare(out,c):
 ...
 graph={
  ...
  '14':node('SaveVideo',video=['13',0],filename_prefix=f'bargebow_crews_h3/{out.name}/original',**{'format':'mp4','format.codec':'h264'}),
  '15':node('SaveImage',images=['11',0],filename_prefix=f'bargebow_crews_h3/{out.name}/frames/decoded')}
 for i in range(len(refs)):graph[str(30+i)]=node('LoadImage',image=f'bargebow_crews_{out.name}_guide_{i}.png')
 previous='5'
...
def submit(out,c):
 ...
 for i in range(len(c['references'])):
  name=f'bargebow_crews_{out.name}_guide_{i}.png';boundary='----BargebowCrewsH3'
 ...
 write(out/'workflow_api.json',g);start=time.time();r=request(URL,'/prompt',dict(prompt=g,client_id='bargebow_crews_h3'))
...
def key(rgb):
 ...
 # This operator contains red, ivory, teal, brown, steel and brass but no green
...
def process(out,c):
 ...
 with av.open(str(out/'original_lossless.mkv')) as video:
  for i,frame in enumerate(video.decode(video=0)):
   ...
   if detail['mode']!='flat_green_chroma_unmix':raise ValueError('Backdrop changed to a color unsafe for red coat, teal sash, steel armor and wood/brass ballista; reject this interval')
   p=out/'matte'/f'rgba_{i:03}.png';im.save(p);hashes.append(sha(p));details.append(detail)
 assert len(hashes)==124
 write(out/'matte.json',dict(frames=124,fps=24,rgba_sha256=hashes,background_frames=details,recipe='Measured uniform green plate absent from bargebow operator palette; corner spread<=10, chroma separation>=80; color-specific alpha unmix and edge-only despill; alpha>=8 regions retained when containing alpha>=128 pixels. Original geometry unchanged.'))

def review(out,c):
 target=ROOT/'.artifacts/bargebow_crews_h3'/out.name;target.mkdir(parents=True,exist_ok=True)
 for part in range(2):
...
def assemble():
 ...
 unit['preserved_accepted_clips']=['idle'];unit['visual_review']=dict(status='pending',notes='Selected Lantern Sapper H3 actions; native review required. Preserve original accepted articulated idle.')
