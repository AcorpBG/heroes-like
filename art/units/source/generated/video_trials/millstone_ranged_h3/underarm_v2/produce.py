...
INPUT_SCALE = .95
RUNTIME_SCALE = .6
PROMPT = 'One continuous underarm sling throw by the same female slinger. From the ready stance she draws her throwing hand down and back beside her hip, then swings forward in one compact smooth arc at waist height, releases one cord, and naturally recovers and gathers the sling. Her other hand balances near her chest. The sling remains a palm-sized leather cup with two forearm-length cords. Never raise the throwing hand above the shoulder; never spin overhead or enlarge the sling. Keep face, clothing, pouches, two arms, two legs and planted boots stable. Clear shoulder, elbow and wrist articulation, natural anticipation and follow-through. No visible projectile. Fixed camera, no cuts, unchanged flat solid magenta background. Crisp individual poses without motion trails, motion blur or doubled outlines. The sling has exactly ONE small cup throughout; the release opens one cord, never a second cup.'
...
def prepare():
    ...
    contacts = [next(f for f in originals if f['name']=='ranged_00'),dict(name='prerelease',clip='ranged',source='art/units/source/generated/video_trials/millstone_ranged_h3/key_guides/backswing_v1.png',rects=[[0,0,1247,1261]],anchor=[770,1200],scale=.195,alpha_noise_cutoff=8),dict(name='release',clip='ranged',source='art/units/source/generated/video_trials/millstone_ranged_h3/key_guides/release_v1.png',rects=[[0,0,1247,1261]],anchor=[600,1200],scale=.195,alpha_noise_cutoff=8),dict(name='swing',clip='ranged',source='art/units/source/generated/video_trials/millstone_ranged_h3/key_guides/swing_v1.png',rects=[[0,0,1239,1270]],anchor=[770,1200],scale=.195,alpha_noise_cutoff=8)]
    refs = []
    for original,filename in zip(contacts,['input_magenta.png','peak_magenta.png','release_magenta.png','swing_magenta.png']):
        frame = original.copy()
    ...
    graph = {
        ...
        '21':node('LoadImage',image='millstone_swing.png'),
        '22':node('MiniMaxH3AddGuide',positive=['17',0],latent=['5',1],frame_idx=42,vae=['3',0],image=['21',0]),
        '19':node('MiniMaxH3AddGuide',positive=['22',0],latent=['5',1],frame_idx=54,vae=['3',0],image=['18',0]),
        ...
        '7':node('RandomNoise',noise_seed=2026092502),
        ...
        '14':node('SaveVideo',video=['13',0],filename_prefix='millstone_ranged_h3/underarm_v2/original',**{'format':'mp4','format.codec':'h264'}),
        '15':node('SaveImage',images=['11',0],filename_prefix='millstone_ranged_h3/underarm_v2/frames/decoded'),
    }
...
def submit():
    ...
    for key,filename in [('4','input_magenta.png'),('16','peak_magenta.png'),('18','release_magenta.png'),('21','swing_magenta.png')]:
        boundary='----MillstoneH3H3'
        data=(f'--{boundary}\r\nContent-Disposition: form-data; name="image"; filename="millstone_ranged_finish_v2_{filename}"\r\nContent-Type: image/png\r\n\r\n'.encode()+(OUT/filename).read_bytes()+f'\r\n--{boundary}--\r\n'.encode())
...
def review():
    target=ROOT/'.artifacts/millstone_ranged_h3/underarm_v2';target.mkdir(parents=True,exist_ok=True)
    for part in range(2):
...
def build():
    ...
    entry=dict(unit_id=UNIT,reference_height=256,source_facing='right',frames=frames,alpha_noise_cutoff=0,clips={'ranged':dict(indices=list(range(len(frames))),frame_msec=selection['frame_msec'],loop=False,static_frame=0)},source_scale_reason='Legacy ready .58 and both original underarm guides .195 match anatomical standing height. Input scale is source scale/.6; extraction fixed .6; boot-ground anchors map to480,475. No per-frame normalization.',provenance=provenance,visual_review=dict(status='pending',notes=selection['review_note']))
    ...
    target=ROOT/'.artifacts/millstone_ranged_h3/underarm_v2';target.mkdir(parents=True,exist_ok=True);write(target/'combined_handoff.json',dict(schema_version=1,units=[candidate]))
