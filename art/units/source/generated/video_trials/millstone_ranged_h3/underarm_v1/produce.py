...
def prepare():
    ...
    graph = {
        ...
        '7':node('RandomNoise',noise_seed=2026092501),
        ...
        '14':node('SaveVideo',video=['13',0],filename_prefix='millstone_ranged_h3/underarm_v1/original',**{'format':'mp4','format.codec':'h264'}),
        '15':node('SaveImage',images=['11',0],filename_prefix='millstone_ranged_h3/underarm_v1/frames/decoded'),
    }
...
def submit():
    ...
    for key,filename in [('4','input_magenta.png'),('16','peak_magenta.png'),('18','release_magenta.png')]:
        ...
        data=(f'--{boundary}\r\nContent-Disposition: form-data; name="image"; filename="millstone_ranged_finish_{filename}"\r\nContent-Type: image/png\r\n\r\n'.encode()+(OUT/filename).read_bytes()+f'\r\n--{boundary}--\r\n'.encode())
...
def review():
    target=ROOT/'.artifacts/millstone_ranged_h3/underarm_v1';target.mkdir(parents=True,exist_ok=True)
    for part in range(2):
...
def build():
    ...
    target=ROOT/'.artifacts/millstone_ranged_h3/underarm_v1';target.mkdir(parents=True,exist_ok=True);write(target/'combined_handoff.json',dict(schema_version=1,units=[candidate]))
