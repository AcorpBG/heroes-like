...
def prepare():
    ...
    source='art/units/source/generated/fluid_animation/batch_b/unit_prism_adept/death_v2.png'
    contacts = [recipe['frames'][0].copy(),dict(name='grounded_corpse',clip='death',source=source,rects=[[570,1130,1136,1385]],anchor=[850,1332],scale=.54,alpha_noise_cutoff=8),dict(name='kneeling_support',clip='death',source=source,rects=[[0,490,565,830]],anchor=[290,804],scale=.54,alpha_noise_cutoff=8),dict(name='buckling',clip='death',source=source,rects=[[60,45,535,481]],anchor=[286,464],scale=.54,alpha_noise_cutoff=8),dict(name='side_lowering',clip='death',source=source,rects=[[570,490,1136,830]],anchor=[850,804],scale=.54,alpha_noise_cutoff=8),dict(name='elbow_rest',clip='death',source=source,rects=[[0,830,570,1120]],anchor=[290,1085],scale=.54,alpha_noise_cutoff=8)]
    refs = []
    for original,filename in zip(contacts,['input_green.png','corpse_green.png','kneel_green.png','buckle_green.png','side_green.png','elbow_green.png']):
        frame = original.copy()
    ...
    graph = {
        ...
        '20':node('LoadImage',image='prism_buckle.png'),
        '21':node('LoadImage',image='prism_side.png'),
        '22':node('LoadImage',image='prism_elbow.png'),
        '23':node('MiniMaxH3AddGuide',positive=['5',0],latent=['5',1],frame_idx=18,vae=['3',0],image=['20',0]),
        '24':node('MiniMaxH3AddGuide',positive=['23',0],latent=['5',1],frame_idx=34,vae=['3',0],image=['17',0]),
        '25':node('MiniMaxH3AddGuide',positive=['24',0],latent=['5',1],frame_idx=50,vae=['3',0],image=['21',0]),
        '26':node('MiniMaxH3AddGuide',positive=['25',0],latent=['5',1],frame_idx=66,vae=['3',0],image=['22',0]),
        '27':node('MiniMaxH3AddGuide',positive=['26',0],latent=['5',1],frame_idx=84,vae=['3',0],image=['16',0]),
        '6':node('BasicGuider',model=['1',0],conditioning=['27',0]),
        '7':node('RandomNoise',noise_seed=2026092436),
        ...
        '14':node('SaveVideo',video=['13',0],filename_prefix='prism_adept_h3/death_guided_v3/original',**{'format':'mp4','format.codec':'h264'}),
        '15':node('SaveImage',images=['11',0],filename_prefix='prism_adept_h3/death_guided_v3/frames/decoded'),
    }
...
def verify():
    ...
    assert graph['6']['inputs']['conditioning']==['27',0] and [graph[str(k)]['inputs']['frame_idx'] for k in range(23,28)]==[18,34,50,66,84]
...
def submit():
    ...
    for key,filename in [('4','input_green.png'),('16','corpse_green.png'),('17','kneel_green.png'),('20','buckle_green.png'),('21','side_green.png'),('22','elbow_green.png')]:
...
def review():
    target=ROOT/'.artifacts/prism_adept_h3/death_guided_v3';target.mkdir(parents=True,exist_ok=True)
    for part in range(2):
...
def build():
    ...
    target=ROOT/'.artifacts/prism_adept_h3/death_guided_v3';target.mkdir(parents=True,exist_ok=True);write(target/'combined_handoff.json',dict(schema_version=1,units=[candidate]))
