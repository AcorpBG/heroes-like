...
def prepare():
    ...
    contacts = [recipe['frames'][0].copy(),dict(name='grounded_corpse',clip='death',source='art/units/source/generated/fluid_animation/batch_b/unit_prism_adept/death_v2.png',rects=[[570,1130,1136,1385]],anchor=[850,1332],scale=.54,alpha_noise_cutoff=8)]
    refs = []
    for original,filename in zip(contacts,['input_green.png','corpse_green.png']):
        frame = original.copy()
    ...
    graph = {
        ...
        '7':node('RandomNoise',noise_seed=2026092434),
        ...
        '14':node('SaveVideo',video=['13',0],filename_prefix='prism_adept_h3/death_v1/original',**{'format':'mp4','format.codec':'h264'}),
        '15':node('SaveImage',images=['11',0],filename_prefix='prism_adept_h3/death_v1/frames/decoded'),
    }
...
def submit():
    ...
    for key,filename in [('4','input_green.png'),('16','corpse_green.png')]:
...
def review():
    target=ROOT/'.artifacts/prism_adept_h3/death_v1';target.mkdir(parents=True,exist_ok=True)
    for part in range(2):
...
def build():
    ...
    target=ROOT/'.artifacts/prism_adept_h3/death_v1';target.mkdir(parents=True,exist_ok=True);write(target/'combined_handoff.json',dict(schema_version=1,units=[candidate]))
