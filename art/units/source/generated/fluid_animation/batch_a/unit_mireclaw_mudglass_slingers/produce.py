"""Rebuild original Mudglass Slingers H3 takes and pending candidates."""
import argparse
import hashlib
import json
import sys
...
def prepare(out,c):
 ...
 graph={
  ...
  '14':node('SaveVideo',video=['13',0],filename_prefix=f'mudglass_slingers_h3/{out.name}/original',**{'format':'mp4','format.codec':'h264'}),
  '15':node('SaveImage',images=['11',0],filename_prefix=f'mudglass_slingers_h3/{out.name}/frames/decoded')}
 if c.get('tiled_decode'):
  graph['11']=node('VAEDecodeTiled',samples=['10',0],vae=['3',0],**c['tiled_decode'])
 for i in range(len(refs)):graph[str(30+i)]=node('LoadImage',image=f'mudglass_slingers_{out.name}_guide_{i}.png')
 previous='5'
...
def submit(out,c):
 ...
 for i in range(len(c['references'])):
  name=f'mudglass_slingers_{out.name}_guide_{i}.png'
  if c.get('reuse_uploaded_guides'):
   ...
   assert not item.get('subfolder') and name.startswith('mudglass_slingers_') and '/' not in name and '\\' not in name
 ...
 write(out/'workflow_api.json',g);start=time.time();r=request(URL,'/prompt',dict(prompt=g,client_id='mudglass_slingers_h3'))
...
def key(rgb,c):
 ...
 # This slinger contains olive green, yellow reeds and brown leather/hair but no magenta
 ...
 # Original opaque clothing has at most 15 green-channel excess. A 32-unit
 ...
 # keeping fine sling cords, hair, reeds and equipment edges attached
...
def review(out,c):
 target=ROOT/'.artifacts/mudglass_slingers_h3'/out.name;target.mkdir(parents=True,exist_ok=True)
