...
for source,cols,rows,scale,names in [('move_v1',2,4,.61,['move']*8),('attack_v1',2,4,.61,['attack']*8),('reactions_v1',2,4,.61,['hit']*4+['defend']*4),('death_v1',2,4,.55,['death']*8),('cast_v1',2,4,.61,['cast']*8)]:
...
replacement=D/'attack_anticipation_v1.png'
replacement_components,_=components(replacement)
assert len(replacement_components)==1
c=replacement_components[0]
rel=replacement.relative_to(ROOT).as_posix(); scales[rel]=.184
frames[clips['attack']['indices'][1]]={'name':'attack_01','clip':'attack','source':rel,'rects':rectangles(c),'anchor':[700,c[3]],'scale':.184}
provenance.append({'source':rel,'sha256':hashlib.sha256(replacement.read_bytes()).hexdigest(),'prompt':replacement.with_suffix('.prompt.txt').relative_to(ROOT).as_posix(),'prompt_sha256':hashlib.sha256(replacement.with_suffix('.prompt.txt').read_bytes()).hexdigest()})
...
unit={'unit_id':uid,'reference_height':256,'source_facing':'right','alpha_noise_cutoff':8,'frames':frames,'clips':clips,'accepted_clips':[],'preserve_clips':['idle'],'source_scale_by_image':scales,'source_scale_reason':'One fixed scale per original source; normal 360-pixel hood-to-foot body at .61 matches inherited 221-pixel idle. Larger 400-pixel death standing figure uses .55. Single regenerated anticipation source at 1190 pixels uses .184. No per-pose size normalization.','visual_review':{'status':'pending','notes':'Artist inspected all 40 new poses plus existing eight-pose idle: original articulates both hands lifting/gathering chain and bell, retained pending root motion review. New actions preserve crescent hook and bell, one chain lash, physical bell support and monotonic collapse. Attack_v1 pose2 missing hook replaced by new original attack_anticipation_v1 painting; attack_v2 edit rejected for alpha contamination.'}}
