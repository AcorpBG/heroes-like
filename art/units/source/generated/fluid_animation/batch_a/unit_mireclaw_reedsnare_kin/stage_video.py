...
def run(take):
 ...
 if not (out/'sampling_submission.json').exists():
  ...
  graph['16']=p.node('SaveLatent',samples=['17',0],filename_prefix=f'reedsnare_kin_h3/{take}/sampled')
 ...
 assert loc['type']=='output' and loc['subfolder'].replace('\\','/').startswith(f'reedsnare_kin_h3/{take}')
 relative=PurePosixPath(loc['subfolder'].replace('\\','/'))/loc['filename']
 assert '..' not in relative.parts and relative.is_relative_to(PurePosixPath('reedsnare_kin_h3')/take)
 ...
 if not (out/'decode_submission.json').exists():
  ...
  graph={
   ...
   '14':p.node('SaveVideo',video=['13',0],filename_prefix=f'reedsnare_kin_h3/{take}/original',**{'format':'mp4','format.codec':'h264'}),
   '15':p.node('SaveImage',images=['11',0],filename_prefix=f'reedsnare_kin_h3/{take}/frames/decoded')}
  q=p.request(p.URL,'/queue');assert not q['queue_running'] and not q['queue_pending']
  p.write(out/'workflow_api.json',graph)
  result=p.request(p.URL,'/prompt',dict(prompt=graph,client_id='reedsnare_kin_h3'))
