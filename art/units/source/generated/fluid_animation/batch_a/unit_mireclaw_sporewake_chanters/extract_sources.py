...
def components(path):
    ...
    big=sorted(data,key=lambda r:r['area'],reverse=True)[:8]
    # The largest eight connected figures define the original sheet order.
    big.sort(key=lambda d:d['box'][1])
    ordered=[]
    for n in range(0,8,2): ordered.extend(sorted(big[n:n+2],key=lambda d:d['box'][0]))
    for item in data:
...
if __name__=='__main__':
    import json
    out={stem:components(HERE/(stem+'.png')) for stem in ['idle_v1','attack_v1','reactions_v1','death_v1','cast_v1','ranged_v1']}
