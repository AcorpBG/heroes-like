"""Rebuild original Shard Wardens H3 takes and pending candidates."""
import argparse
import hashlib
import json
import sys
...
def prepare(out,c):
 ...
 graph={
  ...
  '14':node('SaveVideo',video=['13',0],filename_prefix=f'shard_wardens_h3/{out.name}/original',**{'format':'mp4','format.codec':'h264'}),
  '15':node('SaveImage',images=['11',0],filename_prefix=f'shard_wardens_h3/{out.name}/frames/decoded')}
 if c.get('tiled_decode'):
  graph['11']=node('VAEDecodeTiled',samples=['10',0],vae=['3',0],**c['tiled_decode'])
 for i in range(len(refs)):graph[str(30+i)]=node('LoadImage',image=f'shard_wardens_{out.name}_guide_{i}.png')
 previous='5'
...
def submit(out,c):
 ...
 for i in range(len(c['references'])):
  name=f'shard_wardens_{out.name}_guide_{i}.png'
  if c.get('reuse_uploaded_guides'):
   ...
   assert not item.get('subfolder') and name.startswith('shard_wardens_') and '/' not in name and '\\' not in name
 ...
 write(out/'workflow_api.json',g);start=time.time();r=request(URL,'/prompt',dict(prompt=g,client_id='shard_wardens_h3'))
...
def key(rgb,c):
 ...
 # This original guard contains white/gold armor, blue/violet crystals and brown skin/hair but no saturated green
 ...
 # The protected foreground chroma band preserves muted body/rope colors. The
 # reviewed 32-unit band removes green plate spill from fine edges while
 ...
 # keeping fine shield points, mace ends, fingers and hanging tabard strips attached
...
def review(out,c):
 target=ROOT/'.artifacts/shard_wardens_h3'/out.name;target.mkdir(parents=True,exist_ok=True)
 bounds=[Image.open(out/'matte'/f'rgba_{i:03}.png').getbbox() for i in range(124)]
...
def assemble():
 ...
 unit['preserved_accepted_clips']=['idle'];unit['visual_review']=delivery.get('visual_review',dict(status='pending',notes='Selected Shard Wardens H3 actions; native review required. Six dedicated actions; preserve the reviewed original eight-pose idle.'))
