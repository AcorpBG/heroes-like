"""Rebuild original Sporeglass Menders H3 takes and pending candidates."""
import argparse
import hashlib
import json
import sys
...
def prepare(out,c):
 ...
 graph={
  ...
  '14':node('SaveVideo',video=['13',0],filename_prefix=f'sporeglass_h3/{out.name}/original',**{'format':'mp4','format.codec':'h264'}),
  '15':node('SaveImage',images=['11',0],filename_prefix=f'sporeglass_h3/{out.name}/frames/decoded')}
 if c.get('tiled_decode'):
  graph['11']=node('VAEDecodeTiled',samples=['10',0],vae=['3',0],**c['tiled_decode'])
 for i in range(len(refs)):graph[str(30+i)]=node('LoadImage',image=f'sporeglass_{out.name}_guide_{i}.png')
 previous='5'
...
def submit(out,c):
 ...
 for i in range(len(c['references'])):
  name=f'sporeglass_{out.name}_guide_{i}.png'
  if c.get('reuse_uploaded_guides'):
   ...
   assert not item.get('subfolder') and name.startswith('sporeglass_') and '/' not in name and '\\' not in name
 ...
 write(out/'workflow_api.json',g);start=time.time();r=request(URL,'/prompt',dict(prompt=g,client_id='sporeglass_h3'))
...
def key(rgb,c):
 ...
 if c.get('key_rgb')==[0,0,255]:
  # This creature has green/brown/ivory clothing and grey steel, not saturated blue.
  # Use a separate measured blue plate so green herbs and red ties are preserved.
 ...
 # This original mender wears green/brown/ivory, with no saturated magenta
 ...
 # keeping fine twigs, sickle blade and basket straps attached
...
def review(out,c):
 target=ROOT/'.artifacts/sporeglass_h3'/out.name;target.mkdir(parents=True,exist_ok=True)
...
def assemble():
 ...
 unit['preserved_accepted_clips']=['idle'];unit['visual_review']=delivery.get('visual_review',dict(status='pending',notes='Selected Sporeglass Menders H3 actions; native review required. Six dedicated actions; preserve the reviewed original eight-pose articulated idle.'))
