...
for stem,rows,scale,count,parts in [
 ('idle_v2',2,.49,8,[('idle',0,8,135,True)]),
 ('move_v2',2,.58,8,[('move',0,8,110,True)]),
 ('attack_v4',4,.82,8,[('attack',0,8,95,False)]),
 ('cast_v1',2,.49,8,[('cast',0,8,110,False)]),
 ('hit_defend_v1',2,.50,8,[('hit',0,4,90,False),('defend',4,8,120,False)]),
 ('death_v1',4,.50,8,[('death',0,8,125,False)]),
]:
...
unit={'unit_id':'unit_shard_guard_veteran','reference_height':256,'source_facing':'right','frames':frames,'clips':clips,'accepted_clips':[],'preserve_clips':[],'source_scale_by_image':{f['source']:f['scale'] for f in frames},'source_scale_reason':'One rigid source-specific normalization matching roughly210px standing armored body; layout corrections changed source figure resolution. Never per-pose fit.','visual_review':{'status':'pending','notes':'Original dedicated sentinel actions. Glaive layout corrected to prevent clipping. Full eight-frame march requires root gait continuity review; shield/tabard partly obscure knee crossing. Death keeps armor, pavise and weapon.'}}
...
src=ROOT/'art/animation/source/poses/unit_shard_guard_veteran/unit_shard_guard_veteran_source-alpha.png'
(HERE/'identity_reference.recipe.json').write_text(json.dumps({'operation':'crop_only','source':src.relative_to(ROOT).as_posix(),'source_sha256':hashlib.sha256(src.read_bytes()).hexdigest(),'rect':[0,0,700,625],'output_sha256':hashlib.sha256((HERE/'identity_reference.png').read_bytes()).hexdigest()},indent=2)+'\n',encoding='utf-8')
