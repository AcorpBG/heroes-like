"""Rebuild original Graft Matriarchs H3 takes and pending candidates."""
import argparse
import hashlib
import json
import sys
...
def prepare(out,c):
 ...
 graph={
  ...
  '14':node('SaveVideo',video=['13',0],filename_prefix=f'graftmatriarch_h3/{out.name}/original',**{'format':'mp4','format.codec':'h264'}),
  '15':node('SaveImage',images=['11',0],filename_prefix=f'graftmatriarch_h3/{out.name}/frames/decoded')}
 if c.get('tiled_decode'):
  graph['11']=node('VAEDecodeTiled',samples=['10',0],vae=['3',0],**c['tiled_decode'])
 for i in range(len(refs)):graph[str(30+i)]=node('LoadImage',image=f'graftmatriarch_{out.name}_guide_{i}.png')
 previous='5'
...
def submit(out,c):
 ...
 for i in range(len(c['references'])):
  name=f'graftmatriarch_{out.name}_guide_{i}.png'
  if c.get('reuse_uploaded_guides'):
   ...
   assert not item.get('subfolder') and name.startswith('graftmatriarch_') and '/' not in name and '\\' not in name
 ...
 write(out/'workflow_api.json',g);start=time.time();r=request(URL,'/prompt',dict(prompt=g,client_id='graftmatriarch_h3'))
...
def key(rgb,c):
 ...
 if c.get('key_rgb')==[0,0,255]:
  # This creature has green/brown/ivory bark and red bindings, not saturated blue.
  # Use a separate measured blue plate so green bark and red cords are preserved.
  bgc=bg[2]-max(bg[0],bg[1])
  if bgc<80:raise ValueError(f'unsafe blue backdrop {bg}')
  chroma=a[:,:,2]-np.maximum(a[:,:,0],a[:,:,1]);protected=float(c.get('protected_foreground_chroma',20))
  ...
  color[:,:,2]-=np.maximum(color[:,:,2]-np.maximum(color[:,:,0],color[:,:,1]),0)*(alpha<.98)
 ...
 # This original matriarch has green/brown/ivory bark, with no saturated magenta
 ...
 # The protected foreground chroma band preserves muted armor and lens colors. The
 # unit-specific measured band removes green plate spill from fine edges while
 ...
 # keeping fine crown twigs, bowstring and spreading root tips attached
...
def process(out,c):
 if (out/'extraction_settings.json').exists():
  ...
  c=dict(c,protected_foreground_chroma=settings['protected_foreground_chroma'])
 ...
 with av.open(str(out/'original_lossless.mkv')) as video:
  for i,frame in enumerate(video.decode(video=0)):
   ...
   expected='flat_blue_chroma_unmix' if c.get('key_rgb')==[0,0,255] else ('flat_green_chroma_unmix' if c.get('key_rgb')==[0,255,0] else 'flat_magenta_chroma_unmix')
 ...
 write(out/'matte.json',dict(frames=124,fps=24,rgba_sha256=hashes,background_frames=details,protected_foreground_chroma=c.get('protected_foreground_chroma',0),recipe='Measured uniform guide plate; corner spread<=10, chroma separation>=80; protected foreground chroma band, color-specific alpha unmix and edge-only despill; alpha>=8 regions retained when containing alpha>=128 pixels. Original geometry unchanged.'))

def review(out,c):
 target=ROOT/'.artifacts/graftmatriarch_h3'/out.name;target.mkdir(parents=True,exist_ok=True)
 available=[i for i,h in enumerate(json.loads((out/'matte.json').read_bytes())['rgba_sha256']) if h]
 bounds=[Image.open(out/'matte'/f'rgba_{i:03}.png').getbbox() for i in available]
 assert all(bounds),'Empty extracted creature frame'
 crop=(max(0,min(b[0] for b in bounds)-12),max(0,min(b[1] for b in bounds)-12),min(960,max(b[2] for b in bounds)+12),min(544,max(b[3] for b in bounds)+12))
 for part in range((len(available)+61)//62):
  ...
  for j,i in enumerate(available[part*62:(part+1)*62]):
   ...
   im=Image.open(out/'matte'/f'rgba_{i:03}.png').crop(crop);im.thumbnail((196,195),Image.Resampling.LANCZOS);sheet.paste(im,(x+(200-im.width)//2,y+22),im);d.text((x+5,y+4),str(i),fill=(160,110,65))
...
def build(out,c):
 ...
 for frame in frames:
  refinement=s.get('original_pixel_exclusions',{}).get(str(frame['video_frame']))
  if refinement:
   # Explicitly reviewed, disconnected background fragments only. Preserve all
   # subject pixels and the original unmodified matte, anchor and scale.
   x0,y0,x1,y1=refinement['background_rect']
   assert 0<x0<x1<960 and 0<y0<y1<544
   source=Image.open(ROOT/frame['source']);pixels=np.asarray(source)[:,:,3]>=8
   labels,count=label(pixels,structure=np.ones((3,3),dtype=np.uint8))
   sizes=np.bincount(labels.ravel());sizes[0]=0;body=int(sizes.argmax())
   assert count>1 and not np.any(labels[y0:y1,x0:x1]==body)
   assert 0<int(pixels[y0:y1,x0:x1].sum())<=150
   frame['rects']=[[0,0,960,y0],[0,y0,x0,y1],[x1,y0,960,y1],[0,y1,960,544]]
   frame['crop_recipe']=dict(tool='produce.py build',**refinement)
 clip=dict(indices=list(range(len(indices))),frame_msec=s['frame_msec'],loop=c['clip'] in ['move','idle'],static_frame=len(indices)-1 if c['clip'] in ['death','defend'] else 0)
 ...
 for n in ['runtime_profile.json','extraction_settings.json','extract_ranges.json','staged_generation.json','sampling_workflow_api.json','sampling_submission.json','sampling_history.json','original.latent']:
...
def assemble():
 ...
 for name,sequence in delivery.get('clip_sequences',{}).items():
  ...
  assembled['clips']={name:dict(sequence['timing'],indices=list(range(len(assembled['frames']))),loop=False,static_frame=(len(assembled['frames'])-1 if name in ['death','defend'] else 0))}
 ...
 unit['preserved_accepted_clips']=['idle'];unit['visual_review']=delivery.get('visual_review',dict(status='pending',notes='Selected Graft Matriarchs H3 actions; native review required. Seven dedicated actions; preserve the reviewed original eight-pose articulated idle.'))
