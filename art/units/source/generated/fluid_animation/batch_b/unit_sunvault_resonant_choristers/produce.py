"""Rebuild original Resonant Choristers H3 takes and pending candidates."""
import argparse
import hashlib
import json
import sys
...
def prepare(out,c):
 ...
 graph={
  ...
  '14':node('SaveVideo',video=['13',0],filename_prefix=f'resonant_choristers_h3/{out.name}/original',**{'format':'mp4','format.codec':'h264'}),
  '15':node('SaveImage',images=['11',0],filename_prefix=f'resonant_choristers_h3/{out.name}/frames/decoded')}
 if c.get('tiled_decode'):
  graph['11']=node('VAEDecodeTiled',samples=['10',0],vae=['3',0],**c['tiled_decode'])
 for i in range(len(refs)):graph[str(30+i)]=node('LoadImage',image=f'resonant_choristers_{out.name}_guide_{i}.png')
 previous='5'
...
def submit(out,c):
 ...
 for i in range(len(c['references'])):
  name=f'resonant_choristers_{out.name}_guide_{i}.png'
  if c.get('reuse_uploaded_guides'):
   ...
   assert not item.get('subfolder') and name.startswith('resonant_choristers_') and '/' not in name and '\\' not in name
 ...
 write(out/'workflow_api.json',g);start=time.time();r=request(URL,'/prompt',dict(prompt=g,client_id='resonant_choristers_h3'))
...
def key(rgb,c):
 ...
 # This original trio contains white/gold/blue robes, blue/violet crystals and varied skin/hair but no saturated green
 ...
 # The protected foreground chroma band preserves muted coat/instrument colors. The
 # reviewed 56-unit band removes green plate spill from fine edges while
 ...
 # keeping fine staff points, instrument rims, fingers and hanging coat strips attached
...
def review(out,c):
 target=ROOT/'.artifacts/resonant_choristers_h3'/out.name;target.mkdir(parents=True,exist_ok=True)
...
def assemble():
 ...
 unit['preserved_accepted_clips']=['idle'];unit['visual_review']=delivery.get('visual_review',dict(status='pending',notes='Selected Resonant Choristers H3 actions; native review required. Seven dedicated actions; preserve the reviewed original eight-pose idle.'))
