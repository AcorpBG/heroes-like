...
for stem,rows,scale,count,parts in [
 ('idle_v1',2,.45,8,[('idle',0,8,135,True)]),
 ('move_v1',2,.45,8,[('move',0,4,110,True)]),
 ('move_opposite_half_v1',2,.35,4,[('move',0,4,110,True)]),
 ('attack_v1',2,.45,8,[('attack',0,8,95,False)]),
 ('ranged_v1',2,.45,8,[('ranged',0,8,95,False)]),
 ('cast_v2',2,.45,8,[('cast',0,8,110,False)]),
 ('hit_defend_v1',2,.45,8,[('hit',0,4,90,False),('defend',4,8,120,False)]),
 ('death_v2',4,.55,8,[('death',0,8,125,False)]),
]:
...
unit={'unit_id':'unit_prism_adept_veteran','reference_height':256,'source_facing':'right','frames':frames,'clips':clips,'accepted_clips':[],'preserve_clips':[],'source_scale_by_image':{f['source']:f['scale'] for f in frames},'source_scale_reason':'One rigid factor per original generated sheet matches approximately205px standing body; movement second half has larger original source resolution and death layout revision smaller source figures. Never pose-dependent scale fitting.','visual_review':{'status':'pending','notes':'Original archer poses. Corrected cast arm ownership and death clipping. Movement uses first four original poses followed by newly authored opposite-half poses; root must assess complete left-right gait continuity. The rejected eight-frame movement retries repeated similar halves. Ranged projectile is explicitly assigned to release frame5 despite crossing invisible cell boundary.'}}
...
src=ROOT/'art/animation/source/poses/unit_prism_adept_veteran/unit_prism_adept_veteran_source-alpha.png'
