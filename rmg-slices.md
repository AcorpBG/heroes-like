...
1. **Inspection summary export**
   ...
   - Historical done evidence: `.artifacts/rmg_small_manual_inspection_summary.md` was generated for the strict Small owner-review set and reported 3/3 cases ok, 3/3 editor loads ok, 3/3 maps-folder index hits, zero unguarded route links, and validator-gated 36x36 topology previews for all three packages.
   - Current tooling rule: rerun RMG export/report flows through Python-owned helpers, starting with `python3 tools/rmg_native_batch_export.py`; do not restore the removed GDScript export/report launchers.
...
1. **Manual inspection package export**
   ...
   - Repeat generated-package export through `python3 tools/rmg_native_batch_export.py` with the matching controlled/manual cases, then inspect the emitted `.amap` / `.ascenario` packages and Python summaries.
