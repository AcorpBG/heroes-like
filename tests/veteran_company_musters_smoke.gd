...
func _run() -> void:
	...
	_expect(atlas != null and atlas.get_size() == Vector2(2304, 192), "The twelve-state recovered veteran-muster atlas must remain exactly 2304x192.")
...
func _run_case(view: Control, case: Dictionary) -> void:
	...
	# These same authored operations were subsequently adopted by Six Sealed
	# Companies. Preserve that production availability and its sixth witness hook.
	var direct_lead: bool = String(session.hero_id) == String(case.get("hero_id", "")) and String(scenario.get("player_faction_id", "")) == String(case.get("faction_id", "")) and String(scenario.get("player_army_id", "")) == String(case.get("army_id", "")) and scenario.get("selection", {}).get("availability", {}) == {"campaign":true,"skirmish":true}
	_expect(direct_lead, "%s did not launch its exact campaign/skirmish hero, faction, and lean company." % scenario_id)
	var map_size: Dictionary = scenario.get("map_size", {})
	_expect(int(map_size.get("width", 0)) == 15 and int(map_size.get("height", 0)) == 9 and scenario.get("towns", []).size() == 2 and scenario.get("resource_nodes", []).size() == 7 and scenario.get("encounters", []).size() == 3 and scenario.get("script_hooks", []).size() == 6, "%s lost its exact 15x9 veteran-operation composition." % scenario_id)
