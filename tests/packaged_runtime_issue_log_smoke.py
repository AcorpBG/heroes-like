...
ARTIFACT_DIR = ROOT / ".artifacts" / "packaged_runtime_issue_log_smoke"
PACK_PATH = ARTIFACT_DIR / "heroes-like.pck"
REPORT_PATH = ARTIFACT_DIR / "report.json"
SCENE_REPORT_PATH = ARTIFACT_DIR / "scene_report.json"
REPORT_ID = "PACKAGED_RUNTIME_ISSUE_LOG_SMOKE"
SCHEMA_ID = "packaged_runtime_issue_log_smoke_v1"
PRESET_NAME = "Linux Release"
PACKAGED_SCENE = "res://tests/packaged_runtime_issue_log_report.tscn"
...
def output_summary(output: str) -> dict:
    ...
    script_errors = [line for line in lines if "SCRIPT ERROR" in line or "Parse Error" in line]
    return {
        "line_count": len(lines),
        "warning_count": sum(1 for line in lines if "WARNING:" in line),
        "error_like_count": len(script_errors),
        "fatal_pattern_matches": [pattern for pattern in FATAL_PATTERNS if pattern in output],
        "head": lines[:20],
        "tail": lines[-80:],
        "error_like_tail": script_errors[-20:],
    }
