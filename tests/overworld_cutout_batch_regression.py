...
def expected_assets(recipe, expected_dir, output):
    ...
    for key,row in recipe['assets'].items():
        ...
        claimed=route_arcane or recipe.get('schema_id') in ('claimed_dwelling_cutout_recipe_v1','early_state_cutout_recipe_v1','landmark_state_cutout_recipe_v1','command_site_cutout_recipe_v1','training_site_cutout_recipe_v1')
...
def main():
    ...
    parser.add_argument('--batch',choices=['batch04','map_sheets','decorations','legacy_families','passages','recurring_encounters','recurring_sites','claimed_dwellings','early_states','landmark_states','route_arcane','command_sites','recruitment_sites','training_sites'],default='batch04')
    ...
    if args.batch in ('recurring_encounters','recurring_sites','claimed_dwellings','early_states','command_sites','recruitment_sites'):
        captures_ok=captures_ok and len(report.get('galleries',[]))==3 and (report.get('backend')=='headless' or all((output/name).exists() for name in report['galleries']))
    if args.batch in ('landmark_states','training_sites'):
