...
def expected_assets(recipe, expected_dir, output):
    ...
    for key,row in recipe['assets'].items():
        ...
        remaining=recipe.get('schema_id') in ('remaining_encounter_cutout_recipe_v1','artifact_cutout_recipe_v1','hero_cutout_recipe_v1')
...
def main():
    ...
    parser.add_argument('--batch',choices=['batch04','map_sheets','decorations','legacy_families','passages','recurring_encounters','recurring_sites','claimed_dwellings','early_states','landmark_states','route_arcane','command_sites','recruitment_sites','training_sites','contract_encounters','remaining_encounters','artifacts','remaining_sites','heroes'],default='batch04')
