...
const REPORT_ID := "BATTLE_CLAUSEWORKS_BEACON_WARDENS_PRODUCTION_LINE_REPORT"
const SCENARIO_ID := "clauseworks-counterclaim"
const PLACEMENT_ID := "clauseworks_beacon_wardens"
const SAVE_SLOT := 7
const PRODUCTION_STACKS := [
	{"unit_id": "unit_embercourt_lantern_sappers", "count": 6},
	{"unit_id": "unit_embercourt_fordhook_cadets", "count": 7},
	{"unit_id": "unit_embercourt_bargebow_crews", "count": 2},
]
const LEGACY_STACKS := [
...
func _run() -> void:
	...
	var bridge_before: Dictionary = _scenario_encounter(scenario, "clauseworks_bridge_levies").duplicate(true)
	...
	if (
		String(encounter.get("encounter_id", "")) != "encounter_beacon_wardens"
		or String(encounter.get("difficulty", "")) != "high"
		or int(encounter.get("combat_seed", 0)) != 21103
		or Vector2i(int(encounter.get("x", -1)), int(encounter.get("y", -1))) != Vector2i(8, 0)
		or String(enemy_army.get("id", "")) != "army_clauseworks_beacon_wardens"
		...
		_fail("Clauseworks Beacon Wardens production identity or objective authority drifted.", {"encounter": encounter})
		return
	...
	var expected_abilities := {
		"unit_embercourt_lantern_sappers": ["counter_ambush_flare"],
	...
	if session.to_dict() != session_authority_before or _battle_enemy_stack_contract(battle_payload) != PRODUCTION_STACKS or _battle_enemy_ability_contract(battle_payload) != expected_abilities:
		_fail("Public battle payload missed the exact Clauseworks production line or mutated the session.", {"stacks": _battle_enemy_stack_contract(battle_payload), "abilities": _battle_enemy_ability_contract(battle_payload)})
		return
	if _save_restore(session) == null:
		return
	if _stack_health(PRODUCTION_STACKS) != 142 or _stack_health(LEGACY_STACKS) != 137:
		_fail("Clauseworks production/legacy stack-health contract drifted.", {})
		return
	...
	legacy_army["id"] = "army_clauseworks_beacon_wardens_legacy_control"
	...
	if (
		not _sample_exact(production_normal, 3, 69, 15)
		or not _sample_exact(production_hard, 4, 42, 25)
		or not _sample_exact(legacy_normal, 4, 45, 23)
		or not _sample_exact(legacy_hard, 3, 55, 24)
		or _scenario_encounter(scenario, "clauseworks_archive_wardens") != archive_before
		or _scenario_encounter(scenario, "clauseworks_bridge_levies") != bridge_before
		...
		_fail("Clauseworks production, legacy, adjacent-front, or scenario contract drifted.", {"production_normal": _compact_sample(production_normal), "production_hard": _compact_sample(production_hard), "legacy_normal": _compact_sample(legacy_normal), "legacy_hard": _compact_sample(legacy_hard)})
		return
...
func _objective_exact(scenario: Dictionary) -> bool:
	for value in scenario.get("objectives", {}).get("victory", []):
		if value is Dictionary and String(value.get("id", "")) == "clear_clauseworks_beacon_wardens":
			return value == {"id": "clear_clauseworks_beacon_wardens", "label": "Clear the Clauseworks Beacon Wardens", "type": "encounter_resolved", "placement_id": PLACEMENT_ID}
	return false
...
func _save_restore(session: SessionStateStore.SessionData) -> SessionStateStore.SessionData:
	...
	if not bool(save_result.get("ok", false)):
		_fail("Clauseworks runtime save failed.", {"save_result": save_result})
		return null
	...
	if restored == null:
		_fail("Clauseworks runtime save did not restore.", {})
		return null
	...
	if _session_signature(restored) != signature_before or String(summary.get("resume_target", "")) != "overworld":
		_fail("Clauseworks runtime save/restore authority drifted.", {"before": signature_before, "after": _session_signature(restored), "summary": summary})
		return null
	return restored
...
func _outcome_contract() -> Dictionary:
	...
	victory.overworld["resolved_encounters"] = ["clauseworks_archive_wardens", "clauseworks_bridge_levies", PLACEMENT_ID]
	...
	if String(victory_result.get("status", "")) != "victory" or String(defeat_result.get("status", "")) != "defeat":
		_fail("Clauseworks victory/defeat boundaries are not independently reachable.", {"victory": victory_result, "defeat": defeat_result})
		return {}
...
func _sample_exact(sample: Dictionary, rounds: int, player_health: int, enemy_damage: int) -> bool:
	return bool(sample.get("completed", false)) and String(sample.get("outcome_state", "")) == "victory" and String(sample.get("pacing_band", "")) == "standard" and int(sample.get("round_reached", 0)) == rounds and int(sample.get("player_health_remaining_pct", -1)) == player_health and int(sample.get("enemy_health_remaining_pct", -1)) == 0 and int(sample.get("damage_per_round", {}).get("enemy", -1)) == enemy_damage and int(sample.get("invalid_order_count", -1)) == 0
