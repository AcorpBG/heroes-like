...
func _run_viewport(viewport_size: Vector2i) -> Dictionary:
	...
	for profile_value in fallback_profiles:
		...
		if String(profile.get("town_placement_id", "")) == String(first_town.get("placement_id", "")):
			fallback_exact = String(profile.get("faction_id", "")) == "" and String(profile.get("sprite_asset_id", "")) == "frontier_town" and bool(profile.get("uses_default_sprite", false)) and not bool(profile.get("uses_faction_sprite", true))
			break
...
func _validate_profiles(shell: Node, profiles: Array) -> Dictionary:
	...
	return {
		"ok": seen_factions.size() == EXPECTED_FACTION_ASSETS.size() and seen_assets.size() == EXPECTED_FACTION_ASSETS.size() and visible_asset_exact and footprint_authority_exact and owner_pennant_exact,
