...
func _catalog_contract() -> Dictionary:
	...
	return {
		"ok": target_count == _target_building_ids().size() and specific_count == 48 and fallback_count == 85 and fallback_exact and source_hashes.size() == _target_building_ids().size() and icon_hashes.size() == _target_building_ids().size() and icon_paths.size() == _target_building_ids().size() and _unique(source_hashes) and _unique(icon_hashes) and _unique(icon_paths),
...
func _live_case(viewport_size: Vector2i) -> Dictionary:
	...
	var fallback_id := "building_embercourt_granary_lock_exchange"
