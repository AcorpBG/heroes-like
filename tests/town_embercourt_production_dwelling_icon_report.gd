...
func _catalog_contract() -> Dictionary:
	...
	return {
		"ok": target_count == _target_building_ids().size() and specific_count == 69 and fallback_count == 64 and fallback_exact and source_hashes.size() == _target_building_ids().size() and icon_hashes.size() == _target_building_ids().size() and icon_paths.size() == _target_building_ids().size() and _unique(source_hashes) and _unique(icon_hashes) and _unique(icon_paths),
