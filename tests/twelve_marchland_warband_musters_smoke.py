#!/usr/bin/env python3
"""Run the one consolidated Twelve Marchland Warband Musters smoke."""
...
REPORT_ID = "TWELVE_MARCHLAND_WARBAND_MUSTERS_SMOKE"
REPORT_PATH = ROOT / ".artifacts/twelve_marchland_warband_musters_smoke/report.json"
CAPTURE_DIR = ROOT / ".artifacts/twelve_marchland_warband_musters_smoke/captures"
MANIFEST = ROOT / "art/overworld/source/generated/resource_sites/marchland_warband_musters_wave1/manifest.json"
SCENE = "res://tests/twelve_marchland_warband_musters_smoke.tscn"


def verify_manifest() -> None:
    manifest = json.loads(MANIFEST.read_text(encoding="utf-8"))
    if manifest.get("generation_mode") != "built_in_imagegen" or len(manifest.get("items", [])) != 12:
        raise SystemExit("unexpected Marchland-warband generated-source provenance")
...
def main() -> int:
    ...
    env = os.environ.copy()
    env["TWELVE_MARCHLAND_WARBAND_CAPTURE_DIR"] = str(CAPTURE_DIR)
    ...
    expected = {"ok":True,"case_count":12,"exact_launch_count":12,"production_battle_count":48,"production_claim_count":12,"exact_art_count":12,"objective_victory_count":12,"save_round_trip_count":12,"capture_count":12,"save_version":9,"single_consolidated_smoke":True}
    failures = [f"{key}={report.get(key)!r}, expected {value!r}" for key, value in expected.items() if report.get(key) != value]
    if len(report.get("rows", [])) != 12 or any(int(row.get("completion_day", 99)) >= 21 for row in report.get("rows", [])):
        failures.append("all twelve rows must win and save exactly before their Day 21 or Day 24 deadline")
    captures = sorted(CAPTURE_DIR.glob("*.png"))
    if len(captures) != 12:
        failures.append(f"capture_count={len(captures)}, expected 12")
    if failures:
