...
const REPORT_ID := "BATTLE_RIVER_PASS_HOLLOW_MIRE_PRODUCTION_LINE_REPORT"
const SCENARIO_ID := "river-pass"
const PLACEMENT_ID := "river_pass_hollow_mire"
...
func _run() -> void:
	...
	if (
		String(encounter.get("encounter_id", "")) != "encounter_hollow_mire"
		or String(encounter.get("difficulty", "")) != "medium"
		or int(encounter.get("combat_seed", 0)) != 1202
		or Vector2i(int(encounter.get("x", -1)), int(encounter.get("y", -1))) != Vector2i(6, 4)
		or String(enemy_army.get("id", "")) != "army_river_pass_hollow_mire_watch"
		or String(enemy_army.get("faction_id", "")) != "faction_mireclaw"
		or _stack_contract(enemy_army.get("stacks", [])) != PRODUCTION_STACKS
		or guard_link != {"guard_role": "route_block", "target_kind": "route", "target_id": "river_pass_mire_lane", "target_placement_id": "", "blocks_approach": true, "clear_required_for_target": false}
		or String(encounter.get("object_id", "")) != "object_neutral_encounter_river_pass_hollow_mire_stack"
		or String(encounter.get("object_placement_id", "")) != "object_placement_river_pass_hollow_mire"
		or String(encounter.get("primary_class", "")) != "neutral_encounter"
		or _guarded_resource_ids(scenario, PLACEMENT_ID) != ["duskfen_bastion_peatwax_front", "river_pass_duskfen_bastion_rare_exchange"]
	):
		_fail("River Pass Hollow Mire production encounter identity or guard topology drifted.", {"encounter": encounter, "guarded_resources": _guarded_resource_ids(scenario, PLACEMENT_ID)})
		return
	var session = ScenarioFactory.create_session(SCENARIO_ID, "normal", SessionState.LAUNCH_MODE_CAMPAIGN)
	if session == null:
		_fail("River Pass Hollow Mire could not create a live campaign session.", {})
		return
	...
	if session.to_dict() != session_authority_before:
		_fail("Public battle payload mutated the River Pass session.", {})
		return
	...
	if _battle_enemy_stack_contract(battle_payload) != PRODUCTION_STACKS or _battle_enemy_ability_contract(battle_payload) != expected_abilities:
		_fail("Public battle payload missed the exact Hollow Mire production line.", {"stacks": _battle_enemy_stack_contract(battle_payload), "abilities": _battle_enemy_ability_contract(battle_payload)})
		return
	if _stack_health(PRODUCTION_STACKS) != 87 or _stack_health(LEGACY_STACKS) != 70:
		_fail("Hollow Mire production/legacy stack-health contract drifted.", {})
		return
	...
	legacy_army["id"] = "army_river_pass_hollow_mire_legacy_control"
	...
	if (
		not _sample_exact(production_normal, "victory", "player_advantaged", "standard", 3, 32, 0, 28)
		or not _sample_exact(production_hard, "victory", "player_advantaged", "standard", 3, 31, 0, 17)
		or not _sample_exact(legacy_normal, "victory", "player_advantaged", "standard", 3, 34, 0, 27)
		or not _sample_exact(legacy_hard, "victory", "player_advantaged", "standard", 3, 34, 0, 28)
		or scenario != scenario_authority_before
	):
		_fail("Hollow Mire production or independent legacy battle contract drifted.", {"production_normal": _compact_sample(production_normal), "production_hard": _compact_sample(production_hard), "legacy_normal": _compact_sample(legacy_normal), "legacy_hard": _compact_sample(legacy_hard)})
		return
	print("%s %s" % [REPORT_ID, JSON.stringify({
		...
		"guarded_resources": _guarded_resource_ids(scenario, PLACEMENT_ID),
...
func _guarded_resource_ids(scenario: Dictionary, guard_front_id: String) -> Array:
	var result: Array = []
	for resource_value in scenario.get("resource_nodes", []):
		if resource_value is Dictionary and String(resource_value.get("guard_front_id", "")) == guard_front_id:
			result.append(String(resource_value.get("placement_id", "")))
	result.sort()
	return result
