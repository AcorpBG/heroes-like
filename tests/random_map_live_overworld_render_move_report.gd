...
func _assert_generated_visual_summary(summary: Dictionary, label: String) -> bool:
	...
	if (
		...
		or not is_equal_approx(float(summary.get("multi_tile_interactive_min_step_tiles", 0.0)), 0.12)
		or not is_equal_approx(float(summary.get("multi_tile_interactive_base_cap_tiles", 0.0)), 0.54)
		or not is_equal_approx(float(summary.get("multi_tile_interactive_cap_step_tiles", 0.0)), 0.15)
	):
	...
	for entry_value in summary.get("resource_entries", []):
		...
		if multi_tile:
			...
			var expected_min_tiles := minf(0.50 + float(footprint_span - 1) * 0.12, 0.84)
			var expected_cap_tiles := minf(0.54 + float(footprint_span - 1) * 0.15, 0.84)
