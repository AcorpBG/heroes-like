...
func _terrain_transition_summary(overworld: Node) -> Dictionary:
	...
	var terrain_detail_source_model_ids: Dictionary = {}
		...
		for x in range(map_size.x):
			...
			if bool(shoreline.get("active", false)):
				...
				for profile_value in shoreline_profiles:
					...
					profiles_exact = profiles_exact and int(shoreline_profile.get("bank_band_point_count", 0)) == 11 and int(shoreline_profile.get("shallow_band_point_count", 0)) == 11 and int(shoreline_profile.get("wet_edge_point_count", 0)) == 9 and int(shoreline_profile.get("foam_segment_count", 0)) == 3 and bool(shoreline_profile.get("geometry_contained", false))
				if String(shoreline.get("model", "")) == "shared_lattice_nine_sample_layered_natural_bank" and String(shoreline.get("authored_edge_art_model", "")) == "authored_texture_feathered_to_shared_lattice_profile" and is_equal_approx(float(shoreline.get("authored_edge_art_alpha", 0.0)), 0.46) and is_equal_approx(float(shoreline.get("authored_edge_clip_depth_factor", 0.0)), 0.125) and int(shoreline.get("profile_sample_count", 0)) == 9 and bool(shoreline.get("shared_lattice_endpoints", false)) and bool(shoreline.get("diagonal_water_corner_hints_suppressed", false)) and is_equal_approx(float(shoreline.get("bank_band_alpha", 0.0)), 0.18) and profiles_exact and not bool(shoreline.get("continuous_bright_outline", true)) and not bool(shoreline.get("full_tile_fill", true)):
