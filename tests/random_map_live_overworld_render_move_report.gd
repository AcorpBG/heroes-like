...
func _assert_generated_visual_summary(summary: Dictionary, label: String) -> bool:
	...
	if String(summary.get("composition_signature", "")).length() != 64:
		...
		return false
	if (
		not is_equal_approx(float(summary.get("multi_tile_interactive_cap_tiles", 0.0)), 0.92)
		or not is_equal_approx(float(summary.get("multi_tile_interactive_base_min_tiles", 0.0)), 0.62)
		or not is_equal_approx(float(summary.get("multi_tile_interactive_min_step_tiles", 0.0)), 0.08)
		or not is_equal_approx(float(summary.get("multi_tile_interactive_base_cap_tiles", 0.0)), 0.70)
		or not is_equal_approx(float(summary.get("multi_tile_interactive_cap_step_tiles", 0.0)), 0.12)
	):
	...
	for entry_value in summary.get("resource_entries", []):
		...
		if multi_tile:
			...
			var expected_min_tiles := minf(0.62 + float(footprint_span - 1) * 0.08, 0.92)
			var expected_cap_tiles := minf(0.70 + float(footprint_span - 1) * 0.12, 0.92)
