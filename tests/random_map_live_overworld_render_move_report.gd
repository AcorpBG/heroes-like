...
func _assert_generated_visual_summary(summary: Dictionary, label: String) -> bool:
	...
	if not bool(summary.get("body_tile_keys_exact", false)) or not bool(summary.get("all_body_assets_loaded", false)) or not bool(summary.get("all_body_assets_terrain_matched", false)):
		...
		return false
	if not is_equal_approx(float(summary.get("body_sprite_extent_tiles", 0.0)), 0.42):
	...
	if String(summary.get("composition_signature", "")).length() != 64:
		...
		return false
	if (
		not is_equal_approx(float(summary.get("multi_tile_interactive_cap_tiles", 0.0)), 0.72)
		or not is_equal_approx(float(summary.get("multi_tile_interactive_base_min_tiles", 0.0)), 0.52)
		or not is_equal_approx(float(summary.get("multi_tile_interactive_min_step_tiles", 0.0)), 0.06)
		or not is_equal_approx(float(summary.get("multi_tile_interactive_base_cap_tiles", 0.0)), 0.58)
	...
	for entry_value in summary.get("resource_entries", []):
		...
		if multi_tile:
			...
			var expected_min_tiles := minf(0.52 + float(footprint_span - 1) * 0.06, 0.72)
			var expected_cap_tiles := minf(0.58 + float(footprint_span - 1) * 0.08, 0.72)
