...
func _run() -> void:
	...
	if not bool(sample.get("completed", false)) or String(sample.get("outcome_state", "")) != "victory":
		_fail_sample("Silt Hunters does not resolve as a bounded player victory.", payload, sample)
		return
	if int(sample.get("terminal_health_margin_pct", 100)) > MAX_TERMINAL_MARGIN_PCT:
		_fail_sample("Silt Hunters remains above the matrix terminal-margin target.", payload, sample)
		return
	if int(sample.get("damage_per_round", {}).get("enemy", 0)) <= 2:
		_fail_sample("Silt Hunters no longer applies meaningful enemy pressure.", payload, sample)
		return
