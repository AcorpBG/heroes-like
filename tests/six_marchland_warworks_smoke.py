#!/usr/bin/env python3
"""Run the single consolidated Six Marchland Warworks smoke."""
...
REPORT = ROOT / ".artifacts/six_marchland_warworks_smoke/report.json"
UNIT_MANIFEST = ROOT / "art/units/source/generated/marchland_warworks/manifest.json"
BUILDING_MANIFEST = ROOT / "art/towns/source/generated/buildings/marchland_warworks/manifest.json"
SCENE = "res://tests/six_marchland_warworks_smoke.tscn"


def verify_manifest(path: Path) -> None:
    manifest = json.loads(path.read_text(encoding="utf-8"))
    if manifest.get("generator_mode") != "built_in_imagegen" or manifest.get("content_batch_id") != "content-six-marchland-warworks-10184" or len(manifest.get("items", [])) != 6:
...
def main() -> int:
    ...
    verify_manifest(UNIT_MANIFEST)
    verify_manifest(BUILDING_MANIFEST)
    report = json.loads(REPORT.read_text(encoding="utf-8"))
    expected = {
        "ok": True, "case_count": 6, "exclusive_route_count": 6,
        "five_stack_company_count": 6, "town_build_count": 6,
        "weekly_growth_count": 6, "town_recruit_count": 6,
        "battle_victory_count": 18, "battle_art_count": 6,
    ...
    if not contact.is_file():
        raise SystemExit("six-warworks contact sheet is missing")
    print("SIX_MARCHLAND_WARWORKS_SMOKE VERIFIED " + json.dumps({key: report[key] for key in expected}, sort_keys=True))
    return 0
