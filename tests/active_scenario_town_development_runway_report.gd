...
func _apply_recruitment_market_coverage(session, placement_id: String, unit_cost_value: Variant) -> Array:
	_set_active_town(session, placement_id)
	var unit_cost: Dictionary = unit_cost_value if unit_cost_value is Dictionary else {}
	var rows := []
	for resource_id in COMMON_MARKET_RESOURCE_IDS:
...
func _runway_error(row: Dictionary) -> String:
	...
	if not bool(row.get("pacing_floor_ok", false)):
		return "town completed outside day-%d-to-day-%d pacing window" % [MIN_COMPLETION_DAY, TARGET_TURNS]
