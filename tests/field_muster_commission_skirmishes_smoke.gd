...
func _run_case(view: Control, case: Dictionary) -> void:
	...
	_expect(direct_lead, "%s did not launch its exact captain, faction, and company as a skirmish-only lead." % scenario_id)
	var map_size: Dictionary = scenario.get("map_size", {})
	_expect(int(map_size.get("width", 0)) == 12 and int(map_size.get("height", 0)) == 8 and scenario.get("towns", []).size() == 2 and scenario.get("resource_nodes", []).size() == 8 and scenario.get("artifact_nodes", []).size() == 1 and scenario.get("encounters", []).size() == 3 and scenario.get("script_hooks", []).size() == 5, "%s lost its compact authored composition: map=%s towns=%d resources=%d artifacts=%d encounters=%d hooks=%d." % [scenario_id, JSON.stringify(map_size), scenario.get("towns", []).size(), scenario.get("resource_nodes", []).size(), scenario.get("artifact_nodes", []).size(), scenario.get("encounters", []).size(), scenario.get("script_hooks", []).size()])
