...
func _validate_content_authority() -> void:
	...
	_expect(String(encounter.get("enemy_group_id", "")) == SHARED_ARMY_ID and String(encounter.get("terrain", "")) == "road" and encounter.get("battlefield_tags", []) == ["open_lane", "fog_bank"] and int(encounter.get("max_rounds", 0)) == 13, "Harbor Pilot House encounter authority changed.")
