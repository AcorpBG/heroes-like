...
func _validate_content_authority() -> void:
	...
	_expect(String(encounter.get("enemy_group_id", "")) == SHARED_ARMY_ID and String(encounter.get("terrain", "")) == "snow" and encounter.get("battlefield_tags", []) == ["fog_bank", "ambush_cover"] and int(encounter.get("max_rounds", 0)) == 13, "Icehook Trapper Lodge encounter authority changed.")
