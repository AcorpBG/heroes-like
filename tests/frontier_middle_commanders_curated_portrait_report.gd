...
const REPORT_ID := "FRONTIER_MIDDLE_COMMANDERS_CURATED_PORTRAIT_REPORT"
const VIEWPORT_SIZES := [Vector2i(1280, 720), Vector2i(1920, 1080)]
const CASES := [
	{
		"hero_id": "hero_veilmourn_ruln_vanehook",
		"hero_name": "Ruln Vanehook",
		"faction_id": "faction_veilmourn",
		"archetype": "harpooncaptain",
		"campaign_id": "campaign_frontier_claims",
		"scenario_id": "fogchart-mooring",
		"chapters": [
			{"campaign_id": "campaign_frontier_claims", "scenario_id": "fogchart-mooring"},
		],
		"source_path": "res://art/heroes/source/curated/hero_veilmourn_ruln_vanehook.png",
		"source_sha256": "e1596dc02e51db1b443f7f2f817c5e19f5f3242c9b2b127196f00d13de9b71f0",
		"portrait_path": "res://art/heroes/portraits/hero_veilmourn_ruln_vanehook.png",
		"portrait_sha256": "f987cba0e1fb2c375c156f8f5422e5c296470eb6ffa89dfa950bc02f8f0477ee",
		"starting_spell_ids": ["spell_obituary_mark", "spell_bulwark_litany", "spell_fogwake_step"],
	...
	{
		"hero_id": "hero_brasshollow_oren_bellfounder",
		"hero_name": "Oren Bellfounder",
		"faction_id": "faction_brasshollow",
		"archetype": "siegecaptain",
		"campaign_id": "campaign_frontier_claims",
		"scenario_id": "clauseworks-counterclaim",
		"chapters": [
			{"campaign_id": "campaign_frontier_claims", "scenario_id": "clauseworks-counterclaim"},
		],
		"source_path": "res://art/heroes/source/curated/hero_brasshollow_oren_bellfounder.png",
		"source_sha256": "b83fd6b64c06d8fc4b7bc8bfd49cfa457d286795409809869213bd1870682b8d",
		"portrait_path": "res://art/heroes/portraits/hero_brasshollow_oren_bellfounder.png",
		"portrait_sha256": "daecf892d3b8e47d9234af4a957196cc67b25959fbe1a13847df2aa4d2e95602",
		"starting_spell_ids": ["spell_pressure_clause", "spell_heat_rite", "spell_quickmarch_hymn"],
		"starting_specialties": ["armsmaster"],
		"battle_traits": ["artillerist", "vanguard"],
		"command": {"attack": 2, "defense": 0, "power": 1, "knowledge": 0},
		"base_movement": 10,
		"recruit_cost": {"gold": 1250},
	},
]
...
func _validate_curated_provenance() -> bool:
	...
	for case_value in CASES:
		...
		if source_image == null or source_image.get_size() != Vector2i(1254, 1254) \
			...
			return false
		var chapters: Array = case.get("chapters", [])
		if chapters.is_empty() or RandomMapGeneratorRules.DEFAULT_HERO_BY_FACTION.values().has(hero_id):
			_fail("Campaign-lead chapter/default authority changed for %s." % hero_id)
			return false
		for chapter_value in chapters:
			var chapter: Dictionary = chapter_value if chapter_value is Dictionary else {}
			var campaign_id := String(chapter.get("campaign_id", ""))
			var scenario_id := String(chapter.get("scenario_id", ""))
			var campaign := ContentService.get_campaign(campaign_id)
			var scenario := ContentService.get_scenario(scenario_id)
			if not _campaign_has_scenario(campaign, scenario_id) \
					or String(scenario.get("hero_id", "")) != hero_id \
					or String(scenario.get("player_faction_id", "")) != String(case.get("faction_id", "")):
				_fail("Campaign chapter authority changed for %s/%s." % [campaign_id, scenario_id])
				return false
	return true
...
func _validate_menu() -> bool:
	for chapter_value in _active_case.get("chapters", []):
		...
		if not bool(shell.call("validation_select_campaign", campaign_id)) \
				or not bool(shell.call("validation_select_campaign_chapter", scenario_id)):
			await _remove_shell(shell)
			_fail("Campaign menu could not select %s/%s." % [campaign_id, scenario_id])
			return false
		await get_tree().process_frame
		...
		if not ok:
			await _remove_shell(shell)
			_fail("Campaign portrait did not follow %s/%s: %s" % [campaign_id, scenario_id, JSON.stringify(snapshot)])
			return false
		await _remove_shell(shell)
	return true
...
func _portrait_contained(frame: Control, portrait: Control, viewport_size: Vector2i) -> bool:
	...
	if not contained:
		print("FRONTIER_MIDDLE_COMMANDERS_PORTRAIT_GEOMETRY ", JSON.stringify({
