...
func _assert_marker_readability_contract(shell: Node) -> bool:
	...
	if hero_on_town_footprint:
		...
		or not is_equal_approx(float(hero_layout.get("sprite_extent_fraction", 0.0)), 0.62):
...
func _assert_visible_sprite_scale_contract(map_node: Node) -> bool:
	...
	for payload in [artifact, pickup, service, objective, multi_tile_service, town]:
		if (
			payload.is_empty()
	...
	if (
		...
		or not is_equal_approx(float(multi_tile_service.get("cap_tiles", 0.0)), 0.60)
		or not is_equal_approx(float(multi_tile_service.get("visible_extent_tiles", 0.0)), 0.60)
	):
		...
		get_tree().quit(1)
		return false
	if not is_equal_approx(float(hero.get("sprite_extent_fraction", 0.0)), 0.62) or not bool(hero.get("sprite_contained_in_tile", false)):
	...
	if not (
		...
		push_error("Overworld smoke: artifact/pickup/structure/hero/town scale ordering is incoherent. artifact=%s pickup=%s objective=%s service=%s multi=%s hero=%s town=%s" % [artifact, pickup, objective, service, multi_tile_service, hero, town])
		get_tree().quit(1)
		return false
	if (
