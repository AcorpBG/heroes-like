...
const RESOURCE_SITE_LANDMARK_CASES := [
	...
	{"site_id": "site_reedbarge_mooring", "asset_id": "resource_site_neutral_reedbarge_mooring", "unclaimed_asset_id": "mapobj_reedbarge_mooring", "tile": Vector2i(1, 2), "region": Rect2(240, 0, 48, 48)},
	{"site_id": "site_glowcap_croft", "asset_id": "resource_site_neutral_glowcap_croft", "unclaimed_asset_id": "mapobj_glowcap_croft", "tile": Vector2i(3, 2), "region": Rect2(288, 0, 48, 48)},
	{"site_id": "site_dustjack_yard", "asset_id": "resource_site_neutral_dustjack_yard", "unclaimed_asset_id": "mapobj_dustjack_yard", "tile": Vector2i(5, 2), "region": Rect2(336, 0, 48, 48)},
	{"site_id": "site_cinder_kiln", "asset_id": "resource_site_neutral_cinder_kiln", "unclaimed_asset_id": "mapobj_cinder_kiln", "tile": Vector2i(7, 2), "region": Rect2(384, 0, 48, 48)},
	{"site_id": "site_frostbeacon_bothy", "asset_id": "resource_site_neutral_frostbeacon_bothy", "unclaimed_asset_id": "mapobj_frostbeacon_bothy", "tile": Vector2i(1, 4), "region": Rect2(432, 0, 48, 48)},
	{"site_id": "site_bramble_hedge", "asset_id": "resource_site_neutral_bramble_hedge", "unclaimed_asset_id": "mapobj_bramble_hedge", "tile": Vector2i(3, 4), "region": Rect2(480, 0, 48, 48)},
	{"site_id": "site_tidepool_skiffyard", "asset_id": "resource_site_neutral_tidepool_skiffyard", "unclaimed_asset_id": "mapobj_tidepool_skiffyard", "tile": Vector2i(5, 4), "region": Rect2(528, 0, 48, 48)},
	{"site_id": "site_switchback_hostel", "asset_id": "resource_site_neutral_switchback_hostel", "unclaimed_asset_id": "mapobj_switchback_hostel", "tile": Vector2i(7, 4), "region": Rect2(576, 0, 48, 48)},
	{"site_id": "site_saltpan_camp", "asset_id": "resource_site_neutral_saltpan_camp", "unclaimed_asset_id": "mapobj_saltpan_camp", "tile": Vector2i(0, 1), "region": Rect2(624, 0, 48, 48)},
...
func _run() -> void:
	...
	if not _assert_field_readiness_recap_contract(shell):
		return
	if not _assert_active_site_order_handoff_contract(shell):
		return
...
func _configure_resource_site_landmark_fixture(session) -> void:
	...
	for case_value in RESOURCE_SITE_LANDMARK_CASES:
		...
		resource_nodes.append({
			...
			"x": tile.x,
			"y": tile.y,
			"collected": case.has("unclaimed_asset_id"),
			"collected_by_faction_id": "player" if case.has("unclaimed_asset_id") else "",
		})
	session.overworld["resource_nodes"] = resource_nodes
	session.overworld["towns"] = []
	session.overworld["artifact_nodes"] = []
...
func _assert_resource_site_landmark_contract(shell: Node, map_node: Node, session) -> Dictionary:
	...
	for case_value in RESOURCE_SITE_LANDMARK_CASES:
		...
		var entry: Dictionary = object_assets.get(asset_id, {}) if object_assets.get(asset_id, {}) is Dictionary else {}
		var texture = map_node.call("_object_texture_for_asset", asset_id)
		var unclaimed_resolves_existing := true
		if unclaimed_asset_id != "":
			unclaimed_resolves_existing = String(map_node.call("_resource_asset_id", {
				"site_id": site_id,
				"collected": false,
				"collected_by_faction_id": "",
			})) == unclaimed_asset_id
...
func _assert_objective_stakes_ui_contract(shell: Node) -> bool:
	...
	if not _assert_text_contains_all(
		...
		["Objective Board", "Victory 0/5", "Defeat risks 0/4 triggered", "Claim Duskfen Bastion", "Avoid Defeat", "Current progress:", "Next step:"]
	):
		return false
...
func _assert_active_site_order_handoff_contract(shell: Node) -> bool:
	if not shell.has_method("validation_select_tile") or not shell.has_method("validation_snapshot"):
		push_error("Overworld smoke: shell is missing active-site order handoff validation hooks.")
		get_tree().quit(1)
		return false
	var session = SessionState.ensure_active_session()
	...
	var handoff: Dictionary = snapshot.get("active_site_order", {})
	var readiness: Dictionary = snapshot.get("field_readiness", {})
	var readiness_handoff: Dictionary = readiness.get("active_site_order", {})
	var joined := "\n".join([
		String(snapshot.get("active_site_order_visible_text", "")),
		String(snapshot.get("active_site_order_tooltip_text", "")),
		String(handoff.get("target_label", "")),
		String(handoff.get("order_label", "")),
	...
	if String(snapshot.get("primary_action_id", "")) != "collect_resource":
		push_error("Overworld smoke: active-site order handoff did not inspect a resource collection order. snapshot=%s" % snapshot)
		get_tree().quit(1)
		return false
	if not _assert_text_contains_all(
		"overworld active-site order handoff",
		[joined],
		[
			"Site handoff:",
			"Active Site Handoff",
			...
			"Resolve",
			"State change:",
			"Enter/Space commits this current-tile order",
			"Field Readiness",
			"Primary order:",
		]
	):
		return false
	if not _assert_no_ai_score_leak("overworld active-site order handoff", joined):
		return false
	...
	session.overworld["movement"] = original_movement
	OverworldRules.refresh_fog_of_war(session)
	shell.call("validation_select_tile", int(original_hero_position.get("x", 0)), int(original_hero_position.get("y", 0)))
	shell.call("_refresh")
	return true
...
func _assert_route_decision_clarity_contract(shell: Node) -> bool:
	...
	if String(interceptor_profile.get("hero_id", "")) != "hero_vaska" \
		...
		or not is_equal_approx(float(interceptor_profile.get("visible_extent_tiles", 0.0)), 0.88):
...
func _assert_marker_readability_contract(shell: Node) -> bool:
	...
	if String(hero_sprite.get("sprite_asset_id", "")) != "hero_signature_lyra" or not bool(hero_sprite.get("uses_identity_sprite", false)) or bool(hero_sprite.get("uses_faction_sprite", true)):
		push_error("Overworld smoke: active River Pass hero did not use Lyra's exact signature map sprite. presentation=%s" % hero_presentation)
		get_tree().quit(1)
		return false
	...
	if String(hero_sprite.get("sprite_silhouette_model", "")) != "eight_direction_alpha_silhouette_outline" \
		...
		or not bool(hero_command_pennant.get("asset_contained", false)) \
		or bool(hero_command_pennant.get("procedural_fallback", true)) \
	...
	if hero_on_town_footprint:
		...
		if not bool(focus_layout.get("hero_uses_compact_town_footprint_rect", false)) \
			...
			or bool(focus_layout.get("selection_uses_cartographic_tile_reticle", true)) \
			...
			or not tile_selection_visual_profile.is_empty() \
		...
		or not bool(focus_layout.get("selection_uses_cartographic_tile_reticle", false)) \
		or String(focus_layout.get("selection_visual_model", "")) != "open_cartographic_tile_corner_and_midpoint_ticks" \
		or not town_selection_visual_profile.is_empty() \
		or _focus_rect_from_payload(tile_selection_visual_profile.get("perimeter_rect", {})) != selection_focus_rect.grow(-expected_tile_selection_inset) \
		or not is_equal_approx(float(tile_selection_visual_profile.get("perimeter_inset_px", 0.0)), expected_tile_selection_inset) \
		or not is_equal_approx(float(tile_selection_visual_profile.get("corner_alpha", 0.0)), 0.82) \
		or not is_equal_approx(float(tile_selection_visual_profile.get("corner_length_px", 0.0)), maxf(8.0, tile_selection_extent * 0.18)) \
		or not is_equal_approx(float(tile_selection_visual_profile.get("corner_width_px", 0.0)), maxf(1.5, tile_selection_extent * 0.022)) \
		or not is_equal_approx(float(tile_selection_visual_profile.get("midpoint_alpha", 0.0)), 0.42) \
		or not is_equal_approx(float(tile_selection_visual_profile.get("midpoint_length_px", 0.0)), maxf(5.0, tile_selection_extent * 0.08)) \
		or not is_equal_approx(float(tile_selection_visual_profile.get("midpoint_width_px", 0.0)), maxf(1.25, tile_selection_extent * 0.016)) \
		or bool(tile_selection_visual_profile.get("continuous_outline", true)) \
		or not is_zero_approx(float(tile_selection_visual_profile.get("interior_fill_alpha", -1.0))) \
...
func _assert_overworld_art_contract(shell: Node) -> bool:
	...
	if not _assert_art_sprite(town_presentation, "town_identity_riverwatch", false):
		return false
	...
	if String(wood_resource_layout.get("model", "")) != "ordinary_resource_footprint" \
			...
			or not is_equal_approx(float(wood_resource_layout.get("visible_extent_cap_tiles", -1.0)), 0.0) \
	...
	if String(rare_exchange_layout.get("model", "")) != "compact_outward_edge_town_footprint_resource" \
			...
			or not is_equal_approx(float(rare_exchange_layout.get("visible_extent_cap_tiles", 0.0)), 0.56) \
	...
	for town_structure_case in town_structure_cases:
		...
		if String(structure_layout.get("model", "")) != "compact_outward_edge_town_footprint_resource" \
				...
				or not is_equal_approx(float(structure_layout.get("visible_extent_cap_tiles", 0.0)), 0.56) \
	...
	if generated_supply_art.get("sprite_asset_ids", []) != ["mapobj_moss_oath_cache"] \
			...
			or not is_equal_approx(float(generated_supply_layout.get("visible_extent_cap_tiles", 0.0)), 0.56) \
	...
	if String(artifact_field.get("icon_asset_id", "")) != "artifact_icon_trailsinger_boots" or String(artifact_field.get("icon_path", "")) != "res://art/artifacts/runtime/trailsinger_boots.png":
		...
		get_tree().quit(1)
		return false
	if String(artifact_field.get("sprite_path", "")) != "res://art/overworld/runtime/objects/artifacts/trailsinger_boots.png" or bool(artifact_field.get("uses_artifact_icon", true)) or bool(artifact_field.get("uses_default_sprite", true)) or not bool(artifact_field.get("uses_distinct_field_sprite", false)) or not bool(artifact_field.get("field_sprite_matches_artifact", false)) or not bool(artifact_field.get("inventory_icon_separate_from_field_sprite", false)) or not bool(artifact_field.get("field_sprite_contained_in_tile", false)):
...
func _assert_visible_sprite_scale_contract(map_node: Node) -> bool:
	...
	var tall_waypoint: Dictionary = map_node.call("validation_object_sprite_scale_payload", "mapobj_withered_rootgate_marker", "transit_object", Vector2i(1, 2), {"primary_class": "transit_route_object", "footprint_tier": "small"})
	var multi_tile_service: Dictionary = map_node.call("validation_object_sprite_scale_payload", "mapobj_contract_scribe_booth", "repeatable_service", Vector2i(2, 2), {"primary_class": "interactable_site", "footprint_tier": "medium"})
	var large_service: Dictionary = map_node.call("validation_object_sprite_scale_payload", "mapobj_contract_scribe_booth", "repeatable_service", Vector2i(3, 2), {"primary_class": "interactable_site", "footprint_tier": "large"})
	var clipped_large_service: Dictionary = map_node.call("validation_object_sprite_scale_payload", "mapobj_contract_scribe_booth", "repeatable_service", Vector2i(3, 2), {"primary_class": "interactable_site", "footprint_tier": "large"}, Vector2i(1, 2))
	var hero: Dictionary = map_node.call("validation_hero_draw_layout", Vector2i(7, 4), true)
	...
	for payload in [artifact, pickup, decoration, generic_object, encounter, blocker, waypoint, service, objective, wide_service, tall_waypoint, multi_tile_service, large_service, clipped_large_service]:
		if (
			payload.is_empty()
			...
			get_tree().quit(1)
			return false
	if String(artifact.get("semantic_scale_class", "")) != "handheld_artifact" or not is_equal_approx(float(artifact.get("visible_extent_tiles", 0.0)), 0.58):
		push_error("Overworld smoke: handheld artifact did not retain its legible compact presentation rank. payload=%s" % artifact)
		get_tree().quit(1)
		return false
	if String(pickup.get("semantic_scale_class", "")) != "loose_pickup" or not is_equal_approx(float(pickup.get("visible_extent_tiles", 0.0)), 0.68):
		push_error("Overworld smoke: pickup visible scale no longer remains subordinate. payload=%s" % pickup)
		get_tree().quit(1)
		return false
	if (
		String(decoration.get("semantic_scale_class", "")) != "ground_detail" or not is_equal_approx(float(decoration.get("visible_extent_tiles", 0.0)), 0.46)
		or String(generic_object.get("semantic_scale_class", "")) != "map_object" or not is_equal_approx(float(generic_object.get("visible_extent_tiles", 0.0)), 0.62)
		or String(encounter.get("semantic_scale_class", "")) != "encounter" or not is_equal_approx(float(encounter.get("visible_extent_tiles", 0.0)), 0.88)
		or String(blocker.get("semantic_scale_class", "")) != "terrain_blocker" or not is_equal_approx(float(blocker.get("visible_extent_tiles", 0.0)), 0.92)
		or String(waypoint.get("semantic_scale_class", "")) != "waypoint" or not is_equal_approx(float(waypoint.get("visible_extent_tiles", 0.0)), 0.78)
	):
		...
		get_tree().quit(1)
		return false
	if String(service.get("semantic_scale_class", "")) != "durable_structure" or not is_equal_approx(float(service.get("visible_extent_tiles", 0.0)), 0.82):
		push_error("Overworld smoke: durable service visible scale is not readable at one-tile presentation. payload=%s" % service)
		get_tree().quit(1)
		return false
	if String(objective.get("semantic_scale_class", "")) != "landmark" or not is_equal_approx(float(objective.get("visible_extent_tiles", 0.0)), 0.94):
		push_error("Overworld smoke: scenario objective visible scale changed outside its family hierarchy. payload=%s" % objective)
		get_tree().quit(1)
		return false
	if bool(decoration.get("interactive_silhouette", true)) \
		or String(decoration.get("interactive_silhouette_model", "")) != "none" \
		or not is_equal_approx(float(decoration.get("visible_modulate_alpha", 0.0)), 0.82) \
		or not bool(artifact.get("interactive_silhouette", false)) \
		or not bool(pickup.get("interactive_silhouette", false)) \
		or not bool(encounter.get("interactive_silhouette", false)) \
		or not bool(service.get("interactive_silhouette", false)) \
		or not bool(objective.get("interactive_silhouette", false)) \
		or String(service.get("interactive_silhouette_model", "")) != "eight_direction_alpha_silhouette_outline" \
		or float(service.get("interactive_silhouette_width_px", 0.0)) < 1.15 \
		or not is_equal_approx(float(service.get("visible_modulate_alpha", 0.0)), 1.0):
		push_error("Overworld smoke: interactive silhouettes no longer separate gameplay objects from subdued decoration. decoration=%s artifact=%s pickup=%s encounter=%s service=%s objective=%s" % [decoration, artifact, pickup, encounter, service, objective])
		get_tree().quit(1)
		return false
	if (
		not bool(wide_service.get("uses_multi_tile_visual_cap", false))
		...
		or not is_equal_approx(float(wide_service.get("min_tiles", 0.0)), 0.88)
		or not is_equal_approx(float(wide_service.get("cap_tiles", 0.0)), 1.06)
		or not is_equal_approx(float(wide_service.get("visible_extent_tiles", 0.0)), 0.88)
		or not is_equal_approx(float(tall_waypoint.get("min_tiles", 0.0)), 0.88)
		or not is_equal_approx(float(tall_waypoint.get("cap_tiles", 0.0)), 1.06)
		or not is_equal_approx(float(tall_waypoint.get("visible_extent_tiles", 0.0)), 0.88)
		or not is_equal_approx(float(multi_tile_service.get("visible_extent_tiles", 0.0)), 1.22)
		or not is_equal_approx(float(large_service.get("min_tiles", 0.0)), 1.10)
		or not is_equal_approx(float(large_service.get("cap_tiles", 0.0)), 1.35)
		or not is_equal_approx(float(large_service.get("visible_extent_tiles", 0.0)), 1.35)
		or not (
		...
		get_tree().quit(1)
		return false
	if clipped_large_service.get("footprint", {}) != {"width": 3, "height": 2} \
		or clipped_large_service.get("visible_footprint_span", {}) != {"width": 1, "height": 2} \
		or not is_equal_approx(float(clipped_large_service.get("world_tile_extent_px", 0.0)), 100.0) \
		or not bool(clipped_large_service.get("footprint_clipped", false)) \
		or clipped_large_service.get("visible_footprint_rect", {}) != {"x": 200.0, "y": 0.0, "width": 100.0, "height": 200.0} \
		or clipped_large_service.get("sprite_center", {}) != {"x": 250.0, "y": 127.0} \
		or not bool(clipped_large_service.get("sprite_contained_in_visible_footprint", false)) \
		or bool(large_service.get("footprint_clipped", true)) \
		or not is_equal_approx(float(clipped_large_service.get("visible_extent_tiles", 0.0)), 0.96) \
		or not is_equal_approx(float(clipped_large_service.get("min_tiles", 0.0)), float(large_service.get("min_tiles", -1.0))) \
		or not is_equal_approx(float(clipped_large_service.get("cap_tiles", 0.0)), float(large_service.get("cap_tiles", -1.0))) \
		or float(clipped_large_service.get("visible_extent_tiles", 0.0)) >= float(large_service.get("visible_extent_tiles", 0.0)):
	...
	if not is_equal_approx(float(town.get("visible_extent_tiles", 0.0)), 3.72) \
		...
		get_tree().quit(1)
		return false
	var authored_pickup_profile: Dictionary = authored_pickup.get("content_profile", {})
	var authored_service_profile: Dictionary = authored_service.get("content_profile", {})
	if (
		authored_pickup_profile != {
		...
		or not is_equal_approx(float(authored_service.get("visible_extent_tiles", 0.0)), 0.88)
	):
	...
	if (
		...
		or float(town.get("visible_extent_tiles", 0.0)) < float(artifact.get("visible_extent_tiles", 0.0)) * 3.00
	):
		push_error("Overworld smoke: physical role separation collapsed into same-scale board tokens. artifact=%s pickup=%s objective=%s town=%s" % [artifact, pickup, objective, town])
		get_tree().quit(1)
		return false
	if (
		float(artifact.get("painted_extent_fraction", 1.0)) >= 0.70
		or float(pickup.get("painted_extent_fraction", 1.0)) >= 0.90
...
func _assert_art_sprite(presentation: Dictionary, expected_asset_id: String, remembered: bool) -> bool:
	...
	var is_town := expected_asset_id == "frontier_town" or expected_asset_id.begins_with("town_faction_") or expected_asset_id.begins_with("town_identity_")
