...
func _assert_marker_readability_contract(shell: Node) -> bool:
	...
	if hero_on_town_footprint:
		...
		or not is_equal_approx(float(hero_layout.get("sprite_extent_fraction", 0.0)), 0.68):
...
func _assert_visible_sprite_scale_contract(map_node: Node) -> bool:
	...
	if (
		...
		or not is_equal_approx(float(multi_tile_service.get("cap_tiles", 0.0)), 0.64)
		or not is_equal_approx(float(multi_tile_service.get("visible_extent_tiles", 0.0)), 0.64)
	):
		...
		get_tree().quit(1)
		return false
	if not is_equal_approx(float(hero.get("sprite_extent_fraction", 0.0)), 0.68) or not bool(hero.get("sprite_contained_in_tile", false)):
		push_error("Overworld smoke: field hero art is not contained at its proportional tile rank. payload=%s" % hero)
		get_tree().quit(1)
		return false
	if not is_equal_approx(float(town.get("visible_extent_tiles", 0.0)), 1.40):
