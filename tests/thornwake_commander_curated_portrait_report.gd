...
const CASES := [
	{
		...
		"hero_name": "Ardren Briar-Marshal",
		"faction_id": "faction_thornwake",
		"archetype": "briarmarshal",
		"surface_kind": "generated_default",
		"campaign_id": "",
		"scenario_id": "",
	...
	},
	{
		...
		"portrait_path": "res://art/heroes/portraits/hero_thornwake_tova_rootwright.png",
		"portrait_sha256": "a00562af41fbf557a67538fd1e3aaa2f2ec228563178c8d5f2817e1f9d8e5ace",
		"starting_spell_ids": ["spell_briar_bind", "spell_stone_veil", "spell_graft_mend"],
...
var _generated_session_payload: Dictionary = {}
var _generated_scenario_id := ""
...
func _run() -> void:
	...
	if not _prepare_generated_default_session():
		return
	...
	print(REPORT_ID, " ", JSON.stringify({
		...
		"surfaces": ["component", "generated_default_session", "campaign_menu", "overworld", "town", "battle_player", "battle_enemy_authored", "outcome"],
...
func _validate_curated_provenance() -> bool:
	...
	for case_value in CASES:
		...
		if source_image == null or source_image.get_size() != Vector2i(1254, 1254) \
			...
			return false
		if String(case.get("surface_kind", "")) == "generated_default":
			if RandomMapGeneratorRules.DEFAULT_HERO_BY_FACTION.get(String(case.get("faction_id", "")), "") != hero_id:
				_fail("Generated-map default authority changed for %s." % hero_id)
				return false
		else:
			...
			if not _campaign_has_scenario(campaign, String(case.get("scenario_id", ""))) \
				...
				_fail("Campaign/default authority changed for %s." % hero_id)
				return false
	return true
...
func _campaign_has_scenario(campaign: Dictionary, scenario_id: String) -> bool:
	for chapter_value in campaign.get("scenarios", []):
		var chapter: Dictionary = chapter_value if chapter_value is Dictionary else {}
		if String(chapter.get("scenario_id", "")) == scenario_id:
			return true
	return false


func _validate_menu() -> bool:
	if String(_active_case.get("surface_kind", "")) == "generated_default":
		return true
...
func _prepare_generated_default_session() -> bool:
	...
	var generated: Dictionary = RandomMapGeneratorRules.generate(config)
	if not bool(generated.get("ok", false)):
		_fail("Generated Thornwake portrait fixture failed: %s" % JSON.stringify(generated.get("report", {})))
		return false
	var generated_map: Dictionary = generated.get("generated_map", {}) if generated.get("generated_map", {}) is Dictionary else {}
	var session: SessionStateStoreScript.SessionData = ScenarioFactory.create_generated_draft_session(generated_map, "normal")
	if session.scenario_id == "":
		_fail("Generated Thornwake portrait fixture did not create a session.")
		return false
	OverworldRules.normalize_overworld_state(session)
	...
	if String(hero.get("id", "")) != "hero_thornwake_ardren_briarmarshal" \
			or String(scenario.get("hero_id", "")) != "hero_thornwake_ardren_briarmarshal" \
			or String(scenario.get("player_faction_id", "")) != "faction_thornwake" \
			or not bool(session.flags.get("generated_random_map_draft", false)):
		_fail("Generated Thornwake session did not use Ardren exactly.")
		return false
	_generated_scenario_id = session.scenario_id
	_generated_session_payload = session.to_dict()
	return true
...
func _portrait_contained(frame: Control, portrait: Control, viewport_size: Vector2i) -> bool:
	...
	if not contained:
		print("THORNWAKE_COMMANDER_PORTRAIT_GEOMETRY ", JSON.stringify({
...
func _other_authored_hero_id() -> String:
	return "hero_thornwake_tova_rootwright" if String(_active_case.get("hero_id", "")) == "hero_thornwake_ardren_briarmarshal" else "hero_thornwake_ardren_briarmarshal"
