...
func _write_matrix_markdown(report: Dictionary) -> void:
	var lines := [
		...
		"",
		"| Case | Gate | Template | Size | Roads | Rivers | Decor Body | Guarded/Guardable | Poor Zones | Fair Warn | Contest Dist | Guard Spread | Town-Resource | Row Coverage | Quadrants | ms/Route | Status |",
		"| --- | --- | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |",
	]
	for result in report.get("cases", []):
		...
		lines.append("| %s | %s | %s | %dx%d | %d | %d | %d | %d/%d | %d | %d/%d | %d/%d | %d | %d | %.3f | %d/4 | %.1f | %s |" % [
