...
func _cases() -> Array:
	return [
		...
		{"id": "medium_translated_land_002_probe_a", "seed": "rmg-visual-medium-002-a-10184", "width": 72, "height": 72, "water_mode": "land", "level_count": 1, "player_count": 4, "template_id": "translated_rmg_template_002_v1", "profile_id": "translated_rmg_profile_002_v1", "strict_gate": false},
	]
...
func _inspection_summary(result: Dictionary) -> Dictionary:
	...
	if total_chars > 0 and float(longest_blank_run) / float(max(1, width_max)) > 0.55:
		observations.append("large terrain-only visual band in ASCII inspection")
...
func _summary(results: Array) -> Dictionary:
	...
	for result in results:
		...
		totals["max_blank_run_ratio"] = max(float(totals.get("max_blank_run_ratio", 0.0)), float(inspection.get("longest_blank_run_ratio", 0.0)))
...
func _case_log_line(result: Dictionary) -> Dictionary:
	...
	return {
		...
		"blank_run_ratio": float(inspection.get("longest_blank_run_ratio", 0.0)),
		"failures": result.get("failures", []),
	}
...
func _write_matrix_markdown(report: Dictionary) -> void:
	var lines := [
		...
		"",
		"| Case | Gate | Template | Size | Roads | Rivers | Decor Body | Guarded/Guardable | Poor Zones | Blank Run | Status |",
		"| --- | --- | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | --- |",
	]
	for result in report.get("cases", []):
		...
		lines.append("| %s | %s | %s | %dx%d | %d | %d | %d | %d/%d | %d | %.3f | %s |" % [
			...
			float(inspection.get("longest_blank_run_ratio", 0.0)),
			status,
		])
	lines.append("")
	lines.append("ASCII legend: `T` town, `G` guard/encounter, `A` artifact, `M` mine/resource producer, `R` reward/resource, `#` blocker body, `.` road, `C` connection-controlled road, `~` river candidate, `=` water.")
	lines.append("")
	for result in report.get("cases", []):
		if not (result is Dictionary):
			continue
