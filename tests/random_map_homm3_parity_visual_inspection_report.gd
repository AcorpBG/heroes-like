...
func _write_matrix_markdown(report: Dictionary) -> void:
	var lines := [
		...
		"",
		"| Case | Gate | Template | Size | Roads | Rivers | Decor Body | Guarded/Guardable | Poor Zones | Fair Warn | Contest Dist | Guard Spread | Town-Resource | Row Coverage | Quadrants | ms/Route | Rendered | Status |",
		"| --- | --- | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- | --- |",
	]
	for result in report.get("cases", []):
		...
		lines.append("| %s | %s | %s | %dx%d | %d | %d | %d | %d/%d | %d | %d/%d | %d/%d | %d | %d | %.3f | %d/4 | %.1f | %s | %s |" % [
	...
	lines.append("")
	lines.append("ASCII legend: `T` town, `G` guard/encounter, `A` artifact, `M` mine/resource producer, `R` reward/resource, `#` blocker body, `.` road, `C` connection-controlled road, `~` river candidate, `=` water. Fair Warn shows fail-threshold warnings over total fairness warnings from the source-backed resource/encounter fairness report; distance/pressure spread columns are diagnostic layout-quality evidence, not rendered art comparison.")
	lines.append("")
	for result in report.get("cases", []):
		if not (result is Dictionary):
			continue
...
func _write_rendered_gallery(report: Dictionary) -> void:
	var lines := [
		...
		"<table><tr><th>Case</th><th>Gate</th><th>Template</th><th>Roads</th><th>Fair Warnings</th><th>Rendered Preview</th></tr>",
	]
	for result in report.get("cases", []):
		if not (result is Dictionary):
			continue
		...
		lines.append("<tr><td>%s</td><td>%s</td><td>%s</td><td>%d</td><td>%d/%d</td><td><a href=\"%s\">svg</a></td></tr>" % [
