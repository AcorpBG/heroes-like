...
const REPORT_ID := "EIGHT_COMMANDERS_PROVING_ROADS_SMOKE"
const OUTPUT_DIR := "res://.artifacts/eight_commanders_proving_roads_smoke"
...
func _run() -> void:
	...
	if _errors.is_empty():
		print("%s %s" % [REPORT_ID, JSON.stringify({"ok":true,"case_count":8,"production_battle_count":24,"specialty_choice_count":24,"scenario_victory_count":8,"single_consolidated_smoke":true})])
	view.queue_free()
	await get_tree().process_frame
...
func _run_case(view: Control, case: Dictionary) -> void:
	...
	_expect(exact_art, "%s did not resolve its exact proving-road atlas region." % scenario_id)
	...
	for suffix in ["landmark_guard", "north_examination", "south_examination"]:
	...
	_expect(save_exact, "%s did not preserve eight commanders proving roads through save version %d." % [scenario_id, SessionStateStoreScript.SAVE_VERSION])
...
func _capture_if_requested(scenario_id: String) -> String:
	var directory := OS.get_environment("EIGHT_COMMANDERS_PROVING_ROADS_CAPTURE_DIR")
	if directory == "":
		return ""
