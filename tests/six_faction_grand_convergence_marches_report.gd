...
func _validate_case(view: Control, case: Dictionary) -> void:
	...
	_expect(int(map_size.get("width", 0)) == 14 and int(map_size.get("height", 0)) == 9 and (scenario.get("towns", []) as Array).size() == 2 and (scenario.get("resource_nodes", []) as Array).size() == 16 and (scenario.get("artifact_nodes", []) as Array).size() == 6 and (scenario.get("encounters", []) as Array).size() == 6 and (scenario.get("script_hooks", []) as Array).size() == 8 and (scenario_objectives.get("victory", []) as Array).size() == 7, "%s lost its complete 14x9 grand-convergence contract." % scenario_id)
