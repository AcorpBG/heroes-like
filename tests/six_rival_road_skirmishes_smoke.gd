...
func _run() -> void:
	...
	if _errors.is_empty():
		print("%s %s" % [REPORT_ID, JSON.stringify({"ok":true,"scenario_count":6,"battle_victory_count":30,"exact_encounter_art_count":30,"scenario_victory_count":6,"save_version":SessionStateStoreScript.SAVE_VERSION,"single_consolidated_smoke":true})])
