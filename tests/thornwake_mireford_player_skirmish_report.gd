...
const REPORT_ID := "THORNWAKE_MIREFORD_PLAYER_SKIRMISH_REPORT"
const SCENARIO_ID := "mireford-skirmish"
const HERO_ID := "hero_thornwake_silsa_bramblehound"
const ARMY_ID := "army_rootbound_mireford_wardens"
const PLAYER_TOWN_PLACEMENT_ID := "graftroot_bridgehead"
const PLAYER_TOWN_ID := "town_thornwake_graftroot_caravan"
const SAVE_SLOT := 3
...
func _run() -> void:
	...
	if JSON.stringify(progression_before) != JSON.stringify(SaveService.load_progression()):
		_fail("Skirmish workflow changed campaign progression storage.")
		return
...
func _assert_setup(setup: Dictionary) -> void:
	if setup.is_empty() or String(setup.get("scenario_id", "")) != SCENARIO_ID:
		_fail("Live skirmish setup did not expose Rootbound Mireford: %s" % JSON.stringify(setup))
		return
	if String(setup.get("difficulty", "")) != "normal":
		_fail("Mireford setup did not preserve normal difficulty.")
		return
	...
	if not bool(availability.get("campaign", false)) or not bool(availability.get("skirmish", false)):
		_fail("Mireford missed dual-mode campaign/skirmish availability: %s" % JSON.stringify(availability))
		return
	if String(scenario.get("player_faction_id", "")) != "faction_thornwake":
		_fail("Mireford setup is not authored for Thornwake.")
...
func _assert_opening_session(session: SessionStateStoreScript.SessionData) -> void:
	if session == null or session.scenario_id != SCENARIO_ID:
		_fail("ScenarioFactory did not launch Rootbound Mireford.")
		return
	if session.launch_mode != SessionStateStoreScript.LAUNCH_MODE_SKIRMISH or session.flags.has("campaign_id"):
		_fail("Rootbound Mireford crossed into campaign session state.")
		return
	if String(session.overworld.get("active_hero_id", "")) != HERO_ID:
		_fail("Silsa is not the active Mireford commander.")
		return
	var hero: Dictionary = session.overworld.get("hero", {})
	if String(hero.get("id", "")) != HERO_ID:
		_fail("Mireford hero state did not materialize Silsa.")
		return
	var army: Dictionary = session.overworld.get("army", {})
	if String(army.get("id", "")) != ARMY_ID or _stack_count(army.get("stacks", []), "unit_thornwake_seedcutters") != 12:
		_fail("Mireford opening army did not materialize its authored Thornwake wardens: %s" % JSON.stringify(army))
		return
	...
	if String(town.get("town_id", "")) != PLAYER_TOWN_ID or String(town.get("owner", "")) != "player":
		_fail("Graftroot Caravan did not materialize as the player opening town.")
...
func _exercise_town(session: SessionStateStoreScript.SessionData) -> Dictionary:
	...
	if String(town.get("placement_id", "")) != PLAYER_TOWN_PLACEMENT_ID:
		_fail("Graftroot Caravan could not be opened through the live town rules.")
		return {}
	var build_actions := TownRulesScript.get_build_actions(session)
	if build_actions.is_empty():
		_fail("Graftroot Caravan exposed no development actions.")
		return {}
	...
	if recruit_action.is_empty():
		_fail("Graftroot Caravan exposed no affordable Thornwake recruit action: %s" % JSON.stringify(recruit_actions))
		return {}
	...
	if not bool(recruit_result.get("ok", false)):
		_fail("Graftroot Caravan recruit action failed: %s" % JSON.stringify(recruit_result))
		return {}
	return {
...
func _exercise_save_restore(session: SessionStateStoreScript.SessionData) -> Dictionary:
	...
	if not bool(save_result.get("ok", false)):
		_fail("Rootbound Mireford runtime save failed: %s" % JSON.stringify(save_result))
		return {}
	...
	if restored == null:
		_fail("Rootbound Mireford runtime save did not restore.")
		return {}
	...
	if JSON.stringify(signature_before) != JSON.stringify(signature_after):
		_fail("Rootbound Mireford changed across save/restore: before=%s after=%s" % [signature_before, signature_after])
		return {}
	return {
		"session": restored,
		"report": {
...
func _exercise_battle_entry(session: SessionStateStoreScript.SessionData) -> Dictionary:
	var encounter := _encounter_by_placement(session, "bridge_ford_reavers")
	var battle: Dictionary = BattleRulesScript.create_battle_payload(session, encounter)
	if battle.is_empty():
		_fail("Ford Reavers did not create a live battle payload.")
		return {}
	if String(battle.get("player_commander_state", {}).get("id", "")) != HERO_ID:
		_fail("Ford Reavers battle did not preserve Silsa as player commander.")
		return {}
	...
	for stack in battle.get("stacks", []):
		...
		if String(stack.get("side", "")) == "player":
			...
			enemy_stack_count += 1
	if player_stack_count < 4 or enemy_stack_count < 2:
		_fail("Ford Reavers battle missed authored armies: %s" % JSON.stringify(battle.get("stacks", [])))
		return {}
	return {
...
func _exercise_outcomes() -> Dictionary:
	...
	if String(defeat_result.get("status", "")) != "defeat":
		_fail("Rootbound Mireford Day 10 defeat boundary did not resolve: %s" % JSON.stringify(defeat_result))
		return {}
