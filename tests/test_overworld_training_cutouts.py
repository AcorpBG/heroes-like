"""Forty exact paintings, scoped RGB repair and immutable logical placement."""
import copy
...
spec=importlib.util.spec_from_file_location('training_art',ROOT/'tools/prepare_overworld_training_cutouts.py')
art=importlib.util.module_from_spec(spec);spec.loader.exec_module(art)


class TrainingCutoutTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_complete_six_atlas_cohort(self):
        self.assertEqual(len(self.sources),40)
        self.assertEqual(sorted(v['before_size'][0]//48 for v in self.recipe['atlases'].values()),[6,6,6,6,8,8])
        self.assertEqual(len(self.recipe['state_mappings']),40)
        self.assertEqual(len({r['source_sha256'] for r in self.recipe['assets'].values()}),40)
    ...
    def test_all_state_routes_and_original_archives_preserved(self):
    ...
    def test_bad_mapping_region_and_source_fail_closed(self):
        ...
        m=copy.deepcopy(self.manifest);m['resource_site_sprites'][site]['asset_id']='hostile_camp';self.reject(art.MANIFEST,m,'State mapping')
    ...
    def test_alpha_crop_density_anchor_and_noise_fail_closed(self):
        ...
        for field,value,pattern in [('source_mode','color_key','Alpha policy'),('source_crop',[0,0,10,10],'Source crop'),('pixel_scale',1,'Density/filter'),('fit_limit',44,'fit/anchor'),('canvas_origin',[6,3],'fit/anchor'),('alignment','bottom','fit/anchor'),('low_alpha_rgb_pixels',0,'RGB noise'),('source_manifest_sha256','0'*64,'Source provenance')]:
