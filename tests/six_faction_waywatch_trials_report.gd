...
func _run() -> void:
	...
	var report := {"ok":_errors.is_empty(),"case_count":CASES.size(),"player_unit_id_count":WAYWATCH_PLAYER_UNIT_IDS.size(),"neutral_unit_id_count":WAYWATCH_NEUTRAL_UNIT_IDS.size(),"atlas_path":ATLAS_PATH,"atlas_size":[1152,192],"save_version":SessionStateStoreScript.SAVE_VERSION,"rows":_rows,"errors":_errors}
