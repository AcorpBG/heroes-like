...
func _run() -> void:
	var case_report := _river_pass_nearby_raids_group_for_town_assault()
	...
	var payload := {
		...
		"behavior_policy": "nearby_same_town_raids_group_before_assault",
...
func _raid_seed(placement_id: String, x: int, y: int, brute_count: int, with_commander: bool, session, config: Dictionary) -> Dictionary:
	var raid := {
...
func _assert_no_saved_task_state(session) -> void:
	for state in session.overworld.get("enemy_states", []):
		if state is Dictionary and state.has("hero_task_state"):
			_fail("Grouping wrote forbidden hero_task_state.")
			return
