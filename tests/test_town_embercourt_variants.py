"""Embercourt town union, exact originals and shared full-state probe controls."""
import copy
import hashlib
import json
import unittest

import town_embercourt_variant_regression as variants
...
class EmbercourtVariantTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_complete_five_town_union(self):
        towns = [t for t in self.towns if t['faction_id']=='faction_embercourt']
        ...
        self.assertEqual(required, set(self.manifest['factions']['faction_embercourt']))

    def test_missing_variants_fail_strict_validation(self):
        malformed = copy.deepcopy(self.manifest)
        for building in variants.IDS:
            del malformed['factions']['faction_embercourt'][building]
        errors = validate_scene_layers(malformed)
        for building in variants.IDS:
            self.assertTrue(any('Missing authored Embercourt variant' in e and building in e for e in errors))

    def test_bastions_remain_distinct_simultaneous_identities(self):
        rows = self.manifest['factions']['faction_embercourt']
        a, b = 'building_charter_bastion', 'building_embercourt_charter_bastion'
        self.assertTrue(any({a,b} <= set(t['buildable_building_ids']) for t in self.towns))
        for key in ('source_sha256', 'runtime_sha256', 'runtime_path', 'ground_anchor'):
            self.assertNotEqual(rows[a][key], rows[b][key])

    def test_shared_sites_cannot_coexist(self):
        for a,b in ((variants.IDS[3], variants.IDS[5]),
                    ('building_charter_bastion', 'building_quartermasters_depot')):
    ...
    def test_shared_rule_input_save_assertions_are_retained(self):
        ...
        self.assertEqual(script.count('check('), shared.script_text().count('check(')+2)
        self.assertIn('Relief quay overlap did not follow actual foreground paint',script)
        self.assertIn('body=Vector2(0.35,0.12)',script)
