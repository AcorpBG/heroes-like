...
func _assert_catalog_and_setup() -> Dictionary:
	...
	if scenario_ids.size() != 20 or SCENARIO_ID not in scenario_ids or scenario_ids[-1] != SCENARIO_ID:
		_fail("Active catalog did not expose Fogchart as exact twentieth scenario: %s" % JSON.stringify(scenario_ids))
		return {}
