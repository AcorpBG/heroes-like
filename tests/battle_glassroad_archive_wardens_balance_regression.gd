...
const PLACEMENT_ID := "glassroad_archive_wardens"
...
func _run() -> void:
	...
	if encounter.is_empty():
		_fail("Glassroad Sundering is missing the Archive Wardens placement.", {})
		return
	...
	if not bool(sample.get("completed", false)) or String(sample.get("outcome_state", "")) != "victory":
		_fail_sample("Archive Wardens no longer resolves as a bounded player victory.", payload, sample)
		return
	if int(sample.get("terminal_health_margin_pct", 100)) > MAX_TERMINAL_MARGIN_PCT:
		_fail_sample("Archive Wardens remains above the matrix terminal-margin target.", payload, sample)
		return
	if int(sample.get("damage_per_round", {}).get("enemy", 0)) <= 2:
		_fail_sample("Archive Wardens still fails to apply meaningful enemy pressure.", payload, sample)
		return
