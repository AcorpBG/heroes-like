...
func _assert_report(first: Dictionary) -> bool:
	...
	for required_field in ["terrain", "encounter_difficulty", "initial_health", "final_health", "player_health_remaining_pct", "enemy_health_remaining_pct", "terminal_health_margin_pct", "action_counts", "action_mix", "damage_totals", "damage_per_round", "pacing_band", "initial_stack_profile", "runtime_consequence_profile"]:
	...
	if String(runtime_profile.get("schema", "")) != "battle_autoplay_runtime_consequence_profile_v1" or String(runtime_profile.get("profile_signature", "")) == "":
		_fail("Battle outcome distribution sample is missing runtime consequence profile evidence: %s" % first_battle_sample)
		return false
...
func _assert_runtime_consequence_gate(battle_summary: Dictionary) -> bool:
	...
	if String(distribution.get("schema", "")) != "battle_autoplay_runtime_consequence_distribution_v1":
		_fail("Battle outcome distribution runtime consequence distribution schema mismatch: %s" % distribution)
		return false
	if String(gate.get("policy", "")) != "report_only_runtime_consequence_thresholds_v1":
		_fail("Battle outcome distribution runtime consequence gate policy mismatch: %s" % gate)
		return false
	if String(gate.get("status", "")) != "pass":
		_fail("Battle outcome distribution runtime consequence gate must pass: %s" % gate)
		return false
	if int(gate.get("sample_count", -1)) != int(battle_summary.get("sample_count", -2)):
		_fail("Battle outcome distribution runtime consequence gate sample count does not align with summary: %s" % gate)
		return false
	if int(distribution.get("samples_with_ability_consequence_count", 0)) <= 0:
		_fail("Battle outcome distribution did not observe ability runtime consequences: %s" % distribution)
		return false
