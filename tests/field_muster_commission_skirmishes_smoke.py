...
REPORT_ID = "FIELD_MUSTER_COMMISSION_SKIRMISHES_SMOKE"
REPORT_PATH = ROOT / ".artifacts" / "field_muster_commission_skirmishes_smoke" / "report.json"
CAPTURE_DIR = ROOT / ".artifacts" / "field_muster_commission_skirmishes_smoke" / "captures"
SCENE = "res://tests/field_muster_commission_skirmishes_smoke.tscn"
TIMEOUT_SECONDS = 300
...
def main() -> int:
    ...
    environment["FIELD_MUSTER_COMMISSION_CAPTURE_DIR"] = str(CAPTURE_DIR)
    try:
    ...
    expected = {
        ...
        "case_count": 6,
        "direct_lead_count": 6,
        "muster_claim_count": 6,
        "battle_victory_count": 6,
    ...
    if len(rows) != 6 or any(not row.get("save_round_trip_exact") or not row.get("deterministic") for row in rows):
        failures.append("all six rows must be deterministic exact save round-trips")
