...
func _asset_summary_exact(summary: Dictionary) -> bool:
	return String(summary.get("manifest_path", "")) == "res://content/overworld_vfx_manifest.json" \
		...
		and int(summary.get("mapped_cue_count", 0)) == 3 \
		and summary.get("mapped_cue_ids", []) == ["vfx_placeholder_capture_flag", "vfx_placeholder_depleted_dim", "vfx_placeholder_object_visit"] \
		and int(summary.get("unique_texture_count", 0)) == 3 \
		and int(summary.get("loaded_texture_count", 0)) == 3 \
