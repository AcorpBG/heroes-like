...
func _validate_content_authority() -> void:
	...
	_expect(String(encounter.get("enemy_group_id", "")) == SHARED_ARMY_ID and String(encounter.get("terrain", "")) == "underground" and int(encounter.get("max_rounds", 0)) == 12, "Lantern Warren encounter authority changed.")
