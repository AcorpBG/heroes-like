...
func _run() -> void:
	...
	if not bool(report.get("ok", false)):
		...
		return
	if int(report.get("artifact_count", 0)) != 39:
		_fail("Expected thirty-nine authored artifact records after the guarded-relic-road slice: %s" % report)
		return
	...
	if int(report.get("faction_affinity_artifact_count", 0)) < 30:
		_fail("Artifact taxonomy report did not include five faction-affinity artifacts per faction: %s" % report)
		return
	...
	if bool(policy.get("save_version_bump", true)) or bool(policy.get("source_reward_tables_active", true)) or not bool(policy.get("set_bonuses_active", false)) or not bool(policy.get("rare_resource_activation", false)):
	...
	var payload := {
		...
		"caveats": [
			"This report proves additive artifact taxonomy/schema metadata, active set-threshold validation, and the rare-income artifact classification; live source execution and save-version migration remain separate runtime concerns.",
		],
	}
