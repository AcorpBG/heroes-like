"""Complete Brasshollow art, upgrade identity, real orders and batch-only fixtures."""
import copy
import hashlib
import json
import unittest

import brasshollow_town_opening as opening
import town_brasshollow_faction_regression as variants
...
class BrasshollowFactionTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.manifest = json.loads((variants.ROOT/'content/town_building_scene_art_manifest.json').read_text())
        cls.towns = [t for t in json.loads((variants.ROOT/'content/towns.json').read_text())['items'] if t['faction_id']=='faction_brasshollow']
    ...
    def test_complete_five_town_union(self):
        ...
        self.assertEqual(len(union),27)
        self.assertEqual(union,set(variants.IDS))
        self.assertEqual(union,set(self.manifest['factions']['faction_brasshollow']))

    def test_all_missing_identities_fail_closed(self):
        malformed=copy.deepcopy(self.manifest)
        malformed['factions']['faction_brasshollow']={}
        errors=validate_scene_layers(malformed)
        for id in variants.IDS:
            self.assertTrue(any('Missing authored Brasshollow scene mapping' in e and id in e for e in errors),id)

    def test_all_originals_are_distinct(self):
        rows=self.manifest['factions']['faction_brasshollow']
        for key in ('source_sha256','runtime_sha256','runtime_path'):
            self.assertEqual(len({rows[id][key] for id in variants.IDS}),27)
    ...
    def test_shared_variant_site_is_mutually_exclusive(self):
        pair={'building_brasshollow_blackbell_grand_assay_bell','building_brasshollow_whitegauge_breach_pressure_foundry'}
    ...
    def test_starter_is_not_fabricated_paid_progression(self):
        ...
        self.assertEqual(len(constructible),26)
        self.assertNotIn('building_brasshollow_ore_tithe_office',constructible)
        self.assertIn('building_brasshollow_brassbound_directorate',constructible)
        script=variants.script_text()
        self.assertIn('exercised.size()==26',script)
    ...
    def test_real_large_request_and_runtime_hero_identity(self):
        ...
        self.assertIn('hero_brasshollow_marka_ironclause',script)
        self.assertIn('town_brasshollow_orevein_gantry',script)
