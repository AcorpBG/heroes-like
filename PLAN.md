...
- Latest native RMG implementation update: `h3maped_rmg_core` now separates the recovered `0x4ba938` selector stack roles. The first selector argument is the `0x4bce6d` neighbor/probability mask and the second selector argument is the current-row preserve value (`-1` for ordinary `0x4bcfc3`, current art row for final-sweep current-record calls). The previous native code incorrectly used the current-row argument as the probability multiplier, which forced ordinary normal-terrain cells into the wrong flag-A row run. Focused no-Godot Medium seed-10 setup-1 still reaches final payload compare and remains blocked at `native_final_tile_stream_mismatch_against_same_run_0x49b2b6_payload`, but the first tile mismatch moved from native art byte `61` to `53` against H3MapEd `54`; terrain byte now matches for the first cell and remaining drift is a one-row TerrainPlacement/RNG/order mismatch, not final serializer behavior. Generated-object payload remains short at `10923` native bytes vs `17057` H3MapEd bytes. Runtime/native map output remains disabled. This is source-backed TerrainPlacement implementation progress, not final-payload parity or runtime generation authorization.
