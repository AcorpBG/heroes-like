...
- Normal small land `native_catalog_auto` generation now routes to `h3maped_small_port_generation_not_ready` rather than the archived generator, so no fake fallback map is produced while the remaining post-assignment h3maped phase sequence is still missing.
