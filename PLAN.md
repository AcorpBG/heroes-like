...
- Current implementation slice: `rmg-small-generalization-hardening-10184` is in progress. The active work is true end-to-end H3MapEd RMG parity for supported one-level land Small/Medium maps: entry inputs, template selection, private-state phase chain, and final map write. The R1-R7 recovery ledger is evidence only, not completion; native RMG parity remains incomplete until native output matches recovered H3MapEd final payloads and phase/private-state checkpoints.
- Latest implementation update: native reward guards now follow the recovered source-order attach path for positive guarded rewards. `0x49cf34`/`0x49d2e0` guard candidate filtering now consumes the active reward RNG at the recovered pre-`0x4aa9b7` order, `0x49d69d`/`0x4aa3e9` source-projected guard members are preferred for guard records, and the old native post-coordinate surrogate is only a fallback when no recovered source guard member is available. Linux native build passed, and `.artifacts/rmg_native_reward_guard_source_order_smoke_current` exported one controlled Small seed with phase snapshot; that smoke exercised `165` source guard-attach invocations/RNG calls, `6` source-projected guard records, and `4` fallback surrogate records. This is still not final RMG parity: exact selected reward descriptor/catalog identity before `0x4aa9b7`, complete coordinate/private-cell parity, route entry adoption, Medium parity, and Windows builds remain unresolved.
- Latest reward/template stream artifact: `.artifacts/rmg_native_small2p_seed58_reward_stream_fix_20260612153509`. Native Small template selection no longer uses the stale 13-template shortcut: Small and Medium now both use recovered catalog order and the focused 20-reference check passes `20/20` observed H3MapEd template selections for 2p Small land. Seed 58 now selects source catalog index `46` / `5SB0a`, matching the owner H3MapEd map, and runtime zone records now expose the correct 9-zone relation surface. The reward scheduler no longer gates `0x4aa354` membership on the inferred runtime byte `+0x3c`; that byte remains only the `0x4aadd2` town-choice counter input. Seed 58 now has `9/9` active reward relation records and `189` native reward attempts versus the recovered source stream's `195`, up from the prior wrong-template `48`. The focused export passes, Linux debug/release builds pass, and `tools/rmg_h3maped_template_selection_reference_check.py` plus `tools/rmg_h3maped_4aa354_source_stream_summary.py` preserve the evidence. This is not final route/materialization parity: the first recovered source `0x4aa9b7` call has two candidates and succeeds, while native still has zero candidates and first succeeds at attempt `16`; native route adoption remains denied because the exact coordinate/private-cell and descriptor/materialization stream is still not matched before `0x4a8260`.
- Current native-vs-H3MapEd finding: Current Linux/Python artifacts are `.artifacts/rmg_native_small2p_seed58_secondary_rng_counters_20260612` plus the prior medium checkpoint `.artifacts/rmg_native_medium_seed10_4a8260_scan_counters_20260612`. Native now exposes primary and secondary `0x4aa1db` reward candidate/template/position RNG accounting before `0x4a8260`: small seed-58 counts `56` value RNG calls, `1,730` primary lookup RNG calls, `2,938` secondary lookup RNG calls, `54` secondary position RNG calls, and `10` coordinate RNG calls. These account for `4,788` reward-phase calls against the observed native reward-to-route delta of `4,793`, leaving only `5` native-local unaccounted calls, but the recovered H3MapEd route entry is still offset `5,002` while native route entry is still offset `4,926`; the source parity blocker remains the same `76` upstream reward/materialization/guard RNG-call gap. `tools/rmg_reward_guard_constructor_gap.py` now models the recovered `0x4a5c07` constructor contract explicitly: descriptor-vector subtype filtering, selected descriptor terrain-mask proxy variants, source table scan order, and quantity-jitter RNG condition. The diagnostic also corrects an earlier comparison error: the current 29 guarded records are native-selected `0x4aa354` records after drift, while the `76` offset is a same-run H3MapEd/native route-entry gap. All modeled policies over those native-selected records still total `116` constructor+attach calls, so broad flattened-candidate constructor adoption remains rejected, but the report does not claim `116` is the H3MapEd same-run count. The source-backed blocker is the live `0x4aa354` callsite stream itself: selected reward descriptor/object, generator `+0x398/+0x39c` descriptor vector contents, selected descriptor `+0x94/+0x95/+0x20` state, and whether that same-run record enters `0x4a5c07/0x49cf34`. Native still exposes the recovered `0x4a8260` entry-vs-internal-scan distinction explicitly: small seed-58 enters the scan at bit26 `0` / bit27 `1,296`, then the source-backed internal `0x4a8291..0x4a82ce` object-vector scan produces post-scan bit26 `1,244` / bit27 `52`. The focused route verifier now separates recovered H3MapEd route mechanics from native adoption: route replay passes with H3MapEd `416` splits, `340` stamps, and `52` `0x4a80dc` cuts, but native adoption is denied because native route entry remains offset `4,926` vs H3MapEd `5,002` (`route_entry_offset_gap=76`), native diagnostic route counts are still `271` splits, `263` stamps, `34` `0x4a80dc` cuts, and native still reports `route_container_0x4a8260_active_adoption=false` / `route_container_0x4a8260_rng_boundary_exact=false`. Medium seed-10 remains non-parity: final tile payload mismatch is still `11,726` bytes with first drift `terrain_generated_cell_word_0x24_before_0x49b2b6`; corrected pre-`0x4a4c8e` private-state mismatches are tracked in the evidence correction below. The next implementation target remains the exact reward object/materialization and guard-attachment state before `0x4a8260` (`0x4a9f1c`, exact same-run `0x4a5c07` descriptor-vector filtering, dynamic `+0xf5c/+0xf58/+0x10b4`, `0x49d471`, `0x49cf34`, `0x49d69d`, `0x4aa9b7`, `0x4aa3e9`) so the route RNG/stamp stream can be adopted from an exact source state. Do not patch this with density scalars, art remaps, brute-force retries, Windows rebuild churn, broad flattened-candidate constructor adoption, or final-map delta tuning. Inner-loop validation is Linux `.so` plus Python parity compares; Windows `.dll` builds are deferred until a Linux-verified final boundary.
- Current evidence correction: `tools/rmg_h3maped_private_state_compare.py` now auto-detects the real generated-cell dump command in winedbg logs instead of parsing earlier `$esp`/`$esi` probes as cells. The corrected Medium seed-10 compare supersedes the stale no-base numbers above: pre-`0x4a4c8e` mismatches are `word_0x20=5,184`, `word_0x24=4,768`, `word_0x28=2,936`, with bit-state drift `bit26 native=5,106` vs H3MapEd `2,478` and `bit27 native=78` vs H3MapEd `1,311`. Native `generated_cell_decoration_bit_state` still reports `exact_upstream_bit_source_claim=false`; the blocker is the exact same-run route/reward/guard upstream state, not the final tile serializer or Windows build output.
- Latest source-stream coverage correction: `.artifacts/rmg_recovery/small_seed58_4aa354_source_stream_full_20260612145829/4aa354_source_stream_summary.json` contains a seed-58 same-run `0x4aa354` callstream summary with `195` completed calls, `150` guard-attach calls, and successful `0x4aa9b7/0x4aa3e9` handoffs. The source-stream coverage helper now classifies that artifact explicitly instead of reporting the seed-58 stream as wholly absent. The focused source-order `0x49cf34` guard attach has now been ported, but this does not authorize broader reward behavior guesses: remaining joins are selected descriptor/object identity, generator `+0x398/+0x39c` descriptor-vector state, selected descriptor `+0x94/+0x95/+0x20` state, and per-call comparison against native reward selection before `0x4a8260`. Windows `.dll` builds remain deferred until Linux/Python parity reaches a final boundary.
- Previous seed-58 handoff comparison: `tools/rmg_h3maped_4aa9b7_native_handoff_compare.py` compares the recovered H3MapEd `0x4aa9b7 -> 0x4aa3e9` success-handoff summary to a native phase snapshot. Against `.artifacts/rmg_native_small2p_seed58_guard_surrogate_rng_accounting_20260612`, the first `7/8` recovered source calls now match native at the coarse zero-candidate failure shape. The exact divergence is recovered source call `8`: H3MapEd has candidate count `2`, selected index `1`, and selected coordinate `(4,3,0)`, while the corresponding native prefix attempt still has candidate count `0` and fails. Native's first success occurs later after `18` false attempts, with candidate count `22`, selected index `12`, and coordinate `(4,12,0)`. One still-later native success has candidate count `2` and selected index `1`, but coordinate `(1,10,0)`. The recovered source trace is wrapper-specific and lacks the heap contents for the two candidate-vector coordinates; native records also do not expose the source wrapper pointer/relation, so this narrows the pre-route parity blocker to `0x4aa9b7` candidate availability/wrapper state without source-backing a native behavior patch yet. New Linux-only diagnostic snapshot `.artifacts/rmg_native_small2p_seed58_coordinate_scan_samples_20260612` samples the call-8 scan decisions and shows native rejects the source-selected coordinate `(4,3,0)` before footprint checks because native generated-cell owner byte is `1` while expected owner byte is `0`; the same cell has generated word `0x28` bit27 blocked. Route replay still denies adoption with the same `76` route-entry offset gap, so this is evidence for upstream generated-cell ownership/blocking drift, not a behavior fix.
- Current seed-58 owner-byte follow-up: Linux-only snapshot `.artifacts/rmg_native_small2p_seed58_source_zone_owner_gate_20260612` aligns the native `0x4aa9b7` expected owner byte with source-zone ids when a runtime record carries `source_zone_id`; generated cells already exposed high owner byte `1` for the source-selected `(4,3,0)` cell while the old expected owner was runtime index `0`. The focused route replay improves from route-entry offset gap `76` to `10` (`H3MapEd 5,002` vs native `4,992`), but full parity is still not achieved: the handoff compare now has only `1/8` prefix matches because native accepts earlier proxy reward candidates before the recovered source's first successful `0x4aa9b7` handoff. A tested guard-attach no-candidate gate over `.artifacts/rmg_native_small2p_seed58_guard_attach_failure_gate_20260612` was rejected because it worsened the route offset gap to `550`. The remaining implementation blocker is still exact same-run reward object/materialization/guard stream parity, not route adoption, density scaling, or Windows build output.
- Current `0x4a9f1c` / `0x4aa195` follow-up: static caller recovery corrects an earlier diagnostic claim. The `0x4a9f1c` stack `+0x20` byte tested before `0x4aa195` comes from the selected treasure-band low value passed through `0x4aa354 arg+0x10` and `0x4aa1db arg+0x10`; it is not the `0x4aab7e` retry-pass literal. Native now ports the recovered `0x4aa195` descriptor mask-extent helper and records the source accepted-vector shadow filter for primary and secondary `0x4a9f1c` lookups, but behavioral adoption remains disabled. Fresh seed-58 evidence in `.artifacts/rmg_native_small2p_seed58_4aa195_shadow_20260612123523` shows all `48` primary lookups request the filter, only `9/48` current native selected objects survive the source-shadow vector, route replay still passes with native adoption denied at route-entry gap `10`, and the handoff compare remains `1/8`. Earlier active probes are still rejected: retry-pass wiring in `.artifacts/rmg_native_small2p_seed58_value_extent_filter_20260612` kept handoff prefix at `1/8` and worsened the route-entry gap from `10` to `-28`; corrected low-byte wiring in `.artifacts/rmg_native_small2p_seed58_value_extent_lowbyte_20260612` improved handoff prefix only to `2/8` but destroyed route parity with gap `-1,228`. Conclusion: `0x4aa195` is implemented as source-backed shadow state, but it must not drive behavior until exact same-run `0x4aa354` selected descriptor/object state plus `0x49a6f9`/`0x4a5c07`/`0x49cf34` materialization are matched before route adoption.
- Current owner-byte extraction correction: recovered `0x4aa9b7` / `0x4a8c15` instructions read generated cell `+0x20` with `shl 8; sar 24`, which extracts signed byte2 bits `16..23`, not the high byte bits `24..31`. Native now uses that byte2 helper consistently in the owner gates and diagnostics. Fresh Linux/Python seed-58 snapshot `.artifacts/rmg_native_small2p_seed58_owner_byte2_current_20260612` confirms the diagnostic labels are current and the route replay still passes source mechanics but denies native adoption with route-entry offset gap `10` (`H3MapEd 5,002` vs native `4,992`), so this is a correctness fix, not final parity. The handoff compare remains `1/8` prefix full matches, and the refreshed constructor-gap diagnostic reports no active guard-constructor policy matching the `10`-call gap: `31` native-selected positive guard lookup records model as `120` constructor/attach calls, while native materializes only `4` surrogate guard-coordinate records and consumes `0` active `0x49cf34` RNG calls. The remaining source-backed blocker is unchanged: exact same-run `0x4aa354` reward/materialization/guard stream parity before `0x4a8260`.
