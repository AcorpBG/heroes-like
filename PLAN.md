...
- Latest native RMG implementation update: `h3maped_rmg_core` now exposes `run_h3maped_rmg_entry_to_writeout_workflow()` as the ordered entry-to-current-blocker workflow owner. `rmg_native_core.cpp` delegates native workflow phase/status/final-writeout state to that shared result and adapts its diagnostic owner-grid payload from the already-executed shared workflow instead of replaying setup/template/coordinate/object state locally. This removes the duplicate native workflow owner, but it is not parity or final map output: the shared workflow still blocks at `reward_guard_materialization` on `0x4ad947_projection_object_and_global_candidate_table_live_input_pending_before_selected_global_entry`.
- Latest native RMG implementation update: `rmg_native_core.cpp` routes the no-Godot CLI through a single native H3MapEd workflow that consumes the currently ported relation/object state instead of hiding it behind a generic partial-state stop. The shared relation-owner records carry the source pointer `+0x04` type and selected terrain policy `+0x0c` from the recovered runtime-zone/terrain-selection chain, and recovered `0x4ad7f7` relation-priority filtering uses those fields instead of a boolean test override. The shared chain now ports the recovered `0x4ad947` projection-driver selection rule before the source-relation lookup: scan 0x90 entries from `[0x57c7cc]+0x0c`, reject disabled entry `+0x10`, reject already-used generator `+0x1024` slots, require flag bit `0x02`, set generator `+0x10b4` when fewer than 20 entries are eligible, call `0x4e7276`, select `rng % eligible_count`, and write the selected global entry index to projection record `+0x1c`. It also carries the recovered `0x49ecf2` setup state used by that driver: generator `+0x10b4` is initialized to zero and the 0x90-byte generator `+0x1024` used-slot array is zero-filled before projection-driver use. The shared chain also ports the recovered `0x4ad947` caller-side source-relation lookup rule for explicit projection coordinates: read the projection cell generated-cell `word_0x20`, interpret its high byte as a signed owner vector index, and use that index into generator `+0x10e4/+0x10e8` before `0x4ad7f7`. The live workflow still sequences relation scan consumers (`0x4a1f3b/0x4a5767`), source-order object replay (`0x4a8d2c/0x4a8db2/0x4a901a/0x4a93a2/0x4a54a7`), relation high-owner propagation (`0x49a318`), and the recovered one-level-land `0x49f95a` reward/guard candidate-vector producer before blocking at `reward_guard_materialization` because the live `0x4aa354` selector relation argument, `0x4ad947` projection object/global table/coordinate triple, and selected descriptor/object feed are not yet owned. This is not parity and not final map output: reward/guard selector/materialization caller order, connection/road/river phases, and final tile/object/header writeout remain unowned.
