...
- The clean module now ports the `0x4a8d2c` direct settlement minimum field order, the owner-byte candidate-scan boundary of direct helper `0x4a93a2`, and the recovered `0x49a1d8` validity precheck used by `0x49aa93` for inspection only: seed 1 reports four player-castle direct calls, three assigned-owner candidate scans with 451 total owner/repaint candidates, 451 candidates passing the terrain validity precheck, and one recovered owner `-1` early-fail call for the unassigned player slot. Full `0x49aa93` footprint/collision eligibility, random tie choice, object stamping, `0x4a901a` weighted placement, and later cleanup remain pending.
- The clean module now exposes the recovered `0x49aa93` full direct-placement gate sequence for inspection only: `0x49a6f9` rectangle/footprint rejection, object metadata flags at `0x57c648 + type*0x10`, `0x49a09c` footprint passability/owner/occupied/water scan, the already ported `0x49a1d8` anchor validity precheck, owner-byte match, bit-22 object collision with metadata secondary flags, and final water/non-water terrain match. It also binds the `0x57c648` runtime object metadata path to recovered `objects.txt`/`objnames.txt` source data for inspection: 232 object type names, 1326 object rows, Town type `98` with 9 template rows, and Random Town type `77` with 1 template row. The Town text-mask body scan from `0x49a6f9` is now ported for inspection with 13 passability/body cells and 1 action cell; seed 1 reports 3 assigned-owner scans, 97 Town-footprint-eligible anchors, 42 eligible anchors for the first player-castle call, 2 unique direct town records stamped into the inspection object layer, and 26 bit-22-style occupied body cells marked. The third player-castle call is correctly left pending on the recovered `0x4a93a2` random tie because its closest Town-footprint set has 3 candidates. The blocker is no longer an unknown town-placement condition; it is the missing random tie consumer, full `0x49a09c` pass, project town writeout, weighted placement, and later guard/reward/object passes.
