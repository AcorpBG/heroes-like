...
- Latest native RMG implementation update: `h3maped_rmg_core` now carries the recovered `0x4602c1 -> 0x4adfe1 -> 0x49ecf2 -> 0x49d914` direct-entry optional handler path through the native no-Godot workflow. The `0x4adfe1` caller supplies a non-null stack-local optional handler as arg9; native records it as the recovered direct-entry handler sentinel, stores it at generator `+0xed4`, marks the `0x49d914` virtual-init callback invoked with offset `width * height + 0x4fcf4 + 0x3bc4`, and treats later optional handler calls as non-map-mutating progress/handler callbacks instead of a private-state blocker. Focused no-Godot Medium seed-10 setup-0 now has full `0x49ecf2` setup args, `0x49d914` applied, same-run H3MapEd authority scope matched, and same-run tile/object payload compare invoked. It still blocks at `native_final_tile_stream_mismatch_against_same_run_0x49b2b6_payload`: tile payload first mismatch offset `0` (`native=4`, `expected=2`) and generated-object payload first mismatch offset `0` (`native=4`, `expected=2`). Runtime/native map output remains disabled. This is source-backed setup/private-state progress, not final-payload parity or runtime generation authorization.
