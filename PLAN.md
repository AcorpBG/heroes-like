...
- Current checkpoint-2 finding: refreshed no-Godot snapshots in `.artifacts/rmg_native_cli_post_object_vector_handoff/` now carry the recovered `0x4a5767 -> 0x4a59e2` full-grid reset evidence, the same-profile template-vector precondition for Medium seed 10 `hc4` setup 0, the same-run selected-candidate relation topology, the source-backed `0x49a318` source-cell clear primitive, the recovered `0x5a2658` 8-neighbor direction table, and the same-run 13-entry `generator+0x10e4..+0x10e8` runtime relation pointer vector captured from `.artifacts/rmg_recovery/medium_seed10_hc4_comp0_4a4c8e_relation_records_20260615/`. The vector is materialized for the reference case (`runtime_relation_vector_record_count=13`, begin `0x03622dc0`, end `0x03622df4`, capacity `0x03622e00`), while flat template links and selected-candidate records remain correctly marked as not that runtime pointer vector. The generated-cell helper and relation-normalization summaries now consume the post-town/object-vector private grid (`input_grid_stage=after_town_object_vector_bit_state`) instead of the raw terrain-live-feedback grid, so relation prerequisites now see native bit26/bit27 `2264/1384` instead of the old all-bit27 `0/5184`. Full native runtime replay is still not complete: `runtime_ordered_replay_materialized=false`, `generated_cell_word_0x20_low_word_propagation_materialized=false`, `generated_cell_word_0x1c_projection_gate_materialized=false`, `generated_cell_projection_triple_materialized=false`, `object_reference_vector_filter_materialized=false`, `descriptor_policy_table_materialized=false`, and `generated_cell_mutation_replay_complete=false`. The focused verifier passes against the recovered relation-normalization summary, R6 semantic closure summary, selected-candidate relation summary, and the post-object-vector private-state compare. Checkpoint 2 remains pending: the Medium `hc4_comp0` setup `0` compare at `after_town_object_vector_bit_state` still mismatches H3MapEd with `word_0x20_low_word_mismatch=4587`, `word_0x20_high_word_mismatch=475`, `owner_byte2_signed_mismatch=475`, `word_0x24_mismatch=4768`, and `word_0x28_mismatch=3241`; native bit26/bit27 is `2264/1384` vs H3MapEd `2478/1311`. The next implementation target is source-backed `0x4a5767/0x49a318` runtime replay over this post-object-vector grid, with descriptor-policy/object-reference filters named as blockers if they cannot be implemented from recovered evidence.
