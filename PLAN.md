...
- Current native-vs-H3MapEd finding: Medium seed-10 input/template identity is now corrected for the controlled 4-player land reference. Native manifest-derived export preserves `human_count=4`, `player_count=4`, `computer_count=0` and selects H3MapEd template 015 / `2SM4d(2)` from a 23-entry accepted vector, matching the H3MapEd selector trace (`71 % 23 = 2`). Native now uses the recovered `0x49acf6`/`0x49b2b6` final flag placement (`cell+0x28` bits 15..21 -> final tile byte 6) and has adopted the recovered `0x4a2ec3`/`0x4a4082` relation-rectangle `cell+0x28` bit28 path for TerrainPlacement eligibility. The pinned Linux Medium seed-10 compare still reports 14,201 final tile byte mismatches and first drift remains `terrain_generated_cell_word_0x24_before_0x49b2b6`; native bit28 is 4,652 cells vs H3MapEd 5,181, with `word_0x20`/`word_0x24` still mismatching all 5,184 cells. The named blocker is source-backed owner/span/terrain private-state parity around `0x4a2777`, `0x4a325d`, and `0x4a3f27`. Do not patch this with density scalars or art remaps. Inner-loop validation is Linux `.so` plus Python parity compares; Windows `.dll` builds are deferred until a Linux-verified final boundary.
