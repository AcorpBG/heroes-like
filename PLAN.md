...
- Native RMG alignment checkpoints, in execution order:
  ...
  3. Reward/object identity: carry source records/descriptors through selection and materialization instead of resolving final identity through placeholder object ids. The recovered `0x49da08` object source-record catalog, metadata `+0x08` `0xe8` `0x49db76` wrapper-bucket lane layer, sampled `0x4af785` wrapper reuse/copy/create support model, sampled `0x4af89f`/`0x4a9e40` selector mechanics, recovered `0x4903e8` descriptor/source join prep for contexts `45/53/54/79`, copied `0x4c` source-record identity, and explicit prepared `0x4a54a7` object commit path are now exposed in shared C++ and mirrored through the native wrapper. Full live weighted materialization still waits on source-order object-record-key callers such as `0x4a8d2c/0x4a8db2/0x4a93a2/0x4a901a`.
