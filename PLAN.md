...
- Latest native RMG implementation update: `h3maped_rmg_core` now treats generated-cell `+0x2b` as the recovered high byte of `+0x28` instead of an independent unknown byte. Record reset, overlay, projection, object/reference, reward/guard, and connection mutators now keep `+0x2b` synchronized from `+0x28`; `0x49a1d8` validity uses bit25 and terrain exactly as the recovered byte semantics imply; and the simplified `0x49aa93` eligibility path no longer rejects bit27 when the recovered contour call has `require-bit27=false`. This is live workflow behavior, not a report: the no-Godot Medium seed-10 workflow now reaches one weighted `0x4a901a -> 0x4a54a7` source-order commit (`source_order_scheduler_commit_count_0x4a8db2=1`, `object_record_vector_append_count_0x4a54a7=1`) before the same connection-tail blocker. Small seed 11 still stops at missing scoped fallback records, and Medium seed 10 still stops at `0x4a79a3_fallback_materialization_0x4a7605_0x4a5e03_blocked_at_record_0_0x4a5e03_target_word_0x20_mismatch`. The next implementation target remains the upstream generated-cell target-word prestate/caller order that makes the recovered Medium `0x4a7605 -> 0x4a5e03 -> 0x4a54a7` commits apply, then the recovered connection tail, roads/rivers, and final tile/object/header writeout. This is not parity and not final map output.
- Previous native RMG implementation update: `h3maped_rmg_core` now carries recovered relation-owner reward/guard treasure bands `+0xa0/+0xa4/+0xa8` through `+0xc0`, terrain policy `+0x0c`, stale supported-land cursor `+0xf5c = 0x7a1befdf`, zeroed `+0xf60/+0xf64` terrain-pressure state, recovered `0x57cea0` monster terrain rows, and `0x531cc4` type-17 subtype-to-monster rows into the ordered reward/guard selector. The active no-Godot workflow advances through `0x4aab7e -> 0x4aa354 -> 0x4aa1db -> 0x4a9f1c -> 0x4a9e40`, allocates ordinary `0x540a88` selected records through generator key and `+0xf44` sequence state, seeds wrapper selected members, stamps initial wrapper cells, refreshes bounds, completes the reward/guard source-stream phase, invokes recovered `0x49eb8d` flagged-cell pass-1, and now enters a workflow-owned `0x4a79a3` connection-tail handoff with fallback records scoped to the exact recovered case that produced them.
