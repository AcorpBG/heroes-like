...
- Current checkpoint-2 finding: refreshed no-Godot snapshots in `.artifacts/rmg_native_cli_4a79a3_payload_order_snapshot/` expose the existing checkpoint-2 diagnostic surfaces plus the recovered `0x4a79a3` payload order. The standalone CLI now emits `plain_cpp_object_vector_payload_order_summary` with the recovered 19-record H3MapEd payload vector from `0x4a79a3/0x4a7d2c/0x4a7d36/0x4a7d99`: vector entries match record pointers, vtable counts are `0x00540a9c=8` and `0x00540a88=11`, and each row carries record pointer, descriptor pointer/source pointer, coordinates, fields `+0x1c/+0x20/+0x24/+0x28/+0x2c`, and the 12 recovered record dwords. `tools/rmg_h3maped_4a79a3_payload_order_verify.py` passes against `.artifacts/rmg_recovery/4a79a3_payload_trace_summary.json` (`native=19`, `h3maped=19`, `record_mismatches=0`). The R4 descriptor/source identity verifier, `0x49cf34` attach verifier, `0x4a56b6` projection-write verifier, and seed-58 `0x4a8260` route replay verifier still pass. This remains diagnostic-only: `native_object_vector_order_materialized=false`, `generated_cell_mutation_replay_complete=false`, and `projection_write_coordinates_materialized=false`; route adoption remains denied. The corrected Medium `hc4_comp0` setup `0` private-state compare still mismatches H3MapEd at `after_terrain_live_feedback`: `word_0x20_low_word_mismatch=5184`, `word_0x20_high_word_mismatch=475`, `owner_byte2_signed_mismatch=475`, `word_0x24_mismatch=4768`, and `word_0x28_mismatch=4065`; native bit26/bit27 remains `0/5184` vs H3MapEd `2478/1311`. Checkpoint 2 therefore remains pending. The next implementation target is to connect native live object-vector records to this recovered `0x4a79a3` order and replay the downstream `0x4a696b/0x4a7605` generated-cell mutations, while still carrying same-run generator descriptor vector `+0x398/+0x39c` and selected descriptor state `+0x94/+0x95`.
