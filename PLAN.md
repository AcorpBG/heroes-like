...
- Latest native RMG implementation update: `h3maped_rmg_core` now removes another source-backed TerrainPlacement drift and exposes corrected final-object pass-split identity. Base TerrainPlacement `0x4bcfc3` no longer accepts or preserves a current art row; recovered `0x4bcfc3` passes `-1` into the toolkit selector, while current-record preservation remains confined to the separate recovered final-sweep/classified path. Final-object payload pass splitting continues to use the object-record descriptor/provider type carried as native `descriptor_type_0x1c`, matching the recovered `0x4ad1e3` chain `object record +0x04 -> [provider] -> +0x1c`; final-object samples now expose that resolved pass-split type separately from the copied source-record type used for definition identity. `h3maped_rmg_core_selftest` remains green. Focused no-Godot Medium seed-10 setup-0 still fails closed at `native_final_tile_stream_mismatch_against_same_run_0x49b2b6_payload`: tile bytes remain 36288 vs 36288 with first mismatch offset 1 (`native=49`, `h3maped=54`); generated-object payload improved from 9088 to 11344 native bytes but remains short against 17057 H3MapEd bytes with first mismatch offset 0 (`native=21`, `h3maped=2`). This is source-backed implementation progress, not final-payload parity or runtime map-output authorization. The active blockers remain upstream generated-cell/TerrainPlacement private state before `0x49b2b6`, plus generated-object vector/source-pointer materialization parity before `0x4ad3eb` (`native generated objects=901` vs H3MapEd `1212`, native flagged first-pass objects=18 vs H3MapEd much larger same-run `0x4ad36f` callstream).
