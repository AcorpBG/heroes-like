...
72. Only after these basics are deep enough and art-direction evidence starts to exist, return focus to implementation planning, campaign/skirmish maps, town-screen polish, battle-screen polish, battle AI systems, and the overall player loop: turn flow, economy, resource collection, mine capturing, battles, and town development.

Recommended next slice:
- Run `strategic-ai-hero-task-state-save-normalizer-preservation-planning-10184` as the next concrete slice.
- Use `docs/strategic-ai-hero-task-state-adoption-sequencing-plan.md` as the current decision boundary: the next proof should plan report-only preservation checks for future optional `enemy_states[].hero_task_state`, old-save absence, malformed-state tolerance, no `SAVE_VERSION` bump, and `SaveService` semantics staying outside strategic AI task ownership.
- Keep the next slice planning-only: define a future normalizer preservation report and fixture expectations before any schema write, save migration, live behavior switch, or `SAVE_VERSION` change.
...
- Treat commander-role report output, commander-role adoption sequencing, live-turn transcript/report planning, live-turn transcript/report implementation, live-turn transcript/report gate review, hero task-state boundary planning, hero task-state report fixture planning, hero task-state report implementation, hero task-state report gate review, and hero task-state adoption sequencing as passed gates. The next AI slice should plan a report-only save-normalizer preservation proof while keeping schema writes, save migration, durable event logs, full AI hero task-state implementation, defense-specific durable state, coefficient tuning, live commander-role behavior, and live AI hero behavior deferred.
...
- The next active choice is report-only strategic AI hero task-state boundary report implementation after the completed fixture plan, still without production JSON migration, new resource registry, `wood` to `timber` migration, rare-resource activation, market-cap overhaul, pathing/body-tile/approach adoption, full AI hero/task implementation, broad AI rewrite, behavior tuning, editor behavior, renderer behavior, save format changes, generated PNG import, asset import, live commander-role behavior adoption, schema writes, save migration, durable event logs, or new defense-specific durable state.
...
Current target: plan report-only strategic AI hero task-state save-normalizer preservation after completed adoption sequencing.
...
- The next slice plans a report-only save-normalizer preservation proof because `EnemyTurnRules.normalize_enemy_states(...)` currently rebuilds known enemy-state fields and would drop a future optional `hero_task_state` unless intentionally preserved or normalized.
