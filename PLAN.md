...
- Latest native RMG implementation update: `h3maped_rmg_core` now carries the recovered `rand_trn.txt` score rows for decorative dispatch: 109 obstacle score records and 2425 per-template score variants are generated into the native catalog, `0x49e700` applies the recovered terrain-score lookup, and score rows at or below the hard reject value `-5000` are skipped before scorer entry. No-Godot controlled snapshots for Small seed 10, Small seed 58, and Medium seed 10 now all fail closed later at `0x49e700_0x49e1bf_descriptor_0x30_0x40_adjacency_overlap_scoring_replay_unowned`; the first scorer candidate remains source row `312`, type `119`, DEF `AVLd1sn0.def`, with the accepted score row varying by terrain (`snow dead vegetation` for the Small cases, `cave dead vegetation` for the Medium case). This is source-order workflow progress, not parity completion; final payload generation remains disabled until exact `0x49e1bf` descriptor adjacency/overlap/object-reference scoring replay, road/river adjacency, descriptor counter replay, and final writeout are source-owned.
