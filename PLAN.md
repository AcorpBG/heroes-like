...
- Current checkpoint-2 finding: refreshed Python-only Medium seed-10 compares in `.artifacts/rmg_native_checkpoint5_route_gating_current/private_state_compare_pre_0x4a4c8e_same_run.json`, `private_state_compare_after_terrain_live_feedback_same_run.json`, and `private_state_compare_after_object_vector_private_grid_same_run.json` expose the owner surface directly. H3MapEd has 13 non-negative owner ids (`0..12`) for selected template `2SM4d(2)` with 10 source zones, while the earlier native checkpoint had 9 non-negative owner ids. This is not checkpoint parity yet. The no-Godot controlled-case path now resolves supplied setup `+0x44` values and replays the recovered same-level synthetic runtime-zone append surface: setup value `3` consumes one `0x4e7276() % 3` RNG before template selection; Small 2p seed 58 resolves first RNG `228` to mode `0`, skips the synthetic branch, and runs boundary/span fill over 5 runtime zones; Medium 4p seed 10 resolves first RNG `71` to mode `2`, selects `7SB0c`, scans 56 `0x4a3b48` directions, appends 8 `0x49b452` synthetic runtime zones, and runs boundary/span fill over 15 runtime zones. The plain-C++ `0x4a2777` boundary replay now follows the recovered source-edge owner gate and both-end clip traversal: Small seed 58 skips 7 gated source-edge writes and materializes 1296 owner cells; Medium seed 10 skips 33 gated source-edge writes and materializes 5183 owner cells. Phase snapshots emit `after_boundary_span_fill_owner_words`, a generated-cell checkpoint that materializes `0x4a2777/0x4a325d` owner byte-2 writes over constructor-default generated-cell words. Phase snapshots now also emit `after_runtime_terrain_cell_writeout`, a no-Godot checkpoint for recovered `0x49b53d/0x540908` runtime terrain selection and `0x4a3f27` private terrain-code writeout. Focused smoke evidence selected terrain ids `[2,0,2,3,7]` for Small seed 58 and `[0,2,2,2,2,7,5]` for Medium seed 10; Small materialized 1296 selected terrain cells, while Medium materialized 1636 selected terrain cells and left 3548 cells at default terrain `8` because same-level synthetic/runtime-default cells do not have selected terrain ids. Phase snapshots also emit `after_terrain_relation_eligibility`, a no-Godot checkpoint for recovered `0x4a2105/0x4a2ffa` filled-zone geometry and `0x4a2ec3` owner-gated generated-cell word `0x28` bit-28 writes over the terrain-selected owner grid. The remaining implementation target for checkpoint 2 is exact same-run setup `+0x44` capture/defaulting when not supplied, then rerunning pre-`0x4a4c8e` generated-cell word compares and porting live visual-feedback generated-cell mutations needed for parity.
- Current guardrails: do not re-open H3MapEd reverse engineering for this slice unless Python/Ghidra/Wine evidence disproves the recorded recovery, do not add density scalars/gates/brute-force retries/final-map tuning, do not use GDScript for reports/exports, and do not launch Godot/headless Godot for native RMG export on this memory-constrained host. `tools/rmg_native_batch_export.py` now only supports the standalone no-Godot `bin/rmg_native_batch_export_cli` path; the legacy Godot runner has been removed from this wrapper. The no-Godot CLI now owns plain-C++ controlled-case parsing/filtering, checkpoint-2 phase snapshots with constructor-default generated-cell words, post-boundary owner-byte generated-cell checkpoints, runtime terrain selection and private terrain-code writeout checkpoints, post-relation-eligibility bit-28 generated-cell checkpoints, embedded-catalog template selection/runtime-zone records, recovered `0x49ecf2` generator-mode resolution for supplied setup `+0x44`, link-seed extraction, `0x4a1f3b` coordinate replay summaries, `0x4a3a03/0x4cc788/0x4ccb64/0x4ccdfc` source-node footprint summaries, `0x4a3b48/0x49b452` same-level synthetic runtime-zone append replay, owner-gated `0x4a2777/0x4a325d` boundary/span-fill owner-word summaries, `0x49b53d/0x540908/0x4a3f27` runtime terrain writeout summaries, and `0x4a2105/0x4a2ffa/0x4a2ec3` terrain-relation eligibility summaries for supported Small/Medium one-level land cases, but still fails closed for `.amap` generation until the recovered H3MapEd RMG generation core is split from Godot `Dictionary`/`Array`/`String`, `RefCounted`, and `FileAccess` APIs into plain C++ data structures. Do not treat the initial generated-cell grid, post-boundary owner-word grid, post-terrain-writeout grid, post-relation-eligibility grid, template/runtime-zone summary, link-seed summary, coordinate replay summary, synthetic append summary, source-node summary, boundary/span-fill summary, runtime terrain summary, terrain-cell writeout summary, or terrain-relation eligibility summary as pre-`0x4a4c8e` parity; live visual-feedback mutations and exact same-run compares remain pending. Do not build Windows `.dll` outputs until Linux `.so` parity checks reach the final boundary.
