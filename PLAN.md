...
- Latest native RMG implementation update: `h3maped_rmg_core` now ports the recovered reward/guard `0x4aab7e` source-stream caller boundary and the corrected `0x4aa354` argument contract. `0x4aa354` now consumes the caller-owned wrapper, policy word, low/high value band, and selected value before entering `0x4aa1db`, and `0x4aab7e` owns the three-lane source triplet scheduling loop, policy `0`/`1` retry order, and immediate `0x4aa9b7` coordinate-scan handoff. The active no-Godot workflow enters `0x4aab7e` and blocks at `0x4aab7e_source_triplet_0xa0_0xa4_0xa8_pending_before_0x4aa354_0x4aa9b7`, which means the next implementation target is the live source object producer for the `+0xa0/+0xa4/+0xa8` triplet and owner-kind `+0x0c`, then selected descriptor/materialization state, the `0x4ad947` global table/coordinate inputs, wrapper/member handoff, connection/road/river phases, and final tile/object/header writeout. This is not parity and not final map output.
