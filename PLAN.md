...
- Native catalog-auto generation now has generalized size/level-aware object-density floors, eligible underground decoration/scenic/reward-reference distribution, deterministic underground road cells, and strict size-aware town spacing that skips extra same-zone towns. A four-case Large islands/normal-water export wrote `4/4` native packages, and `tools/rmg_fast_validation.py` parsed `18` owner H3Ms plus those `4` AMAPs in about `5.945s` with `0` parse failures, `0` density gaps, and `0` native rule failures. Exact owner count/topology deltas remain diagnostics, not production pass/fail targets.
