...
- Current native-vs-H3MapEd finding: Current Linux/Python artifacts are `.artifacts/rmg_native_medium_seed10_4a8260_route_diagnostic_gate_20260612` and `.artifacts/rmg_native_small2p_seed58_4a8260_route_diagnostic_gate_20260612`. Native now ports the recovered `0x4a8260` route mechanics (`0x4a80dc` cuts plus `0x49a85d` stamps) into the generated-cell phase, but live adoption is gated off (`route_container_0x4a8260_active_adoption=false`, `exact_upstream_bit_source_claim=false`) because the same-run RNG entry boundary is still wrong. Small seed-58 route verification still passes the recovered H3MapEd route stream mechanically (`416` splits, `340` stamps, `52` `0x4a80dc` cuts at H3MapEd route entry offset `5,002`), but native has no matching candidate route-entry state; the closest current native candidate is `reward_preview_rng_after_0x4aa354_uint32` at offset `4,926`, leaving the exact post-reward/materialization RNG boundary unwired. Medium seed-10 remains non-parity: final tile payload mismatch is `11,726` bytes with first drift `terrain_generated_cell_word_0x24_before_0x49b2b6`, and pre-`0x4a4c8e` private-state mismatches are `word_0x20=5,184`, `word_0x24=4,850`, `word_0x28=2,999`. The next implementation target is the exact reward object/materialization and guard-attachment state before `0x4a8260` (`0x4a9f1c`, dynamic `+0xf5c/+0xf58/+0x10b4`, `0x49d471`, `0x49cf34`, `0x49d69d`, `0x4aa9b7`, `0x4aa3e9`) so the route RNG/stamp stream can be adopted from the correct source state. Do not patch this with density scalars, art remaps, brute-force retries, Windows rebuild churn, or final-map delta tuning. Inner-loop validation is Linux `.so` plus Python parity compares; Windows `.dll` builds are deferred until a Linux-verified final boundary.
