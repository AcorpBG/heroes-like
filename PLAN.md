...
Current broad instrumentation follow-up: expand opt-in JSONL profiling through `HEROES_PROFILE_LOG=1` into boot, menu/generated setup, router scene transitions, save/autosave, end-turn, town, and battle surfaces while keeping the legacy `HEROES_OVERWORLD_PROFILE_LOG=1` overworld log compatible. Usage and record semantics are documented in `docs/profile-jsonl-usage.md`. This is evidence collection only; optimization, save-schema migration, route/pathing semantics changes, renderer/fog changes, and generated-map density/content changes remain out of scope.
