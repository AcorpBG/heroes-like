...
71. Only after these basics are deep enough and art-direction evidence starts to exist, return focus to implementation planning, campaign/skirmish maps, town-screen polish, battle-screen polish, battle AI systems, and the overall player loop: turn flow, economy, resource collection, mine capturing, battles, and town development.

Recommended next slice:
- Run `strategic-ai-hero-task-state-adoption-sequencing-planning-10184` as the next concrete slice.
- Use `docs/strategic-ai-hero-task-state-report-gate-review.md` as the current evidence boundary: report output proves deterministic candidate task ids, actor/task ownership, lifecycle, target reservation, invalidation, save/schema absence, and public leak boundaries through derived/report-only candidate tasks.
- Keep the next slice planning-only: compare minimal `hero_task_state` schema planning, another report-only route/execution boundary, live AI hero behavior adoption, and pausing task-state work for another foundation track before any schema or live behavior switch.
...
- Treat commander-role report output, commander-role adoption sequencing, live-turn transcript/report planning, live-turn transcript/report implementation, live-turn transcript/report gate review, hero task-state boundary planning, hero task-state report fixture planning, hero task-state report implementation, and hero task-state report gate review as passed gates. The next AI slice should sequence the next task-state adoption decision while keeping schema writes, save migration, durable event logs, full AI hero task-state implementation, defense-specific durable state, coefficient tuning, live commander-role behavior, and live AI hero behavior deferred.
