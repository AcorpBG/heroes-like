...
- Latest native RMG implementation update: `h3maped_rmg_core` now treats recovered `0x4a7312` direct endpoint materialization as a different target-cell policy from `0x4a5e03` fallback materialization. Direct endpoint records may commit through `0x4a54a7` when the target generated-cell object-reference vector is already non-empty, while fallback/pre-border records still fail closed on non-empty target vectors. Focused no-Godot Small seed-58 setup-3 still reaches final payload compare and blocks because the available same-run authority profile is Medium-scoped. Focused no-Godot Medium seed-10 setup-0 now applies connection-tail replay, completes final header/tile/object payload assembly, writes 1,073 native generated-object records / 13,557 generated-object payload bytes, and blocks at `final_payload_compare` with `native_final_tile_stream_mismatch_against_same_run_0x49b2b6_payload`: tile stream length is 36,288 bytes with first mismatch offset 0 (`native=4`, `h3maped=2`), and generated-object payload first mismatch offset 0 (`native=3`, `h3maped=2`). Runtime/native map output remains disabled. This is source-backed connection-tail implementation progress, not final-payload parity or runtime generation authorization.
