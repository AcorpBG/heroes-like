...
- Current native-vs-H3MapEd finding: Medium seed-10 input/template identity, coordinate replay, selected runtime terrain ids, recovered `0x4a2777` dual endpoint clipping, map-border continuation, per-edge owner writer skip, `0x4bbfcc` final-sweep scratch mask/class correction, the source-backed town-only `0x49abd6 -> 0x4a8260` object-reference vector boundary, the recovered `0x4ac552` reward active-vector scheduling surface, and the recovered `0x4a9e40` reward template selector filter are ported for the controlled land references. The current Linux/Python artifacts `.artifacts/rmg_native_small2p_seed58_selector_4a9e40_20260611` and `.artifacts/rmg_native_medium_seed10_selector_4a9e40_20260611` show the selector now scans the metadata bucket, matches source record `+0x20`, applies source record `+0x24` group behavior, and uses the `+0x18` terrain-lane mask instead of native reward-category terrain shortcuts. Seed-58 now makes `47` reward value/object attempts and native reward preview exits at RNG offset `1,780`, still short of the H3MapEd route-entry offset `5,002`; Medium seed-10 pre-`0x4a4c8e` `word_0x28` mismatches remain `2,999`, while final tile byte mismatches move to `11,707` with first drift still `terrain_generated_cell_word_0x24_before_0x49b2b6`. Native RMG parity remains incomplete. The remaining blocker is exact reward object/materialization and guard-attachment state inside the active scheduler calls (`0x4a9f1c`, dynamic `+0xf5c/+0xf58/+0x10b4`, `0x49d471`, `0x49cf34`, `0x49d69d`, `0x4aa9b7`, `0x4aa3e9`) before the same-run `0x4a8260` route RNG/stamp stream can align. Do not patch this with density scalars, art remaps, brute-force retries, Windows rebuild churn, or final-map delta tuning. Inner-loop validation is Linux `.so` plus Python parity compares; Windows `.dll` builds are deferred until a Linux-verified final boundary.
