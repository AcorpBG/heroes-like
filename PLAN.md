...
- Completed implementation slice: `presentation-battle-resonant-chorus-distinct-audio-10184`. Resonant Chorus now owns a distinct reproducible 290 ms production-layer Battle sound and exact `audio_spell_resonant_chorus` identity while Prism Bastion retains its cue and the existing Effects-bus, priority, cooldown, and accessibility policies remain unchanged. A real public cast proves imported playback, sequenced repeat suppression, unchanged spell/session/VFX authority, and normal/reduced-motion/reduced-flash parity. Animation/cue/spell/Prism/core, repository/editor, Linux export/headless startup, and Windows export/fresh-Wine Boot/MainMenu/native-DLL gates pass. Final sound-design/mastering approval, packaged Battle listening, controller hardware, AT-SPI/UIA, native hardware, signing/publication, whole-game, and release readiness remain unclaimed.
  id: `presentation-battle-resonant-chorus-distinct-audio-10184`
- Completed implementation slice: `presentation-battle-resonant-chorus-distinct-vfx-10184`. Resonant Chorus now owns a distinct original imported 384x384 battle VFX and exact `vfx_spell_resonant_chorus` runtime identity while Prism Bastion retains its visual and both retain the existing shared audio fallback. A real public cast proves normal imported playback, reduced-motion/strong-flash suppression, immutable spell/session authority, and exact 1280x720/1920x1080 draw geometry. Spell/cue, Battle layout, accessibility, core, repository/editor, Linux export/headless startup, and Windows export/fresh-Wine Boot/MainMenu/native-DLL gates pass. Packaged Battle interaction, final Resonant audio, controller hardware, AT-SPI/UIA, native hardware, signing/publication, whole-game, and release readiness remain unclaimed.
  id: `presentation-battle-resonant-chorus-distinct-vfx-10184`
...
- Completed implementation slice: `performance-overworld-refresh-watch-context-reuse-10184`. End Turn Forecast, Command Commitment, Event Feed, and Field Dispatch now share one detached refresh-local command-risk, local-pressure, and management-watch context while direct APIs remain fresh. Focused runtime covers seven states with exact whole outputs, detached rebuild, and session/UI authority; the broad-state median improves from2.988s to1.751s. Overworld visual/route/input, Town return, core, repository/editor, Linux export/headless startup, and Windows export/fresh-Wine Boot/MainMenu/native-DLL gates pass. Packaged interaction, controller hardware, AT-SPI/UIA certification, native hardware, signing/publication, whole-game, and release readiness remain unclaimed.
  id: `performance-overworld-refresh-watch-context-reuse-10184`
...
- Completed implementation slice: `performance-overworld-full-refresh-drawer-readiness-reuse-10184`. One full Overworld status refresh now passes its already-current no-base Field Readiness payload into tooltip/context-drawer handoff synchronization, while distinct event-feed readiness and default callers remain fresh. A same-process legacy control proves the removed derivation adds 191 ms, or 40.3 percent of the optimized 474 ms refresh, with exact session and whole-surface authority. The authored repeat refresh improves from 4.69 to 4.37 seconds; clean first-ready remains within the 8.99-9.23 second control range; the heavy edited-working-copy round trip remains behavior-exact within 1.7 percent wall-time variance, so no broad Play Copy speedup is claimed. Focused, compatibility, repository/editor, Linux export/headless startup, and Windows export/fresh-Wine Boot/MainMenu/native-DLL gates pass. Packaged interaction, controller hardware, AT-SPI/UIA certification, native hardware, signing/publication, whole-game, and release readiness remain unclaimed.
  id: `performance-overworld-full-refresh-drawer-readiness-reuse-10184`
- Completed implementation slice: `performance-map-editor-inspect-route-projection-elision-10184`. Every Map Editor tool now disables the shared gameplay hero-route projection while the shared Overworld MapView remains route-enabled by default for gameplay. Three distant public Inspect clicks on the authored 64x64 Ninefold working copy fall from the 651-673 ms current-HEAD baseline to 20.3-20.8 ms while preserving exact selected tile, terrain/object/link inspection, authoring/readiness contract, read-only tool cue, session, dirty, and terrain-result authority. Broad Map Editor, package load/Save Copy, shared Overworld visual, core, repository/editor, Linux export/headless startup, and Windows export/fresh-Wine Boot/MainMenu/native-DLL startup gates pass. Packaged Editor interaction, controller hardware, AT-SPI/UIA certification, native hardware, signing/publication, whole-game, and release readiness remain unclaimed. Select the next tracker-approved release-readiness implementation slice.
  id: `performance-map-editor-inspect-route-projection-elision-10184`
- Completed implementation slice: `performance-map-editor-terrain-paint-normalization-defer-10184`. Live Map Editor action-tool clicks now defer the validation-only 64x64 final-normalization report and skip gameplay-only hero-route projection; validation snapshots materialize the exact eager report on demand, and the subsequent Inspect-elision slice extends route suppression to every editor tool while leaving gameplay MapView defaults intact. Focused eager-control parity measures complete terrain clicks at about 21-22 ms versus 113-115 ms for the old normalization lane alone, and the full Map Editor smoke falls from about 910 seconds to 238 seconds with its terrain-transition block reduced from about 760 seconds to 99 seconds. Package load/Save Copy, shared Overworld visual behavior, core, repository/editor, Linux export/headless startup, and Windows export/fresh-Wine Boot/MainMenu/native-DLL startup gates pass. Packaged Editor interaction, controller hardware, AT-SPI/UIA certification, native hardware, signing/publication, whole-game, and release readiness remain unclaimed. Select the next tracker-approved release-readiness implementation slice.
  id: `performance-map-editor-terrain-paint-normalization-defer-10184`
- Completed implementation slice: `presentation-aurelion-reach-main-menu-crest-art-10184`. The compact Main Menu logo pocket now renders one original transparent gold-and-navy frontier watchtower/compass crest through its existing 56x52 mark slot, replacing the generic procedural menu glyph without changing the exact `KINGDOM FRONT` eyebrow, `AURELION REACH` title, subtitle, pocket geometry, scenic negative space, or commands. Imported-pixel alpha/corner/coverage/chroma checks, focused menu/outcome runtime, live X11 inspection at 1280x720 and 1920x1080, shell heraldry, accessibility, full physical keyboard navigation, core, repository/editor, Linux export/headless startup, and Windows export/fresh-Wine Boot/MainMenu/native-DLL startup gates pass. Packaged Main Menu interaction, controller hardware, AT-SPI/UIA certification, native hardware, final marketing/trademark approval, signing/publication, whole-game, and release readiness remain unclaimed. Select the next tracker-approved release-readiness implementation slice.
  id: `presentation-aurelion-reach-main-menu-crest-art-10184`
- Completed implementation slice: `accessibility-main-menu-settings-focus-visibility-10184`. The existing Main Menu Cabinet now defers SettingsScroll visibility whenever focus enters any of its twenty native Settings controls and rechecks the retained focus owner after scale-driven panel refreshes. At 1280x720 and 100/130 percent UI scale, physical Tab and Shift+Tab traversal keeps every focused control fully visible, scrolls down to Readability/support/default actions, and returns to Presentation while preserving exact settings, session, save, campaign, route, and focus authority. Fresh X11 visual capture, display/defaults/active-play Settings, accessibility, menu/outcome visual, core, repository/editor, Linux export/headless startup, and Windows export/fresh-Wine Boot/MainMenu/native-DLL startup gates pass. Packaged Settings interaction, controller hardware, AT-SPI/UIA certification, native hardware, signing/publication, whole-game, and release readiness remain unclaimed. Select the next tracker-approved release-readiness implementation slice.
  id: `accessibility-main-menu-settings-focus-visibility-10184`
...
Selected Phase 6 normal-path performance correction. `describe_objective_header_surfaces` first builds `_objective_stakes_surface`, which normalizes the live session, then immediately calls the public progress-recap entry point that normalizes the same session again. The existing normalized-session recap materializer can consume the already-current state while public direct recap callers remain fresh.

Implemented outcome: Objective Header now uses the existing normalized-session recap materializer after its Objective Stakes surface establishes current state. Public direct recap and Objective Stakes fallbacks still normalize fresh. The focused owner is exact across every authored scenario plus ordinary, pressured, day-transition, campaign, and missing-scenario controls; a real full refresh preserves whole authority and materially reduces the owned lane. Bounded compatibility, static/editor, Linux export/headless startup, and Windows export/fresh-Wine startup gates pass.

Implementation target:
- materialize the Objective Header progress recap through the existing normalized-session API only after `_objective_stakes_surface` has established normalized state;
- preserve exact ObjectiveBrief, Objective Stakes, progress recap, session, save, UI, focus, route, and action authority for every authored scenario and representative live state changes;
- keep public direct progress-recap callers unchanged and do not introduce a cache or persistent normalization flag.

Completion criteria:
- independent current and normalized-path controls remain whole-exact across all 24 authored scenarios, ordinary and changed day/objective states, with session authority unchanged;
- a real full Overworld refresh preserves whole UI/session authority and materially reduces the header/objective/status/resources lane from the current 53 ms baseline;
- focused Objective Header, Overworld visual/route/input, Town return, core, repository/editor, Linux export/headless startup, and Windows export/fresh-Wine Boot/MainMenu/native-DLL startup gates pass.

Non-goals:
- no objective, progress, status, forecast, readiness, resource, route, event, save, scenario, normalization, or gameplay policy changes;
- no persistent cache, async work, content, balance, AI, Native RMG, save/schema, or package identity changes;
- no packaged Overworld interaction, controller hardware, AT-SPI/UIA certification, native hardware, signing/publication, whole-game, or release-readiness claim.

## Overworld Refresh Watch Context Reuse

id: `performance-overworld-refresh-watch-context-reuse-10184`

Status: completed.

Selected Phase 6 normal-path performance correction. One full Overworld refresh renders the End Turn Forecast, Command Commitment, Event Feed, and Field Dispatch from the same live session, but the surfaces independently recompute command risk, local visible pressure, and town-management watch state. A current copied materializer preserves all four whole outputs and session authority while one detached refresh-local watch context reduces three-call batches from a1.460s median to0.943s.

Implemented outcome: one normalized refresh-local watch context now owns detached command-risk forecast/surfaces, local pressure, and management watch for the unchanged forecast, commitment, Event Feed, and Dispatch outputs; direct APIs still normalize and derive fresh state. The authoritative seven-state owner is whole-exact with a2.988s direct median versus1.751s shared, detached rebuild and session/UI authority, and real full refresh reports one build/three reuses. Bounded compatibility, static/editor, Linux export/headless startup, and Windows export/fresh-Wine startup gates pass.

Implementation target:
- derive one detached ordered watch context per full status refresh and pass its exact command-risk forecast/surface, local-pressure line, and management-watch line into the existing forecast, commitment, and Event/Dispatch materializers;
- keep public direct forecast, commitment, Event Feed, and Dispatch callers fresh and preserve exact normalization, fallback, ordering, tie, copy, event, warning, and action-context semantics;
- keep the context local to one refresh with no session/day/global cache or cross-refresh reuse.
...
- a real full Overworld refresh builds one watch context, preserves exact forecast, commitment, Event/Dispatch text plus whole session/UI/save/focus/action authority, and materially reduces the combined owned surfaces;
...
Non-goals:
- no risk, forecast, commitment, event, dispatch, readiness, objective, threat, defense, management, action, save, focus, or gameplay policy changes;
- no persistent cache, async work, content, balance, AI, Native RMG, save/schema, or package identity changes;
- no packaged Overworld interaction, controller hardware, AT-SPI/UIA certification, native hardware, signing/publication, whole-game, or release-readiness claim.
