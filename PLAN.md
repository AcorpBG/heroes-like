...
- Latest native RMG implementation update: `h3maped_rmg_core` now replays the recovered first pass of decorative scorer `0x49e1bf` after `0x49e700` `rand_trn.txt` terrain-score filtering: descriptor footprint candidate anchors are scanned in source order, generated-cell terrain is validated through `0x42cc99` against descriptor terrain mask `+0x14`, bit27 cells hard reject, and the recovered low-score threshold is applied before scorer acceptance. No-Godot controlled snapshots for Small seed 10, Small seed 58, and Medium seed 10 now all fail closed later at `0x49e700_0x49e1bf_neighbor_object_reference_descriptor_0x30_0x40_replay_unowned`, with one positive first-pass candidate in each case. This is source-order workflow progress, not parity completion; final payload generation remains disabled until exact `0x49e1bf` neighbor/object-reference flagging plus descriptor `+0x30/+0x40` score-table replay, road/river adjacency, descriptor counter replay, and final writeout are source-owned.
