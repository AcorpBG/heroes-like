...
- Latest native RMG implementation update: `rmg_native_core.cpp` now routes the no-Godot CLI through a single native H3MapEd workflow that consumes the currently ported relation/object state instead of hiding it behind a generic partial-state stop. The workflow now sequences relation scan consumers (`0x4a1f3b/0x4a5767`), source-order object replay (`0x4a8d2c/0x4a8db2/0x4a901a/0x4a93a2/0x4a54a7`), and relation high-owner propagation (`0x49a318`) before blocking at `reward_guard_materialization` on the live `0x4ad7f7` source-relation/wrapper caller feed. This is not parity and not final map output: reward/guard materialization, connection/road/river phases, and final tile/object/header writeout remain unowned.
