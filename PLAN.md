...
59. Strategic AI minimal commander-role state schema planning is completed in `docs/strategic-ai-minimal-commander-role-state-schema-plan.md`. The plan selects `enemy_states[].commander_roster[].commander_role_state` as the future optional location; fixes the minimal fields, enum sets, defaults, old-save compatibility behavior, deterministic assignment id derivation, derived front id rules, public-reason recomputation, fixture-only annotation boundary, validation strategy, migration risks, and adapter path to later full AI hero state without adding schema, report helpers/tests, live behavior, save migration, durable event logs, coefficient tuning, or defense-specific durable state.
60. Strategic AI commander-role report implementation boundary planning is completed in `docs/strategic-ai-commander-role-report-implementation-plan.md`. The plan assigns report-helper ownership to `EnemyAdventureRules.gd`, keeps `EnemyTurnRules.gd` as supporting town-governor evidence only, defines fixture-only normalization policy, report scene shape, public leak checks, validation commands, exact implementation sequence, and selects report-only implementation as the next step instead of fixture-only normalization planning, live-client gate planning, or pausing AI foundation work.
61. Strategic AI commander-role report implementation is completed in `docs/strategic-ai-commander-role-report-implementation-report.md` plus focused Godot coverage in `tests/ai_commander_role_state_report.gd`. The report proves all eight planned `AI_COMMANDER_ROLE_STATE_REPORT` cases as report-only derived views, with compact public role events and leak checks, without live commander-role behavior adoption, schema writes, save migration, coefficient tuning, durable event logs, defense-specific durable state, or production JSON edits.
62. Strategic AI commander-role report gate review is completed in `docs/strategic-ai-commander-role-report-gate-review.md`. The gate passes: the focused report output is sufficient to keep schema writes, save migration, durable event logs, defense-specific durable state, coefficient tuning, full AI hero task state, and live commander-role behavior deferred. The next recommended slice is planning-only commander-role adoption sequencing before any schema or live behavior switch.
63. Strategic AI commander-role adoption sequencing planning is completed in `docs/strategic-ai-commander-role-adoption-sequencing-plan.md`. The plan compares derived live-turn transcript/report surfacing, minimal `commander_role_state` write/read adoption, full AI hero task-state sequencing, and pausing commander roles; it recommends planning a behavior-neutral derived live-turn transcript/report next, while keeping schema writes, save migration, durable event logs, defense-specific durable state, coefficient tuning, full AI hero task state, and live commander-role behavior deferred.
64. Strategic AI commander-role live-turn transcript/report planning is completed in `docs/strategic-ai-commander-role-live-turn-transcript-report-plan.md`. The plan defines future `AI_COMMANDER_ROLE_TURN_TRANSCRIPT_REPORT` ownership, payload shape, fixture setup, enemy-turn phase records, active commander links, derived role proposals before/after turn execution, target assignment/no-op reasons, raid movement/arrival summaries, town-governor supporting event references, compact public transcript events, public leak checks, validation commands, failure conditions, and exact implementation sequence. It recommends report-only implementation next while keeping all transcript records derived/report-only and still deferring schema writes, save migration, durable event logs, defense-specific durable state, coefficient tuning, full AI hero task state, live commander-role behavior, and production JSON edits.
65. Strategic AI commander-role live-turn transcript/report implementation is completed in `docs/strategic-ai-commander-role-live-turn-transcript-report-implementation-report.md`. `EnemyAdventureRules` now derives before/after enemy-turn transcript snapshots, phase records, commander links, role proposals, target assignment/no-op records, raid movement/arrival summaries, town-governor supporting refs, compact public transcript events, and leak/source-marker checks for River Pass and Glassroad fixtures while still observing existing `EnemyTurnRules.run_enemy_turn(...)` behavior only.
...
70. Strategic AI hero task-state boundary report gate review is completed in `docs/strategic-ai-hero-task-state-report-gate-review.md`. The gate passes: the focused report rerun printed `AI_HERO_TASK_STATE_BOUNDARY_REPORT` with `ok: true`, `schema_status: "task_state_boundary_report_only"`, `behavior_policy: "derive_candidate_tasks_only"`, `save_policy: "no_hero_task_state_write"`, seven reviewed cases, nine checked candidate tasks, old-save absence, no save version change, and public leak checks across eight compact public task events. Schema writes, save migration, durable event logs, defense-specific durable state, coefficient tuning, full AI hero task-state implementation, live commander-role behavior, and live AI hero behavior remain deferred.
...
75. Main-menu save play-check implementation is completed as compact player-facing UI in `scenes/menus/MainMenu.gd`, backed by reusable save-summary helpers in `scripts/autoload/SaveService.gd` and focused smoke coverage in `tests/menu_outcome_visual_smoke.gd`. Existing save summary, resume-target, objective/progress, next-action, risk, and town-defense payloads now produce one `Play check:` line without mechanics changes, save schema/version changes, durable event logs, AI tuning, production JSON migration, or internal score-field exposure.
76. Only after these basics are deep enough and art-direction evidence starts to exist, return focus to implementation planning, campaign/skirmish maps, town-screen polish, battle-screen polish, battle AI systems, and the overall player loop: turn flow, economy, resource collection, mine capturing, battles, and town development.
77. Progress implementation audit is completed in `docs/progress-implementation-audit-2026-04-27.md`. The audit found that recent work drifted from the plan into weakly anchored compact UI cue slices. The stopped overworld artifact cue WIP is not approved gameplay or UI work; it is preserved only as patch evidence in `docs/stopped-overworld-artifact-cue-wip-2026-04-27.patch`.
78. The UI cue stream is paused until a later worker has a direct source-doc parent and an approved bounded task. The next actionable implementation slice is `strategic-ai-hero-task-state-save-normalizer-preservation-report-implementation-10184`, sourced from `docs/strategic-ai-hero-task-state-save-normalizer-preservation-plan.md`.

Recommended next slice:
- Do the report-only strategic AI hero task-state save-normalizer preservation implementation from `docs/strategic-ai-hero-task-state-save-normalizer-preservation-plan.md`.
- Keep the proof scoped to `AI_HERO_TASK_STATE_NORMALIZER_PRESERVATION_REPORT`, old-save absence, optional future-field preservation, malformed-state tolerance, unknown-field isolation, and `SaveService` semantic boundaries.
- Do not continue `overworld-artifact-check-cue-10184` or start another compact UI cue unless a later planning/source document explicitly anchors that cue and makes it the selected worker task.
- Keep the completed editor taxonomy, menu save play-check, and overworld field readiness recap slices as history and guardrails only; they do not authorize broad production JSON migration, renderer sprite ingestion, pathing/body-tile/approach adoption, save migration, AI tuning, mechanics changes, or broader dashboard composition.
...
- Use `docs/strategic-ai-commander-role-report-implementation-report.md` as the current evidence boundary: report helpers live in `EnemyAdventureRules.gd`, town-governor evidence stays supporting-only, fixture annotations remain report-only, and the scene prints a single `AI_COMMANDER_ROLE_STATE_REPORT` line.
- Keep any next AI hero task-state work scoped to planning or report-only evidence unless a later gate explicitly approves implementation; do not tune coefficients, change production content JSON, add durable event logs, implement full AI hero task state, add defense-specific durable state, implement live commander-role or live AI hero behavior, write schema/save fields, or change pathing/renderer/editor/save behavior.
...
- Treat commander-role report output, commander-role adoption sequencing, live-turn transcript/report planning, live-turn transcript/report implementation, live-turn transcript/report gate review, hero task-state boundary planning, hero task-state report fixture planning, hero task-state report implementation, hero task-state report gate review, hero task-state adoption sequencing, and hero task-state save-normalizer preservation planning as passed gates. The next AI slice should implement the planned report-only save-normalizer preservation proof while keeping schema writes, save migration, durable event logs, full AI hero task-state implementation, defense-specific durable state, coefficient tuning, live commander-role behavior, and live AI hero behavior deferred.
...
- The completed active slice is reconciliation, not gameplay implementation: audit recorded cue drift, stopped artifact cue WIP is preserved as patch evidence and removed from live code/state, broad next actions are clarified as guardrails, and the next actionable worker task is `strategic-ai-hero-task-state-save-normalizer-preservation-report-implementation-10184`.
- The next AI normalizer proof remains report-only and must not add schema writes, save migration, `SAVE_VERSION` changes, durable event logs, full AI hero task-state implementation, live commander-role behavior, live AI hero behavior, defense-specific durable state, coefficient tuning, production JSON edits, generated asset import, pathing changes, renderer changes, or broad UI/dashboard work.

## Completed Implementation Slice: Main Menu Quit Check Cue
Status: completed on 2026-04-27 as compact player-facing main-menu UI polish.

Purpose:
- Help a manual player understand that Quit closes the client and does not itself write a new resume point.
- Reuse the existing scenic first-view command, footer expedition pocket, command tooltip, and validation snapshot surfaces instead of changing routing or save behavior.

Delivered:
- Added a compact `Quit check:` surface in `scenes/menus/MainMenu.gd`.
- The first-view footer now pairs current expedition context with quit consequence and save-first guidance, while the Quit command tooltip explains the resume boundary.
- Added focused smoke coverage in `tests/menu_outcome_visual_smoke.gd`, including visible-cue, tooltip, snapshot, and no-score-leak assertions.

Validation:
- Passed focused smoke coverage with `godot4 --headless --path /root/dev/heroes-like /root/dev/heroes-like/tests/menu_outcome_visual_smoke.tscn`; full required validation is recorded in the run result.

Limits:
- This is UI/readability polish only. It does not change routing semantics, mechanics, balance, save schema/version, durable event logs, production JSON, strategic AI behavior, coefficients, pathing, body tiles, renderer assets, generated asset import, or broad dashboard composition.

## Completed Implementation Slice: Battle Timing Check Cue
Status: completed on 2026-04-27 as compact player-facing battle UI polish.

Purpose:
- Help a manual player understand whether the current commander spell window, burst risk, support payoff, and protection need should change the next stack order.
- Reuse the existing spell and ability timing board, active stack, selected target, spell action, and battle tab surfaces instead of changing battle mechanics.

Delivered:
- Added a compact `Timing check:` surface in `scenes/battle/BattleShell.gd`.
- The battle Timing rail now pairs the existing spell and ability timing board with readiness, active/target context, ready spell/order context, burst/protection/support context, and next practical action tooltip detail.
- Added focused smoke coverage in `tests/town_battle_visual_smoke.gd`, including visible-cue, tooltip-payload, readiness-state, and no-score-leak assertions.

Validation:
- Passed focused smoke coverage with `godot4 --headless --path /root/dev/heroes-like /root/dev/heroes-like/tests/town_battle_visual_smoke.tscn`; full required validation is recorded in the run result.

Limits:
- This is UI/readability polish only. It does not change mechanics, balance, battle timing semantics, save schema/version, durable event logs, production JSON, strategic AI behavior, coefficients, pathing, body tiles, renderer assets, generated asset import, or broad dashboard composition.

## Completed Implementation Slice: Town Specialty Readiness Cue
Status: completed on 2026-04-27 as compact player-facing town UI polish.

Purpose:
- Help a manual player see whether the active hero has a specialty choice waiting before leaving town.
- Reuse existing hero progression specialty choices and town action surfaces instead of changing hero progression mechanics.

Delivered:
- Added a compact `Specialty check:` surface in `scenes/town/TownShell.gd`.
- The town Muster specialty summary now pairs existing specialty text with ready/blocked/pending/chosen choice state, best choice, readiness, why-it-matters, and next practical action tooltip detail.
- Added focused smoke coverage in `tests/town_battle_visual_smoke.gd`, including visible-cue, payload-count, state-coverage, and no-score-leak assertions.
...
Limits:
- This is UI/readability polish only. It does not change mechanics, hero progression semantics, save schema/version, durable event logs, production JSON, strategic AI behavior, coefficients, pathing, body tiles, renderer assets, generated asset import, or broad dashboard composition.
...
Purpose:
- Help a manual player interpret tactical risk before committing the active stack's next order.
- Reuse the existing risk readiness board, active stack, and action-readiness context instead of changing battle mechanics.

Delivered:
- Added a compact `Risk check:` surface in `scenes/battle/BattleShell.gd`.
- The battle Risk rail now pairs the existing tactical risk board with readiness, outlook, initiative swing, line integrity, objective urgency, and next practical action tooltip detail.
- Added focused smoke coverage in `tests/town_battle_visual_smoke.gd`, including visible-cue, payload-field, readiness-state, and no-score-leak assertions.

Validation:
- Passed `python3 -m json.tool ops/progress.json`, `git diff --check`, `python3 tests/validate_repo.py`, and `godot4 --headless --path /root/dev/heroes-like /root/dev/heroes-like/tests/town_battle_visual_smoke.tscn`.

Limits:
- This is UI/readability polish only. It does not change mechanics, balance, battle risk semantics, save schema/version, durable event logs, production JSON, strategic AI behavior, coefficients, pathing, body tiles, renderer assets, generated asset import, or broad dashboard composition.

## Completed Implementation Slice: Battle Status Check Cue
Status: completed on 2026-04-27 as compact player-facing battle UI polish.

Purpose:
- Help a manual player understand whether active spell/status effects are shaping the current round before committing a stack order.
- Reuse current active stack, selected target, status pressure, and effect-board summaries instead of changing battle mechanics.

Delivered:
- Added a compact `Status check:` surface in `scenes/battle/BattleShell.gd`.
- The battle Effects rail now pairs the existing effect board with status readiness, active/selected pressure, affected-stack count, and next practical action tooltip detail.
- Added focused smoke coverage in `tests/town_battle_visual_smoke.gd`, including visible-cue, tooltip-payload, stable-count, and no-score-leak assertions.

Validation:
- Passed `python3 -m json.tool ops/progress.json`, `git diff --check`, `python3 tests/validate_repo.py`, and `godot4 --headless --path /root/dev/heroes-like /root/dev/heroes-like/tests/town_battle_visual_smoke.tscn`.

Limits:
- This is UI/readability polish only. It does not change mechanics, balance, battle status semantics, save schema/version, durable event logs, production JSON, strategic AI behavior, coefficients, pathing, body tiles, renderer assets, generated asset import, or broad dashboard composition.

## Completed Implementation Slice: Outcome Carryover Check Cue
Status: completed on 2026-04-27 as compact player-facing outcome UI polish.

Purpose:
- Help a manual player distinguish campaign carryover/export behavior from self-contained skirmish results before choosing a follow-up, retry, replay, manual save, or menu return.
- Reuse current outcome summaries, campaign/skirmish handoff data, action selection, and save surface context instead of changing campaign progression or outcome routing.

Delivered:
- Added a compact `Carryover check:` surface in `scenes/results/ScenarioOutcomeShell.gd`.
- The outcome Carry tab and Field Manual now pair existing carryover summary text with next-follow-up, replay/new-run, and manual-save preservation context.
- Added focused smoke coverage in `tests/menu_outcome_visual_smoke.gd`, including visible-cue, guide, validation-payload, and no-score-leak assertions.

Validation:
- Passed `python3 -m json.tool ops/progress.json`, `git diff --check`, `python3 tests/validate_repo.py`, and `godot4 --headless --path /root/dev/heroes-like /root/dev/heroes-like/tests/menu_outcome_visual_smoke.tscn`.

Limits:
- This is UI/readability polish only. It does not change mechanics, balance, campaign progression semantics, save schema/version, durable event logs, production JSON, strategic AI behavior, coefficients, pathing, body tiles, renderer assets, generated asset import, or broad dashboard composition.

## Completed Implementation Slice: Battle Engagement Check Cue
Status: completed on 2026-04-27 as compact player-facing battle UI polish.

Purpose:
- Help a manual player understand whether the selected enemy can be acted on now, which order is preferred, what the immediate consequence preview says, and what to do next from the existing target rail.
- Reuse current selected-target, action surface, target/range, and active consequence payloads instead of changing battle mechanics.

Delivered:
- Added a compact `Engagement check:` surface in `scenes/battle/BattleShell.gd`.
- The battle target rail now pairs the existing target summary with order readiness, consequence preview, target/range, and next-practical-action tooltip detail.
- Added focused smoke coverage in `tests/town_battle_visual_smoke.gd`, including visible-cue, payload-field, and no-score-leak assertions.
...
Purpose:
- Help a manual player tell whether Save Outcome writes into an empty manual slot or overwrites a selected saved expedition before pressing the outcome save action.
- Reuse the current outcome save surface, selected manual slot summary, and save/load tooltip payloads instead of changing save schema, campaign progression, or outcome routing.

Delivered:
- Added a compact `Slot check:` surface in `scenes/results/ScenarioOutcomeShell.gd`.
- The outcome save rail, save status tooltip, save button tooltip, and Field Manual now carry selected-slot and saving-now context.
- Added focused smoke coverage in `tests/menu_outcome_visual_smoke.gd`, including visible-cue and no-score-leak assertions.
...
## Completed Implementation Slice: Outcome Follow-up Check Cue
Status: completed on 2026-04-27 as compact player-facing outcome UI polish.

Purpose:
- Help a manual player tell which outcome follow-up starts fresh, why saving first matters, and what Return to Menu preserves before pressing an outcome action.
- Reuse current outcome action, save surface, return cue, and campaign/skirmish handoff payloads instead of changing campaign progression or outcome routing.

Delivered:
- Added a compact `Follow-up check:` surface in `scenes/results/ScenarioOutcomeShell.gd`.
- The outcome Next Step rail, action tooltips, and Field Manual now pair the existing action cue with primary-follow-up, save-first, return, state-change, and inspection clarity.
- Added focused smoke coverage in `tests/menu_outcome_visual_smoke.gd`, including visible-cue and no-score-leak assertions.
...
## Completed Implementation Slice: Battle Active Stack Check Cue
Status: completed on 2026-04-27 as compact player-facing battle UI polish.

Purpose:
- Help a manual player understand the current acting stack's role, status pressure, target/range, ready order, and next practical action from the existing active-stack rail.
- Reuse current active consequence, action surface, status, and target-range payloads instead of changing battle mechanics.

Delivered:
- Added a compact `Stack check:` surface in `scenes/battle/BattleShell.gd`.
- The active-stack rail now pairs the existing active stack summary with readiness, current order, and next-practical-action tooltip detail.
- Added focused smoke coverage in `tests/town_battle_visual_smoke.gd`, including visible-cue and no-score-leak assertions.
...
Purpose:
- Help a manual player tell whether the selected campaign chapter is the primary campaign push, replay, retry, or blocked selection before pressing Start Chapter.
- Reuse existing campaign chapter action, primary action, launch handoff, continuity, readiness, and action consequence payloads instead of changing campaign progression semantics.

Delivered:
- Added a compact `Campaign check:` payload in `scenes/menus/MainMenu.gd`.
- The Campaign board now shows the check in selected chapter details and carries the same check into Start Chapter tooltips.
- Added focused smoke coverage in `tests/menu_outcome_visual_smoke.gd`, including visible-cue and no-score-leak assertions.
...
## Completed Implementation Slice: Battle Objective Check Cue
Status: completed on 2026-04-27 as compact player-facing battle UI polish.

Purpose:
- Help a manual player connect the selected battle objective pressure to the next practical order from the existing battle footer.
- Reuse current field objective state, target handoff, action surface, and action confirmation payloads instead of changing battle mechanics.

Delivered:
- Added a compact `Objective check:` payload in `scripts/core/BattleRules.gd`.
- The live battle footer now pairs target handoff with objective pressure, readiness, and next-practical-action clarity.
- Added focused smoke coverage in `tests/town_battle_visual_smoke.gd`, including visible-cue and no-score-leak assertions.
...
## Completed Implementation Slice: Main-Menu Save Play Check
Status: completed on 2026-04-26 as compact player-facing loop-readiness UI polish.

Purpose:
- Help a manual player understand the stored loop state from the main menu before resuming an overworld, town, battle, or outcome save.
- Reuse existing save summary, resume target, objective/progress recap, next-action, risk, and town-defense payloads instead of adding mechanics or durable logs.

Delivered:
- Added reusable `Play check:` builders in `scripts/autoload/SaveService.gd`.
- The main menu latest-save pulse, first-view footer, and selected-save details now surface the compact play-check line.
- Added focused smoke coverage in `tests/menu_outcome_visual_smoke.gd`, including no-score-leak assertions.
...
## Completed Implementation Slice: Overworld End-Turn Readiness Confirmation
Status: completed on 2026-04-26 as compact player-facing overworld UI polish.

Purpose:
- Help a manual player decide whether to move, resolve a selected route or encounter, enter town, or end the day from the existing overworld footer control.
- Reuse current field readiness, route, objective/progress, event-feed, and end-turn forecast payloads instead of adding mechanics or durable logs.

Delivered:
- Added a compact end-turn confirmation surface in `scenes/overworld/OverworldShell.gd`.
- The live overworld `End Turn` control now flags pending movement, route, battle, or town action in its label and carries the readiness confirmation in its tooltip.
- Added focused smoke coverage in `tests/overworld_visual_smoke.gd`, including visible-control and no-score-leak assertions.
...
## Completed Implementation Slice: Battle Action Readiness Confirmation
Status: completed on 2026-04-26 as compact player-facing battle UI polish.

Purpose:
- Help a manual player decide whether to attack, move, cast, defend/hold, or confirm an action from the existing battle order rail and controls.
- Reuse current battle action surface, selected target, initiative/turn state, spell-action availability, and consequence payloads instead of adding mechanics or durable logs.

Delivered:
- Added a compact battle action readiness confirmation payload in `scripts/core/BattleRules.gd`.
- The live battle order rail now shows a `Ready check:` confirmation line, and the existing action button tooltips include `Confirm:` guidance.
- Added focused smoke coverage in `tests/town_battle_visual_smoke.gd`, including visible-control and no-score-leak assertions.
...
## Completed Planning Slice: Strategic AI Minimal Commander Role State Schema
Status: completed on 2026-04-26 as documentation-only schema boundary planning after the completed commander-role report fixture plan.

Purpose:
- Decide the exact future minimal commander-role state schema boundary before report implementation, fixture-only normalization, save migration, live behavior adoption, or full AI hero task state.
- Keep coefficient tuning, defense-specific durable state, production JSON migration, and live behavior deferred because the passed proof reports and fixture plan found no ordering, seizure, retake, town-governor, public-surface, or defense-state defect.

Delivered:
- Added `docs/strategic-ai-minimal-commander-role-state-schema-plan.md`.
- Selected future optional field location: `session.overworld.enemy_states[].commander_roster[].commander_role_state`.
- Defined the minimal saved fields, exact enum sets, defaults, malformed-state normalization policy, deterministic `assignment_id` derivation, derived `front_id` rules, public reason recomputation, fixture-only annotations, old-save compatibility, validation strategy, migration risks, and adapter path to later full AI hero state.
- Kept `target_label`, `public_reason`, debug score details, fixture annotations, path plans, defense locks, durable events, and full AI hero task payloads out of the live schema.
- Recommended `strategic-ai-commander-role-report-implementation-planning-10184` as the next slice.

Validation:
- Planned validation for this slice is `python3 -m json.tool ops/progress.json >/tmp/heroes-progress-jsoncheck.txt`, `git diff --check`, `python3 tests/validate_repo.py`, `python3 tests/validate_repo.py --economy-resource-report`, `python3 tests/validate_repo.py --overworld-object-report`, and `python3 tests/validate_repo.py --neutral-encounter-report`.

Limits:
- This is documentation/planning only. It does not implement schema, report helpers/tests, fixture normalizers, live commander-role behavior, save migration, production JSON edits, coefficient tuning, durable event logs, full AI hero task state, defense-specific durable state, pathing/body-tile/approach behavior, renderer/editor behavior, generated PNG import, neutral encounter migration, `content/resources.json`, `wood` to `timber` migration, rare resources, market-cap overhaul, or River Pass rebalance.

## Completed Planning Slice: Strategic AI Commander Role Report Implementation Boundaries
Status: completed on 2026-04-26 as documentation-only implementation boundary planning after the completed fixture and minimal schema plans.

Purpose:
- Define exact ownership and sequencing for a future `AI_COMMANDER_ROLE_STATE_REPORT` implementation without implementing report helpers/tests, schema writes, live commander-role behavior, save migration, durable event logs, coefficient tuning, defense-specific durable state, or full AI hero task state.
- Decide the next step among report implementation, fixture-only state normalization planning, live-client gate planning, or pausing AI foundation work.

Delivered:
- Added `docs/strategic-ai-commander-role-report-implementation-plan.md`.
- Assigned future report-helper ownership to `EnemyAdventureRules.gd`, with `EnemyTurnRules.gd` kept as supporting town-governor evidence only.
- Defined fixture normalization as report-only view construction on fresh deterministic sessions, with fixture annotations remaining under `fixture_state` and never saved.
- Defined the future report scene shape for `tests/ai_commander_role_state_report.gd` and `.tscn`, including one `AI_COMMANDER_ROLE_STATE_REPORT` output line, eight deterministic cases, failure behavior, and payload fields.
- Defined recursive public leak checks for public role events and compact summaries.
- Sequenced exact future implementation steps: pure helpers, front/reason adapters, report-only role views, role proposal helpers, active encounter linkage, public event construction, leak checks, Godot report scene, implementation report, validation, and local commit.
- Recommended `strategic-ai-commander-role-report-implementation-10184` as the next slice, not another normalization-planning slice, not live-client gate planning, and not an AI foundation pause.

Validation:
- Planned validation for this slice is `python3 -m json.tool ops/progress.json >/tmp/heroes-progress-jsoncheck.txt`, `git diff --check`, `python3 tests/validate_repo.py`, `python3 tests/validate_repo.py --economy-resource-report`, `python3 tests/validate_repo.py --overworld-object-report`, and `python3 tests/validate_repo.py --neutral-encounter-report`.

Limits:
- This is documentation/planning only. It does not implement report helpers/tests, fixture normalizers, live commander-role behavior, save migration, schema writes, production JSON edits, coefficient tuning, durable event logs, full AI hero task state, defense-specific durable state, pathing/body-tile/approach behavior, renderer/editor behavior, generated PNG import, neutral encounter migration, `content/resources.json`, `wood` to `timber` migration, rare resources, market-cap overhaul, or River Pass rebalance.

## Completed Implementation Slice: Strategic AI Commander Role Report
Status: completed on 2026-04-26 as report-only implementation after the completed commander-role boundary, fixture, minimal schema, and implementation-boundary plans.

Purpose:
- Implement pure/report-only commander-role state helpers and focused Godot coverage for the planned `AI_COMMANDER_ROLE_STATE_REPORT`.
- Keep live commander-role behavior, schema writes, save migration, durable event logs, coefficient tuning, defense-specific durable state, full AI hero task state, production JSON migration, pathing/body-tile/approach adoption, renderer/editor changes, generated PNG import, neutral encounter migration, resource migration, and River Pass rebalance out of scope.

Delivered:
- Added report-only commander-role constants, front/reason adapters, commander state views, resource target views, role proposal helpers, active encounter linkage, compact public role event construction, and public leak checking in `scripts/core/EnemyAdventureRules.gd`.
- Added `tests/ai_commander_role_state_report.gd` and `tests/ai_commander_role_state_report.tscn`.
- Covered all eight planned cases: Mireclaw Free Company retaker/raider, Signal Post companion, Embercourt Glassroad relay defender/retaker/stabilizer, recovery blocking assignment, and memory continuity.
- Added `docs/strategic-ai-commander-role-report-implementation-report.md`.
- Recommended `strategic-ai-commander-role-report-gate-review-10184` as the next slice.

Validation:
- Planned validation for this slice is `python3 -m json.tool ops/progress.json >/tmp/heroes-progress-jsoncheck.txt`, `git diff --check`, `python3 tests/validate_repo.py`, `python3 tests/validate_repo.py --economy-resource-report`, `python3 tests/validate_repo.py --overworld-object-report`, `python3 tests/validate_repo.py --neutral-encounter-report`, `godot4 --headless --path /root/dev/heroes-like /root/dev/heroes-like/tests/ai_commander_role_state_report.tscn`, `godot4 --headless --path /root/dev/heroes-like /root/dev/heroes-like/tests/ai_site_control_proof_report.tscn`, `godot4 --headless --path /root/dev/heroes-like /root/dev/heroes-like/tests/ai_glassroad_defense_proof_report.tscn`, and `godot4 --headless --path /root/dev/heroes-like /root/dev/heroes-like/tests/ai_town_governor_pressure_report.tscn`.

Limits:
- This is report-only implementation. It does not write `commander_role_state`, change live AI target selection, change raid advancement, tune coefficients, edit production JSON, migrate saves, add durable events, implement full AI hero tasks, add defense-specific durable state, change pathing/body-tile/approach behavior, change renderer/editor behavior, import generated PNGs, migrate neutral encounters, add `content/resources.json`, migrate `wood` to `timber`, activate rare resources, overhaul market caps, or rebalance River Pass.

## Completed Gate Review Slice: Strategic AI Commander Role Report
Status: completed on 2026-04-26 as report-output review before schema or live behavior adoption.

Purpose:
- Review the focused `AI_COMMANDER_ROLE_STATE_REPORT` output and decide whether report-only evidence is enough to keep commander-role schema writes, save migration, durable event logs, defense-specific durable state, coefficient tuning, full AI hero task state, and live commander-role behavior deferred.

Delivered:
- Added `docs/strategic-ai-commander-role-report-gate-review.md`.
- Reran the focused commander-role Godot report and confirmed `"ok": true`, `schema_status: "report_fixture_only"`, eight reviewed cases, and a passing public leak check across eight compact public role events.
- Confirmed Mireclaw Free Company retaker/raider evidence, Signal Post companion evidence, Embercourt Glassroad relay defender/retaker evidence, Starlens stabilizer evidence, recovery blocking, and commander memory continuity.
- Confirmed no blocker requiring coefficient tuning, schema writes, save migration, durable event logs, defense-specific durable state, full AI hero task state, or live commander-role behavior adoption.
- Recommended `strategic-ai-commander-role-adoption-sequencing-planning-10184` as the next narrow planning slice.
...
Limits:
- This is a gate review only. It does not write `commander_role_state`, change live AI target selection, change raid advancement, tune coefficients, edit production JSON, migrate saves, add durable events, implement full AI hero tasks, add defense-specific durable state, change pathing/body-tile/approach behavior, change renderer/editor behavior, import generated PNGs, migrate neutral encounters, add `content/resources.json`, migrate `wood` to `timber`, activate rare resources, overhaul market caps, or rebalance River Pass.

## Completed Planning Slice: Strategic AI Commander Role Adoption Sequencing
Status: completed on 2026-04-26 as documentation-only sequencing after the passed commander-role report gate review.

Purpose:
- Compare the next narrow strategic AI step among derived live-turn transcript/report surfacing, minimal `commander_role_state` write/read adoption, full AI hero task-state sequencing, and pausing commander-role work.
- Define exact adoption gates, rollback/escape hatch, validation commands, save/schema risk boundaries, and non-change boundaries before any implementation.

Delivered:
- Added `docs/strategic-ai-commander-role-adoption-sequencing-plan.md`.
- Recommended `strategic-ai-commander-role-live-turn-transcript-report-planning-10184` as the next slice.
- Selected behavior-neutral derived live-turn transcript/report planning as the next bridge because it can attach commander-role proposals to existing enemy-turn execution without schema writes, save migration, durable event logs, full AI hero task state, defense-specific durable state, coefficient tuning, or live behavior adoption.
- Deferred minimal `commander_role_state` write/read adoption until a later transcript/report gate proves derived state is insufficient.
- Deferred full AI hero task-state sequencing until real AI hero movement, route planning, front selection, pathing constraints, fog/scouting assumptions, spell/artifact evaluation, recovery, UI, and save/load behavior are planned together.
- Rejected pausing commander-role work as the default because the passed report evidence is useful enough to justify one behavior-neutral turn-transcript planning bridge.

Validation:
- Planned validation for this slice is `python3 -m json.tool ops/progress.json >/tmp/heroes-progress-jsoncheck.txt`, `git diff --check`, `python3 tests/validate_repo.py`, `python3 tests/validate_repo.py --economy-resource-report`, `python3 tests/validate_repo.py --overworld-object-report`, and `python3 tests/validate_repo.py --neutral-encounter-report`.

Limits:
- This is documentation/planning only. It does not implement reports/tests, write `commander_role_state`, change live AI target selection, change raid advancement, tune coefficients, edit production JSON, migrate saves, add durable events, implement full AI hero tasks, add defense-specific durable state, change pathing/body-tile/approach behavior, change renderer/editor behavior, import generated PNGs, migrate neutral encounters, add `content/resources.json`, migrate `wood` to `timber`, activate rare resources, overhaul market caps, or rebalance River Pass.

## Completed Planning Slice: Strategic AI Commander Role Live-Turn Transcript Report
Status: completed on 2026-04-26 as documentation-only planning after the completed commander-role adoption sequencing plan.

Purpose:
- Define a future behavior-neutral `AI_COMMANDER_ROLE_TURN_TRANSCRIPT_REPORT` over existing enemy-turn execution.
- Prove the next implementation can attach derived commander-role vocabulary to real turn snapshots without schema writes, save migration, durable event logs, defense-specific durable state, coefficient tuning, full AI hero tasks, or live commander-role behavior.

Delivered:
- Added `docs/strategic-ai-commander-role-live-turn-transcript-report-plan.md`.
- Assigned future fixture orchestration to `tests/ai_commander_role_turn_transcript_report.gd` and `.tscn`.
- Kept `EnemyTurnRules.gd` as the live enemy-turn behavior authority and `EnemyAdventureRules.gd` as the transcript/commander-role report helper owner.
- Defined the future payload shape, River Pass and Glassroad fixture setup, phase records, active commander links, before/after derived role proposals, target assignment/no-op reasons, raid movement/arrival summaries, town-governor supporting event references, compact public transcript events, public leak checks, validation commands, failure conditions, and exact implementation sequence.
- Recommended `strategic-ai-commander-role-live-turn-transcript-report-implementation-10184` as the next slice.
...
## Completed Implementation Slice: Strategic AI Commander Role Live-Turn Transcript Report
Status: completed on 2026-04-26 as report-only implementation after the completed live-turn transcript/report planning slice.

Purpose:
- Implement `AI_COMMANDER_ROLE_TURN_TRANSCRIPT_REPORT` as a behavior-neutral transcript over existing enemy-turn execution.
- Keep live commander-role behavior, schema writes, save migration, durable event logs, coefficient tuning, defense-specific durable state, full AI hero task state, production JSON edits, pathing/body-tile/approach adoption, renderer/editor changes, generated PNG import, neutral encounter migration, resource migration, and River Pass rebalance out of scope.

Delivered:
- Added report-only turn snapshot and transcript assembly helpers in `scripts/core/EnemyAdventureRules.gd`.
- Added `tests/ai_commander_role_turn_transcript_report.gd` and `tests/ai_commander_role_turn_transcript_report.tscn`.
- The focused report calls `EnemyTurnRules.run_enemy_turn(...)` once per deterministic River Pass and Glassroad fixture, then compares before/after snapshots.
- Covered derived phase records, active commander links, before/after role proposals, target assignment/no-op records, raid movement/arrival summaries, town-governor supporting refs, compact public transcript events, public leak checks, and source-marker checks.
- Added `docs/strategic-ai-commander-role-live-turn-transcript-report-implementation-report.md`.
- Recommended `strategic-ai-commander-role-live-turn-transcript-report-gate-review-10184` as the next slice.

Validation:
- Planned validation for this slice is `python3 -m json.tool ops/progress.json >/tmp/heroes-progress-jsoncheck.txt`, `git diff --check`, `python3 tests/validate_repo.py`, `python3 tests/validate_repo.py --economy-resource-report`, `python3 tests/validate_repo.py --overworld-object-report`, `python3 tests/validate_repo.py --neutral-encounter-report`, `godot4 --headless --path /root/dev/heroes-like /root/dev/heroes-like/tests/ai_commander_role_turn_transcript_report.tscn`, `godot4 --headless --path /root/dev/heroes-like /root/dev/heroes-like/tests/ai_commander_role_state_report.tscn`, `godot4 --headless --path /root/dev/heroes-like /root/dev/heroes-like/tests/ai_site_control_proof_report.tscn`, `godot4 --headless --path /root/dev/heroes-like /root/dev/heroes-like/tests/ai_glassroad_defense_proof_report.tscn`, and `godot4 --headless --path /root/dev/heroes-like /root/dev/heroes-like/tests/ai_town_governor_pressure_report.tscn`.

Limits:
- This is report-only implementation. It does not write `commander_role_state`, change live AI target selection, change raid advancement, change raid arrival, change town-governor choices, tune coefficients, edit production JSON, migrate saves, add durable events, implement full AI hero tasks, add defense-specific durable state, change pathing/body-tile/approach behavior, change renderer/editor behavior, import generated PNGs, migrate neutral encounters, add `content/resources.json`, migrate `wood` to `timber`, activate rare resources, overhaul market caps, or rebalance River Pass.

## Completed Gate Review Slice: Strategic AI Commander Role Live-Turn Transcript Report
Status: completed on 2026-04-26 as report-output review before schema or live behavior adoption.

Purpose:
- Review the focused `AI_COMMANDER_ROLE_TURN_TRANSCRIPT_REPORT` output and decide whether derived transcript evidence is enough to keep commander-role schema writes, save migration, durable event logs, defense-specific durable state, coefficient tuning, full AI hero task-state implementation, and live commander-role behavior deferred.

Delivered:
- Added `docs/strategic-ai-commander-role-live-turn-transcript-report-gate-review.md`.
- Reran the focused commander-role live-turn transcript Godot report and confirmed `ok: true`, `schema_status: "derived_turn_transcript_report_only"`, `behavior_policy: "observe_existing_enemy_turn_only"`, and `save_policy: "no_commander_role_state_write"`.
- Confirmed River Pass Mireclaw signal-yard evidence: Vaska Reedmaw moved to `river_free_company`, arrived, flipped controller from `player` to `faction_mireclaw`, and moved from `retaker` before the turn to `defender` after the turn; Sable Muckscribe remained the companion `river_signal_post` raider view.
- Confirmed Glassroad Embercourt relay evidence: Caelen Ashgrove moved to `glassroad_watch_relay`, arrived, flipped controller from `player` to `faction_embercourt`, and moved from `retaker` before the turn to `defender` after the turn; Seren Valechant remained the `glassroad_starlens` stabilizer view.
- Confirmed both cases emitted 9 phase records, passed source-marker checks, used recognized assignment/no-op records, and passed public leak checks across 21 total compact public transcript events.
- Confirmed no blocker requiring coefficient tuning, schema writes, save migration, durable event logs, defense-specific durable state, full AI hero task-state implementation, or live commander-role behavior adoption.
- Recommended `strategic-ai-hero-task-state-boundary-planning-10184` as the next narrow planning-only slice.

Validation:
- Planned validation for this slice is `python3 -m json.tool ops/progress.json >/tmp/heroes-progress-jsoncheck.txt`, `git diff --check`, `python3 tests/validate_repo.py`, `python3 tests/validate_repo.py --economy-resource-report`, `python3 tests/validate_repo.py --overworld-object-report`, `python3 tests/validate_repo.py --neutral-encounter-report`, and `godot4 --headless --path /root/dev/heroes-like /root/dev/heroes-like/tests/ai_commander_role_turn_transcript_report.tscn`.

Limits:
- This is a gate review only. It does not write `commander_role_state`, change live AI target selection, change raid advancement, change raid arrival, change town-governor choices, tune coefficients, edit production JSON, migrate saves, add durable events, implement full AI hero tasks, add defense-specific durable state, change pathing/body-tile/approach behavior, change renderer/editor behavior, import generated PNGs, migrate neutral encounters, add `content/resources.json`, migrate `wood` to `timber`, activate rare resources, overhaul market caps, or rebalance River Pass.

## Completed Planning Slice: Strategic AI Hero Task-State Boundary
Status: completed on 2026-04-26 as documentation-only planning after the passed commander-role live-turn transcript/report gate review.

Purpose:
- Define the future real strategic AI hero task-state boundary after the passed commander-role transcript gate.
- Compare minimal task-state planning/report first, schema adoption, live behavior adoption, and pausing commander-role work before any implementation.
- Keep schema writes, save migration, durable event logs, defense-specific durable state, coefficient tuning, full AI hero task-state implementation, production JSON edits, and live commander-role behavior deferred.

Delivered:
- Added `docs/strategic-ai-hero-task-state-boundary-plan.md`.
- Defined a future faction-owned task layer separate from commander roster continuity, active raid encounter actors, and derived commander-role views.
- Defined deterministic future task id rules, task ownership/lifecycle statuses, first allowed task classes, role-to-task adapter mapping, route/task ownership split, invalidation codes, save/schema risks, old-save compatibility, and rollback/escape hatch.
- Recorded the normalizer risk that future `hero_task_state` under `enemy_states[]` must be explicitly preserved or normalized in `EnemyTurnRules.normalize_enemy_states(...)`.
- Planned report prerequisites for future `AI_HERO_TASK_STATE_BOUNDARY_REPORT` coverage across River Pass Free Company, River Pass Signal Post, Glassroad Relay, Glassroad Starlens, recovery/rebuild blocking, old-save absence, and duplicate target reservation cases.
- Recommended `strategic-ai-hero-task-state-report-fixture-planning-10184` as the next slice.

Validation:
- Planned validation for this slice is `python3 -m json.tool ops/progress.json >/tmp/heroes-progress-jsoncheck.txt`, `git diff --check`, `python3 tests/validate_repo.py`, `python3 tests/validate_repo.py --economy-resource-report`, `python3 tests/validate_repo.py --overworld-object-report`, and `python3 tests/validate_repo.py --neutral-encounter-report`.

Limits:
- This is documentation/planning only. It does not implement reports/tests, write schema, migrate saves, add durable event logs, add defense-specific durable state, tune coefficients, edit production JSON, implement full AI hero task state, adopt live commander-role behavior, change target selection, change raid movement/arrival, change town-governor choices, change pathing/body-tile/approach behavior, change renderer/editor behavior, import generated PNGs, migrate neutral encounters, add `content/resources.json`, migrate `wood` to `timber`, activate rare resources, overhaul market caps, or rebalance River Pass.

## Completed Planning Slice: Strategic AI Hero Task-State Report Fixtures
Status: completed on 2026-04-26 as documentation-only fixture planning after the completed task-state boundary plan.

Purpose:
- Define the exact future `AI_HERO_TASK_STATE_BOUNDARY_REPORT` payload and deterministic fixture cases before report implementation, schema writes, save migration, durable event logs, full AI hero task-state implementation, live commander-role behavior, coefficient tuning, or defense-specific durable state.
- Preserve the passed commander-role transcript boundary while converting it into precise candidate task id, ownership, reservation, invalidation, old-save, and public leak expectations.

Delivered:
- Added `docs/strategic-ai-hero-task-state-report-fixture-plan.md`.
- Defined the top-level report payload, candidate task record shape, compact public task event shape, public leak restrictions, and failure conditions.
- Planned seven cases: River Pass Free Company retake candidate, River Pass Signal Post companion contest candidate, Glassroad relay retake-to-defend transition, Glassroad Starlens stabilizer candidate, commander recovery/rebuild blocking, old-save no-task-state compatibility, and duplicate target reservation report-only handling.
- Defined candidate task id checks, actor ownership checks, target ownership checks, role-to-task source checks, target reservation checks, invalidation checks, old-save absence checks, validation commands, and exact implementation sequence.
- Recommended `strategic-ai-hero-task-state-report-implementation-10184` as the next slice.
...
## Completed Gate Review Slice: Strategic AI Hero Task-State Boundary Report
Status: completed on 2026-04-26 as report-output review before schema or live behavior adoption.

Purpose:
- Review the focused `AI_HERO_TASK_STATE_BOUNDARY_REPORT` output and decide whether report-only candidate task evidence is enough to keep `hero_task_state` schema writes, save migration, durable event logs, defense-specific durable state, coefficient tuning, full AI hero task-state implementation, and live commander-role / AI hero behavior deferred.

Delivered:
- Added `docs/strategic-ai-hero-task-state-report-gate-review.md`.
- Reran the focused hero task-state Godot report and confirmed `ok: true`, `schema_status: "task_state_boundary_report_only"`, `behavior_policy: "derive_candidate_tasks_only"`, `save_policy: "no_hero_task_state_write"`, seven reviewed cases, and a passing public leak check across eight compact public task events.
- Confirmed deterministic candidate ids for River Pass Free Company, River Pass Signal Post, Glassroad relay retake-to-defend transition, Glassroad Starlens stabilization, recovery/rebuild blocking, old-save absence, and duplicate target reservation arbitration.
- Confirmed no blocker requiring coefficient tuning, schema writes, save migration, durable event logs, defense-specific durable state, full AI hero task-state implementation, live commander-role behavior, or live AI hero behavior adoption.
- Recommended `strategic-ai-hero-task-state-adoption-sequencing-planning-10184` as the next narrow planning slice.

Validation:
- Planned validation for this slice is `python3 -m json.tool ops/progress.json >/tmp/heroes-progress-jsoncheck.txt`, `git diff --check`, `python3 tests/validate_repo.py`, `python3 tests/validate_repo.py --economy-resource-report`, `python3 tests/validate_repo.py --overworld-object-report`, `python3 tests/validate_repo.py --neutral-encounter-report`, and `godot4 --headless --path /root/dev/heroes-like /root/dev/heroes-like/tests/ai_hero_task_state_boundary_report.tscn`.

Limits:
- This is a gate review only. It does not write `hero_task_state`, change live AI target selection, change raid movement, change raid arrival, change town-governor choices, tune coefficients, edit production JSON, migrate saves, add durable events, implement full AI hero tasks, add defense-specific durable state, change pathing/body-tile/approach behavior, change renderer/editor behavior, import generated PNGs, migrate neutral encounters, add `content/resources.json`, migrate `wood` to `timber`, activate rare resources, overhaul market caps, or rebalance River Pass.

## Completed Planning Slice: Strategic AI Hero Task-State Adoption Sequencing
Status: completed on 2026-04-26 as documentation-only sequencing after the passed hero task-state boundary report gate.

Purpose:
- Compare the next strategic AI hero task-state step among minimal schema planning, report-only save-normalizer preservation planning, live behavior transcript proof, full AI hero task-state planning, and pausing AI work.
- Define exact gates, risks, and rollback before any schema writes, save migration, `SAVE_VERSION` change, live behavior adoption, durable event logs, or full AI hero task-state implementation.

Delivered:
- Added `docs/strategic-ai-hero-task-state-adoption-sequencing-plan.md`.
- Recommended `strategic-ai-hero-task-state-save-normalizer-preservation-planning-10184` as the next slice.
- Selected report-only save-normalizer preservation planning because `EnemyTurnRules.normalize_enemy_states(...)` currently reconstructs known enemy-state fields and would drop a future optional `hero_task_state` unless a later slice explicitly preserves or normalizes it.
- Kept derived reports as the current evidence boundary: candidate tasks remain diagnostic/report-only and do not drive target selection, raid movement/arrival, town-governor choices, or save writes.
- Deferred minimal schema planning, live behavior transcript proof, full AI hero task-state planning, and pausing AI work until the normalizer/save boundary is planned and proved without schema writes or a save-version change.

Validation:
- Planned validation for this slice is `python3 -m json.tool ops/progress.json >/tmp/heroes-progress-jsoncheck.txt`, `git diff --check`, `python3 tests/validate_repo.py`, `python3 tests/validate_repo.py --economy-resource-report`, `python3 tests/validate_repo.py --overworld-object-report`, and `python3 tests/validate_repo.py --neutral-encounter-report`.

Limits:
- This is documentation/planning only. It does not implement reports/tests, write `hero_task_state`, change live AI target selection, change raid movement, change raid arrival, change town-governor choices, tune coefficients, edit production JSON, migrate saves, bump `SAVE_VERSION`, add durable events, implement full AI hero tasks, add defense-specific durable state, change pathing/body-tile/approach behavior, change renderer/editor behavior, import generated PNGs, migrate neutral encounters, add `content/resources.json`, migrate `wood` to `timber`, activate rare resources, overhaul market caps, or rebalance River Pass.

## Completed Planning Slice: Strategic AI Hero Task-State Save-Normalizer Preservation
Status: completed on 2026-04-26 as documentation-only planning after the completed hero task-state adoption sequencing plan.
...
- Keep `SaveService` as a top-level payload/version/summary boundary instead of making it the owner of strategic AI task semantics.

Delivered:
- Added `docs/strategic-ai-hero-task-state-save-normalizer-preservation-plan.md`.
- Defined future marker `AI_HERO_TASK_STATE_NORMALIZER_PRESERVATION_REPORT`.
- Defined report payload fields for schema status, behavior policy, save policy, SaveService policy, save-version checks, normalized enemy-state checks, malformed-state checks, unknown-field isolation checks, and failures.
- Planned fixture cases for old-save absence, valid optional future-field preservation, explicit empty-board preservation, malformed non-dictionary task state, malformed task records, unknown task-field sanitization, unknown enemy-state junk isolation, SaveService boundary, and commander-roster continuity with malformed task state.
- Defined failure conditions, validation commands, rollback/escape hatch, and exact future implementation sequence.
- Recommended `strategic-ai-hero-task-state-save-normalizer-preservation-report-implementation-10184` as the next slice.

Validation:
- Planned validation for this slice is `python3 -m json.tool ops/progress.json >/tmp/heroes-progress-jsoncheck.txt`, `git diff --check`, `python3 tests/validate_repo.py`, `python3 tests/validate_repo.py --economy-resource-report`, `python3 tests/validate_repo.py --overworld-object-report`, and `python3 tests/validate_repo.py --neutral-encounter-report`.

Limits:
- This is documentation/planning only. It does not implement reports/tests, change `EnemyTurnRules`, write `hero_task_state`, change live AI target selection, change raid movement, change raid arrival, change town-governor choices, tune coefficients, edit production JSON, migrate saves, bump `SAVE_VERSION`, add durable events, implement full AI hero tasks, add defense-specific durable state, change pathing/body-tile/approach behavior, change renderer/editor behavior, import generated PNGs, migrate neutral encounters, add `content/resources.json`, migrate `wood` to `timber`, activate rare resources, overhaul market caps, or rebalance River Pass.
...
## Completed Planning Slice: Strategic AI Economy Pressure
Status: completed on 2026-04-26 as documentation-only planning for the first AI pressure slice after the economy proof gate.

Purpose:
- Use the Riverwatch economy proof facts as the evidence base for the smallest useful opponent-pressure target.
- Keep the slice on existing River Pass content and current `EnemyTurnRules` / `EnemyAdventureRules` pressure scaffolding rather than starting full AI heroes, broad AI state migration, or resource schema migration.

Delivered:
- Added `docs/strategic-ai-economy-pressure-slice-plan.md`.
- Selected the first pressure target: Duskfen/Mireclaw should value and contest `river_free_company` and `river_signal_post` once the player controls them, unless Riverwatch town siege or a visible weak hero is clearly higher priority.
- Defined exact proposed scoring fields for resource targets: base value, persistent income value, recruit value, scarcity value, denial value, route pressure value, town enablement value, objective value, faction bias, travel cost, guard cost, assignment penalty, final priority, and debug reason.
- Defined compact surfacing expectations through report/debug target tables and small event records, without adding large in-game dashboards.
- Defined implementation validation and manual gate expectations for target preference, Duskfen pressure, save/resume continuity, and non-use of hidden bonuses.
- Confirmed no prerequisite blocker exists and sequenced the next slice as `strategic-ai-economy-pressure-implementation-10184`.
...
- Proved that player-controlled `river_signal_post` outranks simple pickups in the focused reachable report, that player-controlled `river_free_company` ranks above `river_signal_post` when both are owned, and that owned persistent signal-yard sites outrank reachable simple pickups such as `north_timber` and `eastern_cache`. `southern_ore` remains a planning comparator but is not present in the focused reachable ordering from the tested raid origin.
...
## Completed Review Slice: Strategic AI Economy Pressure Report Gate
Status: completed on 2026-04-26 as documentation/report-review for the first River Pass strategic AI economy pressure implementation.

Purpose:
- Rerun and review the focused AI economy pressure report.
- Decide whether the first narrow signal-yard pressure target passes, needs coefficient tuning, or should lead into compact AI event/threat reason surfacing.

Delivered:
- Added `docs/strategic-ai-economy-pressure-report-gate-review.md`.
- Reran the focused Godot report and recorded the meaningful target ordering and reasons.
- Marked the gate as passed.
- Confirmed explainable scoring, owned signal-yard site priority over simple pickups, preserved town-pressure dominance, no hidden difficulty bonus, and no broad strategic AI rewrite.
- Recommended `strategic-ai-event-surfacing-planning-10184` as the next slice instead of coefficient tuning or broad AI expansion.
...
## Completed Implementation Slice: Strategic AI Event And Threat Reason Surfacing
Status: completed on 2026-04-26 as the first narrow compact strategic AI event/reason implementation.

Purpose:
- Implement reusable compact AI event records and public reason phrases from existing strategic AI hooks.
- Keep detailed score tables in debug/report surfaces and avoid adding a saved event log or large in-game dashboard.

Delivered:
- Added `docs/strategic-ai-event-surfacing-implementation-report.md`.
- Added reusable event helpers in `EnemyAdventureRules` for `ai_target_assigned`, `ai_pressure_summary`, `ai_site_seized`, and `ai_site_contested`.
- Added compact reason codes/public reasons/public importance to resource and town target candidates.
- Extended `advance_raids(...)` to return ephemeral event records alongside existing compact event messages.
- Added structured site seizure and objective-contest events from existing arrival handlers.
- Added compact visible reason phrases to threat focus and local dispatch summaries.
- Added focused Godot report coverage in `tests/ai_event_surfacing_report.gd` and `.tscn`.
- Confirmed River Pass examples for `river_free_company`, `river_signal_post`, `riverwatch_hold`, resource-site seizure messaging, and objective-site contest records.

Validation:
- Planned validation for this slice is `python3 -m json.tool ops/progress.json >/tmp/heroes-progress-jsoncheck.txt`, `git diff --check`, `python3 tests/validate_repo.py`, `python3 tests/validate_repo.py --economy-resource-report`, `python3 tests/validate_repo.py --overworld-object-report`, `python3 tests/validate_repo.py --neutral-encounter-report`, and the focused Godot AI event surfacing report command.
...
## Completed Implementation Slice: Strategic AI Town Governor Pressure Report
Status: completed on 2026-04-26 as behavior-neutral report/debug surfacing for existing enemy town governor choices.

Purpose:
- Explain existing enemy town build, recruit, garrison, active raid reinforcement, and commander rebuild choices without changing behavior.
- Use River Pass Duskfen / Mireclaw as the first concrete report example.

Delivered:
- Added `docs/strategic-ai-town-governor-pressure-implementation-report.md`.
- Added `EnemyTurnRules.town_governor_pressure_report(...)`, town build candidate reports, recruitment destination reports, and compact derived event helpers.
- Refactored the enemy build score path so the existing chooser and the focused report share the same final-score breakdown.
- Added compact derived event coverage for `ai_town_built`, `ai_town_recruited`, `ai_garrison_reinforced`, `ai_raid_reinforced`, and `ai_commander_rebuilt`.
- Added `tests/ai_town_governor_pressure_report.gd` and `.tscn`, printing `AI_TOWN_GOVERNOR_PRESSURE_REPORT`.
- Confirmed the focused report covers Duskfen build selection, garrison reinforcement, active raid reinforcement, commander rebuild, and public-event leak checks.

Validation:
- Planned validation for this slice is `python3 -m json.tool ops/progress.json >/tmp/heroes-progress-jsoncheck.txt`, `git diff --check`, `python3 tests/validate_repo.py`, `python3 tests/validate_repo.py --economy-resource-report`, `python3 tests/validate_repo.py --overworld-object-report`, `python3 tests/validate_repo.py --neutral-encounter-report`, and the focused Godot town governor pressure report command.

Limits:
- This is report/debug surfacing only. It does not change chosen builds, recruitment destinations, raid launch thresholds, target scoring, resource economy, pathing, saves, production JSON, renderer/editor behavior, generated PNG import, neutral encounter migration, full AI hero task state, broad strategic AI rewrite, or River Pass balance.

## Completed Review Slice: Strategic AI Town Governor Pressure Report Gate
Status: completed on 2026-04-26 as documentation/report-review for report-only enemy town governor pressure surfacing.

Purpose:
- Rerun and review the focused `AI_TOWN_GOVERNOR_PRESSURE_REPORT`.
- Confirm selected build/recruit destination explanations stay behavior-neutral and report/debug-oriented.
- Decide whether compact derived town-governor events leak public score-table fields.
- Decide whether a live-client enemy-turn gate is required before faction personality or commander-role planning.

Delivered:
- Added `docs/strategic-ai-town-governor-pressure-report-gate-review.md`.
- Reran the focused Godot town governor pressure report and recorded meaningful Duskfen / Mireclaw output.
- Reran the focused AI event surfacing and economy pressure reports because the town-governor slice adds derived event types through the shared event-record helper.
- Marked the gate as passed.
- Confirmed Duskfen selects `building_slingers_post`, routes recruitment to garrison stabilization, active raid reinforcement, and commander rebuild in the expected cases, and emits `ai_town_built`, `ai_town_recruited`, `ai_garrison_reinforced`, `ai_raid_reinforced`, and `ai_commander_rebuilt` records without public score-table leakage.
- Deferred the live-client enemy-turn gate until a later AI slice changes visible turn-message composition, turn pacing, arrival frequency, map pressure, or UI layout.
- Recommended `strategic-ai-faction-personality-pressure-planning-10184` as the next slice.

Validation:
- Planned validation for this slice is `python3 -m json.tool ops/progress.json >/tmp/heroes-progress-jsoncheck.txt`, `git diff --check`, `python3 tests/validate_repo.py`, `python3 tests/validate_repo.py --economy-resource-report`, `python3 tests/validate_repo.py --overworld-object-report`, `python3 tests/validate_repo.py --neutral-encounter-report`, and the focused Godot town governor, event surfacing, and economy pressure report commands.

Limits:
- This is documentation/report-review only. It does not edit gameplay code, production content JSON, add a durable AI event log, migrate saves, add `content/resources.json`, migrate `wood` to `timber`, activate rare resources, overhaul markets, change runtime economy/pathing/editor/renderer behavior, import generated PNGs, add assets, migrate neutral encounters, implement full AI hero task state, tune behavior, broadly rewrite strategic AI, or rebalance River Pass.

## Completed Planning Slice: Strategic AI Faction Personality Pressure
Status: completed on 2026-04-26 as documentation-only planning for faction personality evidence from existing strategic AI reports.

Purpose:
- Compare Embercourt and Mireclaw as the first strategic AI personality anchors before coefficient tuning or commander-role state planning.
- Define which existing pressure surfaces can support personality evidence: target preferences, town build reasons, recruitment destination reasons, garrison/raid/commander priorities, and compact public reason phrases.

Delivered:
- Added `docs/strategic-ai-faction-personality-pressure-plan.md`.
- Recorded that Mireclaw currently has direct Duskfen/River Pass report evidence across resource pressure, compact event/threat reasons, and town governor choices.
- Recorded that Embercourt has config and scenario evidence, but still needs equivalent focused report coverage before personality claims should drive tuning.
- Selected `strategic-ai-faction-personality-evidence-report-10184` as the next concrete slice.
- Deferred coefficient/strategy config audit planning, commander-role state planning, capture/counter-capture proof planning, durable AI event logs, and broad AI rewrites until after the evidence report.
...
## Completed Proof Slice: Strategic AI Site-Control Proof Report
Status: completed on 2026-04-26 as focused Godot report/proof scaffolding for the selected Mireclaw River Pass signal-yard path.

Purpose:
- Prove current site-control behavior before coefficient tuning.
- Use the existing `EnemyAdventureRules` resource pressure, assignment event, raid arrival, and site seizure surfaces.
- Record exact blockers if current systems fail, without broadening scope.

Delivered:
- Added `tests/ai_site_control_proof_report.gd` and `tests/ai_site_control_proof_report.tscn`.
- Added `docs/strategic-ai-capture-countercapture-defense-proof-report.md`.
- Proved `river_free_company` and `river_signal_post` rank first and second in the focused Mireclaw resource ordering when both are player controlled.
- Proved `river_free_company` assignment emits compact `ai_target_assigned` output with public reason `recruit and income denial`.
- Proved a staged current-system Mireclaw raid flips `river_free_company` from `player` to `faction_mireclaw` and emits compact `ai_site_seized` output with `denies its logistics route` wording.
- Proved `river_signal_post` keeps companion public reason `income and route vision denial`.
- Confirmed public assignment/seizure events do not leak score-table keys while detailed score rows remain report/debug-only.
- Recorded the valid full selector sanity result: `choose_target(...)` still selects `riverwatch_hold` as the town-front target.
- Selected `strategic-ai-glassroad-defense-proof-planning-10184` as the next slice because no site-control coefficient defect was found.

Validation:
- Passed `python3 -m json.tool ops/progress.json >/tmp/heroes-progress-jsoncheck.txt`, `git diff --check`, `python3 tests/validate_repo.py`, `python3 tests/validate_repo.py --economy-resource-report`, `python3 tests/validate_repo.py --overworld-object-report`, `python3 tests/validate_repo.py --neutral-encounter-report`, `godot4 --headless --path /root/dev/heroes-like /root/dev/heroes-like/tests/ai_site_control_proof_report.tscn`, `godot4 --headless --path /root/dev/heroes-like /root/dev/heroes-like/tests/ai_economy_pressure_report.tscn`, `godot4 --headless --path /root/dev/heroes-like /root/dev/heroes-like/tests/ai_event_surfacing_report.tscn`, and `godot4 --headless --path /root/dev/heroes-like /root/dev/heroes-like/tests/ai_faction_personality_evidence_report.tscn`.

Limits:
- This is focused report/proof scaffolding only. It does not tune coefficients, change AI behavior, edit production content JSON, add durable event logs, migrate saves, implement full AI hero task state, change pathing/body-tile/approach behavior, alter renderer/editor behavior, migrate neutral encounters, add `content/resources.json`, migrate `wood` to `timber`, activate rare resources, overhaul market caps, rebalance River Pass, import generated PNGs, push, or open a PR.

## Completed Planning Slice: Strategic AI Glassroad Defense Proof
Status: completed on 2026-04-26 as documentation-only planning for a bounded Embercourt Glassroad defense/stabilization proof.

Purpose:
- Use the passed Mireclaw site-control proof and current Embercourt evidence to define the next bounded proof without tuning.
- Plan a report around `glassroad-sundering`, the current direct Embercourt enemy fixture.
- Decide whether current compact public/debug event vocabulary is enough for the proof, or whether defense-specific behavior needs explicit state later.

Delivered:
- Added `docs/strategic-ai-glassroad-defense-proof-plan.md`.
- Selected `glassroad_watch_relay` as the primary proof site and `glassroad_starlens` as the companion stabilization/reason surface.
- Preserved `halo_spire_bridgehead` as the valid full-selector town-front sanity check and `glassroad_beacon_wardens` as objective/encounter companion context.
- Defined the follow-up `AI_GLASSROAD_DEFENSE_PROOF_REPORT` payload, fixture sanity checks, expected resource ordering, assignment event, relay seizure/retake or blocker output, Starlens response-profile evidence, Embercourt town-governor garrison stabilization signals, public leak checks, validation commands, manual gate triggers, and out-of-scope boundaries.
- Decided that new defense-specific durable state is not approved for the next slice unless the focused report proves current assignment, seizure, pressure-summary, and town-governor surfaces are insufficient.
- Sequenced the next slice as `strategic-ai-glassroad-defense-proof-report-10184`.

Validation:
- Planned validation for this slice is `python3 -m json.tool ops/progress.json >/tmp/heroes-progress-jsoncheck.txt`, `git diff --check`, `python3 tests/validate_repo.py`, `python3 tests/validate_repo.py --economy-resource-report`, `python3 tests/validate_repo.py --overworld-object-report`, and `python3 tests/validate_repo.py --neutral-encounter-report`.

Limits:
- This is documentation/planning only. It does not edit production content JSON, gameplay behavior, AI coefficients, resource schemas, pathing/body-tile/approach behavior, renderer/editor behavior, saves, generated assets, neutral encounter metadata, River Pass balance, durable event logs, full AI hero task state, defense-specific durable state, broad strategic AI rewrite, behavior tuning, or public UI surfaces.
