...
- The active compact port includes top-level `0x4a3a03` footprint scheduling and the small-land `0x4a3710` finalizer boundary: seed `1` has one land level, six matching runtime zones, six polygon split calls, no synthetic fallback zone, no appended runtime zones, six ordering reset/rebuild calls scheduled, and zero adjacency inserts. This remains inspection-only and does not materialize zone cells.
...
- Pending before runtime generation: `0x4a2777/0x4a325d` boundary traversal and zone span-fill materialization, terrain/cell writeout, owned towns, roads, blockers, guards, mines, rewards, and final writeout.
