...
- Current checkpoint-2 finding: refreshed no-Godot snapshots in `.artifacts/rmg_native_cli_setup_default_postsynthetic_terrain/` now default missing setup `+0x44` to recovered H3MapEd initializer value `3` with explicit provenance fields, while preserving exact equality with explicit `:3` controlled cases. Runtime terrain selection now iterates the post-`0x4a3b48/0x49b452` synthetic runtime-zone vector, so Medium seed 10 setup `3` no longer drops synthetic owner cells during `0x4a3f27` terrain writeout. The matched Medium `hc4_comp0` setup `0` comparison in `.artifacts/rmg_native_cli_4a2777_append_order_hc4_restore/private_state_compare_hc4_setup0_after_live_feedback.json` still uses the same `2SM4d(2)` template and reports matching owner-id coverage (`0..9`) with 475/5184 owner-byte placement mismatches remaining. The no-Godot boundary snapshot in `.artifacts/rmg_native_cli_4a2777_append_order_hc4_restore/medium_4p_seed10_hc4_setup0.phase_snapshot.json` now exposes the source-backed `0x4a2777` boundary-vector append order over currently materialized native source-node walks: selected start vertex at `0x4a2990` before the owner gate, and pre-update current-point vertices at `0x4a2adc/0x4a2b1e`. This is progress inside checkpoint 2, not parity: applying the recovered one-endpoint traversal over the current native descriptor regressed the same-run owner surface, so exact live grid mutation remains blocked until the `0x4a3a03 -> 0x4cca55 -> 0x4a325d` descriptor/seed handoff is ported into plain C++. `word_0x20` low-word projection, `word_0x24` visual/art selection, `word_0x28` bit26/bit27 mutations, exact same-run `generator+0x3f4` vector materialization, and the `0x4a325d` seed/input join still mismatch or remain unported.
