...
- Current native-vs-H3MapEd finding: Medium seed-10 input/template identity, coordinate replay, selected runtime terrain ids, recovered `0x4a2777` dual endpoint clipping, map-border continuation, per-edge owner writer skip, `0x4bbfcc` final-sweep scratch mask/class correction, the source-backed town-only `0x49abd6 -> 0x4a8260` object-reference vector boundary, and the recovered `0x4ac552` reward active-vector scheduling surface are ported for the controlled land references. The current Linux/Python artifacts `.artifacts/rmg_native_small2p_seed58_reward_active_vector_20260611` and `.artifacts/rmg_native_medium_seed10_reward_active_vector_20260611` prove native no longer schedules rewards over every runtime zone: seed-58 now has `2` scheduler records and `45` reward value attempts, matching the recovered H3MapEd `0x4aab7e/0x4aa354` call-count shape; Medium seed-10 now schedules `4` active records instead of `10`. With the same explicit Medium H3MapEd dump, pre-`0x4a4c8e` `word_0x28` mismatches remain `2,999`, and final tile byte mismatches are `11,725` with first drift still `terrain_generated_cell_word_0x24_before_0x49b2b6`. The remaining blocker is no longer reward scheduler scope; it is exact reward object/materialization state inside the active scheduler calls (`0x4a9f1c`, dynamic `+0xf5c/+0xf58/+0x10b4`, `0x49d471`, `0x49cf34`, `0x49d69d`, `0x4aa9b7`, `0x4aa3e9`) before the same-run `0x4a8260` route RNG/stamp stream can align. Do not patch this with density scalars, art remaps, brute-force retries, Windows rebuild churn, or final-map delta tuning. Inner-loop validation is Linux `.so` plus Python parity compares; Windows `.dll` builds are deferred until a Linux-verified final boundary.
