...
- Latest native RMG implementation update: `h3maped_rmg_core` now keeps the single source-order workflow from setup through route/free-cell and into source-backed mine/resource minimum materialization before later relation/reward phases. The generator object private-state builder runs setup, template/runtime seed construction, coordinate/owner/terrain support state, generator private-state vector initialization, pre-route type-98 materialization, source-order `0x4a8c15 -> 0x4a8260 -> 0x4a4c8e` route/free-cell ownership, and `0x4a9d6a` leading-descriptor mine/resource counts. It now materializes the recovered generator `+0x388/+0x38c` mine/resource descriptor bucket from source catalog order, executes the recovered `0x4a9911` selection scan using source field `+0x20`, relation terrain policy `+0x0c`, terrain bitset helper `0x42cc99`, fallback category scan, RNG modulo selection, recovered `0x4a9641` coordinate scan/selection/`0x4a54a7` object commit, recovered `0x4a960a`/`0x4a65a5` guard value scaling, recovered `0x4a9911` guard coordinate adjustment, and positive-value `0x4a5e03 -> 0x4a54a7` ordinary guard commit. Minimum placement callbacks now continue until the first unowned source-order follow-up, then fail closed at `0x4a9d6a_density_followup_0x4a9c7c_0x4a9641_unported_after_minimums`. It does not execute source-order `0x4a9c7c` density followup, `0x4aadd2`, `0x4a5767` relation scan consumers, generic source-order object replay, `0x49a318` high-owner propagation, reward/guard `0x4aab7e -> 0x4aa9b7 -> 0x4aa3e9`, D-014 endpoint prep, `0x49eb8d` decorative dispatch, `0x4a79a3` connection tail, `0x4a7605 -> 0x4a5e03` fallback replay, roads/rivers, or final writeout until mine/resource density/materialization is owned. The next implementation target is the source-order mine/resource density follow-up `0x4a9c7c`, not reward/guard. This is not parity and not final map output.
- Previous native RMG implementation update: `h3maped_rmg_core` now carries recovered relation-owner reward/guard treasure bands `+0xa0/+0xa4/+0xa8` through `+0xc0`, terrain policy `+0x0c`, stale supported-land cursor `+0xf5c = 0x7a1befdf`, zeroed `+0xf60/+0xf64` terrain-pressure state, recovered `0x57cea0` monster terrain rows, and `0x531cc4` type-17 subtype-to-monster rows into the ordered reward/guard selector. The no-Godot workflow advances through `0x4aab7e -> 0x4aa354 -> 0x4aa1db -> 0x4a9f1c -> 0x4a9e40`, allocates ordinary `0x540a88` selected records through generator key and `+0xf44` sequence state, seeds wrapper selected members, stamps initial wrapper cells, refreshes bounds, and attempts reward/guard source-stream materialization. Those attempts currently produce zero successful `0x4aa9b7` commits, so they are the active source-order blocker instead of a license to execute later connection helpers.
...
- Current source-order object materialization status: weighted `0x4a901a`, direct exact-input `0x4a8d2c -> 0x4a93a2 -> 0x4a54a7`, source-backed `0x4a8db2` direct-prepass/weighted-lane scheduler replay, explicit-input reward/guard `0x4aa9b7 -> 0x4aa3e9` scan/commit with recovered `0x4aa603` feasibility filtering, explicit reward/guard wrapper attach/finalizer helpers `0x49ce04/0x49ce64/0x49cf34/0x49cefb`, recovered `0x49d2e0` candidate filtering, and the recovered empty-vector `0x49d7c3` wrapper contour rebuild are now implemented in shared C++ for source-backed inputs and mirrored through native private-state reporting where their callers exist. The live generator-object private-state chain now builds `0x4a8db2` scheduler source records from runtime-zone source payload town/castle count and density fields, then replays the type-98 town/castle scheduler after relation-scan source-pair preservation and before high-owner propagation. Descriptor-joined non-type98 `+0xedc` pairs now replay into recovered `0x4a8d2c` and `0x4a8db2` when their relation/key/anchor context and copied source count/density fields are present; pairs missing descriptor/relation/source-field context fail closed and remain explicit blockers. The `0x4af785` resolver now preserves the actual generator `+0xedc` source-pair payload contents through object-branch and relation-scan state: copied source record at pair `+0x00`, wrapper/context copy for pair `+0x04`, source lane, wrapper lane, and wrapper `+0x10` metadata. Pairs created through `0x4903e8` additionally retain descriptor-join context needed by later source-order replay, and generator object private state carries the actual recovered `+0xed8/+0xee0/+0xee4` player-slot arrays, not only counts. This is not full live object parity: live descriptor producers/raw source-field capture for all generic non-type98 source pairs, live `0x4adb72/0x4ad7f7` reward/guard wrapper/source-relation caller feed, fallback materialization caller feed, downstream caller order, and final payload writer remain unported.
