...
- Current native-vs-H3MapEd finding: Medium seed-10 input/template identity, coordinate replay, and selected runtime terrain ids are corrected for the controlled 4-player land reference. Native now preserves the recovered `0x4a2777` dual endpoint clipping, map-border continuation, and per-edge owner writer skip over the `0x4cca55` source descriptor handoff. The current Linux/Python compare artifact `.artifacts/rmg_native_medium_seed10_dual_clip_owner_gate_20260611` improves the pre-`0x4a4c8e` owner surface from `owner_byte2=1,558` mismatches and 1,002 unassigned native cells to `owner_byte2=475` mismatches and 1 unassigned native cell; the focused Small seed-11 sanity export `.artifacts/rmg_native_small_seed11_dual_clip_owner_gate_sanity_20260611` has 0 unassigned cells and no blocked seeds. Full final tile parity is still incomplete: Medium seed-10 final tile byte mismatches are 15,694 and first drift remains `terrain_generated_cell_word_0x24_before_0x49b2b6`, with remaining work in exact terrain/live-feedback word replay, road/flag lanes, and residual `0x4a2777` descriptor topology. Do not patch this with density scalars, art remaps, brute-force retries, Windows rebuild churn, or final-map delta tuning. Inner-loop validation is Linux `.so` plus Python parity compares; Windows `.dll` builds are deferred until a Linux-verified final boundary.
