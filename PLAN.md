...
- Use `docs/strategic-ai-commander-role-report-implementation-plan.md` as the implementation boundary: report helpers belong in `EnemyAdventureRules.gd`, town-governor evidence stays in `EnemyTurnRules.gd`, fixture annotations remain report-only, and the future scene prints a single `AI_COMMANDER_ROLE_STATE_REPORT` line.
- Keep the next implementation slice scoped; do not tune coefficients, change production content JSON, add durable event logs, implement full AI hero task state, add defense-specific durable state, implement live commander-role behavior, write schema/save fields, or change pathing/renderer/editor/save behavior.
...
- Start commander-role report implementation as the next report-only strategic AI slice, because fixture cases, schema boundaries, helper ownership, fixture normalization policy, report scene shape, and public leak checks are now defined.
...
- The next active choice is report-only commander-role report implementation after the completed implementation boundary plan, still without production JSON migration, new resource registry, `wood` to `timber` migration, rare-resource activation, market-cap overhaul, pathing/body-tile/approach adoption, full AI hero/task implementation, broad AI rewrite, behavior tuning, editor behavior, renderer behavior, save format changes, generated PNG import, asset import, live commander-role behavior adoption, or new defense-specific durable state.
