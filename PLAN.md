...
- Latest native RMG implementation update: `h3maped_rmg_core` now replays the recovered first pass and initial neighbor scan of decorative scorer `0x49e1bf` after `0x49e700` `rand_trn.txt` terrain-score filtering. The scorer carries the recovered scratch footprint, scans the descriptor footprint plus one-cell border, applies the recovered `byte_0x2b & 0x02`/terrain skip, walks generated-cell `+0x04/+0x08` object-reference keys when present, and fails closed on unrepresented `0x49b89c` score-index/cache fields instead of deleting or guessing them. No-Godot controlled snapshots for Small seed 10, Small seed 58, and Medium seed 10 now invoke the neighbor pass once, scan 20 cells, skip 18 empty scratch cells, encounter no object refs for the first accepted candidate, and fail closed later at `0x49e700_weighted_decorative_object_allocation_after_0x49e1bf_unowned`. This is source-order workflow progress, not parity completion; final payload generation remains disabled until decorative allocation commit, exact object-reference score-table replay for ref-bearing candidates, road/river adjacency, descriptor counter replay, and final writeout are source-owned.
