...
- Latest native RMG implementation update: `h3maped_rmg_core` now carries a source-backed model of the recovered `0x4adfe1 -> 0x49ecf2` setup stack into the native no-Godot workflow and phase snapshot. Known stack words are width, height, level count, setup mode `+0x44`, prepared value band `clamp(raw +0x48 + 3, 1, 5)`, and setup field `+0x4c`; the remaining five source words are named as unrecovered inputs (`+0x34/+0x38/+0x3c/+0x40` and caller stack `[EBP+0x0c]`). The same-run final-payload authority guard now enforces setup-stack equality only when the authority ledger actually captured stack words, so missing stack capture no longer masks payload comparison. Focused no-Godot Medium seed-10 setup-0 still writes final payload sections, serializes all 900 generated objects with no serializer block, writes the `0x4ad3db` final zero sentinel, emits a 61,424-byte ordered payload, and exposes the partial setup stack in the phase snapshot. The run now blocks at `native_final_tile_stream_mismatch_against_same_run_0x49b2b6_payload`: tile bytes are 36,288 vs 36,288 with first mismatch offset 0 (`native=4`, `h3maped=2`), and generated-object payload is 12,376 vs 17,057 with first mismatch offset 0 (`native=4`, `h3maped=2`). Runtime/native map output remains disabled. This is source-backed setup-stack modeling and blocker correction, not final-payload parity or runtime generation authorization.
