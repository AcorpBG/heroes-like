...
- Recently completed follow-up: battle boards now render checked-in generated terrain PNGs from `art/battle/terrain/` as the battlefield backdrop when available, with `plains` mapped cleanly to the grass texture and the prior procedural terrain retained as a missing-asset fallback. The textured backdrop keeps the existing hex grid, movement/attack affordances, stack tokens, turn strip, and footer overlays on top. Focused town/battle visual smoke, battle layout smoke, and repo validation are green.
