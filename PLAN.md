...
- Small land generation may emit only explicitly non-authoritative partial packages until roads, blockers, rewards, guards, and final h3maped writeout phases are ported.
...
- Normal 36x36 one-level land `native_catalog_auto` generation now returns a non-authoritative partial h3maped package with status `h3maped_small_clean_restart_phase_package_adoption_partial`; medium/large/XL and other out-of-scope catalog-auto generation returns `archived_legacy_native_rmg_disabled`. The archived native catalog-auto path is no longer reachable through a debug/evidence generation option.
...
- The boundary exposes an executable phase ledger and keeps later map-materialization phases pending: terrain fill/repaint schedule `0x4a3f27/0x4bcff5/0x4bd099`, one-level TerrainPlacement art/index/flip normalization, owner/repaint cell grids, town/castle field consumption, direct candidate scanning, direct validity prechecking, mine/reward source field ledgers, the `0x4a9911` mine template/record/guard handoff ledger, the `0x4a9641` mine candidate scan/selection/body-cell marking, and the `0x4a79a3` raw-link Value/Wide/Border Guard payload semantics are ported. Runtime package adoption now materializes the ported terrain, 3 direct town records, 12 selected mine records, and 5 connection payload records into a structurally valid `MapDocument`, but generalized production object-cell stamping, `0x4a61bc/0x4a696b/0x4a6cf2/0x4a7605` connection geometry, `0x4ab37f` road adapter paths, physical connection blockers, reward/monster placement, guard object materialization, and final cell/object passes remain pending.
- The partial package adoption deliberately sets `native_runtime_authoritative: false`, `full_parity_claim: false`, and `no_authored_writeback: true` while preserving explicit `h3maped_0x4ab52a_0x4ab37f_road_adapter_boundary_recovered_toolkit_pending` and `h3maped_0x4a79a3_link_guard_payload_ported_guard_object_materialization_pending` statuses. The road boundary records `generator+0x14b0` coordinate-vector sources from `0x4a959b` direct town append and `0x4a6bb4` generic object append through `0x4ae1fd`, now materializes 15 partial 12-byte coordinate-vector entries for the already adopted 3 town and 12 mine records, carries those counts into the `0x4ab52a` road-adapter boundary, records the ported `0x4aae2f` generated-cell path-state reset ledger, recovers the `0x4aae7b` path-state seed/propagation boundary with neighbor table `0x5a2658`, generated-cell bit gates, object metadata table `0x57c648`, and special vectors `generator+0x14c0/+0x14d0`, recovers the `0x4ab37f` road-adapter bridge that builds terrain adapter `0x540a14`, road adapter `0x540a34`, calls `0x4b4243`, and follows predecessor coordinates, recovers the `0x4b4243` toolkit entry as initializer/dispatcher through vtables `0x5419d4/0x5419f4`, line helpers `0x458d37/0x458e61`, and neighbor table `0x5a5028`, recovers the `0x458e61` line-visit adapter boundary through road-adapter virtuals `+0x14`/`0x49af7b`, `+0x08`/`0x49aec5`, `+0x04`/`0x49ae47`, `+0x10`/`0x49af1d`, helper `0x458a2f`, neighbor-mask helper `0x4587f7`, edge helper `0x4bf3f4`, and road-shape table `0x538a04..0x538a8f`, and emits no synthetic road segments while clean generated-cell road mutation and complete coordinate-vector parity remain pending.
- `tests/native_rmg_small_h3maped_port_boundary_report.tscn` passed with `status: h3maped_small_clean_restart_template_selection_ready`, `archive_status: previous_native_catalog_auto_generator_archived_debug_only`, `accepted_template_count: 13`, `selected_template: h3maped_template_018`, `generated_validation_status: pass`, `generation_status: h3maped_small_clean_restart_phase_package_adoption_partial`, `generated_town_count: 3`, `generated_mine_count: 12`, `generated_connection_count: 5`, `generated_connection_status: h3maped_0x4a79a3_link_payload_semantics_ported_geometry_roads_guards_pending`, `road_adapter_boundary_status: h3maped_0x4ab52a_0x4ab37f_road_adapter_boundary_recovered_toolkit_pending`, and `out_of_scope_generation_status: archived_legacy_native_rmg_disabled`. The footprint counts changed because the clean order now consumes the two `0x49b53d` terrain RNG calls before `0x4a3a03`.
