...
- Latest native RMG implementation update: `h3maped_rmg_core` now ports recovered reward/guard selected-create dispatch for source-backed inputs: `0x4a9f1c` weighted candidate selection, `0x4aa151` score replay for recovered direct and projection candidate contracts, `0x4aa166` selected-create dispatch, and the `0x540c70/0x540c80 -> 0x49cb83/0x49cc22 -> 0x540b14` projection-object constructor return path. The active no-Godot workflow no longer probes `0x49c0a6` with a null projection object; it blocks at `0x4aa1db_selected_create_caller_feed_pending_before_0x4a9f1c_0x4aa166_0x49c0a6`, which means the next implementation target is the live caller feed that supplies selector/retry/descriptor context to selected-create, then the `0x4ad947` global table/coordinate inputs, wrapper/member handoff, remaining reward/guard selected descriptor flow, connection/road/river phases, and final tile/object/header writeout. This is not parity and not final map output.
