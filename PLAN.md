...
- Current native-vs-H3MapEd finding: Medium seed-10 input/template/player identity passes in the corrected phase-drift audit after fixing the Python report to read wrapped `h3maped_small_port` snapshots and compare against the observed saved H3M identity. Current Linux/Python artifacts are `.artifacts/rmg_native_medium_seed10_4a93a2_rng_fix_20260612` and `.artifacts/rmg_native_small2p_seed58_4a93a2_rng_fix_20260612`. The native `0x4a93a2` direct-town branch now consumes `0x4e7276` selection RNG for every successful best-candidate vector, including singleton vectors, matching the recovered Ghidra call pattern; Medium seed-10 changed from `0` to `6` direct selection RNG calls and `object_rng_state_after_0x4a93a2_uint32` changed from `572769411` to `3425703529`. Native RMG parity remains incomplete: Medium seed-10 first same-identity drift is still `town_castle_endpoint_vector` coordinate drift (`H3M: 16,49 / 20,32 / 39,35 / 50,47 / 55,63 / 9,19`; native: `16,48 / 20,32 / 41,33 / 48,46 / 53,63 / 9,18`). Mines still match `24/24`, rewards remain underproduced (`57` native vs `144` H3M), guards remain underproduced (`47` native vs `126` H3M), roads overdraw (`315` native vs `136` H3M), and Medium pre-`0x4a4c8e` `word_0x28` mismatches remain `2,999`. Small seed-58 route replay still passes mechanically but native route RNG state does not match (`native_match=False`, H3MapEd route entry offset `5,002`). This proves the selection RNG boundary was wrong but not sufficient: the next implementation target is the exact source-backed `0x4a93a2` source-record `+0x10/+0x20` coordinate/rectangle inputs and/or missing `0x49aa93` eligibility predicate that produce the town candidate vector. Do not patch this with density scalars, art remaps, brute-force retries, Windows rebuild churn, or final-map delta tuning. Inner-loop validation is Linux `.so` plus Python parity compares; Windows `.dll` builds are deferred until a Linux-verified final boundary.
