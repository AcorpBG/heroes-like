...
- Latest native RMG implementation update: the final-payload same-run authority is now carried as explicit recovered profile metadata through `SharedRuntimeChainInput`, `H3MapedRmgWorkflowConfig`, and the no-Godot native CLI. The default authority profile is sourced from `.artifacts/rmg_recovery/same_run_final_payload_summary_20260610.json` only when its Medium seed-10 one-level no-water profile and 36,288/17,057 byte counts match the loaded tile/object authority binaries. Focused no-Godot Medium seed-10 setup-0 still serializes 915 generated objects, writes the `0x4ad3db` final zero sentinel, and emits a 61,354-byte ordered payload, but now fails closed at `same_run_payload_authority_profile_loaded_but_0x49ecf2_setup_stack_join_missing`. Same-run byte parity is not invoked until the exact recovered setup stack words for that final payload are joined to the authority profile. Runtime/native map output remains disabled.
