...
- Current checkpoint-2 finding: refreshed Python-only Medium seed-10 compares in `.artifacts/rmg_native_checkpoint5_route_gating_current/private_state_compare_pre_0x4a4c8e_same_run.json`, `private_state_compare_after_terrain_live_feedback_same_run.json`, and `private_state_compare_after_object_vector_private_grid_same_run.json` expose the owner surface directly. H3MapEd has 13 non-negative owner ids (`0..12`) for selected template `2SM4d(2)` with 10 source zones, while the earlier native checkpoint had 9 non-negative owner ids. Native has a standalone no-Godot `0x4a2777/0x4a325d` boundary/span-fill phase snapshot surface for the currently materialized source-node walks: Medium 4p seed 10 materializes 10 zone walks, 579 boundary cells, 3,603 unique span-filled cells, and 4,182 assigned boundary-or-filled cells; Small 2p seed 58 materializes 9 zone walks, 327 boundary cells, 846 unique span-filled cells, and 1,173 assigned boundary-or-filled cells. This is not checkpoint parity yet. The exact branch condition behind the missing same-level owner surface remains source-backed: `0x4a3a9d` tests `level_index == 1 || generator+0x10b8 != 0`, and `generator+0x10b8` is written by `0x49ecf2` from constructor arg8 supplied by RMG setup object `+0x44` at `0x4adfe1`; setup value `3` is randomized through `0x4e7276() % 3` before template selection consumes RNG. The no-Godot controlled-case path now resolves supplied setup `+0x44` values, including setup `3`: Small 2p seed 58 resolves first RNG `228` to mode `0` and can continue through current source-node/boundary summaries, while Medium 4p seed 10 resolves first RNG `71` to mode `2` and correctly remains blocked on the still-unported same-level synthetic runtime-zone append. The remaining implementation target for checkpoint 2 is exact same-run setup `+0x44` capture/defaulting when not supplied plus `0x4a3b48` direction scan and `0x49b452` synthetic runtime-zone append replay before rerunning pre-`0x4a4c8e` generated-cell word compares.
- Current guardrails: do not re-open H3MapEd reverse engineering for this slice unless Python/Ghidra/Wine evidence disproves the recorded recovery, do not add density scalars/gates/brute-force retries/final-map tuning, do not use GDScript for reports/exports, and do not launch Godot/headless Godot for native RMG export on this memory-constrained host. `tools/rmg_native_batch_export.py` now only supports the standalone no-Godot `bin/rmg_native_batch_export_cli` path; the legacy Godot runner has been removed from this wrapper. The no-Godot CLI now owns plain-C++ controlled-case parsing/filtering, checkpoint-2 phase snapshots with constructor-default generated-cell words, embedded-catalog template selection/runtime-zone records, recovered `0x49ecf2` generator-mode resolution for supplied setup `+0x44`, link-seed extraction, `0x4a1f3b` coordinate replay summaries, `0x4a3a03/0x4cc788/0x4ccb64/0x4ccdfc` source-node footprint summaries, and `0x4a2777/0x4a325d` boundary/span-fill owner-word summaries for supported Small/Medium one-level land cases, but still fails closed for `.amap` generation until the recovered H3MapEd RMG generation core is split from Godot `Dictionary`/`Array`/`String`, `RefCounted`, and `FileAccess` APIs into plain C++ data structures. Do not treat the initial generated-cell grid, template/runtime-zone summary, link-seed summary, coordinate replay summary, source-node summary, or boundary/span-fill summary as pre-`0x4a4c8e` parity; same-level runtime-zone append and later generated-cell mutations remain pending. Do not build Windows `.dll` outputs until Linux `.so` parity checks reach the final boundary.
