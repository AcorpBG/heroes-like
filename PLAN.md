...
- Latest native RMG implementation update: `h3maped_rmg_core` now uses the recovered TerrainPlacement final-sweep selector split. `0x4bbfcc` only applies the `2->6`, `8->12`, `5->7`, and `11->13` class upgrades when the recovered `0x4bcb91` or `0x4bcd43` probe predicates pass, and it routes class-0 final-sweep cells through recovered `0x4ba938` current-row preservation instead of the classified bucket path. The no-Godot Medium seed-10 setup-0 snapshot remains fail-closed at `native_final_tile_stream_mismatch_against_same_run_0x49b2b6_payload`: tile stream length matches at 36288 bytes, tile 0 matches, and the first mismatch is offset 8 (`native=51`, `h3maped=60`). Generated-object payload still mismatches, so this is TerrainPlacement source-order implementation progress, not parity completion or runtime map output.
