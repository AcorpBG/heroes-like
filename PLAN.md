...
- Native RMG alignment checkpoints, in execution order:
  ...
  3. Reward/object identity: carry source records/descriptors through selection and materialization instead of resolving final identity through placeholder object ids. The recovered `0x49da08` object source-record catalog and `0xe8` `0x49db76` wrapper-bucket layer are now exposed in shared C++ and no-Godot blocked snapshots, but wrapper/source selection, `0x4903e8` descriptor joins, selected copied records, and materialization remain unported.
