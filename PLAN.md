...
- The `0x4a325d` span-fill helper and `0x4a3710` adjacency finalizer are now disassembled into the boundary report without fake output. The report records `0x00ff0000` unassigned cell sentinel handling, horizontal span fill behavior, above/below span queueing, `runtime_zone+0xc4` adjacency insertion, `runtime_zone+0x3e8` ordering vectors, and the exact span/adjacency/source-polygon schemas still blocking project materialization.
- The runtime polygon layout is now partially recovered from `h3maped.exe` without inventing project geometry. The boundary records `0x49b452` runtime-zone vector offsets, 0x24-byte polygon nodes constructed/finalized through `0x4cc788`, `0x4cc5db`, `0x4ccb64`, and `0x4ccdfc`, and now ports the `0x4a3a03` polygon seed-call schedule: `0x4cc788` initializes the `-200..400` enclosing polygon and `0x4a3a79` calls `0x4ccb64` once per same-level runtime zone using `runtime_zone+0x10` x/y plus the runtime-zone pointer. Seed `1` materializes 6 primary split seed calls on level `0`, executes the `0x4cca55/0x4ccb64` locator/insertion/bridge loop, ports the `0x4cc9cc` edge-removal branch plus `0x4ccc7a/0x4cc68e` crossing cleanup, and reports 24 allocated node pairs / 23 active node pairs after cleanup. It also ports `0x4ccd69/0x4ccdfc` finalized-coordinate fanout, producing 14 finalized triplets and 42 finalized nodes. Feeding those finalized polygon coordinates into `0x4a325d` span fill remains the next blocker before terrain-cell materialization.
