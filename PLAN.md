...
- Latest native RMG implementation update: `h3maped_rmg_core` now passes recovered `water_mode_code` separately from setup/generator mode through `0x4a2777/0x4a325d`, so one-level land maps no longer lose the `0x4a325d` member/repaint flags when setup mode is `2`. No-Godot controlled snapshots for Small seed 10, Small seed 58, and Medium seed 10 now have nonzero `0x4a3f27` terrain repaint writes and no longer stop at `0x4a61bc_live_prefix_no_source_frontier_candidate_after_0x49b3fb`; all three advance to the same current blocker, `0x49eb8d_0x49e700_decorative_dispatch_unported_for_valid_bit26_cells`, before final writeout. This is source-order workflow progress, not parity completion; final payload generation remains disabled until decorative dispatch, road/river adjacency, descriptor counter replay, and final writeout are source-owned.
