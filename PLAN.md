...
64. Only after these basics are deep enough and art-direction evidence starts to exist, return focus to implementation planning, campaign/skirmish maps, town-screen polish, battle-screen polish, battle AI systems, and the overall player loop: turn flow, economy, resource collection, mine capturing, battles, and town development.

Recommended next slice:
- Run `strategic-ai-commander-role-live-turn-transcript-report-planning-10184` as the next concrete slice.
- Use `docs/strategic-ai-commander-role-adoption-sequencing-plan.md` as the current sequencing contract: plan a behavior-neutral derived enemy-turn transcript/report before any schema write, live commander-role behavior, full AI hero task state, or AI foundation pause.
...
- Treat commander-role report output and commander-role adoption sequencing as passed planning gates. The next AI slice should plan `AI_COMMANDER_ROLE_TURN_TRANSCRIPT_REPORT` before any schema write, save migration, durable event log, full AI hero task state, defense-specific durable state, coefficient tuning, or live commander-role behavior adoption.
...
- The next active choice is behavior-neutral commander-role live-turn transcript/report planning after the completed adoption sequencing plan, still without production JSON migration, new resource registry, `wood` to `timber` migration, rare-resource activation, market-cap overhaul, pathing/body-tile/approach adoption, full AI hero/task implementation, broad AI rewrite, behavior tuning, editor behavior, renderer behavior, save format changes, generated PNG import, asset import, live commander-role behavior adoption, schema writes, save migration, durable event logs, report/test implementation, or new defense-specific durable state.
...
Current target: behavior-neutral commander-role live-turn transcript/report planning after the completed adoption sequencing plan.
...
- The next slice plans `AI_COMMANDER_ROLE_TURN_TRANSCRIPT_REPORT` only; it does not implement reports/tests, adopt live behavior, migrate saves, write schema, add durable event logs, tune coefficients, or implement full AI hero task state.
...
- No production JSON migration, durable AI event log, save migration, full AI hero task state, broad AI rewrite, pathing/body-tile/approach adoption, renderer/editor changes, generated PNG import, neutral encounter migration, rare-resource activation, market-cap overhaul, new defense-specific durable state, report/test implementation, or River Pass rebalance is pulled into the planning slice.
