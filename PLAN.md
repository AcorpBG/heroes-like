...
- Current native-vs-H3MapEd finding: Current Linux/Python artifacts are `.artifacts/rmg_native_small2p_seed58_route_rng_source_handoff_20260612` and `.artifacts/rmg_native_medium_seed10_route_rng_source_handoff_20260612`. Native now seeds the diagnostic `0x4a8260` route replay from the post-object-vector reward boundary (`mines_rewards_and_object_vector.reward_scheduler_boundary.preview_rng_state_after_0x4aa354_uint32`) instead of the earlier terrain-placement visual RNG fallback. Small seed-58 route verification still passes the recovered H3MapEd route stream mechanically (`416` splits, `340` stamps, `52` `0x4a80dc` cuts), and the native route seed now explicitly sits at offset `4,926`, the same as `reward_preview_rng_after_0x4aa354_uint32`; the recovered H3MapEd route entry remains offset `5,002`, so the remaining pre-route gap is `76` upstream RNG calls. Live route adoption remains gated off (`route_container_0x4a8260_active_adoption=false`, `route_container_0x4a8260_rng_boundary_exact=false`) until that boundary matches. Medium seed-10 remains non-parity: final tile payload mismatch is still `11,726` bytes with first drift `terrain_generated_cell_word_0x24_before_0x49b2b6`, and pre-`0x4a4c8e` private-state mismatches are still `word_0x20=5,184`, `word_0x24=4,850`, `word_0x28=2,999`. The next implementation target remains the exact reward object/materialization and guard-attachment state before `0x4a8260` (`0x4a9f1c`, dynamic `+0xf5c/+0xf58/+0x10b4`, `0x49d471`, `0x49cf34`, `0x49d69d`, `0x4aa9b7`, `0x4aa3e9`) so the route RNG/stamp stream can be adopted from an exact source state. Do not patch this with density scalars, art remaps, brute-force retries, Windows rebuild churn, or final-map delta tuning. Inner-loop validation is Linux `.so` plus Python parity compares; Windows `.dll` builds are deferred until a Linux-verified final boundary.
