...
- Latest native RMG implementation update: `h3maped_rmg_core` now keeps the single source-order workflow from setup through route/free-cell and into source-backed mine/resource descriptor selection before later relation/reward phases. The generator object private-state builder runs setup, template/runtime seed construction, coordinate/owner/terrain support state, generator private-state vector initialization, pre-route type-98 materialization, source-order `0x4a8c15 -> 0x4a8260 -> 0x4a4c8e` route/free-cell ownership, and `0x4a9d6a` leading-descriptor mine/resource counts. It now materializes the recovered generator `+0x388/+0x38c` mine/resource descriptor bucket from source catalog order, executes the recovered `0x4a9911` selection scan using source field `+0x20`, relation terrain policy `+0x0c`, terrain bitset helper `0x42cc99`, fallback category scan, RNG modulo selection, `0x5044b1` allocation, `0x49ba89` initialization, and selected-object vtable `0x540ab0`, then fails closed at `0x4a9911_coordinate_builder_0x4a9641_unported_after_source_backed_descriptor_selection`. It does not execute source-order `0x4a9641` coordinate building, `0x4a9c7c` density followup, `0x4aadd2`, `0x4a5767` relation scan consumers, generic source-order object replay, `0x49a318` high-owner propagation, reward/guard `0x4aab7e -> 0x4aa9b7 -> 0x4aa3e9`, D-014 endpoint prep, `0x49eb8d` decorative dispatch, `0x4a79a3` connection tail, `0x4a7605 -> 0x4a5e03` fallback replay, roads/rivers, or final writeout until mine/resource coordinate/materialization is owned. The next implementation target is the source-order mine/resource coordinate builder `0x4a9641` and density followup `0x4a9c7c`, not reward/guard. This is not parity and not final map output.
