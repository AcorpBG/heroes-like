...
- The `0x4a6cf2` overlap/candidate-scan precheck is now materialized for the selected seed-1 links without inventing endpoint coordinates: five links inspected, three non-empty bbox overlaps, 502 total overlap cells, recovered ranges `0x4a6d52..0x4a6ddc` and `0x4a6de2..0x4a6f4a`, pending `generator+0x6a8` candidate shape-vector source, `0x49aa93` validation helper, and zero endpoint-coordinate materializations until the helper success path is ported.
- Direct executable inspection now records the next `0x4a6cf2` vector/writeout layout: `generator+0x6a8/+0x6ac` is a 4-byte pointer vector selected through `0x4e7276`, best candidates are 12-byte coordinate records managed through `0x4ae52a/0x4ae1fd`, endpoint records allocate 0x1c bytes through `0x5044b1`, construct through `0x49ba89`, commit through generator vfunc slot `+0x04`, and append endpoint coordinates to runtime-zone `+0x404` through `0x40bb15`. Shape-list content and endpoint-coordinate materialization remain pending.
- Direct executable inspection now records why that connection path is still earlier-blocked: helpers `0x4a61bc`, `0x4a696b`, and `0x4a6cf2` compare both generated-cell owner channels in `cell+0x20`. The port had only materialized bits 16..23; it now exposes bits 24..31 as `owner_high_byte_grid_i8`, initialized to sentinel `-1` by the recovered `0x4a5767 -> 0x4a59e2` reset boundary, and marks `0x49a318` high-owner copy/normalization as the next required clean port before endpoint geometry can succeed.
