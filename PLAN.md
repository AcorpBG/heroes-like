...
- The selected h3maped source template is now resolved through the imported catalog by source provenance instead of translated-name guessing; seed `1` selects source `h3maped_template_018`, resolves to adapted template `translated_rmg_template_019_v1`, and reports its 6 active zones, 5 active links, 4 player-capable start zones, 2 treasure zones, 4 minimum player castles, human/player-capable owner bitmaps, and the recovered 15-step phase sequence.
- The `0x4ac62a..0x4ac6ec` player-slot assignment boundary is ported from the executable. The `0x4adfe1` wrapper writes selected player-color bytes into `generator+0xed8`; when none are supplied, the constructor-zeroed bitmap produces color order `0..7`. Seed `1` with one human and two computer players maps source owners `0,1,2` to actual colors `0,1,2`.
- The high-confidence runtime-zone seed portion of `0x4a218c` is ported into the inspection boundary: one runtime-zone seed per active source zone, owner/color carryover through `generator+0xee4`, retained link endpoints, source `+0x08` minimum-size scan, land divisor `5`, and link-seed scale argument `79` for seed `1`.
- Single-level coordinate seeding and bounding-box rescale are now ported into the boundary from `0x4a1f3b`, `0x4a17f5`, `0x4a1701`, `0x4a1ad8`, and `0x4a19ed`. The seed `1` inspection executes 18 placement steps: 6 initial runtime-zone insertions plus two refinement passes, using the executable angle tables at `0x58dc28/0x58dd28` and `0x4e7276` candidate selection after template RNG state.
- The `0x4a3a03` level-collection boundary is now ported for the small land scope. It reports the one-level loop, all 6 runtime zones matching level `0`, the executable land branch that skips synthetic `0xd4` fallback source-zone allocation, and the concrete next helper trio `0x4a2777`, `0x4a325d`, and `0x4a3710`.
- The first footprint helper, `0x4a2777`, now consumes the recovered real `0x4cca55` source-node cycles for the selected small template instead of stopping at standalone samples. Its endpoint clip dependency `0x4a2b33` is ported with integer truncation, max-bound-minus-one behavior, and early in-bounds return evidence. Its deterministic cell-line writer dependency `0x4a261a` is executable-backed C++: endpoint-inclusive Bresenham-style stepping, zone-word writes at `cell+0x20`, reserved-cell flagging at `cell+0x2b`, 48-byte cell stride, endpoint swapping, and the water-mode reserved-flag suppression branch. Its flagged randomized/interpolated line-writer dependency `0x4a2413` is ported with midpoint subdivision, `0x4e7276` jitter, `0x4cc5ad` distance use, clamp-before-write behavior, and zone-word writes through the same cell writer path using the caller's runtime-zone `+0x1c` size limit. The rectangular fallback, connector segment, and boundary-wrapping branches remain exposed as standalone evidence, and the real-cycle traversal now clips finalized `+0x1c/+0x20` polygon coordinates, appends `runtime_zone+0x3f4` vertices, executes caller-flagged `0x4a2413` connector painting, and reports 262 h3maped boundary cells for seed `1`. These are still executable-buffer cells, not project terrain cells.
- The `0x4a325d` span-fill helper now consumes the real `0x4a2777` boundary buffer for the selected small land template. It still reports the standalone contained sample fill, and the real pass initializes `0x00ff0000` unassigned zone-word cells, starts from copied `runtime_zone+0x10` x/y/level seeds, writes zone ids through `cell+0x20`, marks `cell+0x2b` reserved state, and queues above/below unassigned runs. The caller-flagged randomized span limit now follows `runtime_zone+0x1c` after bbox rescale, not the source template `+0x08` base size; seed `1` therefore uses runtime size `4` instead of source base size `11`, fills 762 unique cells across all 6 selected zones, blocks 0 initial spans/seeds, leaves 272 remaining unassigned cells, and reports 0 out-of-bounds spans. The out-of-bounds seed relocation branch at `0x4a32b2..0x4a338e` remains exposed but unused for the in-bounds seed-1 fixture. The filled zone-word buffer is adapted into a report-only project grid with 1024 assigned cells, 272 explicit unresolved cells, 6 zone buckets `[266, 227, 204, 58, 152, 117]`, owner ids, and terrain codes mapped through project terrain ids. Runtime map generation remains blocked until h3maped `0x4a3f27` terrain fill/repaint and later placement phases are ported.
- The `0x4a3710` finalizer is now exposed for the small one-level land path. The call site passes the runtime-zone vector count captured before the synthetic branch at `0x4a3a86..0x4a3a9a`; because one-level land skips the synthetic source-zone branch, the start index and final runtime-zone count are both `6`. Both adjacency insertion loops therefore skip with `appended_runtime_zone_count: 0` and `materialized_adjacency_count: 0`, while `0x49b61b` ordering reset and `0x4a3554` per-zone order/depth rebuild are scheduled for all 6 selected zones.
- The runtime polygon layout is now partially recovered from `h3maped.exe` without inventing project geometry. The boundary records `0x49b452` runtime-zone vector offsets, 0x24-byte polygon nodes constructed/finalized through `0x4cc788`, `0x4cc5db`, `0x4ccb64`, and `0x4ccdfc`, and now ports the `0x4a3a03` polygon seed-call schedule: `0x4cc788` initializes the `-200..400` enclosing polygon and `0x4a3a79` calls `0x4ccb64` once per same-level runtime zone using `runtime_zone+0x10` x/y plus the runtime-zone pointer. Seed `1` materializes 6 primary split seed calls on level `0`, executes the `0x4cca55/0x4ccb64` locator/insertion/bridge loop, ports the `0x4cc9cc` edge-removal branch plus `0x4ccc7a/0x4cc68e` crossing cleanup, and reports 24 allocated node pairs / 23 active node pairs after cleanup. It also ports `0x4ccd69/0x4ccdfc` finalized-coordinate fanout, producing 14 finalized triplets and 42 finalized nodes. Project-grid terrain remains report-only until `0x4a3f27` fills/repaints the remaining h3maped cells.
- The selected-template polygon model exposes and now feeds the `0x4cca55 -> 0x4a2777` source-node cycle inputs that the helper caller uses at `0x4a3e58..0x4a3e8c`: 6 same-level located node walks, node `+0x10/+0x14` links, payload fields, finalized `+0x1c/+0x20` coordinates, and zero walk guard exhaustion. The helper report consumes all six walks and the second helper consumes the resulting boundary buffer for partial real `0x4a325d` span-fill evidence.
- The `0x4cca55/0x4ccb64` polygon splitter contract is now recovered as explicit boundary evidence, and the seed-specific split model executes the locator, duplicate check, edge test, primary split insertion, `0x4cc643` relink, `0x4ccb1f` bridge loop, `0x4cc9cc` vector erase branch, `0x4ccc7a` crossing tests, `0x4cc68e` collapses, and `0x4ccdfc` finalized-node fanout. For seed `1`, the model records 6 split calls, 13 bridge pairs, 1 edge-removal branch, 24 crossing tests, 6 crossing collapses, 14 finalized triplets, 42 finalized nodes, and 28 active payload-gated nodes. The next unclaimed executable step is h3maped `0x4a3f27` terrain fill/repaint, then the later post-footprint placement phases.
- The `0x4a3f27` terrain fill/repaint schedule is now exposed as report-only evidence from the executable: `0x4a2105` owned-cell bbox prepass, six `0x4a2ffa` runtime-zone recenter calls, one-level land skip for `0x4a30c2`, TerrainPlacement adapter vtable `0x540a14`, whole-map terrain id `8` (`water`) fill through `0x4bd099`, and per-cell repaint gating on owner byte plus `cell+0x28` bit 28. The owner-byte basis is resolved by adapting the translated catalog's one-based `source_zone_id` back to the executable/runtime zero-based zone word id that `0x4a325d` writes through `runtime_zone->source_zone[0]`; the report now records 1024 source-zone repaint-member cells, 1024 runtime-index repaint-member cells, 0 owner-basis mismatches, and 0 unmatched repaint cells. Runtime terrain adoption remains blocked until TerrainPlacement terrain/art/index/flip adoption is ported.
- Normal small land `native_catalog_auto` generation now routes to `h3maped_small_port_generation_not_ready` rather than the archived generator, so no fake fallback map is produced while real span fill and the post-footprint h3maped phase sequence remain incomplete.
- `tests/native_rmg_small_h3maped_port_boundary_report.tscn` passed with `accepted_template_count: 13`, `selected_template: h3maped_template_018`, `adapted_template_id: translated_rmg_template_019_v1`, `selected_zone_count: 6`, `selected_link_count: 5`, `coordinate_seed_status: 0x4a1f3b_0x4a17f5_0x4a1ad8_0x4a19ed_single_level_ported`, `coordinate_step_count: 18`, `level_footprint_phase_status: 0x4a3a03_level_collection_ported_helpers_blocked`, `first_helper_status: 0x4a2777_real_source_node_cycle_traversal_ported_boundary_buffer_available`, `real_source_cycle_traversal_status: 0x4a2777_real_source_node_cycle_traversal_ported_boundary_materialized`, `real_source_cycle_unique_cell_count: 262`, `real_source_cycle_random_span_limit_source: runtime_zone+0x1c`, `real_source_cycle_random_span_limit_runtime_size: 4`, `clip_helper_status: 0x4a2b33_clip_helper_ported`, `line_writer_status: 0x4a261a_cell_line_writer_ported`, `randomized_line_writer_status: 0x4a2413_randomized_line_writer_ported_standalone`, `rectangle_fallback_status: 0x4a2777_rectangle_fallback_branch_ported_standalone`, `connector_segment_status: 0x4a2777_connector_segment_branch_ported_standalone`, `boundary_wrapping_status: 0x4a2777_boundary_wrapping_continuation_ported_standalone`, `second_helper_status: 0x4a325d_real_boundary_span_fill_and_seed_relocation_ported_project_materialization_pending`, `real_boundary_span_fill_status: 0x4a325d_real_0x4a2777_boundary_span_fill_executed`, `real_boundary_filled_cell_count: 762`, `real_boundary_filled_zone_count: 6`, `real_boundary_seed_blocked_count: 0`, `real_boundary_seed_relocated_count: 0`, `real_boundary_blocked_initial_span_count: 0`, `real_boundary_remaining_unassigned_cell_count: 272`, `project_materialization_status: h3maped_span_fill_project_grid_materialized_terrain_repaint_pending`, `project_materialized_cell_count: 1024`, `project_unresolved_cell_count: 272`, `project_materialized_zone_counts: [266, 227, 204, 58, 152, 117]`, `terrain_phase_status: 0x4a3f27_schedule_ported_owner_byte_basis_resolved_terrain_adoption_pending`, `terrain_owner_byte_basis_mismatch_count: 0`, `terrain_source_zone_repaint_member_cell_count: 1024`, `terrain_runtime_index_repaint_member_cell_count: 1024`, `terrain_runtime_index_unmatched_repaint_member_cell_count: 0`, `finalizer_status: 0x4a3710_small_land_no_appended_zone_finalizer_ported`, `finalizer_appended_runtime_zone_count: 0`, `finalizer_order_reset_call_count: 6`, `runtime_layout_status: runtime_polygon_seed_points_split_cleanup_and_finalizer_ported_span_fill_pending`, `polygon_seed_split_count: 6`, `polygon_seed_status: 0x4a3a03_polygon_tree_seed_calls_split_cleanup_and_finalizer_ported_span_fill_pending`, `finalized_triplet_count: 14`, `finalized_node_count: 42`, `source_node_walk_status: 0x4cca55_to_0x4a2777_source_node_cycles_recovered`, `phase_count: 15`, `generation_status: h3maped_small_port_generation_not_ready`, `out_of_scope_generation_status: archived_legacy_native_rmg_disabled`, and `unsupported_scope_status: unsupported_scope`.
