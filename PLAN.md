...
- Latest native RMG implementation update: `h3maped_rmg_core` now keeps the single source-order workflow from setup through route/free-cell and stops at the recovered mine/resource phase before later relation/reward phases. The generator object private-state builder runs setup, template/runtime seed construction, coordinate/owner/terrain support state, generator private-state vector initialization, pre-route type-98 materialization, and source-order `0x4a8c15 -> 0x4a8260 -> 0x4a4c8e` route/free-cell ownership, then invokes `0x4a9d6a` from relation leading-descriptor mine/resource counts and fails closed at `0x4a9d6a_selected_object_callback_0x4a9911_unported_before_mine_resource_commit`. It does not execute source-order `0x4aadd2`, `0x4a5767` relation scan consumers, generic source-order object replay, `0x49a318` high-owner propagation, reward/guard `0x4aab7e -> 0x4aa9b7 -> 0x4aa3e9`, D-014 endpoint prep, `0x49eb8d` decorative dispatch, `0x4a79a3` connection tail, `0x4a7605 -> 0x4a5e03` fallback replay, roads/rivers, or final writeout until `0x4a9d6a -> 0x4a9911 / 0x4a9c7c / 0x4a9641` is owned. The next implementation target is the source-order mine/resource materialization callbacks, not reward/guard. This is not parity and not final map output.
