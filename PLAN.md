...
- The active compact port includes top-level `0x4a3a03` footprint scheduling, the small-land `0x4a3710` finalizer boundary, and the standalone `0x4a325d` span-fill primitive: seed `1` has one land level, six matching runtime zones, six polygon split calls, no synthetic fallback zone, no appended runtime zones, six ordering reset/rebuild calls scheduled, zero adjacency inserts, and a bounded disassembly-derived span-fill contract sample. This remains inspection-only and does not materialize zone cells.
...
- Pending before runtime generation: `0x4a2777` boundary traversal, wiring real h3maped boundary cells into the ported `0x4a325d` span-fill primitive, terrain/cell writeout, owned towns, roads, blockers, guards, mines, rewards, and final writeout.
