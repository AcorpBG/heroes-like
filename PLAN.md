...
- Latest native RMG implementation update: `h3maped_rmg_core` now carries the recovered `0x49ecf2 -> 0x49d914` backing-state constructor boundary through the native no-Godot workflow and generator-object private state. The constructor records width, height, level count, allocation size `width * height + 0x4fcf4`, generator vtable `0x540cb0`, generator `+0x04 = 0x3a`, setup `+0x4c -> generator +0x08`, and source-handler arg9 -> generator `+0xed4`. Same-run final payload authority compare now fails closed before byte comparison when source-handler arg9 from `0x4adfe1` caller stack `[EBP+0x0c]` is unknown; if arg9 is nonzero, the recovered virtual-init branch must be ported before any final payload parity claim. Focused no-Godot Medium seed-10 setup-0 with trace-backed setup fields still writes final payload sections and a final payload binary, but same-run compare is not invoked; the exact blocker is `native_0x49d914_backing_state_constructor_0x49d914_source_handler_arg9_from_0x4adfe1_caller_stack_ebp_0x0c_missing`. Runtime/native map output remains disabled. This is source-backed setup/private-state porting progress, not final-payload parity or runtime generation authorization.
