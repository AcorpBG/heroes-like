...
- Latest native RMG implementation update: `h3maped_rmg_core` now keeps the single source-order workflow from setup through route/free-cell, source-backed mine/resource materialization, and recovered `0x4a9c7c` density followup, then continues through `0x4aadd2` terrain-pressure zeroing, `0x4a5767` relation scan consumers, descriptor-joined source-order object replay, and `0x49a318` high-owner propagation. The no-Godot workflow now blocks at `0x4aab7e_live_source_pointer_argument_ebp8_not_owned_before_reward_guard_materialization`: the earlier all-relation-owner source-stream sweep was removed because recovered `0x4aab7e` takes one caller-owned `[EBP+0x8]` source pointer, reads `[EDI]+0xa0/+0xa4/+0xa8`, and passes that same pointer through `0x4aa354/0x4aa9b7`. The next implementation target is owning that source-order pointer feed and its triplet source records before reward/guard commits, not mine/resource density and not final-map delta tuning. This is not parity and not final map output.
- Previous native RMG implementation update: `h3maped_rmg_core` carries recovered relation-owner reward/guard treasure bands `+0xa0/+0xa4/+0xa8` through `+0xc0`, terrain policy `+0x0c`, stale supported-land cursor `+0xf5c = 0x7a1befdf`, zeroed `+0xf60/+0xf64` terrain-pressure state, recovered `0x57cea0` monster terrain rows, and `0x531cc4` type-17 subtype-to-monster rows. The earlier all-owner source-stream sweep built on top of those fields has been removed as non-source-backed; the active blocker is the caller-owned `0x4aab7e` source pointer described above.
