...
- Latest native RMG implementation update: `h3maped_rmg_core` now follows the recovered `0x4ac552` order through source-record replay, route/free-cell `0x4a8260/0x4a4c8e`, the recovered post-`0x4a4c8e` `0x4a8c15 -> 0x49a962` cleanup scan, the source-backed `0x4a4913` relation-vector loop, the source-order `0x4a8c15 -> 0x4a5767` bridge relation normalization/scan/high-owner propagation, the one-level-land `0x4a4fc5` water-edge scan including source-water candidate handling, and the live `0x4a79a3` connection-tail prefix. This is still not parity and not final map output. No-Godot Small seed 10/58 setup3 and Medium 4-player seed 10 setup3 enter `0x4a79a3`, run the recovered source-frontier scan and relation-owner `0x49b3fb` endpoint pair lookup from source-owned `+0xc8/+0xcc` records, and invoke the source-backed `0x4a61bc` entry/candidate path for each reciprocal pair. Small seed 10 and Medium seed 10 have reciprocal pairs whose `0x4a61bc` entry gates pass but no matching source-frontier candidate, so they stop at `0x4a61bc_live_prefix_no_source_frontier_candidate_after_0x49b3fb`. Small seed 58 now executes the recovered selected-candidate loop: `7` selected loops, `7` RNG selections, `14` `0x4a5a23` projection-chain calls, `16` occupied stamps, `40` cleanup clears, and `14` owner local-vector `+0x404` appends before failing closed at `0x4a61bc_live_0x4a5c07_static_selector_unported_before_0x4a5e03`. Fallback materialization no longer runs before the live pair loop. The required next implementation slice is the direct `0x4a5c07` static selector and `0x4a5e03/0x4a54a7` object construction/commit behavior after the recovered `0x4a61bc` selected loop. Downstream `0x4a9d6a`, reward/guard, connection/decorative/road-river, and final writeout remain blocked.
- Previous native RMG implementation update: `h3maped_rmg_core` carries recovered relation-owner reward/guard treasure bands `+0xa0/+0xa4/+0xa8` through `+0xc0`, terrain policy `+0x0c`, stale supported-land cursor `+0xf5c = 0x7a1befdf`, zeroed `+0xf60/+0xf64` terrain-pressure state, recovered `0x57cea0` monster terrain rows, and `0x531cc4` type-17 subtype-to-monster rows. Those fields remain source-backed helper inputs, but active workflow progress is blocked earlier at the recovered `0x4ac552` relation-pointer source/context bridge.
...
- Current source-order object materialization status: weighted `0x4a901a`, direct exact-input `0x4a8d2c -> 0x4a93a2 -> 0x4a54a7`, source-backed `0x4a8db2` direct-prepass/weighted-lane scheduler replay, explicit-input reward/guard `0x4aa9b7 -> 0x4aa3e9` scan/commit with recovered `0x4aa603` feasibility filtering, explicit reward/guard wrapper attach/finalizer helpers `0x49ce04/0x49ce64/0x49cf34/0x49cefb`, recovered `0x49d2e0` candidate filtering, and the recovered empty-vector `0x49d7c3` wrapper contour rebuild exist in shared C++ for source-backed inputs. They are helper surfaces only until their recovered caller inputs are owned. The active generator-object private-state chain now fails closed before route/free-cell if the recovered `0x4ac552` relation-pointer vector cannot provide the exact `[0x00]` source record and `[0x04]` context pointer required by `0x4a8d2c` and `0x4a8db2`. Reconstructed type98 scheduler/direct replay and descriptor-joined non-type98 `+0xedc` replay are no longer counted as completed workflow progress when that relation-pointer bridge is missing. This is not full live object parity: native still needs the real `0x4ac552` relation-pointer source/context bridge, live descriptor producers/raw source-field capture for all generic non-type98 source pairs, live reward/guard wrapper/source-relation caller feed, fallback materialization caller feed, downstream caller order, and final payload writer.
