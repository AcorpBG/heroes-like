...
- Current native-vs-H3MapEd finding: Medium seed-10 input/template identity is corrected for the controlled 4-player land reference. Native manifest-derived export preserves `human_count=4`, `player_count=4`, `computer_count=0` and selects H3MapEd template 015 / `2SM4d(2)` from a 23-entry accepted vector, matching the H3MapEd selector trace (`71 % 23 = 2`). Native now ports the recovered `0x4a1ad8` span metric and `0x4a1967` visible-runtime-link filter, so the controlled Medium seed-10 coordinate replay matches the H3MapEd `0x4a325d` trace exactly: selected pre-rescale span 154, runtime size 7, and zone coordinates `(29,42)`, `(26,31)`, `(35,54)`, `(44,64)`, `(25,7)`, `(44,44)`, `(17,18)`, `(39,34)`, `(53,34)`, `(44,21)`. Native also consumes the recovered `0x49b4e1` monster-town RNG step after `0x49b53d`, so selected runtime terrain ids now match H3MapEd exactly: `[5,0,3,2,2,3,0,0,7,2]`. The current Linux/Python compare artifact `.artifacts/rmg_native_medium_seed10_current_linux_only_20260611` still reports 14,892 final tile byte mismatches and first drift remains `terrain_generated_cell_word_0x24_before_0x49b2b6`; pre-`0x4a4c8e` private-state mismatches remain `owner_byte2=1,558`, `word_0x20=5,184`, `word_0x24=5,176`, and `word_0x28=5,079`. The named blocker is now the exact `0x4cca55 -> 0x4a325d` boundary/span descriptor handoff inside `0x4a3a03`, then terrain cell writeout private-state parity around `0x4a3f27`; `0x58db78` monster-town table values remain unmaterialized until later object RNG/materialization requires them. Do not patch this with density scalars, art remaps, brute-force retries, Windows rebuild churn, or final-map delta tuning. Inner-loop validation is Linux `.so` plus Python parity compares; Windows `.dll` builds are deferred until a Linux-verified final boundary.
