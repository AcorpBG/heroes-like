...
- The active compact port includes top-level `0x4a3a03` footprint scheduling, standalone `0x4a2b33` clip and `0x4a261a` line-writer primitives, the standalone `0x4a325d` span-fill primitive, and the small-land `0x4a3710` finalizer boundary: seed `1` has one land level, six matching runtime zones, six polygon split calls, no synthetic fallback zone, no appended runtime zones, six ordering reset/rebuild calls scheduled, zero adjacency inserts, bounded clip/line samples, and a bounded disassembly-derived span-fill contract sample. This remains inspection-only and does not materialize zone cells.
