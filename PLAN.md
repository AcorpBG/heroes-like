...
- Current native-vs-H3MapEd finding: Medium seed-10 input/template identity, coordinate replay, selected runtime terrain ids, recovered `0x4a2777` dual endpoint clipping, map-border continuation, per-edge owner writer skip, `0x4bbfcc` final-sweep scratch mask/class correction, and the source-backed first `0x4a8260` object-vector scan are ported for the controlled 4-player land reference. The current Linux/Python artifact `.artifacts/rmg_native_medium_seed10_4a8260_blocker_20260611` has pre-`0x4a4c8e` `owner_byte2=475`, `word_0x20=5,184`, `word_0x24=4,768`, and `word_0x28=3,049` mismatches; this improves `word_0x28` from `4,065` after correcting the checkpoint boundary and from `3,909` in the prior post-land-edge artifact. Native checkpoint bit lanes are now bit26=`4,590` and bit27=`594` versus H3MapEd bit26=`2,478` and bit27=`1,311`, which identifies the next blocker as the unreplayed same-run `0x4a8260` route-list coordinate vector, route RNG entry state, `0x49a85d` stamp stream, and dependent final `0x49a962` sweep. A sweep-only probe was rejected because it moved `word_0x28` only `3,049 -> 3,044` while collapsing native bit27 to `15`, proving the final sweep must not be adopted before the route stamps. Full final tile parity is still incomplete: Medium seed-10 final tile byte mismatches are `11,726` and first drift remains `terrain_generated_cell_word_0x24_before_0x49b2b6`. Do not patch this with density scalars, art remaps, brute-force retries, Windows rebuild churn, or final-map delta tuning. Inner-loop validation is Linux `.so` plus Python parity compares; Windows `.dll` builds are deferred until a Linux-verified final boundary.
