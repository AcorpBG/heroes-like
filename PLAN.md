...
- `tools/rmg_fast_audit.py` now separates native guard body tiles from guard control-zone tiles when computing object-only town-route topology, and `tools/rmg_fast_validation.py --closure-shape-gate` exposes guard-mediated closure gaps without starting Godot. Default full fast validation still passes, but the stricter closure-shape diagnostic reports `13` cases where owner evidence has town pairs reachable with terrain/objects only and closed after guards, while native packages are already closed before guards matter. The next production target is guarded crossing shape, not more parser/report Godot work.
