...
- The runtime polygon layout is now partially recovered from `h3maped.exe` without inventing project geometry. The boundary records `0x49b452` runtime-zone vector offsets, 0x24-byte polygon nodes constructed/finalized through `0x4cc788`, `0x4cc5db`, `0x4ccb64`, and `0x4ccdfc`, and now ports the `0x4a3a03` polygon seed-call schedule: `0x4cc788` initializes the `-200..400` enclosing polygon and `0x4a3a79` calls `0x4ccb64` once per same-level runtime zone using `runtime_zone+0x10` x/y plus the runtime-zone pointer. Seed `1` materializes 6 primary split seed calls on level `0`; the exact `0x4cca55/0x4ccb64` pointer topology remains the next blocker before terrain-cell materialization.
- The `0x4cca55/0x4ccb64` polygon splitter contract is now recovered as explicit boundary evidence without fake execution. The report records locator orientation branches, node-pair ownership fields, vector membership, helper roles for `0x4cc643`, `0x4cc670`, `0x4cc68e`, `0x4cc6f2`, `0x4ccb1f`, `0x4ccc7a`, `0x4ccd69`, and `0x4ccdfc`, and keeps `materialized_split_node_count: 0` until those pointer mutations are executed in a project-owned model. The first allocation model now materializes the four initial `0x4cc788` enclosing polygon node pairs plus the `0x4cc904 -> 0x4ccb1f` constructor bridge pair as 10 vector nodes rooted at `initial_pair_0_primary`; the four inline splice swaps from `0x4cc82b..0x4cc901` and the constructor bridge relinks are represented, while the runtime `0x4ccb64` splitter loop remains pending.
