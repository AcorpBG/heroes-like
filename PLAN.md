...
- Current native-vs-H3MapEd finding: Medium seed-10 input/template identity is now corrected for the controlled 4-player land reference. Native manifest-derived export preserves `human_count=4`, `player_count=4`, `computer_count=0` and selects H3MapEd template 015 / `2SM4d(2)` from a 23-entry accepted vector, matching the H3MapEd selector trace (`71 % 23 = 2`). Native now uses the recovered `0x49acf6`/`0x49b2b6` final flag placement (`cell+0x28` bits 15..21 -> final tile byte 6) instead of the previous local bit-12/13 projection, but the Linux Medium seed-10 compare still reports 14,201 final tile byte mismatches and first drift remains `terrain_generated_cell_word_0x24_before_0x49b2b6`. The named blocker is source-backed TerrainPlacement replay: recovered `0x4a3f27` iterates the generator relation vector at `+0x10e4/+0x10e8`, while native still reconstructs live terrain placement from runtime zones/span fill. Do not patch this with density scalars or art remaps.
