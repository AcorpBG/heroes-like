...
- Latest native RMG implementation update: `h3maped_rmg_core` now follows the recovered `0x4adfe1 -> 0x49ecf2` setup-value preparation for object `+0x48`: the caller supplies `clamp(raw + 3, 1, 5)` to `0x49ecf2`, and controlled cases carry the raw setup field separately from the prepared callee value. The reward/guard final-object path also ports the recovered `0x540c20 -> 0x49c8f3 -> 0x540b3c` constructor fields and `0x49c273` serializer: candidate `+0x14` payload is preserved, `0x4ad640` allocates a slot from generator byte array `+0xf88`, exact record fields `+0x20/+0x24/+0x28` are carried through `0x4aa166 -> 0x4a54a7`, and `0x4ad3eb` serializes `0x49c273` records instead of blocking after the base prefix. The final payload authority guard no longer accepts the old three-field setup proxy; it requires the exact 11-word `0x4adfe1 -> 0x49ecf2` call stack recovered from the same authority run before comparing payload bytes. `bin/h3maped_rmg_core_selftest` is green. Focused no-Godot Medium seed-10 setup-0 now writes final payload sections, serializes all 900 generated objects with no serializer block, writes the `0x4ad3db` final zero sentinel, and emits a 61,424-byte ordered payload. The run still blocks at `final_payload_compare` with `same_run_payload_authority_profile_loaded_but_0x49ecf2_setup_stack_words_missing` because the current stitched final-payload authority ledger does not contain the setup-call event; independent compare remains blocked before byte mismatch reporting. Runtime/native map output remains disabled. This is source-backed constructor/serializer and setup-authority guard implementation progress, not final-payload parity or runtime generation authorization.
