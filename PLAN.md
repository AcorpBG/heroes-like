...
- The high-confidence runtime-zone seed portion of `0x4a218c` is ported into the inspection boundary: one runtime-zone seed per active source zone, owner/color carryover through `generator+0xee4`, retained link endpoints, source `+0x08` minimum-size scan, land divisor `5`, and link-seed scale argument `79` for seed `1`. Coordinate seeding and bounding-box rescale through `0x4a1f3b/0x4a19ed` remain blocked instead of faked.
- Normal small land `native_catalog_auto` generation now routes to `h3maped_small_port_generation_not_ready` rather than the archived generator, so no fake fallback map is produced while the remaining post-runtime-zone-seed h3maped phase sequence is still missing.
- `tests/native_rmg_small_h3maped_port_boundary_report.tscn` passed with `accepted_template_count: 13`, `selected_template: h3maped_template_018`, `adapted_template_id: translated_rmg_template_019_v1`, `selected_zone_count: 6`, `selected_link_count: 5`, `phase_count: 15`, `generation_status: h3maped_small_port_generation_not_ready`, and `unsupported_scope_status: unsupported_scope`.
