...
- Latest native RMG implementation update: `h3maped_rmg_core` now carries recovered setup object `+0x4c` / generator field `+0x08` into `GeneratorObjectPrivateState`, and recovered `0x49e700` decorative type gating now reads that field instead of map `level_count`. Missing generator `+0x08` now fails closed as `0x49e700_generator_plus_0x08_missing_for_type_gate`; setup-zero Medium seed-10 no longer admits decorative source types `>= 0xa5`. Focused no-Godot Medium seed-10 setup-0 moves from 862 to 902 generated objects, base-object vtable records from 682 to 722, and generated-object payload from 12,138 to 12,618 bytes. It still fails closed at `native_final_tile_stream_mismatch_against_same_run_0x49b2b6_payload`: tile first mismatch offset 1 (`native=55`, `h3maped=54`), generated-object payload 12,618 native bytes vs 17,057 H3MapEd bytes with first mismatch offset 0 (`native=30`, `h3maped=2`). This is source-backed decorative/source-order behavior progress, not final-payload parity or runtime map-output authorization.
