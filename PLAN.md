...
- Latest native RMG implementation update: `rmg_native_core.cpp` now exposes a single native H3MapEd workflow result and `rmg_native_batch_export_cli` routes case status through that workflow instead of independently treating helper/count/snapshot surfaces as progress. The workflow executes the currently ported ordered phases once and records the current blocker as `relation_object_materialization`. This is not parity and not final map output: relation/object caller order, reward/guard materialization, connection/road/river phases, and final tile/object/header writeout remain unowned.
