...
- The clean module now ports runtime-zone record construction `0x4a218c` for inspection only: it rebuilds the runtime-zone vector report, derives scale inputs, initializes records with `0x49b452` semantics, applies owner-color mapping through `generator+0xee4`, adapts `0x49b3c1` town/faction choice and `0x49b53d` terrain choice to project ids, and resolves 5 endpoint-only link seeds for the selected seed-1 template.
...
- The clean module now replays one-level coordinate-candidate math from `0x4a1f3b/0x4a17f5/0x4a1701/0x4a1ad8/0x4a19ed` for inspection only: seed 1 reports 18 candidate-selection steps, 18 coordinate RNG calls after the current runtime-zone report, and bbox span `82` onto the 36-tile small map. The report explicitly keeps `rng_order_status: pending_full_0x4a218c_interleaved_replay`, so this is not yet authoritative generation.
- The boundary exposes an executable phase ledger and keeps map-materialization phases pending: exact interleaved `0x4a218c` replay, zone footprint placement `0x4a3a03/0x4a2777/0x4a325d/0x4a3710`, terrain placement `0x4a3f27/0x4bcff5/0x4bd099`, object placement, guard/reward/monster placement, and final cell/object passes.
- `tests/native_rmg_small_h3maped_port_boundary_report.tscn` passed with `status: h3maped_small_clean_restart_template_selection_ready`, `archive_status: previous_native_catalog_auto_generator_archived_debug_only`, `accepted_template_count: 13`, `selected_template: h3maped_template_018`, `assignment_status: 0x4ac62a_player_slot_assignment_ported_inspection_only`, `runtime_zone_build_status: 0x4a218c_runtime_zone_records_ported_inspection_only`, `early_link_placement_status: 0x4a1f3b_endpoint_control_flow_ported_inspection_only`, `coordinate_placement_status: 0x4a1f3b_coordinate_candidate_math_replayed_inspection_only`, `coordinate_rng_order_status: pending_full_0x4a218c_interleaved_replay`, `generation_status: h3maped_small_clean_restart_generation_not_ready`, and `out_of_scope_generation_status: archived_legacy_native_rmg_disabled`.
