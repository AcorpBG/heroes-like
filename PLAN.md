...
- Latest native RMG implementation update: `h3maped_rmg_core` now enters the source-backed `0x49eb8d -> 0x49e700` decorative prefix instead of stopping at a generic valid-bit26 guard. It counts bit26 cells, computes the recovered `0x4374c / bit26` budget, scans valid bit26 cells in source order, uses the recovered 45-entry `0x54092c` decorative type table, filters rand_trn-backed source records, and reaches the descriptor footprint/scorer boundary. No-Godot controlled snapshots for Small seed 10, Small seed 58, and Medium seed 10 now all fail closed at `0x49e700_0x49e1bf_record_0x10_descriptor_0x30_0x40_scoring_replay_unowned`; the first scorer candidate is source row `312`, type `119`, DEF `AVLd1sn0.def`. This is source-order workflow progress, not parity completion; final payload generation remains disabled until exact `0x49e1bf` scoring data/replay, road/river adjacency, descriptor counter replay, and final writeout are source-owned.
