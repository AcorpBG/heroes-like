...
validationResults:
- `/root/Downloads/h3maped.exe` was re-verified as the PE32 MapEd binary and the recovered `random-map-generation-h3maped-full-spec.md` was audited against our translated template catalog. The concrete translation drift found was the known `rmg.txt` monster-mask `Forge` header quirk: the importer now maps that source header to the elemental/conflux-compatible original faction slot and records the compatibility note in generated catalog metadata. `tests/random_map_template_catalog_grammar_report.tscn`, `tests/validate_repo.py`, `git diff --check`, and `jq empty content/random_map_template_catalog.json content/random_map_template_import_summary.json` passed.
...
- Medium two-level islands now caps generated reward and decoration totals for this compact islands profile, which also brings the reward-ratio guard floor into line. Focused `m_4players_2levels_islands` now matches broad owner counts exactly: decoration `340/340`, reward `132/132`, guard `69/69`, object `190/190`, town `6/6`, and total objects `737/737`; route and road deltas remain `0`, and merged 18-case quick validation plus `rmg_python_validation_gate.py` passed while preserving `production_ready=false`.
- Medium one-level normal-water now has scoped generated catalog-auto object/scenic/decoration/reward/guard/town/road profile floors plus a global spaced supplemental-town fallback for profiles where owner evidence has more towns than high-level zones. Focused `m_normalw_4players` now matches broad owner counts exactly: decoration `335/335`, guard `80/80`, object `160/160`, reward `172/172`, town `7/7`, total objects `754/754`, road cells `221/221`, and object-route openness `21/21`; merged 18-case quick validation plus `rmg_python_validation_gate.py` passed while preserving `production_ready=false`.
- XL two-level islands now uses the surface global spaced supplemental-town fallback, a generated route-closure loop that treats open-route allowance as a final threshold instead of a per-pass skip budget, and a profile-specific compact decorative route-mask target. Focused `xl_islands_2levels` now exports with native validation `pass`, reaches the owner town count at `20/20`, and improves production-gap severity from `520` to `233`; merged 18-case quick validation passed with `18/18` matches and `0` parse/native/density/policy/topology/coverage/closure-shape gaps while preserving `production_ready=false`. The focused manifest also confirms `town_pair_route_guard_closure` is still the XL hotspot at about `29.610s`.
- Medium two-level normal-water now has a stricter owner-rule-aligned town-spacing floor, profile-scoped broad count caps/floors, object-route mask targeting, surface/underground object distribution, and a stronger underground rock shape. Focused `m_normalw_4players_2levels` severity improved from `429` to `96`, terrain delta improved from `-40` to `+15`, object-route delta improved from `+13` to `+1`, and broad category absolute delta improved from `10` to `5`; merged 18-case quick validation plus `rmg_python_validation_gate.py` passed while preserving `production_ready=false`.
- Medium two-level islands surface/underground terrain shape now has scoped land/rock fractions for the compact islands profile. Focused `m_4players_2levels_islands` terrain-blocked delta improved from `+391` to `+3` while preserving exact decoration/guard/object/reward/town counts, exact road total, and `0/0` object/guarded route deltas; merged 18-case quick validation plus `rmg_python_validation_gate.py` passed while preserving `production_ready=false`.
- Large two-level islands terrain shape now has scoped land/rock fractions for that profile. Focused `l_islands_randomplayers_2level` production-gap severity improved from `203` to `145`, terrain-blocked delta improved from `+196` to `+140`, object and guarded route deltas stayed `0/0`, and focused validation passed; merged 18-case quick validation plus `rmg_python_validation_gate.py` passed while preserving `production_ready=false`.
- XL two-level normal-water package route shape now uses a package-only guarded-route allowance, compact vertical guard masks, and a profile object-route mask target. Focused `xl_water_2levels` matched owner object-route openness at `16/16`, improved guarded-route openness from `0/10` to `2/10`, and improved production-gap severity from `417` to `356`; merged 18-case quick validation plus `rmg_python_validation_gate.py` passed while preserving `production_ready=false`.
- XL two-level land now has a scoped category/level-distribution profile: owner-scale scenic and decoration floors, a reward cap, a lower underground object share, and a lower underground rock fraction to compensate for the new object openings. Focused `xl_nowater_2levels` severity improved from `367` to `47`, total object delta from `-86` to `-3`, terrain-blocked delta from `-151` to `-7`, and category absolute delta from `96` to `3`; merged 18-case quick validation plus `rmg_python_validation_gate.py` passed while preserving `production_ready=false`.
- XL two-level normal-water now uses a recovered profile-aware required-town spacing floor, scoped surface water land fractions, a lower underground rock fraction, and a bounded one-cluster guard closure pass after corridor cuts. Focused `xl_water_2levels` now reaches the owner town count and split at `16/16` with `11/5` surface/underground towns, keeps object-route openness at `16/16`, improves terrain-blocked delta from `+114` to `-46`, and improves production-gap severity from `356` to `263`; guarded-route openness is still under owner at `3/10`. Merged 18-case quick validation plus `rmg_python_validation_gate.py` passed with `0` parse/native/density/policy/topology/coverage/closure-shape gaps while preserving `production_ready=false`; focused timing still shows `town_pair_route_guard_closure` as the XL hotspot at about `26.36s`.
- Large two-level normal-water now has a scoped category/level-distribution and underground-rock profile: reward floor/cap, decoration cap, owner-like underground decoration/object/reward shares, and a higher underground rock fraction. Focused `l_normalwater_randomplayers_2level` production-gap severity improved from `342` to `55`, category absolute delta from `144` to `13`, terrain-blocked delta from `-187` to `+5`, and reward count to exact `722/722`; route openness moved from `0/0` to `-1/0`, leaving a small object-route shape debt. Merged 18-case quick validation plus `rmg_python_validation_gate.py` passed with `0` parse/native/density/policy/topology/coverage/closure-shape gaps while preserving `production_ready=false`.
