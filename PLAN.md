...
68. Only after these basics are deep enough and art-direction evidence starts to exist, return focus to implementation planning, campaign/skirmish maps, town-screen polish, battle-screen polish, battle AI systems, and the overall player loop: turn flow, economy, resource collection, mine capturing, battles, and town development.

Recommended next slice:
- Run `strategic-ai-hero-task-state-report-fixture-planning-10184` as the next concrete slice.
- Use `docs/strategic-ai-hero-task-state-boundary-plan.md` as the current evidence boundary: future AI hero task state should be a faction-owned task board with deterministic task ids, actor/task ownership, lifecycle, target reservation, invalidation, save/schema, and rollback rules, first proven through derived/report-only candidate tasks.
- Keep the next slice planning-only: define the exact `AI_HERO_TASK_STATE_BOUNDARY_REPORT` payload, River Pass and Glassroad fixture cases, candidate task id checks, role-to-task source checks, target reservation checks, old-save absence checks, public leak checks, validation commands, and failure conditions without implementing report helpers/tests or adopting schema/live behavior.
...
- Treat commander-role report output, commander-role adoption sequencing, live-turn transcript/report planning, live-turn transcript/report implementation, live-turn transcript/report gate review, and hero task-state boundary planning as passed gates. The next AI slice should plan report-only task-state fixtures while keeping schema writes, save migration, durable event logs, full AI hero task-state implementation, defense-specific durable state, coefficient tuning, and live commander-role behavior deferred.
...
- The next active choice is planning-only strategic AI hero task-state report/fixture planning after the completed task-state boundary plan, still without production JSON migration, new resource registry, `wood` to `timber` migration, rare-resource activation, market-cap overhaul, pathing/body-tile/approach adoption, full AI hero/task implementation, broad AI rewrite, behavior tuning, editor behavior, renderer behavior, save format changes, generated PNG import, asset import, live commander-role behavior adoption, schema writes, save migration, durable event logs, or new defense-specific durable state.
...
Current target: planning-only strategic AI hero task-state report/fixture planning after the completed task-state boundary plan.
