...
- Latest native RMG implementation update: `h3maped_rmg_core` now follows the recovered `0x4ac552` order through source-record replay, route/free-cell `0x4a8260/0x4a4c8e`, the recovered post-`0x4a4c8e` `0x4a8c15 -> 0x49a962` cleanup scan, the source-backed `0x4a4913` relation-vector loop, the source-order `0x4a8c15 -> 0x4a5767` bridge relation normalization/scan/high-owner propagation, the one-level-land `0x4a4fc5` water-edge scan, and the zero-source-water `0x4a79a3` connection-tail invocation. This is still not parity and not final map output. No-Godot Small seed 58 setup3 now executes the zero-source-water `0x4a4fc5` path (`scan=1296`, `source_water=0`), invokes `0x4a79a3`, and fails closed at `0x4a79a3_fallback_materialization_0x4a7605_0x4a5e03_source_records_missing_for_current_scope` because the source-backed fallback record feed is missing for the current Small scope. No-Godot Medium seed 10 setup3 reaches `0x4a4fc5` but fails closed at `0x4a4fc5_source_water_mutation_path_unowned_for_current_land_scope` because the current generated-cell terrain word surface exposes source-water cells before that mutation path is owned. Downstream `0x4a9d6a`, reward/guard, connection/decorative/road-river, and final writeout remain blocked.
