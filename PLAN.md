...
- Latest source-stream coverage artifact: `.artifacts/rmg_recovery/reward_guard_source_stream_coverage_summary_20260612.json` now recursively scans the recovered corpus, not just the curated artifact list. It confirms the existing recovered artifact set does not contain a full same-run seed-58 H3MapEd `0x4aa354` selected reward/guard stream with generator `+0x398/+0x39c` descriptor-vector state and selected descriptor `+0x94/+0x95` state: `691` recovered JSON files and raw text/log hits were scanned, with `46` total `0x4aa354` event hits (`45` broad unseeded plus one seed-10 attach-order sample), `0` seed-58 constructor ledgers, and `0` raw seed-58 text/log hits for `0x4aa354`. No native behavior change is source-backed from this artifact; Windows `.dll` builds remain deferred until Linux/Python parity reaches a final boundary.
