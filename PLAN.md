...
- Current native-vs-H3MapEd finding: Medium seed-10 input/template/player identity now passes in the corrected phase-drift audit after fixing the Python report to read wrapped `h3maped_small_port` snapshots and compare against the observed saved H3M identity. Current Linux/Python artifacts are `.artifacts/rmg_native_medium_seed10_report_fix_current_20260611` and `.artifacts/rmg_native_small2p_seed58_report_fix_current_20260611`. Native RMG parity remains incomplete: Medium seed-10 first same-identity drift is `town_castle_endpoint_vector` coordinate drift (`H3M: 16,49 / 20,32 / 39,35 / 50,47 / 55,63 / 9,19`; native: `16,48 / 20,32 / 41,33 / 48,46 / 53,63 / 9,18`), while mines still match `24/24`, rewards remain underproduced (`53` native vs `144` H3M), guards remain underproduced (`51` native vs `126` H3M), roads overdraw (`315` native vs `136` H3M), and Medium pre-`0x4a4c8e` `word_0x28` mismatches remain `2,999`. Small seed-58 route replay still passes mechanically but native route RNG state does not match (`native_match=False`, H3MapEd route entry offset `5,002`). The selected-town-subtype mask probe was tested and rejected because it changed downstream RNG/road/reward behavior without fixing the controlled town coordinates. The next implementation target is the exact source-backed town coordinate/footprint placement subrule before reward/materialization or road tuning. Do not patch this with density scalars, art remaps, brute-force retries, Windows rebuild churn, or final-map delta tuning. Inner-loop validation is Linux `.so` plus Python parity compares; Windows `.dll` builds are deferred until a Linux-verified final boundary.
