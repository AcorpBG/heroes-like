...
- Latest seed-58 handoff comparison: `tools/rmg_h3maped_4aa9b7_native_handoff_compare.py` compares the recovered H3MapEd `0x4aa9b7 -> 0x4aa3e9` success-handoff summary to a native phase snapshot. Against `.artifacts/rmg_native_small2p_seed58_guard_surrogate_rng_accounting_20260612`, H3MapEd has `7` false `0x4aa9b7` calls before the first success, candidate count `2`, selected index `1`, and selected coordinate `(4,3,0)`, while native has `18` false attempts before first success, candidate count `22`, selected index `12`, and coordinate `(4,12,0)`. One later native success has candidate count `2` and selected index `1`, but coordinate `(1,10,0)`. This narrows the pre-route parity blocker to upstream reward/object candidate state before or at `0x4aa9b7`; it still does not source-back changing native `0x4aa354` or guard-constructor behavior without the missing selected descriptor stream.
