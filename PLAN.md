...
- Latest native RMG implementation update: `h3maped_rmg_core` now carries the recovered `0x4adfe1 -> 0x49ecf2` setup fields `+0x34/+0x38/+0x3c/+0x40` through the native no-Godot workflow, setup stack model, generator setup result, generator-object private state, controlled-case schema, and phase snapshot. It also applies the recovered caller normalization that zeroes `+0x34/+0x3c` when their sum is below `2`. Known stack words are now width, height, level count, normalized setup `+0x34`, setup `+0x38`, normalized setup `+0x3c`, setup `+0x40`, setup mode `+0x44`, prepared value band `clamp(raw +0x48 + 3, 1, 5)`, and setup field `+0x4c`; the remaining source word is caller stack `[EBP+0x0c]` / arg9. Focused no-Godot Medium seed-10 setup-0 with trace-backed setup fields still writes final payload sections and blocks at `native_final_tile_stream_mismatch_against_same_run_0x49b2b6_payload`: tile bytes are 36,288 vs 36,288 and generated-object payload is 12,376 vs 17,057. Runtime/native map output remains disabled. This is source-backed setup/private-state porting progress, not final-payload parity or runtime generation authorization.
