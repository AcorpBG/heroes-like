...
- Current checkpoint-2 finding: refreshed no-Godot snapshots in `.artifacts/rmg_native_cli_49cf34_attach_mutation_snapshot/` now include the prior plain-C++ generated-cell bit-helper contract surface for recovered `0x49a1d8`, `0x49aa63`, `0x49a932`, and `0x49a962`; a plain-C++ `0x49cf34` attach-mutation helper replay for the recovered generated-cell write pairs; a plain-C++ `0x4a54a7` commit-mutation helper replay for recovered `0x4a61bc`, `0x4a7605`, and `0x4aa3e9` samples; a plain-C++ `0x4a56b6` projection-write helper replay for the recovered `0x4aa3e9 -> 0x4aa44d -> 0x4a54a7` projection stream; the standalone object-vector prerequisite contract for the recovered `0x4aa354/0x49cf34/0x4aa9b7/0x4aa3e9/0x4a79a3` surfaces; and the standalone `0x4a8260` route-boundary contract imported from the recovered seed-58 route replay. The `0x49cf34` helper replay now materializes and matches the recovered 17/17 attach write pairs, with 17 unique recovered cell pointers, 8 primary writes, 9 neighbor writes, 15 changed pairs, 5 bit26 clears, and 15 bit27 sets-from-clear. `tools/rmg_h3maped_49cf34_attach_mutation_verify.py` compares those native rows against the recovered `49cf34_cell_mutation_replay_summary_20260610.json` and passes (`native=17`, `h3maped=17`, `unique=17/17`). The `0x4a54a7` helper replay matches all 6 recovered samples: 5 endpoint/border-guard style samples clear the GeneratedCell `+0x20` low word and set the action/occupied surface, and 1 sampled `0x4aa3e9` reward callback lowers the low word and clears bit25. The `0x4a56b6` helper replay now materializes and matches the full 90/90 ordered projection-write stream from the recovered Wine ledger, with 90 unique recovered cell pointers, ordinal coverage 4..93, and every replay preserving the GeneratedCell `+0x20` high word while lowering the low word. `tools/rmg_h3maped_projection_write_stream_verify.py` compares the native snapshot rows against the recovered ledger and passes (`native=90`, `h3maped=90`, `unique=90/90`). The seed-58 source stream evidence has `0x4aa354=195`, `0x49cf34=150`, `0x4aa9b7=195`, and `0x4aa3e9=14`, but it still does not dump generator descriptor vector `+0x398/+0x39c` or selected descriptor state `+0x94/+0x95`, and the `0x4a79a3` samples stop short of replaying downstream GeneratedCell mutations from ordered native object-vector records and materialized projection-write coordinates. The route verifier still passes mechanically against the seed-58 replay (`416/416` splits, `340/340` stamps, `52/52` `0x4a80dc` calls, `11/11` far cuts, unique route RNG offset `5002` and entry state `2776862028`), but native adoption is explicitly denied because `native_object_vector_order_materialized=false`, `same_run_descriptor_state_complete=false`, `generated_cell_mutation_replay_complete=false`, `projection_write_coordinates_materialized=false`, `native_route_rng_boundary_materialized=false`, and `live_grid_mutation_adopted=false`. Earlier no-Godot work in `.artifacts/rmg_native_cli_phase_snapshot_only_hc4/` and related hc4 artifacts preserves the recovered `0x4a3a03/0x4cca55` source descriptor table, replays boundary walks from locator node indices, and ports the recovered `0x4a2777` no-selected-segment fallback rectangle branch; the matched Medium `hc4_comp0` setup `0` reference does not exercise that fallback branch. The corrected hc4 compare still uses the same `2SM4d(2)` template and reports matching owner-id coverage (`0..9`) but not private-state parity: `word_0x20_low_word_mismatch=5184`, `word_0x20_high_word_mismatch=475`, `owner_byte2_signed_mismatch=475`, `word_0x24_mismatch=4768`, and `word_0x28_mismatch=4065`. The bit gap remains explicit: native after-live-feedback has `bit26=0` and `bit27=5184`, while H3MapEd has `bit26=2478` and `bit27=1311`. This is progress inside checkpoint 2, not parity; exact live grid mutation remains blocked on source-backed normal-path owner/vector materialization plus ordered score/object-vector/route bit replay, with no Godot export path and no density/final-map tuning.
- Current guardrails: do not re-open H3MapEd reverse engineering for this slice unless Python/Ghidra/Wine evidence disproves the recorded recovery, do not add density scalars/gates/brute-force retries/final-map tuning, do not use GDScript for reports/exports, and do not launch Godot/headless Godot for native RMG export on this memory-constrained host. `tools/rmg_native_batch_export.py` now only supports the standalone no-Godot `bin/rmg_native_batch_export_cli` path; the legacy Godot runner has been removed from this wrapper, the wrapper refuses to run while a Godot process is already active, the wrapper refuses full `.amap` export attempts before spawning the CLI unless `--allow-blocked-full-export-probe` is explicitly supplied, and the Python freshness gate tracks CLI sources plus `bin/rmg_native_batch_export_cli` instead of the old in-engine scene/runner or GDExtension `.so` timestamps. The no-Godot CLI now owns plain-C++ controlled-case parsing/filtering, checkpoint-2 phase snapshots with constructor-default generated-cell words, setup `+0x44` supplied/defaulted provenance, post-boundary owner-byte generated-cell checkpoints, post-synthetic runtime terrain selection and private terrain-code writeout checkpoints, post-relation-eligibility bit-28 generated-cell checkpoints, post-live-feedback word `0x24`/`0x28` generated-cell checkpoints, embedded-catalog template selection/runtime-zone records, recovered `0x49ecf2` generator-mode resolution for supplied or defaulted setup `+0x44`, link-seed extraction, `0x4a1f3b` coordinate replay summaries, `0x4a3a03/0x4cc788/0x4ccb64/0x4ccdfc` source-node footprint summaries, `0x4a3b48/0x49b452` same-level synthetic runtime-zone append replay, owner-gated `0x4a2777/0x4a325d` boundary/span-fill owner-word summaries, source-backed `0x4a2777` boundary-vector append-order diagnostics with exact same-run `generator+0x3f4` vector still marked unported, `0x49b53d/0x540908/0x4a3f27` runtime terrain writeout summaries, `0x4a2105/0x4a2ffa/0x4a2ec3` terrain-relation eligibility summaries, and `0x4a4025/0x4a4082/0x4bb74b/0x4bc5f0/0x4bbfcc` live visual-feedback summaries for supported Small/Medium one-level land cases. `--phase-snapshot-only` is now the successful no-Godot trigger for private-state snapshots; `.amap` generation still fails closed until the recovered H3MapEd RMG generation core is split from Godot `Dictionary`/`Array`/`String`, `RefCounted`, and `FileAccess` APIs into plain C++ data structures. Do not treat the initial generated-cell grid, post-boundary owner-word grid, boundary-vector append-order diagnostics, post-terrain-writeout grid, post-relation-eligibility grid, post-live-feedback grid, template/runtime-zone summary, link-seed summary, coordinate replay summary, synthetic append summary, source-node summary, boundary/span-fill summary, runtime terrain summary, terrain-cell writeout summary, terrain-relation eligibility summary, or terrain live-feedback summary as pre-`0x4a4c8e` parity; exact same-run compares still mismatch. Do not build Windows `.dll` outputs until Linux `.so` parity checks reach the final boundary.
