...
- Latest native RMG implementation update: `h3maped_rmg_core` now refuses same-run final tile/object payload byte comparison unless the recovered H3MapEd authority includes the exact `0x49ecf2` setup-stack words and the native setup stack matches them. The previous focused Medium seed-10 setup-0 compare was fail-open: the recovered final-payload ledger provides tile/object bytes but does not include the `0x49ecf2` setup-stack authority, while the native run still has unknown setup args `3..6` (`+0x34/+0x38/+0x3c/+0x40`, later generator `+0xf48..+0xf54`). The standalone no-Godot CLI now blocks at `same_run_payload_authority_0x49ecf2_stack_join_missing` and its manifest points the next slice at recovering or supplying that same-run setup authority before using final-byte deltas. Runtime/native map output remains disabled. This is a blocker correction and fail-closed comparison fix, not final-payload parity or runtime generation authorization.
