...
- Latest native RMG implementation update: `h3maped_rmg_core` now follows the recovered `0x4ac552` order through source-record replay, route/free-cell `0x4a8260/0x4a4c8e`, the recovered post-`0x4a4c8e` `0x4a8c15 -> 0x49a962` cleanup scan, and the source-backed `0x4a4913` relation-vector loop. The active workflow now fails closed at the next unported `0x4a8c15` bridge call, `0x4a5767`, before `0x4a4fc5`, `0x4a79a3`, `0x4a9d6a`, relation scan, reward/guard, connection/decorative/road-river, or final writeout can run. The current blocker is `0x4a8c15_bridge_0x4a5767_unported_after_relation_loop_0x4a4913_before_0x4a4fc5`; the next implementation target is source-backed replay of that `0x4a5767` bridge in the active shared workflow. This is not parity and not final map output. In the sampled Small/Medium one-level land controlled cases, `0x4a4913` is applied but all relation records skip the type-8 body path.
