...
- Latest native RMG implementation update: `h3maped_rmg_core` now advances decorative `0x49eb8d -> 0x49e700` beyond the old allocation placeholder. After `rand_trn.txt` terrain-score filtering and recovered `0x49e1bf` scoring, native builds the recovered positive-candidate/weight vectors (`0x49e904`/`0x49e91f`), performs the weighted `0x49e9ad` RNG selection, allocates/initializes the selected decorative object (`0x5044b1`/`0x49ba89`), and runs one recovered `0x4a54a7` footprint commit before failing closed at the still-unported local coordinate worklist replay. The focused no-Godot snapshots now show Small seed 10 reaching 335 candidate appends, weight total 8995, one RNG selection, and one commit before `0x49e700_coordinate_worklist_replay_after_first_decorative_commit_unported`. Small seed 58 and Medium seed 10 expose the separate ref-bearing-object blocker before RNG: `0x49b89c_object_record_score_cache_descriptor_0x3c_mask_unowned`, where the recovered `0x49b89c` cache builder needs descriptor `+0x3c`/`0x41e915` mask data and object-record local score-cache storage. This is source-order workflow progress, not parity completion; final payload generation remains disabled until `0x49b89c` score-cache data, the full decorative coordinate worklist/repeated commit loop, road/river adjacency, descriptor counter replay, and final writeout are source-owned.
