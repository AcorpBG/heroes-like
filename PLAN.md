...
- Native parity reality: native still diverges before object/route/package consumers. Checkpoint 2 remains the active implementation blocker until the native pre-`0x4a4c8e` generated-cell words match H3MapEd private state in Python-owned comparisons. Shared core owns the record-level `0x4a5767`/`0x49a318` projection helpers, source-backed `0x49a318` high-owner propagation from relation-owner coordinate seeds including source projection clear, same-owner projection/`+0x1c` low-word writes, and cross-owner `+0x1c` high-word/`+0x28` direction/`+0x20` owner-byte writes, reset-time known-empty object-reference vectors from `0x499e65/0x499ea3`, recovered `0x499ee8` object-reference removal/empty-cell reset helper behavior, recovered explicit-input `0x4a606b` connection-region writing, no-object `0x4a5a23` projection-chain behavior, recovered explicit-call `0x4a54a7` object-footprint commit afterstate including descriptor-offset source-cell projection semantics, recovered `0x4903e8` descriptor/source join prep for contexts `45/53/54/79`, `0x4af785` resolver wiring into that prep, copied `0x4c` source-record identity through explicit object commit prep, recovered `0x4a93a2` weighted-record allocation / generator `+0xf44` sequence ownership, recovered `0x4a8db2` weighted scheduler density-lane threshold state, source-backed weighted `0x4a93a2 -> 0x4a901a -> 0x4a54a7` type-98 record-tail commit behavior for recovered `0x540a9c` records, and recovered D-014 projection/exclusion/prep state for endpoint/connection materialization. The `0x4a1f3b` non-sentinel scan-bound writer/scan-consumer payload, remaining `0x49a318` object-metadata branch, generic source-order object materialization producer/selection order beyond source-owned weighted records, successful endpoint `+0xd8/+0xdc`/`+0xf5c` materialization, town/reward/guard materialization, roads/rivers, and final writeout are still unported.
...
- Native RMG alignment checkpoints, in execution order:
  ...
  3. Reward/object identity: carry source records/descriptors through selection and materialization instead of resolving final identity through placeholder object ids. The recovered `0x49da08` object source-record catalog, metadata `+0x08` `0xe8` `0x49db76` wrapper-bucket lane layer, sampled `0x4af785` wrapper reuse/copy/create support model, sampled `0x4af89f`/`0x4a9e40` selector mechanics, recovered `0x4903e8` descriptor/source join prep for contexts `45/53/54/79`, copied `0x4c` source-record identity, explicit prepared `0x4a54a7` object commit path, recovered `0x4a93a2` weighted-record allocation / generator `+0xf44` sequence ownership, recovered `0x4a8db2` density-lane threshold argument, and source-backed weighted type-98 `0x4a93a2` record-tail commit through `0x4a901a -> 0x4a54a7` are now exposed in shared C++ and mirrored through the native wrapper. Full live weighted materialization still waits on the generic source-order producer feed, candidate/vector selection, object-record-key path, and downstream caller order around `0x4a8d2c/0x4a8db2/0x4a93a2/0x4a901a`.
...
- Current checkpoint-2 snapshot surface: the blocked no-Godot shared-chain snapshot emits only a partial generated-cell surface. It now carries the recovered generator-object private-state shape for generated-cell buffer `+0x14`, dimensions `+0x18/+0x1c/+0x20`, accepted candidate vector `+0x10d4/+0x10d8`, source-owner/player-slot counts, active relation-vector count from selected `0x4ac552 -> 0x4a218c` owner adoption, recovered `0x49b452` relation-owner constructor/default fields with scan-bound sentinel values explicitly left unknown until the non-sentinel `0x4a1f3b` writer is ported, recovered `0x4a1f3b/0x4a19ed` relation-owner selected coordinate triple `+0x10..+0x18`, source-zone endpoint vector `+0xc8/+0xcc` contents/count for `0x4a1f3b`, `0x4a17f5/0x4a1ad8` coordinate candidate vectors per relation owner, recovered reciprocal `0x49f7c4` relation-owner records from selected runtime links, recovered `0x4a5767` full-grid projection reset, source-backed `0x49a318` projection/high-owner word mutations and counters, reset-time known-empty generated-cell object-reference vectors from `0x499e65/0x499ea3`, recovered `0x499ee8` object-reference removal/empty-cell reset helper behavior, setup-zeroed endpoint field `+0xf58` while leaving `+0xf5c` unclaimed, recovered `0x49f95a` endpoint byte-state vector `+0x1104/+0x1108` zero-init relationship to endpoint pointer vector `+0xd8/+0xdc`, the recovered static `0x4a5e73` endpoint helper contract for explicit inputs, recovered explicit-input `0x4a606b` 3x3 connection-region writing, recovered no-object `0x4a5a23` projection-chain occupancy/cleanup behavior, recovered explicit-call `0x4a54a7` object-footprint commit afterstate including descriptor-offset source-cell projection semantics, recovered `0x4903e8` descriptor/source join prep for contexts `45/53/54/79`, copied `0x4c` source-record identity through explicit object commit prep, recovered `0x4a93a2` weighted-record allocation / generator `+0xf44` sequence ownership, recovered `0x4a8db2` weighted scheduler density-lane threshold list, D-014 endpoint projection/exclusion/caller-prep state, descriptor counter table `+0x1110` zero initialization from `0x49ecf2` (`0x3a0` bytes / 232 dwords), recovered stride-`0x30` record shape, reset/owner/terrain word overlays, partial `+0x2b` bit-knowledge reporting, the recovered `0x49da08` object source-record catalog identity summary, the recovered metadata `+0x08` `0xe8` `0x49db76` wrapper-bucket summary, sampled `0x4af785` wrapper reuse/copy/create behavior, and sampled `0x4af89f`/`0x4a9e40` source selector behavior. Live relation/object caller order, `0x4a1f3b` scan-bounds updates/source-order scan consumers, the remaining `0x49a318` object-metadata branch, source-order object materialization callers/object-reference appends/removals, endpoint pointer vector `+0xd8/+0xdc` contents/count required for concrete byte-state contents, live `+0xf5c` cursor seed, full weighted object-record-key caller order, full descriptor counter increment/decrement replay beyond explicit prepared calls, and downstream source-order relation/object caller mutations are still not source-owned by the live chain. It reports owner-grid materialization counts, `generated_cell_private_state_comparable=false`, and the missing mutating phases from `docs/native-rmg-generated-cell-mutation-chain.md`. The active source-order chain still calls the recovered one-level land `0x4a3710` finalizer after `0x4a2777 -> 0x4a325d`, but this proves only ordering/owner-grid support; it does not prove checkpoint-2 parity or authorize runtime map output.
