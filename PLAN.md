...
- Native RMG alignment checkpoints, in execution order:
  ...
  3. Reward/object identity: carry source records/descriptors through selection and materialization instead of resolving final identity through placeholder object ids. The recovered `0x49da08` object source-record catalog, metadata `+0x08` `0xe8` `0x49db76` wrapper-bucket lane layer, and sampled `0x4af89f`/`0x4a9e40` selector mechanics are now exposed in shared C++ and no-Godot blocked snapshots, but the live `0x4af785` wrapper reuse/copy/create chain, `0x4903e8` descriptor joins, selected copied records carried through object selection, and materialization remain unported.
