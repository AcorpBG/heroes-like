...
- Current native-vs-H3MapEd finding: Medium seed-10 input/template identity, coordinate replay, selected runtime terrain ids, recovered `0x4a2777` dual endpoint clipping, map-border continuation, per-edge owner writer skip, `0x4bbfcc` final-sweep scratch mask/class correction, the source-backed town-only `0x49abd6 -> 0x4a8260` object-reference vector boundary, the recovered `0x4ac552` reward active-vector scheduling surface, the recovered `0x4a9e40` reward template selector filter, the source-projected `0x4aa3e9` reward-guard package surface, and the source-backed active reward RNG consumption for secondary `0x4aa1db -> 0x49d471` member materialization are ported for the controlled land references. The current Linux/Python artifacts `.artifacts/rmg_native_small2p_seed58_guard_branch_20260611` and `.artifacts/rmg_native_medium_seed10_guard_branch_20260611` add the source-backed `0x4aa354` guard-attachment branch rule: native no longer projects a diagnostic `0x49cf34/0x49d69d` guard member for scaled-zero reward wrappers or dynamic-monster reward candidates. Active `0x49cf34` RNG adoption was tested and rejected because it regressed Small seed-58 reward-preview exit to offset `4,015`; it stays blocked until the exact `0x4aa9b7` wrapper candidate state and active call-time RNG order are proven from the recovered H3MapEd state. Native RMG parity remains incomplete: seed-58 route replay still passes mechanically and native reward preview exits at RNG offset `4,905`, while the H3MapEd route entry remains offset `5,002`; Medium seed-10 pre-`0x4a4c8e` `word_0x28` mismatches remain `2,999`, and final tile byte mismatches are `11,724` with first drift still `terrain_generated_cell_word_0x24_before_0x49b2b6`. The remaining blocker is exact reward object/materialization and guard-attachment state inside active scheduler calls (`0x4a9f1c`, dynamic `+0xf5c/+0xf58/+0x10b4`, `0x49cf34`, `0x49d69d`, `0x4aa9b7`, `0x4aa3e9`) before the same-run `0x4a8260` route RNG/stamp stream can align. Do not patch this with density scalars, art remaps, brute-force retries, Windows rebuild churn, or final-map delta tuning. Inner-loop validation is Linux `.so` plus Python parity compares; Windows `.dll` builds are deferred until a Linux-verified final boundary.
