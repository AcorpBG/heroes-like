...
- Latest native RMG implementation update: `h3maped_rmg_core` now follows the recovered `0x4ac552` order through source-record replay, route/free-cell `0x4a8260/0x4a4c8e`, the recovered post-`0x4a4c8e` `0x4a8c15 -> 0x49a962` cleanup scan, the source-backed `0x4a4913` relation-vector loop, the source-order `0x4a8c15 -> 0x4a5767` bridge relation normalization/scan/high-owner propagation, the one-level-land `0x4a4fc5` water-edge scan including source-water candidate handling, and the `0x4a79a3` connection-tail invocation. This is still not parity and not final map output. No-Godot Small seed 10/58 setup3 and Medium 4-player seed 10 setup3 now reach `0x4a79a3` and fail closed at `0x4a79a3_fallback_materialization_0x4a7605_0x4a5e03_source_records_missing_for_current_scope` because the source-backed fallback record feed is missing for those current scopes. The sampled recovered Medium seed-10 fallback records are now scoped to the recovered 1-human/2-player trace and are no longer consumed by 4-player workflows. The old connection-tail summary-counter success path has been removed; even after exact fallback materialization, `0x4a79a3` remains blocked until the real `0x49b3fb -> 0x4a61bc -> 0x4a696b/0x4a7605` pair loop is ported. Downstream `0x4a9d6a`, reward/guard, connection/decorative/road-river, and final writeout remain blocked.
