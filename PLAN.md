...
- Current native-vs-H3MapEd finding: Current Linux/Python artifacts are `.artifacts/rmg_native_small2p_seed58_4a8260_scan_counters_20260612` and `.artifacts/rmg_native_medium_seed10_4a8260_scan_counters_20260612`. Native now exposes the recovered `0x4a8260` entry-vs-internal-scan distinction explicitly: small seed-58 enters the scan at bit26 `0` / bit27 `1,296`, then the source-backed internal `0x4a8291..0x4a82ce` object-vector scan produces post-scan bit26 `1,244` / bit27 `52`. The focused route verifier now reports native route counters beside the recovered H3MapEd stream; recovered H3MapEd route mechanics remain `416` splits, `340` stamps, `52` `0x4a80dc` cuts, while native diagnostic replay is still `271` splits, `263` stamps, `34` `0x4a80dc` cuts and `native_match=false`. Native route seed remains offset `4,926`; recovered H3MapEd route entry remains offset `5,002`, leaving the same `76` upstream RNG-call gap. Live route adoption remains gated off (`route_container_0x4a8260_active_adoption=false`, `route_container_0x4a8260_rng_boundary_exact=false`). Medium seed-10 remains non-parity: final tile payload mismatch is still `11,726` bytes with first drift `terrain_generated_cell_word_0x24_before_0x49b2b6`, and pre-`0x4a4c8e` private-state mismatches remain `word_0x20=5,184`, `word_0x24=4,850`, `word_0x28=2,999`. The next implementation target remains the exact reward object/materialization and guard-attachment state before `0x4a8260` (`0x4a9f1c`, dynamic `+0xf5c/+0xf58/+0x10b4`, `0x49d471`, `0x49cf34`, `0x49d69d`, `0x4aa9b7`, `0x4aa3e9`) so the route RNG/stamp stream can be adopted from an exact source state. Do not patch this with density scalars, art remaps, brute-force retries, Windows rebuild churn, or final-map delta tuning. Inner-loop validation is Linux `.so` plus Python parity compares; Windows `.dll` builds are deferred until a Linux-verified final boundary.
