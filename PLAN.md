...
- Latest native RMG implementation update: `h3maped_rmg_core` now carries recovered relation-owner reward/guard treasure bands `+0xa0/+0xa4/+0xa8` through `+0xc0`, terrain policy `+0x0c`, stale supported-land cursor `+0xf5c = 0x7a1befdf`, zeroed `+0xf60/+0xf64` terrain-pressure state, recovered `0x57cea0` monster terrain rows, and `0x531cc4` type-17 subtype-to-monster rows into the ordered reward/guard selector. The active no-Godot workflow advances through `0x4aab7e -> 0x4aa354 -> 0x4aa1db -> 0x4a9f1c`, consumes the recovered direct `0x49c54d`, dynamic `0x49c64b`, projection `0x49ca8b/0x49cb60/0x49cd97`, and type-17 `0x49c849` scorer contracts, then blocks at `0x4aa1db_generator_descriptor_vector_0x398_0x39c_pending_before_0x4a5c07_selected_object_create`. The next implementation target is selected descriptor/materialization state for `0x4aa1db -> 0x4a5c07 -> 0x49cf34`, then the `0x4ad947` global table/coordinate inputs, wrapper/member handoff, connection/road/river phases, and final tile/object/header writeout. This is not parity and not final map output.
