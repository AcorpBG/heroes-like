...
- Direct executable inspection now materializes the baseline `0x4a5767 -> 0x49a318` high-owner propagation used by connection helpers `0x4a61bc`, `0x4a696b`, and `0x4a6cf2`: seed 1 reports 1111 high-owner cells, 185 sentinel cells, overlap precheck totals 472 materialized / 30 sentinel high-owner cells, and zero endpoint coordinates. The remaining gap is no longer the baseline owner channel; it is the extra `0x49a318` object-metadata branch plus `generator+0x6a8` shape-list content and helper success geometry.
