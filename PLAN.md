...
- The first footprint helper, `0x4a2777`, is disassembled into the boundary report without faking full materialization. Its endpoint clip dependency `0x4a2b33` is now ported as a standalone executable-backed helper with integer truncation, max-bound-minus-one behavior, and early in-bounds return evidence. Its deterministic cell-line writer dependency `0x4a261a` is also ported as executable-backed C++: endpoint-inclusive Bresenham-style stepping, zone-word writes at `cell+0x20`, reserved-cell flagging at `cell+0x2b`, 48-byte cell stride, endpoint swapping, and the water-mode reserved-flag suppression branch. Its flagged randomized/interpolated line-writer dependency `0x4a2413` is now ported as a standalone executable-backed helper: midpoint subdivision, `0x4e7276` jitter, `0x4cc5ad` distance use, clamp-before-write behavior, and zone-word writes through the same cell writer path. Its rectangular fallback branch at `0x4a2847..0x4a290c` is also standalone-ported: four `0x4a261a` boundary edges and four `runtime_zone+0x3f4` footprint vertices. Its non-fallback connector segment branch at `0x4a2911..0x4a29f9` is now standalone-ported: one clipped connector segment, one `runtime_zone+0x3f4` appended endpoint, and deterministic `0x4a261a` or flagged `0x4a2413` painting. Its continuation branch at `0x4a29f9..0x4a2b23` is standalone-ported for boundary wrapping: clipped current/next endpoints, border-walk through `0x4a2a5b..0x4a2af2`, `0x4a261a` wrap/final painting, and intermediate footprint vertex appends. Full `0x4a2777` remains blocked on the exact source-node traversal and source rectangle/list payloads.
- The `0x4a325d` span-fill helper is now ported as a standalone executable-backed boundary: it consumes `0x00ff0000` unassigned zone-word cells, fills horizontal spans, writes the current zone id through `cell+0x20`, marks reserved state through `cell+0x2b`, queues above/below unassigned runs, and reports a contained sample fill with no boundary overwrite. Project cell materialization remains blocked until real `0x4a2777` footprint boundaries and source polygon/list inputs are wired into that fill. The `0x4a3710` adjacency finalizer remains disassembled without fake output, with `runtime_zone+0xc4` adjacency insertion and `runtime_zone+0x3e8` ordering vectors still blocking project link materialization.
- The runtime polygon layout is now partially recovered from `h3maped.exe` without inventing project geometry. The boundary records `0x49b452` runtime-zone vector offsets, 0x24-byte polygon nodes constructed/finalized through `0x4cc788`, `0x4cc5db`, `0x4ccb64`, and `0x4ccdfc`, and now ports the `0x4a3a03` polygon seed-call schedule: `0x4cc788` initializes the `-200..400` enclosing polygon and `0x4a3a79` calls `0x4ccb64` once per same-level runtime zone using `runtime_zone+0x10` x/y plus the runtime-zone pointer. Seed `1` materializes 6 primary split seed calls on level `0`, executes the `0x4cca55/0x4ccb64` locator/insertion/bridge loop, ports the `0x4cc9cc` edge-removal branch plus `0x4ccc7a/0x4cc68e` crossing cleanup, and reports 24 allocated node pairs / 23 active node pairs after cleanup. It also ports `0x4ccd69/0x4ccdfc` finalized-coordinate fanout, producing 14 finalized triplets and 42 finalized nodes. Full `0x4a2777` footprint traversal remains the next blocker before terrain-cell materialization can feed real boundaries into the standalone-ported `0x4a325d` span fill.
- The selected-template polygon model now exposes the `0x4cca55 -> 0x4a2777` source-node cycle inputs that the helper caller uses at `0x4a3e58..0x4a3e8c`: 6 same-level located node walks, node `+0x10/+0x14` links, payload fields, finalized `+0x1c/+0x20` coordinates, and zero walk guard exhaustion. This is still boundary evidence; the next step is to consume those node cycles in the actual `0x4a2777` traversal instead of standalone samples.
- The `0x4cca55/0x4ccb64` polygon splitter contract is now recovered as explicit boundary evidence, and the seed-specific split model executes the locator, duplicate check, edge test, primary split insertion, `0x4cc643` relink, `0x4ccb1f` bridge loop, `0x4cc9cc` vector erase branch, `0x4ccc7a` crossing tests, `0x4cc68e` collapses, and `0x4ccdfc` finalized-node fanout. For seed `1`, the model records 6 split calls, 13 bridge pairs, 1 edge-removal branch, 24 crossing tests, 6 crossing collapses, 14 finalized triplets, 42 finalized nodes, and 28 active payload-gated nodes. The next unclaimed executable step is wiring finalized polygon output through full `0x4a2777` boundary traversal before `0x4a325d` span fill can safely paint cells.
- Normal small land `native_catalog_auto` generation now routes to `h3maped_small_port_generation_not_ready` rather than the archived generator, so no fake fallback map is produced while the remaining footprint helper trio and post-footprint h3maped phase sequence are still missing.
- `tests/native_rmg_small_h3maped_port_boundary_report.tscn` passed with `accepted_template_count: 13`, `selected_template: h3maped_template_018`, `adapted_template_id: translated_rmg_template_019_v1`, `selected_zone_count: 6`, `selected_link_count: 5`, `coordinate_seed_status: 0x4a1f3b_0x4a17f5_0x4a1ad8_0x4a19ed_single_level_ported`, `coordinate_step_count: 18`, `level_footprint_phase_status: 0x4a3a03_level_collection_ported_helpers_blocked`, `first_helper_status: 0x4a2777_disassembled_4a2b33_4a261a_4a2413_helpers_ported_runtime_layout_blocked`, `clip_helper_status: 0x4a2b33_clip_helper_ported`, `line_writer_status: 0x4a261a_cell_line_writer_ported`, `randomized_line_writer_status: 0x4a2413_randomized_line_writer_ported_standalone`, `rectangle_fallback_status: 0x4a2777_rectangle_fallback_branch_ported_standalone`, `connector_segment_status: 0x4a2777_connector_segment_branch_ported_standalone`, `boundary_wrapping_status: 0x4a2777_boundary_wrapping_continuation_ported_standalone`, `second_helper_status: 0x4a325d_cell_span_fill_ported_standalone_project_materialization_blocked`, `runtime_layout_status: runtime_polygon_seed_points_split_cleanup_and_finalizer_ported_span_fill_pending`, `polygon_seed_split_count: 6`, `polygon_seed_status: 0x4a3a03_polygon_tree_seed_calls_split_cleanup_and_finalizer_ported_span_fill_pending`, `executed_split_call_count: 6`, `pre_crossing_inserted_bridge_pair_count: 13`, `edge_removal_branch_count: 1`, `crossing_test_count: 24`, `crossing_collapse_count: 6`, `post_crossing_cleanup_active_node_pair_count: 23`, `post_crossing_cleanup_allocated_node_pair_count: 24`, `finalized_triplet_count: 14`, `finalized_node_count: 42`, `active_payload_node_count: 28`, `source_node_walk_status: 0x4cca55_to_0x4a2777_source_node_cycles_recovered`, `phase_count: 15`, `generation_status: h3maped_small_port_generation_not_ready`, and `unsupported_scope_status: unsupported_scope`.
