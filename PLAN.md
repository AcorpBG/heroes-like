...
- Current checkpoint-2 finding: refreshed no-Godot snapshots in `.artifacts/rmg_native_cli_4a79a3_endpoint_dispatch_snapshot/` expose the existing checkpoint-2 diagnostic surfaces, the recovered `0x4a79a3` payload order, and the recovered `0x4a79a3 -> 0x4a696b/0x4a7605` endpoint-dispatch exclusion/delegation contract. The standalone CLI emits `plain_cpp_object_vector_payload_order_summary` with the recovered 19-record H3MapEd payload vector from `0x4a79a3/0x4a7d2c/0x4a7d36/0x4a7d99`, and `tools/rmg_h3maped_4a79a3_payload_order_verify.py` still passes (`native=19`, `h3maped=19`, `record_mismatches=0`). It now also emits `plain_cpp_object_vector_endpoint_dispatch_summary`: recovered evidence shows `0x4a696b` has a static direct `GeneratedCell+0x28` mutation surface, but the supported one-level land corpus has zero owner/relation pair matches and zero direct mutation hits (`combined_4a696b_entries=150`, `combined_source_relation_match_hits=0`, `combined_direct_mutation_hits=0`); `0x4a7605` is a fallback endpoint coordinator with delegated `0x4a7312/0x4a746b/0x4a5e03` after-state still pending. `tools/rmg_h3maped_4a79a3_endpoint_dispatch_verify.py` passes against the recovered filter-dispatch, static-surface, target-mode, grid-scan, and cell-mutation artifacts (`trace_4a696b=4`, `trace_4a7605=1`, `direct_hits=0`, `replay_complete=false`). The R4 descriptor/source identity verifier, `0x49cf34` attach verifier, `0x4a56b6` projection-write verifier, and seed-58 `0x4a8260` route replay verifier still pass. This remains diagnostic-only: `native_object_vector_order_materialized=false`, `direct_4a696b_mutation_adopted=false`, `delegated_4a7605_afterstate_materialized=false`, `generated_cell_mutation_replay_complete=false`, and route adoption remains denied. The corrected Medium `hc4_comp0` setup `0` private-state compare still mismatches H3MapEd at `after_terrain_live_feedback`: `word_0x20_low_word_mismatch=5184`, `word_0x20_high_word_mismatch=475`, `owner_byte2_signed_mismatch=475`, `word_0x24_mismatch=4768`, and `word_0x28_mismatch=4065`; native bit26/bit27 remains `0/5184` vs H3MapEd `2478/1311`. Checkpoint 2 therefore remains pending. The next implementation target is to materialize native live object-vector records in recovered `0x4a79a3` order and recover/adopt delegated `0x4a7605` after-state, while still carrying same-run generator descriptor vector `+0x398/+0x39c` and selected descriptor state `+0x94/+0x95`.
