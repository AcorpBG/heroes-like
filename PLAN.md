...
Owner-directed unit-art progress on 2026-05-23: every authored unit now has deterministic generated PNG art records for portrait, battle icon, and overworld icon surfaces under `art/units/`, with `content/unit_art_manifest.json` as the source manifest and `tools/generate_unit_art_assets.py` as the reproducible generator. `ContentService` validates the manifest and `BattleBoardView` loads battle icons for stack tokens with the old procedural glyph as fallback. `tests/unit_art_manifest_report.tscn` checks all unit records, dimensions, unique paths, and live battle icon loading; `tests/validate_repo.py` now includes the same manifest and runtime-hook gate. This is a full-roster generated-art baseline and battle-surface integration slice, not a claim that final hand-painted unit art direction or every future UI/map usage surface is complete.
