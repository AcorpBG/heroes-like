...
- Latest native RMG implementation update: `h3maped_rmg_core` now aligns descriptor mask helper bit order for recovered `0x49a6f9` reward/guard footprint filtering. Native `0x4268eb`/`0x41e951` mask reads now use `47 - (x * 8) - y` over the 8x6 descriptor field, matching H3MapEd's column-major helper order instead of the stale row-major interpretation; `h3maped_rmg_core_selftest` covers the `0x4aa603 -> 0x49a6f9` rejection trace for `(mask_x=1, mask_y=0)`. The no-Godot Medium seed-10 setup-0 run still fails closed at `native_final_tile_stream_mismatch_against_same_run_0x49b2b6_payload`: tile stream length remains 36288 bytes, generated-object payload is now 8960 native bytes vs 17057 H3MapEd bytes, and reward/guard still has zero successful `0x4aa9b7` commits. The first `0x4aa603` footprint reject moved from existing bit26 to owner-byte mismatch, so the next blocker is source-backed owner/generated-cell state for reward/guard candidates, not bit26 weakening, density tuning, or final-map deltas.
