...
- The runtime polygon layout is now partially recovered from `h3maped.exe` without inventing project geometry. The boundary records `0x49b452` runtime-zone vector offsets, 0x24-byte polygon nodes constructed/finalized through `0x4cc788`, `0x4cc5db`, `0x4ccb64`, and `0x4ccdfc`, and the remaining blocker that the imported template catalog has source zone/link grammar but not the executable polygon-tree payload needed to feed terrain-cell materialization.
