...
Current tactical chain: owner-directed reset archives the native catalog-auto RMG implementation and the overgrown h3maped inspection/report code as legacy evidence/debug code, then restarts from a small 36x36-only h3maped-derived boundary. After the 2026-05-13 strict restart correction, the active compiled module exposes only `/root/Downloads/h3maped.exe` verification, small land scope gate, recovered size/water scoring, h3maped RNG template-selection evidence, strict `0x4ac62a..0x4ac6ec` player-slot assignment/mapping slots, strict `0x4a218c` / `0x49b452` runtime-zone records, strict `0x4a1f3b` link-seed setup, strict `0x4a17f5` / `0x4a1701` / `0x4a1ad8` / `0x4a19ed` coordinate replay, strict `0x4a3a03` / `0x4cc788` / `0x4cc955` / `0x4ccb64` / `0x4ccdfc` zone-footprint source-node setup, strict `0x4a2777` / `0x4a2b33` / `0x4a261a` / `0x4a2413` / `0x4a325d` boundary and span fill, strict small-land footprint finalizer ordering, strict runtime terrain selection, strict private terrain cell writeout, strict TerrainPlacement visual/live feedback, strict `0x49b2b6` terrain/art/flag tile-byte candidates, strict private `0x4a8d2c` / `0x4a8db2` / `0x4a93a2` town/castle candidates, strict private `0x4a9d6a` / `0x4a9911` / `0x4aa354` / `0x4a9f1c` / `0x4aa9b7` mine/reward/object-vector prerequisites, explicit pending executable-port backlog, and runtime generation refusal. Reward coordinate filtering/mutation, roads, blockers, guards, and final writeout are pending strict executable ports, not active output. The replacement must derive behavior from `/root/Downloads/h3maped.exe` and the recovered h3maped spec, then adapt only the resulting object/content references to our runtime assets and registries. Completed child slices remain valid evidence for failure history and specific gates, but they are not the active generator direction and do not close broad HoMM3-style production parity.
