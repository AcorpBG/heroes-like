...
- Native RMG alignment checkpoints, in execution order:
  ...
  3. Reward/object identity: carry source records/descriptors through selection and materialization instead of resolving final identity through placeholder object ids. The recovered `0x49da08` object source-record catalog and metadata `+0x08` `0xe8` `0x49db76` wrapper-bucket lane layer are now exposed in shared C++ and no-Godot blocked snapshots, but wrapper/source selection, `0x4903e8` descriptor joins, selected copied records, and materialization remain unported.
...
- Current checkpoint-2 snapshot surface: the blocked no-Godot shared-chain snapshot emits only a partial generated-cell surface. It now carries the recovered stride-`0x30` record shape, reset/owner/terrain word overlays, partial `+0x2b` bit-knowledge reporting, the recovered `0x49da08` object source-record catalog identity summary, and the recovered metadata `+0x08` `0xe8` `0x49db76` wrapper-bucket summary, with object-reference contents, `+0x14/+0x18`, selected copied source records, descriptor/source joins, and source-order relation/object caller mutations still not source-owned by the live chain. It reports owner-grid materialization counts, `generated_cell_private_state_comparable=false`, and the missing mutating phases from `docs/native-rmg-generated-cell-mutation-chain.md`. The active source-order chain still calls the recovered one-level land `0x4a3710` finalizer after `0x4a2777 -> 0x4a325d`, but this proves only ordering/owner-grid support; it does not prove checkpoint-2 parity or authorize runtime map output.
