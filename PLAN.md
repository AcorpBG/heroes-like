...
- Latest native RMG implementation update: `h3maped_rmg_core` now carries recovered `0x4ac552` relation-pointer source records into the source-order town/castle materialization path. Small/Medium no-Godot workflows replay `0x4a8d2c`, `0x4a8db2`, route/free-cell, mine/resource, relation scan, generic source-order replay, and `0x49a318`, then stop at `reward_guard_materialization` with `reward_guard_materialization_0x4aab7e_zero_successful_0x4aa9b7_commits_before_connection_tail`. The next implementation target is source-backed successful reward/guard commits through `0x4aab7e -> 0x4aa9b7 -> 0x4aa3e9` before connection/decorative/road-river/final writeout. This is not parity and not final map output.
