...
- Current implementation slice: `strategic-ai-spawn-saved-task-commander-selection-10184`, selected to make enemy raid spawning prefer deployable commanders with reachable saved strategic tasks before falling back to roster rotation.
...
- Current strategic AI target: build from the baseline KPI audit into production behavior. Live commander/raid target choices now write normalized `enemy_states[].hero_task_state`, reuse saved active tasks for later target selection, reserve exclusive targets across active task boards, complete captured resource tasks, reconcile saved tasks against current target ownership/existence, suspend or invalidate saved tasks whose commander actor is missing, recovering, or unable to deploy, reactivate suspended saved tasks once their commander is deployable again, and now make new raid spawning prefer deployable commanders with reachable saved tasks before falling back to roster rotation without a save-version bump.
