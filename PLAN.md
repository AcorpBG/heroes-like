...
- The runtime polygon layout is now partially recovered from `h3maped.exe` without inventing project geometry. The boundary records `0x49b452` runtime-zone vector offsets, 0x24-byte polygon nodes constructed/finalized through `0x4cc788`, `0x4cc5db`, `0x4ccb64`, and `0x4ccdfc`, and now ports the `0x4a3a03` polygon seed-call schedule: `0x4cc788` initializes the `-200..400` enclosing polygon and `0x4a3a79` calls `0x4ccb64` once per same-level runtime zone using `runtime_zone+0x10` x/y plus the runtime-zone pointer. Seed `1` materializes 6 primary split seed calls on level `0`, executes the `0x4cca55/0x4ccb64` locator/insertion/bridge loop, ports the `0x4cc9cc` edge-removal branch plus `0x4ccc7a/0x4cc68e` crossing cleanup, and reports 24 allocated node pairs / 23 active node pairs after cleanup; `0x4ccdfc` remains the next blocker before terrain-cell materialization.
- The `0x4cca55/0x4ccb64` polygon splitter contract is now recovered as explicit boundary evidence, and the seed-specific split model executes the locator, duplicate check, edge test, primary split insertion, `0x4cc643` relink, `0x4ccb1f` bridge loop, `0x4cc9cc` vector erase branch, `0x4ccc7a` crossing tests, and `0x4cc68e` collapses. For seed `1`, the model records 6 split calls, 13 bridge pairs, 1 edge-removal branch, 24 crossing tests, and 6 crossing collapses. The report records helper roles for `0x4cc670`, `0x4ccd69`, and `0x4ccdfc`, and keeps finalized split-cell materialization pending until `0x4ccdfc` fans finalized coordinates into the related polygon nodes.
...
- `tests/native_rmg_small_h3maped_port_boundary_report.tscn` passed with `accepted_template_count: 13`, `selected_template: h3maped_template_018`, `adapted_template_id: translated_rmg_template_019_v1`, `selected_zone_count: 6`, `selected_link_count: 5`, `coordinate_seed_status: 0x4a1f3b_0x4a17f5_0x4a1ad8_0x4a19ed_single_level_ported`, `coordinate_step_count: 18`, `level_footprint_phase_status: 0x4a3a03_level_collection_ported_helpers_blocked`, `runtime_layout_status: runtime_polygon_seed_points_ported_split_algorithm_pending`, `polygon_seed_split_count: 6`, `polygon_seed_status: 0x4a3a03_polygon_tree_seed_calls_and_split_cleanup_ported_finalizer_pending`, `executed_split_call_count: 6`, `pre_crossing_inserted_bridge_pair_count: 13`, `edge_removal_branch_count: 1`, `crossing_test_count: 24`, `crossing_collapse_count: 6`, `post_crossing_cleanup_active_node_pair_count: 23`, `post_crossing_cleanup_allocated_node_pair_count: 24`, `phase_count: 15`, `generation_status: h3maped_small_port_generation_not_ready`, and `unsupported_scope_status: unsupported_scope`.
