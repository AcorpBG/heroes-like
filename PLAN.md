...
- Current native-vs-H3MapEd finding: Medium seed-10 input/template identity, coordinate replay, selected runtime terrain ids, recovered `0x4a2777` dual endpoint clipping, map-border continuation, per-edge owner writer skip, `0x4bbfcc` final-sweep scratch mask/class correction, and the source-backed town-only `0x49abd6 -> 0x4a8260` object-reference vector boundary are ported for the controlled 4-player land reference. The current Linux/Python artifact `.artifacts/rmg_native_medium_seed10_town_action_final_20260611` proves the pre-`0x4a4c8e` generated-cell object-reference vector now matches the H3MapEd dump at `78` non-empty cells, with town action/control cells `6` and non-action town body bit25 clears `72`; with the same explicit H3MapEd dump, `word_0x28` mismatches improve from the rerun baseline `3,065` to `2,999`. The remaining blocker is the same-run `0x4a8260` route-list coordinate vector/RNG/stamp stream: a source-ported route replay probe on top of the corrected town input was rejected because it moved bit counts closer (`bit26=2,795`, `bit27=1,169`) but worsened actual `word_0x28` mismatches to `3,438`. Full final tile parity is still incomplete: Medium seed-10 final tile byte mismatches are `11,727` and first drift remains `terrain_generated_cell_word_0x24_before_0x49b2b6`. Do not patch this with density scalars, art remaps, brute-force retries, Windows rebuild churn, or final-map delta tuning. Inner-loop validation is Linux `.so` plus Python parity compares; Windows `.dll` builds are deferred until a Linux-verified final boundary.
