...
- Current checkpoint-2 finding: refreshed no-Godot snapshots in `.artifacts/rmg_native_cli_phase_snapshot_only_hc4/` now preserve the recovered `0x4a3a03/0x4cca55` source descriptor table and replay boundary walks from locator node indices while using the standalone `--phase-snapshot-only` trigger. The recovered `0x4a2777` no-selected-segment fallback rectangle branch is now ported in the plain-C++ boundary replay, including the four perimeter line writes and `generator+0x3f4` append anchors `0x4a28c8/0x4a28dc/0x4a28f3/0x4a2907`; the matched Medium `hc4_comp0` setup `0` reference does not exercise it (`boundary_fallback_zone_count=0`). For that reference, descriptor handoff is materialized for all 10 runtime-zone walks (70 descriptor nodes, 51 replay nodes, no missing/invalid/guard-exhausted handoffs), and every `0x4a325d` seed is already in bounds, so exact out-of-bounds seed relocation also remains unexercised. The compare in `.artifacts/rmg_native_cli_phase_snapshot_only_hc4/private_state_compare_hc4_setup0_after_live_feedback.json` still uses the same `2SM4d(2)` template and reports matching owner-id coverage (`0..9`). The split mismatch surface remains explicit: `word_0x20` mismatches all 5184 cells because `word_0x20_low_word_mismatch=5184` leaves native at constructor low word `0x7fbc`, while `word_0x20_high_word_mismatch=475` matches the 475/5184 `owner_byte2_signed_mismatch`; `word_0x24` mismatches 4768 and `word_0x28` mismatches 4065. This is progress inside checkpoint 2, not parity: exact live grid mutation remains blocked on the normal-path `0x4a2777` `generator+0x3f4` owner/vector surface plus ordered score/bit mutation replay, with no Godot export path and no density/final-map tuning.
