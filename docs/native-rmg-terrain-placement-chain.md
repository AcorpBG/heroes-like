...
5. `0x4bb74b/0x4bc5f0` live feedback:
   ...
   - `0x4bd0d1` owns the `0x4bc5f0` drain/final-sweep finalizer reached through the `0x4bd077 -> 0x4bd0b4` wrapper destruction path;
   ...
   - class-0 final-sweep cells route through the recovered `0x4ba938` current-row/base selector path instead of the classified `+0x14` bucket selector; when that path selects or reselects a class-0 row, native now keeps the recovered zeroed flag outputs instead of copying stale selected-row flags;
   - count-zero final-sweep cells route through the recovered `0x4bc5a3 -> 0x4bce6d -> vtable+0x10` selector path instead of unconditionally copying the current scratch visual record;
   - the recovered `0x4ba938` base selector argument surface is now ported: the first selector argument is the `0x4bce6d` neighbor/probability mask, the second selector argument is `-1` for ordinary `0x4bcfc3` base selection or the current art row for final-sweep current-record selection, and the probability threshold uses the source unsigned logical shift.
...
The known terrain toolkit vfunc drift is fixed in the active shared core: native no longer treats every nonzero same-terrain neighbor art row as connectable, and now uses recovered `0x4ba91d` flag-A semantics for complex same-terrain continuity. The final-sweep selector now uses the recovered `0x4bcb91`/`0x4bcd43` correction predicates, the recovered `0x4ba938` class-0 current-row/base selector path, the recovered count-zero final-sweep selector call path, and the recovered `0x4ba938` first-argument mask / second-argument current-row surface. The active core also ports the recovered set-A/set-B ordered-tree first-node drain behavior, byte5-zero same-class retouch routing, the `0x4bb74b` byte5-zero live-feedback branch, and the recovered `0x4bb681` same-terrain direct rewrite vs changed-terrain feedback split. This is still not a TerrainPlacement parity claim.

The focused same-run Medium comparison continues to block at `native_final_tile_stream_mismatch_against_same_run_0x49b2b6_payload`: the 36288-byte tile stream length matches, but the first mismatch is offset 1 (`cell=0`, `byte_in_cell=1`, `native=53`, `h3maped=54`). This moved the first-cell art from the previous wrong flag-A run (`61`) into the correct class-0 normal run, but still leaves a one-row drift inside that run. The generated-object payload remains short (`10923` native bytes vs `17057` H3MapEd bytes). The recovered `0x49b2b6` final tile serializer was rechecked against the same-run first-cell words: native is already carrying a different `word_0x24` before final writeout, so the first byte-1 mismatch is upstream generated-cell/private-state drift, not a source-backed serializer patch. Rechecked `0x4bc5f0` ordered set drain, `0x4bdbcd` ordered-tree comparator, and `0x4a3f27` TerrainPlacement caller order did not expose a new source-backed TerrainPlacement scope-order change in this slice. Any remaining TerrainPlacement queue/final-sweep order/RNG drift must be treated as live until phase/private-state comparison proves otherwise.
