...
The faction signature unit-building ladders now use a 2/4/6 rare-resource tier curve on tiers 5, 6, and 7. Selected late unique town buildings keep additional faction-rare costs, so every authored town now spends at least `MIN_RARE_DEVELOPMENT_SPEND = 14` faction rare resources during full development.

The deterministic balance gate also caps post-completion rare surplus with `MAX_ENDING_RARE_AFTER_COMPLETION = 13`. Current evidence covers 15 authored towns, all still completing inside the 30-turn target and at or after the day-20 production pacing floor.
