...
| --- | --- | --- | --- | --- |
| D-001 | Blocking | Ordered spine starts at `0x4602c1`, runs through `0x4adfe1`, `0x4ac552`, private-state phases, `0x4ad1e3`, `0x49b2b6`, object/header writeout, and `0x4ae09a`. | `case_shared_h3maped_state_chain_blocked_json()` emits blocked JSON with `generation_output_written=false`, `runtime_generation_allowed=false`, and `native_rmg_end_to_end_parity_complete=false` (`rmg_native_core.cpp:803-844`). | No entrypoint-to-writeout generator exists in this file. |
| D-002 | Blocking | Source behavior starts from H3MapEd executable state, candidate/source setup, and generator private fields. | `ControlledCase` is a synthetic string schema: `id:size_class:players:seed:water_mode:level_count[:human_count[:computer_count[:setup_object_0x44]]]` (`rmg_native_core.cpp:501-554`, `rmg_native_core.hpp:10-25`). | Native input is not the recovered H3MapEd generator object/private-state entrypoint. |
...
| D-008 | Blocking | Reset defaults are not valid substitutes for later relation/object/private-state mutations. | The partial word surface marks `+0x10/+0x1c` and `+0x2c` as partial reset-derived sources, but it still exports records and hashes from reset-derived arrays before later recovered relation/object mutations run (`rmg_native_core.cpp:300-353`). | Native still exposes reset/default fields where the recovered ledger requires later source-order mutation before consumers. |
| D-009 | Blocking | Relation/candidate/action/occupied caller order must include `0x4a5767`, `0x49a318`, `0x4a59e2`, `0x49aa63`, `0x49a932`, `0x49abd6`, `0x49a85d`, `0x49a962`, `0x499ee8`, `0x4a5a23`, `0x49cf34`, `0x4aa3e9`, and related inputs. | `generated_cell_mutation_phase_blockers()` only records three grouped missing strings (`rmg_native_core.cpp:144-149`) and does not execute these callers. | Native marks the block too narrowly and does not port the recovered caller order. |
...
| D-016 | Blocking | Final writeout requires `0x4ad1e3`, `0x49b2b6`, `0x4ad309`, `0x4ad3eb`, header/player/metadata stream writes, stream adapter/sink, and success return. | Native explicitly writes no generation output and no `.amap` (`rmg_native_core.cpp:818-822`). `CaseReport` still has inert `native_map_json_written` fields (`rmg_native_core.hpp:83-94`). | Native has no final tile/object/header writeout authority. |
...
`controlled case -> optional 0x49ecf2 -> optional recovered catalog 0x4ac552 helper with accepted candidate-container records and source-owner/player-slot buffers -> 0x4a218c/0x4a1f3b/0x4a19ed -> 0x4a3a03/0x4cca55 -> 0x4a2777/0x4a325d/0x4a3710 -> 0x49b53d/0x4a3f27/TerrainPlacement support -> blocked JSON`.
...
- full raw copied source records and descriptor identity;
...
6. Preserve full copied `0x4c` source records and descriptor/source identity through runtime zones and object selection.
7. Expand generated-cell state to real stride-`0x30` records, including object-reference vectors, `+0x14/+0x18`, and `+0x2b`, before claiming a checkpoint.
8. Port relation/scoring/object caller order before route/object/package consumers.
