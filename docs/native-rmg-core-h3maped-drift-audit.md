...
| D-004 | Blocking | Template links preserve guard value, `wide`, and Border Guard payloads for later connection behavior. | `SharedRuntimeLinkInput` carries only `from_index` and `to_index` (`rmg_native_core.hpp:41-44`). The shared core's runtime link seed also carries only endpoints after filtering, while source template links have `guard_value`, `wide`, and `border_guard` before collapse (`h3maped_rmg_core.hpp:274-284`, `236-239`). | Link payload needed by connection/blocker/guard phases is dropped before native can port those phases. |
...
| D-007 | Misleading surface | A pre-`0x4a4c8e` checkpoint requires source-order private-state ownership and comparison. | `append_generated_cell_checkpoint_json()` emits schema `h3maped_private_state_checkpoint_0x4a4c8e_generated_cells_v1` and status `recovered_owner_grid_payload_materialized` when the partial payload is built (`rmg_native_core.cpp:286-340`, `461-464`). | The JSON name looks like a checkpoint even though `generated_cell_private_state_comparable=false` is set elsewhere. |
| D-008 | Blocking | Reset defaults are not valid substitutes for later relation/object/private-state mutations. | Output labels `+0x10/+0x1c` and `+0x2c` as reset-owned and exports hashes/records from reset-derived arrays (`rmg_native_core.cpp:297-300`, `749-754`). | Native still exposes reset/default fields where the recovered ledger requires later source-order mutation before consumers. |
...
| D-014 | Blocking | Endpoint/cursor and fallback behavior includes `0x4a5e73`, `0x4a606b`, `0x4a696b`, `0x4a7312`, `0x4a7605`, and `0x4a5e03`. | No endpoint vector/cursor state exists in `RecoveredOwnerGridPayload`; link payload is already reduced to endpoints; no connection/fallback materialization code exists. | Native cannot port recovered connection blockers/guards from the current adapter state. |
...
| D-017 | Misleading surface | The recovery ledger distinguishes H3MapEd recovery from native parity. | JSON says `required_source="shared_recovered_h3maped_private_state_chain"` while the callable path is only coordinate/owner-grid/TerrainPlacement support (`rmg_native_core.cpp:805-811`, `649-760`). | The output wording overstates the partial shared-chain surface. |
| D-018 | Misleading surface | Missing native-port phases include source records, descriptor identity, relation/scoring, town/object/reward/guard, endpoint/connection, roads/rivers, and writeout. | `next_required_native_core_slice` names only `port_later_relation_object_generated_cell_mutation_caller_order` (`rmg_native_core.cpp:813-814`). | The next-slice marker underreports remaining drift after relation/object mutation. |
| D-019 | Misleading surface | Terrain selection/repaint can only be trusted in the source-order chain and after comparison. | `terrain_selection_input_blockers()` and `terrain_repaint_input_blockers()` return empty (`rmg_native_core.cpp:152-158`). | Locally true for the helper call, but misleading as a parity signal because upstream private state and downstream consumers remain unported/uncompared. |
...
2. Preserve full runtime link payloads (`guard_value`, `wide`, `border_guard`) into runtime/link state instead of reducing links to endpoints.
