...
- `0x4ab52a` road/river object-adjacency prefix now owns the recovered generator `+0x14b0` coordinate vector for source-backed `0x4a901a`/`0x4a93a2` allocations, the road-type RNG selection, outer/inner coordinate pair order, `0x4aae2f` path-state reset of low-word/predecessor fields, and `0x4aae7b` low-word path-cost/predecessor propagation before candidate acceptance. The active blocker is the downstream road toolkit mutation/write path `0x4ab37f -> 0x4b4243`; native does not synthesize road art or final payload output.
...
- Exact caller sequence after the `0x4a5767` bridge is still not fully owned through the road toolkit and final writeout.
...
- No-Godot Small seed 10/58 setup3 and Medium 4-player seed 10 setup3 snapshots show the active bridge reset visits both generated-cell planes, applies relation scan consumers for all relation owners, applies `0x49a318` high-owner propagation, applies the one-level-land `0x4a4fc5` scan, runs mine/resource materialization, runs reward/guard materialization, applies decorative coordinate-worklist replay/pass3, applies source-backed connection-tail replay, replays the `0x4ab52a` road-prefix path, and then blocks at `0x4ab37f -> 0x4b4243`. Small seed 10 reaches 4 road records, 1 accepted pair, and 2517 relaxed path edges. Small seed 58 reaches 4, 1, and 2882. Medium seed 10 reaches 7, 1, and 12953. All three still have zero successful direct `0x4aa9b7` reward/guard commits, so this is still not parity or final writeout.
...
- Native still lacks successful reward/guard coordinate commits from the live `0x4aab7e -> 0x4aa354 -> 0x4aa9b7 -> 0x4aa603/0x4aa3e9` source stream, but recovered caller order does not allow that mismatch to stop downstream phase replay by itself. The current focused no-Godot Small/Medium one-level land blocker is `0x4ab37f_0x4b4243_road_toolkit_unported_after_0x4ab52a_pair_prefix`. Descriptor counters, final writeout, and same-run private-state/final-payload comparison remain blocked. The implemented `0x4a5e73`, `0x4a606b`, `0x4a5a23`, `0x4903e8`, exact-input `0x4a8db2`, exact-input `0x4a901a`, exact-input direct `0x4a8d2c -> 0x4a93a2`, exact-prestate fallback replay, direct `0x4a5c07 -> 0x4a5e03`, `0x49b89c` object-record score-cache support, `0x49e700` coordinate worklist replay, `0x4a79a3` connection-tail replay, `0x4ab52a` road-prefix replay, and explicit `0x4a54a7` prep helpers must not be wired to guessed endpoint/object state or run before their source-order owner phase is complete.
...
1. Port the source-backed road toolkit mutation/write path `0x4ab37f -> 0x4b4243` after the now-applied `0x4ab52a` road/river object-adjacency prefix.
...
4. Resume descriptor counter replay, final writeout, and same-run private-state comparison only after the source-order road/river/object mutations above are owned.
