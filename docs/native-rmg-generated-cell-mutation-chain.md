...
- recovered `0x4ba868` toolkit range slots, `0x4ba91d` complex neighbor probe, `0x4baa81` simple/rock neighbor rejection, and `0x4ba938` base-row probability selection with its source second-argument/probability surface;
...
- The focused same-run Medium payload still blocks at `native_final_tile_stream_mismatch_against_same_run_0x49b2b6_payload`; after the `0x4bbfcc/0x4ba938` selector port the current first tile-stream mismatch is offset `1`, native art byte `61` vs H3MapEd `54`. Remaining TerrainPlacement queue/final-sweep order drift is not ruled out until phase/private-state comparison proves it.
