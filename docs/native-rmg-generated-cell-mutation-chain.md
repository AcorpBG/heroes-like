...
- `0x4a8db2` source-order scheduler replay now owns recovered exact-input behavior for source-backed preserved `+0xedc` pairs: pair `+0x00/+0x04` payload gating, direct prepass call order over source fields `+0x24/+0x20/+0x34/+0x30`, recovered lane arguments into `0x4a901a`, weighted density order `+0x2c/+0x28/+0x3c/+0x38`, threshold formula, weighted lane score/increment order, tie behavior, and disable-on-false. Live producer/caller order feeding every real source pair into this replay remains pending.
...
- Native now applies the recovered `0x4a5767` full-grid projection reset over the live generated-cell grid, materializes `0x4a1f3b` non-sentinel relation scan bounds from generated-cell owner-byte rectangles, runs the recovered scan-consumer projection chain over those bounds including the `0x4a5a23` object branch when generator state/RNG are present, runs source-backed `0x49a318` high-owner byte propagation, carries reset-time known-empty object-reference vectors, owns the `0x4a93a2` record allocator/`+0xf44` sequence state, owns exact-input `0x4a8db2` scheduler replay, weighted `0x4a901a`, and direct `0x4a8d2c -> 0x4a93a2` candidate/selection/commit behavior, and exposes explicit-call `0x4a54a7` object-vector/object-reference/generator-counter/relation-counter/descriptor-offset projection mutations, but still lacks exact `0x49a318` call-site order/same-run private-state comparison around the now-ported bit22 policy branch, generic live source-order object materialization callers beyond the scan-consumer/weighted/direct exact-input paths, true source-record `+0x30/+0x34` direct branches, and downstream relation/object caller replay.
- Native now carries recovered `0x49b452` relation-owner constructor/default fields plus concrete zeroed `+0x44` descriptor-counter tables, the recovered `0x4a1f3b/0x4a19ed` selected coordinate triple `+0x10..+0x18`, `0x4a1f3b` relation scan bounds `+0x20..+0x2c`, source-zone endpoint vector `+0xc8/+0xcc` contents/count, supported-land endpoint cursor vector `+0xd8/+0xdc` compact key count, `0x4a17f5/0x4a1ad8` coordinate candidate vectors consumed by `0x4a1f3b`, the recovered scan-consumer pass over those bounds, and the recovered `0x49f95a` endpoint byte-state vector zero-init relationship to endpoint pointer vector `+0xd8/+0xdc`. The remaining relation-owner/private-state gap is exact `0x49a318` call-site order/same-run private-state comparison around the now-ported bit22 policy branch and later source-order object callers.
- Native still lacks the live `+0xf5c` cursor success producer for unsupported/non-sampled endpoint modes, live source-pair producer/caller order for generic weighted/direct object-record-key creation, and the generic supported-land fallback payload feed/caller order after target-mode-excluded `0x4a606b` / `0x4a696b` paths. The exact `0x4a7605 -> 0x4a5e03` fallback payload replay is now invoked by the active generator private-state builder for the two sampled seed-controlled records, requires recovered target `+0x20/+0x24/+0x28` pre-state words, fails closed on mismatch, and delegates successful commits through `0x4a54a7`; the implemented `0x4a5e73`, `0x4a606b`, `0x4a5a23`, `0x4903e8`, exact-input `0x4a8db2`, exact-input `0x4a901a`, exact-input direct `0x4a8d2c -> 0x4a93a2`, exact-prestate fallback replay, and explicit `0x4a54a7` prep helpers must not be wired to guessed endpoint/object state.
...
1. Replay `0x49a318` at the recovered call sites and prove same-run private-state parity around the now-ported bit22 object-metadata policy before route/object/package consumers run.
2. Port the source-order candidate, occupied/action, score, object-record-key, and reward attachment callers into the active shared native chain.
