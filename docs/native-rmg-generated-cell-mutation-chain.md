...
Status: the active shared native workflow now owns the source-order `0x4a8c15 -> 0x4a5767` bridge relation-normalization call after the recovered post-`0x4a4c8e` cleanup scan and `0x4a4913` relation-vector loop, the one-level-land `0x4a4fc5` water-edge scan including source-water candidate handling, and the `0x4a79a3` connection-tail invocation. Small supported-land cases and the current Medium 4-player setup3 case advance past `0x4a4fc5`, invoke `0x4a79a3`, and fail closed because the source-backed `0x4a7605/0x4a5e03` fallback record feed is missing for those scopes. The two sampled recovered Medium seed-10 fallback records are scoped to the recovered 1-human/2-player trace and are no longer consumed by 4-player workflows. Downstream object/reward/guard, road/river, and final-writeout caller order remain unowned.
...
- Native now carries recovered `0x49b452` relation-owner constructor/default fields plus concrete zeroed `+0x44` descriptor-counter tables, the recovered `0x4a1f3b/0x4a19ed` selected coordinate triple `+0x10..+0x18`, source-zone endpoint vector `+0xc8/+0xcc` contents/count, and `0x4a17f5/0x4a1ad8` coordinate candidate vectors from the source setup path. The active generator private-state builder now continues through source-order relation record replay, route/free-cell `0x4a8260 -> 0x4a4c8e`, the recovered post-`0x4a4c8e` `0x4a8c15 -> 0x49a962` cleanup scan, the recovered `0x4a4913` relation-vector loop, the `0x4a8c15 -> 0x4a5767` full-grid reset / relation scan / `0x49a318` high-owner propagation bridge, the one-level-land `0x4a4fc5` water-edge scan, and the `0x4a79a3` connection-tail invocation. Small supported-land cases and the current Medium 4-player setup3 case now fail closed inside `0x4a79a3` because the source-backed `0x4a7605/0x4a5e03` fallback record feed is missing for those current scopes. It does not run `0x4a9d6a` mine/resource, reward/guard, connection/decorative, roads/rivers, or final writeout until the bridge tail is source-owned.
- No-Godot Small seed 58 setup3 and Medium 4-player seed 10 setup3 snapshots show the active bridge reset visits and changes every generated-cell record (`1296/1296` and `5184/5184`), applies relation scan consumers for all relation owners, and applies `0x49a318` high-owner propagation. Small seed 58 setup3 applies the zero-source-water `0x4a4fc5` scan. Medium 4-player seed 10 setup3 applies the source-water-aware `0x4a4fc5` scan, reaches `0x4a79a3`, and blocks because no recovered fallback record feed exists for that player scope. That is still a prefix, not parity.
...
1. Port the live `0x4a79a3` relation-pair loop (`0x49b3fb -> 0x4a61bc -> 0x4a696b/0x4a7605`) so fallback materialization is produced from current source-owned relation state instead of sampled record feeds.
2. Recover/provide source-backed fallback feeds only where the live loop still has named unrecovered inputs; current Small supported-land and Medium 4-player scopes have no exact fallback record feed.
