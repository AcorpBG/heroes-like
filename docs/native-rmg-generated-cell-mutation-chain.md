...
- Native now applies the recovered `0x4a5767` full-grid projection reset over the live generated-cell grid, materializes `0x4a1f3b` non-sentinel relation scan bounds from generated-cell owner-byte rectangles, runs the recovered scan-consumer projection chain over those bounds including the `0x4a5a23` object branch when generator state/RNG are present, runs source-backed `0x49a318` high-owner byte propagation, carries reset-time known-empty object-reference vectors, owns the `0x4a93a2` weighted record allocator/`+0xf44` sequence state, and exposes explicit-call `0x4a54a7` object-vector/object-reference/generator-counter/relation-counter/descriptor-offset projection mutations, but still lacks the `0x49a318` object-metadata branch, generic source-order object materialization callers beyond the scan-consumer branch and weighted record path, and downstream relation/object caller replay.
...
- Native still lacks the live `+0xf5c` cursor success producer for unsupported/non-sampled endpoint modes, source-owned weighted object-record-key callers beyond the recovered weighted record-tail commit path, and the downstream supported-land fallback materialization caller order after target-mode-excluded `0x4a606b` / `0x4a696b` paths. The implemented `0x4a5e73`, `0x4a606b`, `0x4a5a23`, `0x4903e8`, and explicit `0x4a54a7` prep helpers must not be wired to guessed endpoint/object state.
