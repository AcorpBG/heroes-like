...
Status: the active shared native workflow now owns the source-order `0x4a8c15 -> 0x4a5767` bridge relation-normalization call after the recovered post-`0x4a4c8e` cleanup scan and `0x4a4913` relation-vector loop, the one-level-land `0x4a4fc5` water-edge scan including source-water candidate handling, mine/resource materialization, and the live `0x4a79a3` connection-tail path. Small supported-land cases and the current Medium 4-player setup3 case advance past `0x4a4fc5`, enter `0x4a79a3`, run the recovered source-frontier scan and source-owned relation endpoint lookup through `0x49b3fb`, and invoke the source-backed `0x4a61bc` entry/candidate path for reciprocal pairs. Small seed 10 and Medium seed 10 stop at `0x4a61bc_live_prefix_no_source_frontier_candidate_after_0x49b3fb`. Small seed 58 now executes the recovered `0x4a61bc` selected-candidate loop through direct `0x4a5c07` monster selection and `0x4a5e03/0x4a54a7` commit, appending 7 connection guard records before stopping at `reward_guard_materialization_0x4aab7e_zero_successful_0x4aa9b7_commits_before_connection_tail`. The current reward/guard blocker is zero successful `0x4aa9b7` commits after 90 source-stream attempts, with the first `0x4aa603` rejection at `0x49a6f9` footprint filtering. Fallback materialization no longer runs before the live `0x49b3fb -> 0x4a61bc` pair loop. Downstream reward/guard source-stream feasibility, generic source-order replay, road/river, and final-writeout caller order remain unowned.
...
- Native now carries recovered `0x49b452` relation-owner constructor/default fields plus concrete zeroed `+0x44` descriptor-counter tables, the recovered `0x4a1f3b/0x4a19ed` selected coordinate triple `+0x10..+0x18`, source-zone endpoint vector `+0xc8/+0xcc` contents/count, and `0x4a17f5/0x4a1ad8` coordinate candidate vectors from the source setup path. The active generator private-state builder now continues through source-order relation record replay, route/free-cell `0x4a8260 -> 0x4a4c8e`, the recovered post-`0x4a4c8e` `0x4a8c15 -> 0x49a962` cleanup scan, the recovered `0x4a4913` relation-vector loop, the `0x4a8c15 -> 0x4a5767` full-grid reset / relation scan / `0x49a318` high-owner propagation bridge, the one-level-land `0x4a4fc5` water-edge scan, mine/resource materialization, and the live `0x4a79a3` connection-tail path. Small seed 58 now invokes the source-backed `0x4a61bc` entry/candidate prefix from source-owned `0x49b3fb` pair records, selects direct `0x4a5c07` monsters, commits them through `0x4a5e03/0x4a54a7`, then fails closed at reward/guard materialization. It does not run decorative, road/river, or final writeout until reward/guard commits are source-owned.
- No-Godot Small seed 10/58 setup3 and Medium 4-player seed 10 setup3 snapshots show the active bridge reset visits every generated-cell record, applies relation scan consumers for all relation owners, applies `0x49a318` high-owner propagation, applies the one-level-land `0x4a4fc5` scan, then enters `0x4a79a3`. Small seed 10 and Medium seed 10 still stop at `0x4a61bc_live_prefix_no_source_frontier_candidate_after_0x49b3fb`. Small seed 58 reaches `reward_guard_materialization`: the connection tail is applied, `0x4a61bc` selects 7 guard lanes, constructor blocked count is 0, `0x4a5e03/0x4a54a7` appends 7 connection guard records, and the object vector reaches 28 records before `0x4aab7e` makes 90 reward/guard source-stream attempts with zero successful `0x4aa9b7` commits. That is still not parity or final writeout.
...
- Native still lacks successful reward/guard coordinate commits from the live `0x4aab7e -> 0x4aa354 -> 0x4aa9b7 -> 0x4aa603/0x4aa3e9` source stream after direct connection guards are committed. Fallback records are no longer the first required implementation path for current supported scopes; fallback must remain behind the live pair loop. Downstream decorative caller order, generic supported-land fallback payload feed/caller order after target-mode-excluded `0x4a606b` / `0x4a696b` paths, roads/rivers, and final writeout remain blocked behind reward/guard source ownership. The implemented `0x4a5e73`, `0x4a606b`, `0x4a5a23`, `0x4903e8`, exact-input `0x4a8db2`, exact-input `0x4a901a`, exact-input direct `0x4a8d2c -> 0x4a93a2`, exact-prestate fallback replay, direct `0x4a5c07 -> 0x4a5e03`, and explicit `0x4a54a7` prep helpers must not be wired to guessed endpoint/object state or run before their source-order owner phase is complete.
...
1. Port the live `0x4a61bc -> 0x4a5e03/0x4a54a7` descriptor payload construction and object commit path inside the recovered `0x4a79a3 -> 0x49b3fb` relation-pair loop.
2. Continue the recovered `0x4a79a3` helper order through `0x4a696b/0x4a7605` only after the source-owned `0x4a61bc` pair path is active; fallback feeds must not run before that loop.
3. Resume `0x4a9d6a`, relation/object caller order, and reward/guard only after the `0x4a8c15` bridge private-state boundary through the real `0x4a79a3` loop is owned.
