...
- Native now applies the recovered `0x4a5767` full-grid projection reset over the live generated-cell grid, runs source-backed `0x49a318` high-owner byte propagation, carries reset-time known-empty object-reference vectors, and exposes explicit-call `0x4a54a7` object-vector/object-reference/descriptor-counter/descriptor-offset projection mutations, but still lacks source-order object materialization callers, `0x4a1f3b` scan-bounds/scan-consumer updates, the `0x49a318` object-metadata branch, and downstream relation/object caller replay.
