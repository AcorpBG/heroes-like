...
Status: the active shared native workflow now owns the source-order `0x4a8c15 -> 0x4a5767` bridge relation-normalization call after the recovered post-`0x4a4c8e` cleanup scan and `0x4a4913` relation-vector loop, the one-level-land `0x4a4fc5` water-edge scan including source-water candidate handling, mine/resource materialization, reward/guard materialization, and the source-backed `0x49eb8d -> 0x49e700` decorative dispatch prefix in recovered caller order. The earlier native stop that treated zero successful direct `0x4aa9b7` reward/guard commits as a hard phase gate was a native-only guard; recovered `0x4ac552` continues after `0x4aab7e`, so native now continues while preserving the zero-commit detail as a parity diagnostic. Decorative dispatch now scans recovered `0x54092c` source rows, applies `rand_trn.txt` terrain-score filtering, replays recovered `0x49e1bf` scoring, appends positive candidate/weight entries (`0x49e904`/`0x49e91f`), selects by weighted `0x49e9ad` RNG, allocates/initializes the selected object (`0x5044b1`/`0x49ba89`), and runs one recovered `0x4a54a7` footprint commit. Native now carries `0x49b89c` object-record local score-cache storage for source-carried object records and uses that cache for ref-bearing `0x49e1bf` overlap/adjacent score adjustments instead of failing at the former ref-bearing blocker. Focused no-Godot Small seed 10/58 and Medium seed 10 now all proceed past the former `0x49b89c_object_record_score_cache_descriptor_0x3c_mask_unowned` stop and fail closed later at `0x49e700_coordinate_worklist_replay_after_first_decorative_commit_unported`; score-cache misses are zero and cache-known counts match object-record counts for those snapshots. Final writeout remains blocked. The current cache body-mask adoption is bounded to the native catalog's existing `.msk` footprint fields until later same-run/final-payload parity proves whether independent descriptor `+0x3c` separation matters.
...
- Native still lacks successful reward/guard coordinate commits from the live `0x4aab7e -> 0x4aa354 -> 0x4aa9b7 -> 0x4aa603/0x4aa3e9` source stream, but recovered caller order does not allow that mismatch to stop downstream phase replay by itself. The current focused no-Godot Small/Medium one-level land blocker is the `0x49e700` local coordinate worklist/repeated-commit loop after the first selected decorative commit. Roads/rivers, adjacency, descriptor counters, and final writeout remain blocked. The implemented `0x4a5e73`, `0x4a606b`, `0x4a5a23`, `0x4903e8`, exact-input `0x4a8db2`, exact-input `0x4a901a`, exact-input direct `0x4a8d2c -> 0x4a93a2`, exact-prestate fallback replay, direct `0x4a5c07 -> 0x4a5e03`, `0x49b89c` object-record score-cache support, and explicit `0x4a54a7` prep helpers must not be wired to guessed endpoint/object state or run before their source-order owner phase is complete.
...
1. Complete the `0x49e700` local coordinate worklist/repeated-commit loop after the first selected decorative object commit.
...
4. Resume downstream road/river adjacency, descriptor counter replay, final writeout, and same-run private-state comparison only after the source-order object/decorative mutations above are owned.
