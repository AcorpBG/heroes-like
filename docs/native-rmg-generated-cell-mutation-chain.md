...
- No-Godot Small seed 10/58 setup3 and Medium 4-player seed 10 setup3 snapshots show the active bridge reset visits both generated-cell planes, applies relation scan consumers for all relation owners, applies `0x49a318` high-owner propagation, applies the one-level-land `0x4a4fc5` scan, runs mine/resource materialization, runs reward/guard materialization, and then advances into downstream connection/decorative replay. Small seed 10 now reaches decorative weighted candidate selection and one selected-object commit before blocking on the unported `0x49e700` coordinate worklist/repeated-commit loop. Small seed 58 and Medium seed 10 reach the recovered object-reference neighbor scorer path and block on `0x49b89c` because native does not yet store the descriptor `+0x3c`/`0x41e915` mask or object-record score-cache table. All three still have zero successful direct `0x4aa9b7` reward/guard commits, so this is still not parity or final writeout.
...
1. Add the recovered descriptor `+0x3c`/`0x41e915` mask to native source-object records and implement the `0x49b89c` object-record local score-cache table before ref-bearing decorative scorer candidates can proceed.
2. Complete the `0x49e700` local coordinate worklist/repeated-commit loop after the first selected decorative object commit.
