...
Status: the active shared native workflow now owns the source-order `0x4a8c15 -> 0x4a5767` bridge relation-normalization call after the recovered post-`0x4a4c8e` cleanup scan and `0x4a4913` relation-vector loop, the one-level-land `0x4a4fc5` water-edge scan including source-water candidate handling, mine/resource materialization, reward/guard materialization, and the source-backed `0x49eb8d -> 0x49e700` decorative prefix in recovered caller order. The earlier native stop that treated zero successful direct `0x4aa9b7` reward/guard commits as a hard phase gate was a native-only guard; recovered `0x4ac552` continues after `0x4aab7e`, so native now continues while preserving the zero-commit detail as a parity diagnostic. Small seed 10, Small seed 58, and the current Medium 4-player setup3 case now fail closed during decorative dispatch at `0x49e700_0x49e1bf_descriptor_0x30_0x40_adjacency_overlap_scoring_replay_unowned`: native has counted bit26 cells, entered the recovered `0x54092c` type-table scan, loaded the recovered `rand_trn.txt` terrain/adjacent/overlap score rows, applied the `-5000` hard terrain-score reject, and reached source row `312` / type `119` / DEF `AVLd1sn0.def`. It stops before guessed `0x49e1bf` descriptor `+0x30/+0x40` adjacency/overlap/object-reference scoring replay. Final writeout remains blocked.
...
- No-Godot Small seed 10/58 setup3 and Medium 4-player seed 10 setup3 snapshots show the active bridge reset visits both generated-cell planes, applies relation scan consumers for all relation owners, applies `0x49a318` high-owner propagation, applies the one-level-land `0x4a4fc5` scan, runs mine/resource materialization, runs reward/guard materialization, and then advances into downstream connection/decorative replay. Small seed 10 reaches 36 object records, 144 reward/guard attempts, and blocks at `0x4a61bc`; Small seed 58 reaches 22 object records, 90 reward/guard attempts, and blocks at `0x49eb8d`; Medium seed 10 reaches 59 object records, 126 reward/guard attempts, and blocks at `0x4a61bc`. All three still have zero successful direct `0x4aa9b7` commits, so this is still not parity or final writeout.
...
1. Fix the live `0x4aab7e -> 0x4aa354 -> 0x4aa9b7 -> 0x4aa603/0x4aa3e9` reward/guard source stream so supported Small/Medium one-level land cases produce source-owned reward/guard commits before any connection-tail replay.
2. Resume generic source-order object replay only after reward/guard commits are source-owned.
3. Continue the recovered `0x4a79a3` helper order through `0x49b3fb`, `0x4a61bc`, `0x4a696b`, and `0x4a7605` only after the reward/guard phase succeeds; fallback feeds must not run before their source-order caller phase.
