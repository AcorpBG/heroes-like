...
- recovered `0x4ba868` toolkit range slots, `0x4ba91d` complex neighbor probe, `0x4baa81` simple/rock neighbor rejection, and `0x4ba938` base-row probability selection with its source first-argument mask and second-argument current-row surface;
...
- The focused same-run Medium payload still blocks at `native_final_tile_stream_mismatch_against_same_run_0x49b2b6_payload`; after the `0x4bbfcc/0x4ba938` selector-mask stack-slot fix the current first tile-stream mismatch is offset `1`, native art byte `53` vs H3MapEd `54`. Native has moved from the previous wrong flag-A row run into the correct class-0 normal row run, but remaining TerrainPlacement queue/final-sweep order or RNG drift is not ruled out until phase/private-state comparison proves it.
...
- `0x4ab52a` road/river object-adjacency helpers are reached by the source-owned workflow, including source-owned `+0x14b0` coordinate handling and the downstream `0x4ab37f -> 0x4b4243` toolkit. The focused Medium seed-10 setup-1 run reaches this phase without captured connection RNG replay, but its pre-road RNG state/value and road output still differ from H3MapEd.
...
- The recovered `0x49da08` descriptor bucket expression is `generator + 0x34 + metadata[type].bucket_index * 0x10`. Native now source-populates the type-43 one-way entrance lane at `+0x2e8/+0x2ec`, type-44 one-way exit lane at `+0x2f8/+0x2fc`, and type-45 two-way lane at `+0x308/+0x30c`, with eight records in each lane. Catalog indices (`1150`, `1158`, `1166` for each first record) and descriptor raw keys (`source_row - 22`) are carried separately.
- The recovered `0x4a79a3` cursor dispatch now scans the two-way lane before paired entrance/exit lanes, calls source-owned `0x4a696b -> 0x4a7605 -> 0x4a7312`, appends successful keys to `+0x14c0/+0x14d0`, appends coordinates to relation owner `+0x404`, and marks reciprocal endpoint records only after dispatch success. The focused authority case records two direct `0x4a7312` commits and zero captured connection RNG calls.

Known blockers:

- The source-owned caller sequence now advances through `0x4a79a3` and downstream final writeout. The focused Medium seed-10 setup-1 result remains `blocked` at `native_final_tile_stream_mismatch_against_same_run_0x49b2b6_payload`: native has `1258` objects versus `1326`, object payload `18055` versus `18830` bytes, and first tile mismatch offset `5226` (cell `746`, road-type lane, native `0`, expected `3`).
- The exact remaining upstream proof gap is the object materialization and RNG call order before `0x4ab52a`: native reaches road selection with RNG state `599433559`, emits value `3030`, and selects road type `1`, while the same-run authority ledger records road RNG value `25892`. The shorter object vector and changed RNG state prevent treating the road byte mismatch as a serializer defect.
- Earlier Small/Medium snapshots that reached roads or final writeout through captured private state remain diagnostic history only and cannot be used as native ownership or parity evidence.
...
- Native still lacks complete live object-materialization and RNG parity before roads. The current focused blocker is the 68-object shortfall and pre-`0x4ab52a` RNG divergence; captured coordinate/RNG/object/road/river streams are not acceptable inputs.
