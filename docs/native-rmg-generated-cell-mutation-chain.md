...
- Native now applies the recovered `0x4a5767` full-grid projection reset over the live generated-cell grid, materializes `0x4a1f3b` non-sentinel relation scan bounds from generated-cell owner-byte rectangles, runs source-backed `0x49a318` high-owner byte propagation, carries reset-time known-empty object-reference vectors, owns the `0x4a93a2` weighted record allocator/`+0xf44` sequence state, and exposes explicit-call `0x4a54a7` object-vector/object-reference/descriptor-counter/descriptor-offset projection mutations, but still lacks full source-order object materialization callers, source-order scan consumers after those bounds, the `0x49a318` object-metadata branch, and downstream relation/object caller replay.
- Native now carries recovered `0x49b452` relation-owner constructor/default fields plus the recovered `0x4a1f3b/0x4a19ed` selected coordinate triple `+0x10..+0x18`, `0x4a1f3b` relation scan bounds `+0x20..+0x2c`, source-zone endpoint vector `+0xc8/+0xcc` contents/count, `0x4a17f5/0x4a1ad8` coordinate candidate vectors consumed by `0x4a1f3b`, and the recovered `0x49f95a` endpoint byte-state vector zero-init relationship to endpoint pointer vector `+0xd8/+0xdc`. The remaining relation-owner/private-state gap is endpoint pointer-vector contents/count required for concrete byte-state contents and later source-order scan consumers.
...
1. Port source-order scan consumers after the now-native `0x4a1f3b` scan bounds, plus the remaining `0x49a318` object-metadata branch before route/object/package consumers run.
