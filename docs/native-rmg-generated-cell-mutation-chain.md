...
Status: the active shared native workflow now owns the source-order `0x4a8c15 -> 0x4a5767` bridge relation-normalization call after the recovered post-`0x4a4c8e` cleanup scan and `0x4a4913` relation-vector loop, the one-level-land `0x4a4fc5` water-edge scan including source-water candidate handling, and the live `0x4a79a3` connection-tail prefix. Small supported-land cases and the current Medium 4-player setup3 case advance past `0x4a4fc5`, enter `0x4a79a3`, run the recovered source-frontier scan and source-owned relation endpoint lookup through `0x49b3fb`, and invoke the source-backed `0x4a61bc` entry/candidate path for reciprocal pairs. Small seed 10 and Medium seed 10 stop at `0x4a61bc_live_prefix_no_source_frontier_candidate_after_0x49b3fb`. Small seed 58 now executes the recovered `0x4a61bc` selected-candidate loop: `7` selected loops, `7` RNG selections, `14` `0x4a5a23` projection-chain calls, `16` occupied stamps, `40` cleanup clears, and `14` owner local-vector `+0x404` appends before stopping at `0x4a61bc_live_0x4a5c07_static_selector_unported_before_0x4a5e03`. Fallback materialization no longer runs before the live `0x49b3fb -> 0x4a61bc` pair loop. Downstream direct `0x4a5c07` static selector, `0x4a5e03/0x4a54a7` object construction/commit, object/reward/guard, road/river, and final-writeout caller order remain unowned.
...
- Native now carries recovered `0x49b452` relation-owner constructor/default fields plus concrete zeroed `+0x44` descriptor-counter tables, the recovered `0x4a1f3b/0x4a19ed` selected coordinate triple `+0x10..+0x18`, source-zone endpoint vector `+0xc8/+0xcc` contents/count, and `0x4a17f5/0x4a1ad8` coordinate candidate vectors from the source setup path. The active generator private-state builder now continues through source-order relation record replay, route/free-cell `0x4a8260 -> 0x4a4c8e`, the recovered post-`0x4a4c8e` `0x4a8c15 -> 0x49a962` cleanup scan, the recovered `0x4a4913` relation-vector loop, the `0x4a8c15 -> 0x4a5767` full-grid reset / relation scan / `0x49a318` high-owner propagation bridge, the one-level-land `0x4a4fc5` water-edge scan, and the live `0x4a79a3` connection-tail prefix. Small supported-land cases and the current Medium 4-player setup3 case now invoke the source-backed `0x4a61bc` entry/candidate prefix from source-owned `0x49b3fb` pair records, then fail closed before descriptor payload construction and object commit. It does not run `0x4a9d6a` mine/resource, reward/guard, connection/decorative, roads/rivers, or final writeout until the bridge tail is source-owned.
- No-Godot Small seed 10/58 setup3 and Medium 4-player seed 10 setup3 snapshots show the active bridge reset visits every generated-cell record, applies relation scan consumers for all relation owners, applies `0x49a318` high-owner propagation, applies the one-level-land `0x4a4fc5` scan, then enters `0x4a79a3`. Small seed 10/58 scan `1296` cells; Medium seed 10 scans `5184`. The live relation-owner pair counts after `0x49b3fb` are Small seed 10 `22`, Small seed 58 `16`, and Medium seed 10 `16`, and the `0x4a61bc` prefix is invoked for every pair. The `0x4a61bc` entry gate passes all pairs in these probes. Small seed 58 finds `65` source-frontier scan candidates, `59` matching candidate lanes, `10` selected candidate lanes, and `7` constructor blockers; Small seed 10 and Medium seed 10 have no matching source-frontier candidates after the pair loop. All three stop before fallback. That is still a prefix, not parity.
