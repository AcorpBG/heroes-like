...
## Runtime Asset Ingestion Boundary

Status: explicit deferral. No generated PNG, generated crop, traced derivative, prompt-output sheet, or renamed candidate file is approved for runtime or source-asset import.

Approval authority:

- AcOrP must approve any future concept-art asset-ingestion slice in writing before import begins.
- The future slice must name the approved game surface, approved source studies, runtime target paths, rollback owner, and validation commands.
- A code/content worker may not infer approval from accepted concept evidence, implementation briefs, selected tracks, or this pipeline document.

Provenance/source requirements for any later import:

- The imported work must be original game-ready production art derived from the approved brief, not the generated PNG itself.
- The asset packet must record source study ids, exact prompt text when available, image-generation tool/model/version, generation date, curator decision, human production/editing steps, license/rights statement, and derivative-risk review.
- The packet must list rejected generated details that were not copied, including exact layouts, banners, symbols, labels, palette chips, and distinctive silhouettes.
- Files with external candidate names or generation UUIDs may remain only in concept/reference areas such as `art/concept/` and review notes; runtime/source-art files need production names tied to game ids.

Import location rules:

- Concept/reference studies stay under `art/concept/` or a documented review folder and must not be registered in scenes, manifests, content JSON, renderer lookup tables, or runtime export paths.
- Source production art may use a future documented source-art path, but only after the approval slice defines ownership, naming, and export rules.
- Runtime-ready derivatives may enter runtime asset folders only through the future ingestion slice, with stable game-id filenames and matching `.import` files generated by Godot as part of validation.
- No future import may alter save schema, gameplay rules, renderer behavior, map generation, AI logic, or content JSON outside the paths named by that slice.

Rollback path:

- Every future ingestion slice must keep the import in one coherent commit or patch set that can be reverted without schema/data migration.
- The slice must list all added source files, runtime files, `.import` files, manifests, scene references, and content references.
- Rollback must restore previous placeholder references or leave the affected surface on the existing placeholder path, then rerun validation.
- If provenance, rights, derivative-risk, or AcOrP approval is later disputed, the asset must be removed from runtime paths before further art work builds on it.

Validation gate for any later import:

- Baseline validation must include `python3 tests/validate_repo.py` and `git diff --check`.
- The future slice must add or run a focused asset gate that proves generated candidate files are not registered as runtime/source assets, all referenced runtime files exist, Godot import metadata is present where needed, and rollback instructions name every touched asset/reference.
- Completion requires the approval note, provenance packet, import manifest, rollback note, and validation result to be recorded in `ops/progress.json`.
