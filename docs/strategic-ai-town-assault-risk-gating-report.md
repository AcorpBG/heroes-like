...
- Independent newly assigned town, encounter, and hero targets now run a proactive known-target risk pass before movement, so a deployable host that becomes too weak for the selected objective can regroup or stage before spending turns marching into a bad fight.
- Hosts already committed to `awaiting_support` or `active_front_support` are left to the existing grouping pass instead of retasking themselves before support can merge.
...
- The same report now proves `weak_retake_front_regroups_before_marching`.
