...
Date: 2026-05-01
Status: slice-3 cached selected-route execution follow-up implemented; staged async plan remains
...
7. Movement-rule profiling now exposes `cached_execution_mode`, `post_action_recap_skipped`, `scenario_eval_skipped`, `interaction_dispatch_mode`, and exact `fallback_reason` so cached open movement, descriptor interaction movement, and full fallback movement are distinguishable in F3 snapshots and persistent JSONL records.
