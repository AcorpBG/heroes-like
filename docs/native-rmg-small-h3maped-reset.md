...
1. Assemble real `0x4a2777` boundary traversal from the recovered source-node walk plus the ported `0x4a2b33`/`0x4a261a` helpers, then wire its real boundary buffer into the ported `0x4a325d` span-fill primitive. The rectangle fallback is not sufficient for runtime generation.
