...
The `0x4a3f27` terrain fill/repaint schedule is now disassembly-backed in the inspection boundary. The report records the `0x4a2105` owned-cell bbox prepass, the per-runtime-zone `0x4a2ffa` recenter pass, the land-scope skip for the islands connector helper `0x4a30c2`, and the TerrainPlacement adapter vtable `0x540a14` used with `0x4bcff5` and `0x4bd099`. For the current one-level 36x36 land fixture, h3maped first schedules a whole-map fill with terrain id `8` (`water`) through `0x4bd099(0,0,width,height)`, then scans runtime-zone rectangles and repaints cells whose `cell+0x20` owner byte matches the runtime-zone loop index and whose `cell+0x28` bit 28 is set. That comparison exposes the current executable-port blocker: the filled zone-word buffer currently carries source-zone ids `1..6`, while `0x4a3f27` compares against runtime-zone indexes `0..5`. The report therefore stays read-only and records 1024 source-zone repaint-member cells, 907 runtime-index repaint-member cells, and 117 source-zone-id-6 cells that would be unmatched if terrain adoption were enabled now. Runtime generation remains blocked until that owner-byte basis is resolved from the executable path.
