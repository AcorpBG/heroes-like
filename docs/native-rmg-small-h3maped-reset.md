...
10. Ports the top-level zone-footprint schedule `0x4a3a03` for inspection: one land level collects six runtime zones, records the `0x4cc788` initial source-node rectangle `-200..400`, schedules six polygon split calls, skips the synthetic fallback zone, and records the helper sequence `0x4a2777 -> 0x4a325d -> 0x4a3710`.
