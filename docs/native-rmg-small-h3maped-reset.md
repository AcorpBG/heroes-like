...
The active port does not materialize public runtime map packages, terrain art, towns, roads, blockers, guards, mines, rewards, or final map packages. Coordinate, terrain selection, terrain/cell writeout, terrain byte-zero/grid inspection, generated-grid final-sweep boundary counters, partial generated-grid TerrainPlacement projection, footprint scheduling, the small-land no-appended-zone finalizer boundary, standalone boundary/span helper primitives, standalone `0x4a2777` branch and wrapping samples, and standalone `0x4a2413` randomized writer coverage are exposed as inspection data only and are not package generation.
