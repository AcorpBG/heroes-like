...
The clean module now ports the field-consumption boundary for the `0x4a8d2c` direct settlement minimum pass and the owner-byte candidate-scan boundary of direct helper `0x4a93a2`. For the seed `1` boundary case the selected template reports four required player castles, zero required player towns, zero neutral towns/castles, 24 density fields, no positive settlement density weights, three assigned-owner direct candidate scans with 451 total owner/repaint candidates, and one recovered owner `-1` early-fail call for the unassigned player slot. `0x49aa93` eligibility, random tie choice, object stamping, `0x4a901a` weighted placement, town object templates, footprints, ownership writeout, and later cleanup/link processing remain pending.
...
6. `object_category_placement` - `0x4a8d2c`, `0x4a93a2`, `0x4a8db2`, `0x4a8c15` - town/castle field consumption and direct owner-byte candidate scanning ported for inspection only; eligibility, object-cell materialization, weighted placement, and cleanup still pending.
