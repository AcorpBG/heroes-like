...
The runtime polygon layout is now partially recovered from `h3maped.exe` and reported as explicit evidence. `0x49b452` constructs runtime zones with initialized vectors at `runtime_zone+0x3e4`, `+0x3f4`, and `+0x404`; `0x4cc788`, `0x4cc5db`, `0x4ccb64`, and `0x4ccdfc` construct, split, and finalize 0x24-byte polygon nodes whose finalized coordinates live at node `+0x1c/+0x20`. The imported template catalog carries source zone/link grammar and provenance, but not the executable polygon-tree payload needed to feed those helpers. The boundary therefore records `runtime_polygon_layout_partially_recovered_catalog_payload_missing` and still refuses to materialize terrain cells.
