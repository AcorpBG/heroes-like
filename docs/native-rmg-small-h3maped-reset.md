...
The clean module now ports the `0x4a3a03` per-level footprint phase boundary through the `0x4a2777` boundary traversal for inspection only. For the seed `1` boundary case it reports one level, 6 same-level runtime zones, 6 `0x4ccb64` polygon split calls, no synthetic fallback zone for one-level land, `0x4cc788` initial bounds `-200..400`, `0x4ccdfc` finalized node fanout, 6 `0x4cca55` source-node walks for `0x4a2777`, and `0x4a2777` real source-node boundary traversal with 238 unique boundary cells. Actual zone interiors are still not painted because `0x4a325d` span fill and `0x4a3710` adjacency finalization are still pending; this reset must not invent replacement footprint cells.
...
4. `zone_footprint_placement` - `0x4a3a03`, `0x4cc788`, `0x4cca55`, `0x4ccb64`, `0x4ccdfc`, `0x4a2777`, `0x4a325d`, `0x4a3710` - level collection, polygon source-node walks, and `0x4a2777` boundary traversal ported for inspection only; span fill and adjacency finalization pending clean port.
