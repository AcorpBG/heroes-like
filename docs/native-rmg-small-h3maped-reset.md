...
2026-05-13 correction: the report-treadmill active implementation was archived out of the build to `src/gdextension/src/archived_h3maped_small_rmg_report_treadmill_20260513.cpp`. The compiled active module is now a small h3maped executable boundary with these internal state phases only: binary verification, 36x36 one-level land scope gate, recovered size/water score, h3maped RNG template selection, `0x4ac62a..0x4ac6ec` player-slot assignment, `0x4a218c` / `0x49b452` runtime-zone records from the adapted project template, `0x4a1f3b` link-seed setup, one-level `0x4a17f5` / `0x4a1701` / `0x4a1ad8` / `0x4a19ed` coordinate replay, private `0x4a3a03` zone footprints through `0x4cc788` / `0x4ccb64` / `0x4ccdfc` / `0x4a2777` / `0x4a325d` / `0x4a3710`, private `0x49b53d` runtime terrain selection, private `0x4a3f27` terrain cell writeout over the real zone-word buffer, private `0x4bcff5` TerrainPlacement visual-table/toolkit decode from `/root/Downloads/h3maped.exe`, private `0x4bb74b`/`0x4bc5f0` live TerrainPlacement feedback, private `0x49b2b6` terrain tile-byte candidates, private `0x4a8d2c`/`0x4a8db2`/`0x4a93a2` town/castle candidates, private `0x4a9d6a`/`0x4aab7e` mine/reward object-vector prerequisite state, explicit phase backlog, and runtime generation refusal.
...
The active module must not expose `small_generation_state`, `private_generation_context`, private town/object ledgers, or partial package payloads. Player slots, runtime zones, template link seeds, coordinate replay, private zone footprint/cell ownership buffers, private terrain cell writeout, decoded TerrainPlacement visual tables, live TerrainPlacement feedback, tile-byte candidates, town/castle candidates, and mine/reward object-vector prerequisites are active non-public internal state. Private mine coordinate records now exist for the small scope, but private reward coordinate filter evaluation, generated-cell object mutation, roads, blockers, guards, public package adoption, and final writeout are pending runtime ports.

Seed `1`, one human, three total players currently selects `h3maped_template_018` -> `translated_rmg_template_019_v1`, assigns source owners `0/1/2` to player colors `0/1/2`, leaves source owner `3` unassigned, builds six runtime-zone records with actual owner colors `[0, 1, -1, 2, -1, -1]`, preserves five template link seeds with guard values `[3000, 3000, 3000, 6000, 6000]`, replays 18 coordinate-placement steps to scaled zone centers `(23,11)`, `(21,22)`, `(12,23)`, `(18,4)`, `(18,30)`, and `(12,11)`, then builds private source-node footprint state with six zone walks, 301 boundary writes, 238 unique boundary cells, 869 span-filled cells, 1107 boundary-or-filled cells, 189 remaining unassigned cells, and owner cell counts `177/91/226/177/207/229`. It then runs `0x49b53d` terrain selection to h3maped terrain ids `[2,0,7,7,4,5]` / project terrains `[grass,dirt,lava,lava,swamp,rough]`, with two terrain RNG calls from `255755822` to `2166683160`, writes private terrain counts water `189`, grass `177`, dirt `91`, lava `403`, swamp `207`, rough `229`, decodes five TerrainPlacement visual tables at `0x543108`, `0x543380`, `0x5434f0`, `0x5435b0`, and `0x542f88` totaling 230 rows, projects three private owned player castle candidates synchronized to starts at `(26,16)`, `(23,23)`, and `(18,5)`, and materializes the private object-vector prerequisite boundary with 704 single-level candidate-vector records, 118 CRTRAITS-backed monster candidates, 18 total mine minimum/density weights, 18 private mine coordinate records, partial coordinate-vector count `21`, 18 eligible reward bands, six reward scheduler zones, 18 first-cycle reward value previews, 18 reward preview RNG calls, 18 private `0x4aa1db`/`0x4a9f1c` reward object lookups, 18 selected proxy-backed reward candidates, 18 reward object lookup RNG calls, candidate scan source `proxy_backed_recovered_static_candidates_from_0x49f95a`, and land budget argument total `300`. This is still not map output; reward coordinate filter/mutation, roads, guards, blockers, package adoption, and final writeout remain blocked.
...
The active module no longer exposes the old top-level terrain art, blocker, guard, final-writeout records, or the private phase ledger. The active generation state beyond template selection is limited to player-slot assignment, runtime-zone records, link seeds, coordinate replay, private zone-footprint/cell ownership buffers through `0x4a3710`, private terrain cell writeout through `0x49b53d` / `0x4a3f27`, decoded TerrainPlacement visual tables through `0x4bcff5`, live TerrainPlacement feedback through `0x4bb74b`/`0x4bc5f0`, terrain tile-byte candidates through `0x49b2b6`, town/castle candidates through `0x4a8d2c`/`0x4a93a2`, and mine/reward object-vector prerequisites through `0x4a9d6a`/`0x4aab7e`/`0x4a9f1c`. It materializes no runtime players, public map cells, package tiles, public roads, rivers, public objects, or public output. Mine/reward coordinate filter/mutation, road/river, blocker/guard, and final writeout work must be reintroduced as actual runtime generator implementation derived from `h3maped.exe`, not as broad inspection-ledger expansion.
...
3. Runtime-zone records: `0x4a218c` / `0x49b452` (active non-public generation state; no public map-cell materialization). Town selector `0x49b3c1` is consumed for coordinate-replay town-choice RNG.
...
5. Terrain writeout and TerrainPlacement: `0x49b53d` runtime terrain selection, `0x4a3f27` private terrain cell writeout, and `0x4bcff5` visual table/toolkit decode are active non-public state. `0x4bb74b`, `0x4bc5f0`, and `0x49b2b6` live TerrainPlacement/tile-byte work remain pending in the active compiled restart. Package output remains blocked until terrain art adoption, town, road, object, guard, and final-writeout phases are executable-derived.
...
7. Mines, rewards, and object-vector producers: `0x4a9d6a`, `0x4a9911`, and `0x4a9641` now produce private mine coordinate records for the small scope, and `0x4a9c7c`, `0x4aab7e`, `0x4aa354`, `0x4a9f1c`, and `0x4aa9b7` are active as non-public prerequisite/scheduler boundaries. Reward coordinate commits, generated-cell mutation, object adoption, guards, and public package output remain pending.
