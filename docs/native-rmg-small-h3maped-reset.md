...
The active module must not expose `active_generation_state`, `small_generation_state`, `private_generation_context`, private town/object ledgers, or partial package payloads. Previous private terrain, town, mine, reward, road, blocker, and guard ledgers are archived evidence only; they are not accepted as active generation progress. The next active implementation step is the narrow executable port of `0x4a2777` / `0x4a325d` boundary and span fill over the source-node output.

Seed `1`, one human, three total players currently selects `h3maped_template_018` -> `translated_rmg_template_019_v1` through h3maped RNG first value `41` and selected vector index `2`, fills `generator+0xee0/+0xee4` assignment/mapping slots as `[0, 1, 2, -1, -1, -1, -1, -1]`, builds six runtime-zone records with owner colors `[0, 1, -1, 2, -1, -1]`, builds five link seeds with guard values `[3000, 3000, 3000, 6000, 6000]`, replays coordinates to scaled zone centers `(23,11)`, `(21,22)`, `(12,23)`, `(18,4)`, `(18,30)`, and `(12,11)`, then builds strict source-node geometry with six split calls, 23 active source-node pairs, 14 finalized triplets, 42 finalized nodes, and six source-node walks. Boundary/span fill, terrain, towns, mines, rewards, roads, guards, blockers, package adoption, and final writeout remain pending executable ports.
...
The active module no longer exposes the old top-level terrain art, blocker, guard, final-writeout records, or the private phase ledger. The active generation state beyond template selection is limited to player-slot assignment, runtime-zone records, link seeds, coordinate replay, and source-node geometry. It materializes no boundary trace, span fill, terrain, runtime players, public map cells, package tiles, public roads, rivers, public objects, or public output. Boundary/span fill, terrain, town, mine/reward, road/river, blocker/guard, and final writeout work must be reintroduced as actual runtime generator implementation derived from `h3maped.exe`, not as broad inspection-ledger expansion.
...
4. Coordinate replay and zone-footprint phase: `0x4a1f3b`, `0x4a17f5`, `0x4a1701`, `0x4a1ad8`, and `0x4a19ed` are active through coordinate replay and bbox rescale. `0x4a3a03`, `0x4cc788`, `0x4cc955`, `0x4ccb64`, and `0x4ccdfc` are active through source-node setup only. `0x4a2777`, `0x4a2b33`, `0x4a261a`, `0x4a2413`, `0x4a325d`, and small-land `0x4a3710` remain pending.
