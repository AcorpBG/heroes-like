...
The clean module now also rebinds the selected template's mine and treasure fields from the recovered h3maped source catalog because the adapted project catalog currently drops those fields as null. Phase `0x4a9d6a` mine minimum/density source fields and phase `0x4aab7e` treasure-band triplets are ported as source ledgers for inspection: seed `1` reports 6 bound source rows, 42 mine minimum fields, 42 mine density fields, 18 positive mine minimum fields, 18 positive mine density fields, total minimum mine count 18, total mine density weight 18, 18 eligible treasure bands, and total treasure density weight 96. Physical mine/resource/reward placement through `0x4a9911`, `0x4a9c7c`, `0x4a9641`, and `0x4aa354` remains pending.
...
7. `guard_reward_monster_placement` - `0x4a9d6a`, `0x4aab7e` - mine/reward source field ledgers ported for inspection; `0x4a9911`, `0x4a9c7c`, `0x4a9641`, `0x4aa354`, object placement, and guarding still pending.
