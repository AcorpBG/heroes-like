...
- ports late connection payload and same-level dispatch evidence from `0x4a79a3`, `0x4a61bc`, `0x4a696b`, `0x4a6cf2`, `0x4a7605`, `0x4a65a5`, `0x4a5e03`, `0x4a5e73`, and `0x4a5a23`; this marks five unique links and ten reciprocal records, scales guard values through the recovered strength helper, records that `0x4a6cf2` returns false for all current one-level links, identifies `0x4a61bc`/`0x4a696b` as the same-level endpoint helpers, exposes the low/high owner-byte precondition they need, and now ports the `0x49a318` direction-bit handoff plus `0x4a79a3` transition-vector candidate scan as inspection-only evidence, but does not materialize road/river geometry, endpoint objects, or guards;
...
The late connection payload report for the same seed consumes the five `0x4a1f3b` link seeds through the recovered `0x4a79a3` postprocessor. It reports five unique links, ten reciprocal link records, ten processed-marker writes at `+0x0a`, raw guard values `[3000,3000,3000,6000,6000]`, scaled `0x4a65a5` guard values `[2000,2000,2000,5000,5000]`, zero `Wide` suppressions, and zero `Border Guard` special links. The corrected `0x4a6cf2` report records the `0x4a6d3d..0x4a6d40` same-level return-false gate for all five current one-level links, with zero overlap cells and zero endpoint coordinates. The `0x4a79a3` dispatch report records first-pass helpers `0x4a61bc`, `0x4a696b`, `0x4a6cf2`, second-pass helpers `0x4a696b`, `0x4a7605` if still unprocessed, five same-level links ready for `0x4a61bc`/`0x4a696b`, and zero endpoint, road, or guard materialization. The same-level owner-channel report now carries `cell+0x20` low owner bytes from the real `0x4a325d` zone words and high owner bytes from the `0x4a5767`/`0x49a318` propagation path. For seed `1`, low owner bytes cover 1107 cells with counts `0:177`, `1:91`, `2:226`, `3:177`, `4:207`, `5:229`; high owner bytes cover 1107 cells with counts `0:173`, `1:162`, `2:200`, `3:262`, `4:134`, `5:176`, leaving 189 sentinel cells. The clean port now also writes the recovered `0x49a318` materialized bit25 and direction bits 12..14 into the private generated-cell `+0x28` word, then runs the `0x4a79a3` transition-vector gate over the private grid. Seed `1` reports 324 transition-vector candidates, 211 directional candidates across the five template links, and four links with directional candidates; link `4-5` still has zero `0x4a61bc` directional candidates and remains dependent on the later fallback/object placement path. Remaining blockers are the extra `0x49a318` bit22/object metadata branch and the actual endpoint write/guard success paths in `0x4a61bc`/`0x4a696b`/`0x4a7605`. Runtime road geometry, endpoint object stamping, guard placement, blockers, mines, rewards, and final package writeout remain blocked.

The active module materializes private inspection-only zone boundary buffers for `0x4a2777`, private inspection-only span-fill buffers for `0x4a325d`, private `0x4a3710` small-land finalizer scheduling evidence, private `0x4a3f27` terrain/cell word evidence, private TerrainPlacement static visual-table evidence, private town/start placement candidates, and private late connection payload/same-level helper dispatch evidence. It still does not materialize terrain art, runtime players, public towns, roads, blockers, guards, mines, rewards, public runtime grids, or packages.
