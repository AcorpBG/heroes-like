...
The active port does not materialize public runtime map packages, terrain art, towns, roads, blockers, guards, mines, rewards, or final map packages. Coordinate, terrain selection, terrain/cell writeout, terrain byte-zero/grid inspection, generated-grid final-sweep boundary counters, footprint scheduling, the small-land no-appended-zone finalizer boundary, standalone boundary/span helper primitives, standalone `0x4a2777` branch and wrapping samples, and standalone `0x4a2413` randomized writer coverage are exposed as inspection data only and are not package generation.
...
1. TerrainPlacement classifier, row selection, scratch/writeback, and queue normalization over the inspected `0x4a3f27` generated terrain grid, followed by art/index/flip byte selection only after those h3maped phases are ported.
