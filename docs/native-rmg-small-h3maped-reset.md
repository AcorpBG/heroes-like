...
The same terrain-fill boundary now exposes the terrain portion of final generated-cell writeout from `0x49b2b6`. For each of the 1296 small-map cells, it packs generated-cell terrain id bits into tile byte 0, terrain art bits into tile byte 1, and terrain flip bits into tile byte 6 bits 0..1. Road and river overlay bytes remain pending and are not synthesized.
