...
10a. Wires the real `0x4a2777` boundary buffer into the ported `0x4a325d` span fill for inspection. Seed `1` fills five of six runtime zones directly, reports 706 filled interior cells, 927 total boundary-or-filled cells, 369 remaining unassigned cells, and exposes the real blocker that runtime zone 2 reaches a non-unassigned boundary cell. The recovered `0x4a32b2..0x4a338e` relocation branch is ported as an out-of-bounds-seed branch and is not applicable to that in-bounds boundary hit.
...
1. Resolve the remaining in-bounds non-unassigned runtime-zone seed case from executable evidence; do not substitute a nearest-open-cell repair.
2. Terrain/cell writeout from the real `0x4a325d` zone-word buffer, then owned town placement, roads, guards, blockers, mines, rewards, and final package adoption.
