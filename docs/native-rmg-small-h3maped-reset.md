...
10. Ports the top-level zone-footprint schedule `0x4a3a03` for inspection: one land level collects six runtime zones, records the `0x4cc788` initial source-node rectangle `-200..400`, materializes the first `0x4cca55` locator/duplicate-endpoint guard and first `0x4ccb64` pre-crossing insertion/bridge loop against that graph, records the remaining `0x4ccb64` split-insertion gate, records the `0x4ccdfc` finalized-coordinate gate through `0x4ccd69`, skips the synthetic fallback zone, and records the helper sequence `0x4a2777 -> 0x4a325d -> 0x4a3710`.
...
1. Materialize `0x4ccc7a`/`0x4cc68e` crossing cleanup after the first pre-crossing insertion, continue later `0x4cca55`/`0x4ccb64` source-node graph updates against the mutated graph, materialize `0x4ccdfc` finalized source-node cycles, then assemble real `0x4a2777` boundary traversal from those cycles plus the ported `0x4a2b33`/`0x4a261a`/`0x4a2413` helpers, then wire its real boundary buffer into the ported `0x4a325d` span-fill primitive. Standalone rectangle/connector/writer samples are not sufficient for runtime generation.
