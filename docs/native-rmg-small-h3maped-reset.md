...
The active module no longer exposes the old top-level terrain art, blocker, guard, final-writeout records, or the private phase ledger. The active generation state beyond template selection is limited to player-slot assignment, runtime-zone records, link seeds, coordinate replay, source-node geometry, and private boundary/span-fill buffers. It materializes no terrain, runtime players, public map cells, package tiles, public roads, rivers, public objects, or public output. Footprint finalizer, terrain, town, mine/reward, road/river, blocker/guard, and final writeout work must be reintroduced as actual runtime generator implementation derived from `h3maped.exe`, not as broad inspection-ledger expansion.
