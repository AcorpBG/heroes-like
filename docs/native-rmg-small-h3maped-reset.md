...
8. Roads and rivers: `0x4ab52a` and `0x4aae7b` are active for private coordinate-vector pair costs using town/castle `generator+0x14b0` records, and accepted predecessor chains now materialize private road overlay bytes through the recovered `0x458a2f` / `0x458893` art/flip classifier and `0x49b2b6` road byte staging. Public package adoption, rivers, connection blockers, guards, and final writeout remain blocked.
