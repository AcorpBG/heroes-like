...
The standalone battle difficulty sweep now proves that the current normal launch row is clear, the hard launch row stays within a bounded report-only watch budget, and the two rows still differ. This slice makes that evidence part of the shared headless simulation harness so routine headless balance runs catch difficulty-effect and queue regressions.
...
- `tests/headless_simulation_harness_report.gd` asserts normal and hard rows stay report-only `clear` or `watch`, carry queue signatures, pass combat-feel and balance-matrix gates, use default sample breadth, and keep `no_observed_effect: false`.
...
- Expected current `sweep_signature`: `4361f7f7`
- Current headless case signature: `6e40b418`
- Current headless harness signature: `fb4b3efd`
- Expected current normal queue signature: `829808c9`
- Expected current hard queue signature: `c286b819`
- Expected current normal queue item count: `0`
- Expected current hard queue item count: `3`
- Expected current normal `tuning_queue_status: clear`
- Expected current hard `tuning_queue_status: watch`
- Expected current normal `tuning_queue_item_count: 0`
- Expected current hard `tuning_queue_item_count: 3`
- Expected current `no_observed_effect: false`

## Boundaries

- No runtime encounter retuning.
