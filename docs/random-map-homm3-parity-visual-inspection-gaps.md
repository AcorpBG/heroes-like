...
Latest focused cheap-gate run, 2026-05-04 after the `6c14f35` large-layout-quality follow-up:
- Visual inspection report passed in 57144 ms.
...
- Runtime summary: 5/5 strict cases passed the strict per-case budget; the `translated_rmg_template_002_v1` diagnostic probe ran in 19492 ms, under its explicit 24000 ms diagnostic cap, with 185.638 ms per route and a recorded note that it exceeds the strict 18000 ms fixture budget.
- Layout-quality summary: the visual matrix now exposes source-backed fairness warning counts and route/resource/guard spread metrics. Current cheap coverage has 18 total fairness warnings, 8 fail-threshold warnings, max contest-route spread 40/66, max contest-guard pressure spread 18000, max route-guard pressure spread 500, and max town-to-resource spread 38. These are diagnostic layout-quality signals, not rendered-art comparison.
...
Latest large-template diagnostic run, 2026-05-04 after `6c14f35` large-layout-quality follow-up:
- Large visual inspection report passed in 41479 ms with one bounded diagnostic case.
...
- Layout-quality metrics: fairness status remains warning, with 4 total fairness warnings and 4 fail-threshold warnings. The matrix now exposes contest-route distance spread 39/56, contest-guard pressure spread 3000, and town-to-resource route spread 42. These counters come from the existing source-backed resource/encounter fairness report; this slice does not change route/pathing/zone/object generation semantics.
- Runtime metrics: generation/inspection case time 41221 ms, report time 41479 ms, 438.521 ms per route segment. This exceeds the strict 18000 ms cheap-gate fixture budget but stays under the explicit 90000 ms large diagnostic budget.
- Outcome: the next highest-value gap is no longer just runtime or ASCII-vs-rendered evidence. The current 108x108 template 042 seed has strong road/guard/decor/richness counters but visible layout fairness spread defects, so a later correction should reduce route/resource/guard spread before strict promotion.
