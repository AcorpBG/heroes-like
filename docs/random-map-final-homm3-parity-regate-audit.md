...
Status: final re-gate report; RMG parity gate met with accepted original-game translations.
Date: 2026-04-29.
Slice: `random-map-final-homm3-parity-regate-audit-10184`.

## Decision

HoMM3-style random map generation parity, translated into AcOrP's original systems, is **met for the Phase 2 RMG parity gate**.
...
- `random_map_player_setup_retry_ux_report.tscn`: passed; setup exposes seed/template/profile/player/water/underground controls and bounded visible retry failure before launch.
...
| Player-facing setup and retry UX | Implemented | Main-menu skirmish setup exposes generated-map controls, valid launch handoff, visible validation failure/retry exhaustion, and disabled launch on invalid generated setup without turning the menu into a report dashboard. |
...
1. Oversized source-template requirements: AcOrP currently caps headless materialized generated maps at `64x48x2`. Downscaling templates whose source minimum exceeds that cap would create false evidence. The translated records are preserved, but they are not required to materialize under the current original runtime size cap.
...
The RMG parity queue can close for Phase 2. The correct status is: HoMM3-style random-map-generation functional parity is met in AcOrP's original systems for generated skirmish/session use, with explicit non-parity boundaries for oversized templates, disconnected source graphs, campaign/writeback exclusion, save-version policy, and creative originality.
