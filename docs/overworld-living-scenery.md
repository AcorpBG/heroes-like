...
## Owner visibility correction, 2026-09-12

The original sub-one-percent displacement and squared canopy falloff looked effectively static in the delivered clips. Canopy amplitudes are now 0.032/0.024 of original image width (previously 0.009/0.006), with smooth rather than squared height falloff and a 1.45-radian/second primary breeze (previously 0.85). Painted smoke/water displacement increases four to five times; fire and water luminance variation is stronger but remains restricted to their existing painted pixels. Roots, rocks and masonry remain fixed. Transparent draw margins now derive from maximum displacement in screen pixels plus a two-pixel safety margin instead of a fixed two pixels.

Evidence under `.artifacts/overworld_living_scenery_20260912/`: `stronger-small` passes 445 source checks; `stronger-large` passes 516 with the real 108x108 seed-10 generated map; `stronger-linux` passes 445 in the isolated Linux package. The subsequently added fourteen displacement-margin assertions pass in `stronger-reduced` (423 checks) and `stronger-windows` (407 checks). All preserve fog, original disabled-mode art, grounding, building rigidity and simulation/save state. Large viewport: eight animated trees, 17 cached batches, 297 microseconds for 120 idle ticks without cache rebuilding; five deliberate state rebuilds total 77601 microseconds. These remain CPU component samples, not a frame-rate promise.

Official `.artifacts/packaging_linux_export_smoke/report.json` and `.artifacts/packaging_windows_export_smoke/report.json` pass, including Windows generated Overworld/Town entry and construction. Both packages contain 5605 members and are 313217288 bytes; only `project.binary` differs. Manifest, shader and compiled batch owner match across platforms. Windows/Wine is headless, not physical GPU certification.

Inspected small-window gallery and normal-zoom Large gameplay keyframes. `stronger-map.mp4` is 1920x1080; `stronger-assets.mp4` is a 1280x720 inspection gallery after the settings test applies the smaller window. Both contain 96 actual shader-clock samples at 24 fps, four seconds at normal animation speed. Delivered through OpenClaw to Discord `#proca-chat`, media receipts `1548394798346670090` and `1548394797906010112`. No extra motion is added by encoding. Retained clips, five keyframes per sequence, static/original pixel controls, reports and final packages. Cleanup removed 182 rebuildable encoded PNG intermediates (171973814 bytes); managed Wine wrappers removed disposable prefixes and retained user data. Caches and unrelated files are untouched.

The same gameplay viewport's mean RGB difference across retained one-second keyframes increases from 0.03194 to 0.16006 on the 0–255 scale, about five times the previous motion signal; this complements visual inspection rather than substituting for it. The first full repository process was terminated with exit 143 and no report by the command runner. The managed `heroes-scenery-stronger-repo-20260912.service` retry completed the full scan and reported exactly one error: the status helper had retained the old `completedAt` timestamp when reopening the slice. Removed that stale timestamp and reran `validate_progress_tracker_state` and `validate_overworld_scenery_animation`: both pass. Full-scan log: `/tmp/heroes-scenery-stronger-repo-complete-20260912.log`. No runtime/content check failed in that scan; it is not misreported as a zero-exit full run. `git diff --check` passes. Visibility correction completed, with the rest of the original scope and limitations unchanged.
