...
produce fresh packages, but test/report/export control stays in Python. The
Python exporter invokes a native GDExtension runner scene with no GDScript
launcher.
...
diagnostics, and export/test/report orchestration. Godot/GDExtension owns native
generation and package serialization execution when a fresh `.amap` must be
created.
