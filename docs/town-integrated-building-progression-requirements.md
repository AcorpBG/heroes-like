...
- Pointer ownership follows raster alpha and the shared cover crop for both scene-matched layers and retained catalog paintings. Empty margins must pass clicks to visible art behind them; actual foreground pixels retain depth priority. Keyboard/controller focus keeps the full accessible button bounds. The approved full-match presentation continuation must validate mixed migrated/catalog compositions, not only all-migrated towns.
