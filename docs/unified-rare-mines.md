...
Dedicated rare mines produce one unit of their rare resource daily while controlled, with no gold income. Capturing one awards one unit of its rare resource and no gold. This applies to all nine compatible site aliases, including existing saves after reload; gold already earned is retained. Capture rules and output timing are unchanged. Existing saves/maps gain the new appearance and game-level footprint when loaded; no regeneration is required for the artwork. Historical placements are not moved, so older tightly packed maps may not have reserved all six cells. Native placement rules and source/package masks remain immutable; this is an explicit owner-selected game geometry override, not a native parity change.
...
Owner income revision, 2026-09-20: removed daily and capture gold from all nine aliases. The revised rare-mine regression passes 333 checks; no full suite was run.
