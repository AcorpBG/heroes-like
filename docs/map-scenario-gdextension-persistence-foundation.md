...
- Generated package filenames use a shared deterministic lowercase kebab stem for the paired `.amap`/`.ascenario` files. The stem shape is `size-creative-name-hash`: the user-facing size token, a deterministic creative name derived from normalized seed/config, and an 8-hex deterministic hash suffix derived from normalized seed/config identity. Template/profile/player-count/water-mode/dimensions/seed details stay in package metadata and refs instead of the filename.
...
2026-05-06 correction: `native-rmg-full-parity-gate-10184` now acts as a
production-claim boundary gate for the current comparison fixture scope:
36x36 `homm3_small` maps using
...
For those scoped structural profiles, native C++ output reports
`status: scoped_structural_profile_supported`, keeps
`native_runtime_authoritative: false`, and keeps `full_parity_claim: false`
while preserving the feature gate and leaving runtime call-site adoption false.
