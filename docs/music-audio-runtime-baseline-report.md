...
`tests/music_audio_runtime_report.tscn` proves exact manifest and imported asset coverage for all thirty-three layers, six direct faction routes plus generic/unknown fallback, 44.1 kHz stereo eight-second imports, forward-loop metadata, all three players still active after a full segment, stable non-restart behavior, changed-context replacement, generated fallback, bus selection, player cap exposure, a live shell route through `MainMenu`, and all six real faction `TownShell` routes at 1280x720 and 1920x1080. Repository validation separately checks 16-bit source PCM, bounded peaks, non-silent distinct channels, exact loop boundaries, unique hashes, and deterministic generation.
