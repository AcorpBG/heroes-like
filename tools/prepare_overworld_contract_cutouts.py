#!/usr/bin/env python3
"""Recover 52 contract/expedition paintings with unchanged logical registrations.
...
locked. Nine historical families used different fit/alignment/filtering rules.
"""
import argparse
import hashlib
...
PACKET = ROOT/'art/overworld/source/generated/cutout_recovery_20260909/contract_encounters'
...
spec = importlib.util.spec_from_file_location('contract_original_projection', SHARED)
shared = importlib.util.module_from_spec(spec); spec.loader.exec_module(shared)
base = shared.base; owner = shared.owner; project = shared.recover
FAMILIES = {'dissident_fronts': 6, 'standalone_contracts': 6, 'outer_reach_contracts': 6, 'mire_sun_contracts': 4, 'ascendant_companies': 6, 'waywatch_trials': 6, 'spellwright_expeditions': 6, 'ritual_relay_circuits': 6, 'grand_convergence_marches': 6}
...
def family_rule(row):
    ...
    return (44 if family in ('dissident_fronts','waywatch_trials','spellwright_expeditions','ritual_relay_circuits') else 42,
            'center' if family in ('waywatch_trials','ritual_relay_circuits','grand_convergence_marches') else 'bottom')
...
def registration(source, row, old):
    ...
    # Original manifests fix fit and anchor; historical raster filters differ.
    # This bounded comparison is registration evidence, not pixel equality.
    if row['historical_registration']!='same_fit_cross_filter' or result['alpha_mae']>5 or result['opaque_rgb_mae']>9:
...
def inputs():
    ...
    paths = {f'res://art/overworld/runtime/objects/encounters/{f}/{f}_atlas.png' for f in FAMILIES}
    if recipe.get('schema_id')!='contract_encounter_cutout_recipe_v1' or len(recipe['assets'])!=52 or set(recipe['atlases'])!=paths or recipe['preserved_controls']:
    ...
    if set(recipe['identity_mappings'])!={r['original_manifest_entry']['assigned_encounter_id'] for r in recipe['assets'].values()}:
        raise ValueError('Incomplete encounter coverage')
    if any(manifest['encounter_identity_sprites'][k]!=v for k,v in recipe['identity_mappings'].items()):
        raise ValueError('Identity mapping changed')
    ...
    for key, row in recipe['assets'].items():
        ...
        trim = ROOT/'art/overworld/source/trimmed/cutout_recovery_20260909/contract_encounters'/(key+'.png')
...
def prepare(output, install=False):
    ...
    for page,start in enumerate(range(0,52,6)):
    ...
    proof=dict(schema_id='contract_encounter_cutout_recovery_v1',recipe_sha256=base.digest(RECIPE),tools=tool_hashes(),assets=rows,files=files,
               processing='Original alpha and opaque paint unchanged. Only saturated near-transparent alpha 1..4 RGB is replaced by nearest original opaque paint. Historical family-specific 42/44px centered or bottom alignment retained and projected directly at fourfold density, not upscaled from 48px. No alpha/color key, invented state marker or gameplay changes.')
    if install:
...
def validate_assets():
    ...
    if proof['schema_id']!='contract_encounter_cutout_recovery_v1' or proof['recipe_sha256']!=base.digest(RECIPE) or proof['tools']!=tool_hashes():raise ValueError('Preparation provenance changed')
