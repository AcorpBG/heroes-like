...
FUNCTIONS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "remaining_gap": "Runtime ordered replay is still pending for the 0x4a746b/0x4a5e73 delegated endpoint-writer path in a sample where [ESI+0x09] != 0 or another branch reaches 0x4a746b, 0x4a5e03 outcomes, generated-cell before/after state, full candidate-vector contents around 0x4a7312 appends/RNG, and exact source/relation/vector-entry semantic names.",
    },
    {
        "address": "0x49b3fb",
    ...
    },
    {
        ...
        "remaining_gap": "Continue from the runtime boundary summary: the sampled 0x4a7605 ESI record is not the generator+0xc8 header itself, and no post-consumer +0x09 write occurs. Do not treat 0x4b3c03 as the semantic producer of connection_recipe.endpoint_stamping_enabled without new evidence. The next target is the allocator or vector append path that fills the edge/control-record +0x08 dword before 0x4a79a3 selects the pair.",
    },
    {
        "address": "0x4a7dcd",
        "name": "connection_edge_control_record_pair_selector",
        "status": "partial_runtime_boundary_checkpoint_producer_pending",
        "reads": [
        ...
        "runtime_trace": "Boundary summary .artifacts/rmg_recovery/connection_record_runtime_boundary_summary.json proves the sampled control byte is already initialized before 0x4a7605, the consumed ESI record is separate from the generator+0xc8 header, and 0x4b3c4e/0x4b3d3c do not fire before the sampled fallback consumer.",
        "remaining_gap": "Recover the owner and population path for the edge/control-record block iterated by 0x4a79a3 before 0x4a7dcd/0x4a7dd0 selects a pair. The next live checkpoint should snapshot the iterator records and break on the vector append or allocator that fills +0x08.",
    },
    {
        "address": "0x49cf34",
