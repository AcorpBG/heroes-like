...
def render(catalog_path: Path, catalog_json_path: Path, metadata_path: Path, msk_dirs: list[Path], rows: list[dict[str, str]], metadata_bucket_by_type: dict[int, int]) -> str:
...
def main() -> int:
    ...
    args.out.write_text(render(args.catalog, args.catalog_json, args.metadata, msk_dirs, rows, metadata_bucket_by_type), encoding="utf-8")
    return 0
