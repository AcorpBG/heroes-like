...
def load_ledger(trace_log: Path, ledger_path: Path | None) -> tuple[dict[str, Any], str]:
    if ledger_path is not None:
        return json.loads(ledger_path.read_text(encoding="utf-8")), str(ledger_path)
    return parse_winedbg_log(trace_log), str(trace_log)


def summarize(log_path: Path, ledger_path: Path | None = None) -> dict[str, Any]:
    ledger, source = load_ledger(log_path, ledger_path)
    ...
    return {
        ...
        "source_ledger": source if ledger_path is not None else None,
