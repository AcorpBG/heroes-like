...
Dictionary h3maped_repaint_order_queue_seed_report(const std::vector<uint32_t> &zone_words, const Array &selected_terrain_codes, const PackedInt32Array &final_terrain_code_u16, int32_t width, int32_t height, int32_t level_count) {
	Dictionary report;
	report["status"] = "0x4a3f27_0x4bb74b_0x4bba59_repaint_order_queue_seed_projection";
	report["source"] = "replays the h3maped 0x4a3f27 water-then-zone repaint order on a copied terrain grid and records 0x4bba59 set-B candidates seeded by changed-cell updates";
	report["ported_addresses"] = Array::make("0x4a3f27", "0x4a4025", "0x4a4082", "0x4a415a", "0x4bb681", "0x4bb74b", "0x4bba59", "0x4bc5f0");
	report["materializes_queue_drain"] = false;
	report["materializes_package_tiles"] = false;
	report["adopts_into_runtime_grid"] = false;
	const int32_t level_tile_count = width * height;
	PackedInt32Array replay_grid;
	replay_grid.resize(int32_t(zone_words.size()));
	for (int32_t index = 0; index < replay_grid.size(); ++index) {
		replay_grid.set(index, 8);
	}
	std::set<int64_t> set_b_candidates;
	std::set<int64_t> set_a_candidates;
	Array candidate_samples;
	int32_t changed_cell_update_count = 0;
	auto append_candidate = [&](int32_t level, int32_t x, int32_t y, int32_t current_terrain, const char *source_branch) {
		if (x < 0 || y < 0 || x >= width || y >= height) {
			return;
		}
		const int32_t neighbor = terrain_at_grid_index(replay_grid, width, height, level_tile_count, level, x, y, current_terrain);
		if (neighbor == current_terrain) {
			return;
		}
		const int64_t key = h3maped_grid_key(level, x, y);
		set_b_candidates.insert(key);
		if (candidate_samples.size() < 12) {
			Dictionary sample;
			sample["x"] = x;
			sample["y"] = y;
			sample["level"] = level;
	...
	auto seed_4bba59 = [&](int32_t level, int32_t x, int32_t y, int32_t current_terrain) {
		append_candidate(level, x, y - 1, current_terrain, "0x4bba59_north_south_cardinal");
		append_candidate(level, x, y + 1, current_terrain, "0x4bba59_north_south_cardinal");
		append_candidate(level, x - 1, y, current_terrain, "0x4bba59_west_east_cardinal");
		append_candidate(level, x + 1, y, current_terrain, "0x4bba59_west_east_cardinal");
		...
		for (const auto &delta : diagonals) {
			...
			const int32_t neighbor = terrain_at_grid_index(replay_grid, width, height, level_tile_count, level, nx, ny, current_terrain);
			if (!h3maped_toolkit_byte5_allows_same_class_gate(neighbor)) {
				continue;
			}
			append_candidate(level, nx, ny, current_terrain, "0x4bba59_diagonal_byte5_zero_neighbor");
		}
	};
	auto seed_4bb74b_neighbor_branch = [&](int32_t level, int32_t x, int32_t y, int32_t current_terrain, bool horizontal_pair_wrapper, const char *source_branch) {
		...
		const bool gate = horizontal_pair_wrapper
				? h3maped_horizontal_pair_gate_4bc674(replay_grid, width, height, level_tile_count, level, x, y, neighbor)
				: h3maped_vertical_pair_gate_4bc6e0(replay_grid, width, height, level_tile_count, level, x, y, neighbor);
		if (!gate) {
			seed_4bba59(level, x, y, current_terrain);
			append_candidate(level, x, y, current_terrain, source_branch);
		}
	};
		...
		for (int32_t level = 0; level < level_count; ++level) {
			for (int32_t y = 0; y < height; ++y) {
				for (int32_t x = 0; x < width; ++x) {
					...
					if (set_terrain_at_grid_index(replay_grid, width, height, level_tile_count, level, x, y, terrain)) {
						changed_cell_update_count += 1;
						const int64_t key = h3maped_grid_key(level, x, y);
						if (h3maped_candidate_gate_4bc988_grid(replay_grid, width, height, level_tile_count, level, x, y)) {
							set_a_candidates.insert(key);
						} else {
	...
	int32_t missing_bucket_count = 0;
	int32_t missing_bucket_in_set_b_count = 0;
	int32_t missing_bucket_candidate_gate_true_count = 0;
	Array missing_bucket_queue_samples;
	for (int32_t level = 0; level < level_count; ++level) {
		for (int32_t y = 0; y < height; ++y) {
			for (int32_t x = 0; x < width; ++x) {
				const int32_t index = level * level_tile_count + y * width + x;
				...
				const int64_t key = h3maped_grid_key(level, x, y);
				const bool in_set_b = set_b_candidates.find(key) != set_b_candidates.end();
				const bool candidate_gate = h3maped_candidate_gate_4bc988_grid(final_terrain_code_u16, width, height, level_tile_count, level, x, y);
				if (in_set_b) {
					missing_bucket_in_set_b_count += 1;
				}
				if (candidate_gate) {
					missing_bucket_candidate_gate_true_count += 1;
				}
				if (missing_bucket_queue_samples.size() < 8) {
					Dictionary sample;
					sample["x"] = x;
					sample["y"] = y;
					sample["level"] = level;
					...
					sample["in_repaint_order_set_b"] = in_set_b;
					sample["candidate_gate_0x4bc988"] = candidate_gate;
					missing_bucket_queue_samples.append(sample);
				}
			}
		}
	}
	report["changed_cell_update_count"] = changed_cell_update_count;
	report["set_a_candidate_count"] = int32_t(set_a_candidates.size());
	report["set_b_candidate_count"] = int32_t(set_b_candidates.size());
	report["missing_bucket_cell_count"] = missing_bucket_count;
	report["missing_bucket_cells_in_repaint_order_set_b"] = missing_bucket_in_set_b_count;
	report["missing_bucket_set_b_coverage_complete"] = missing_bucket_count > 0 && missing_bucket_in_set_b_count == missing_bucket_count;
	report["missing_bucket_candidate_gate_true_count"] = missing_bucket_candidate_gate_true_count;
...
Dictionary h3maped_drained_scratch_feedback_projection_report(const PackedInt32Array &drained_terrain_code_u16, const Dictionary &drained_visual_projection, int32_t width, int32_t height, int32_t level_count) {
	Dictionary report;
	report["status"] = "0x4bad0f_drained_grid_scratch_word_projection_inspection_only";
	report["source"] = "projects the copied 0x4bc5f0 drained-grid visual records into full-grid 0x4bad0f scratch words and round-trips those words back to 0x49acf6 generated-cell fields without live queue feedback adoption";
	report["ported_addresses"] = Array::make("0x4bad0f", "0x49acf6", "0x49b2b6", "0x4bb74b", "0x4bcfc3", "0x4bce6d");
	report["scratch_write_address"] = "0x4bad0f";
	report["generated_cell_write_address"] = "0x49acf6";
	report["changed_cell_update_address"] = "0x4bb74b";
	report["visual_record_resolve_address"] = "0x4bcfc3";
	report["neighbor_mask_address"] = "0x4bce6d";
	report["scratch_word_contract"] = "bit0 dirty, bits1..4 terrain id, bits5..11 terrain art row, bit12 flag A, bit13 flag B";
	report["requires_live_feedback_sequencing"] = true;
	report["live_feedback_materialized"] = false;
	report["adopts_into_runtime_grid"] = false;
	report["materializes_package_tiles"] = false;
	...
	PackedInt32Array scratch_word_u16;
	PackedInt32Array roundtrip_cell_word_0x24_u32;
	PackedInt32Array roundtrip_cell_word_0x28_u32;
	scratch_word_u16.resize(tile_count);
	roundtrip_cell_word_0x24_u32.resize(tile_count);
	roundtrip_cell_word_0x28_u32.resize(tile_count);
	int32_t dirty_cell_count = 0;
	int32_t nonzero_art_cell_count = 0;
	int32_t terrain_flag_cell_count = 0;
	int32_t terrain_mismatch_count = 0;
	int32_t roundtrip_mismatch_count = 0;
	int32_t missing_projection_cell_count = 0;
	Array samples;
	for (int32_t index = 0; index < tile_count; ++index) {
		const bool has_projection = index < projected_cell_word_0x24_u32.size() && index < projected_cell_word_0x28_u32.size();
		const uint32_t source_word_0x24 = has_projection ? uint32_t(int32_t(projected_cell_word_0x24_u32[index])) : uint32_t(drained_terrain_code_u16[index] & 0x3f);
		const uint32_t source_word_0x28 = has_projection ? uint32_t(int32_t(projected_cell_word_0x28_u32[index])) : 0U;
		if (!has_projection) {
			missing_projection_cell_count += 1;
		}
		const int32_t terrain_id = int32_t(source_word_0x24 & 0x3fU);
		const int32_t art_id = int32_t((source_word_0x24 >> 6U) & 0xffU);
		const int32_t flag_a = int32_t((source_word_0x28 >> 15U) & 0x01U);
		const int32_t flag_b = int32_t((source_word_0x28 >> 16U) & 0x01U);
		const uint32_t scratch_word = h3maped_scratch_word_4bad0f(terrain_id, art_id, flag_a, flag_b);
		const int32_t scratch_terrain_id = int32_t((scratch_word >> 1U) & 0x0fU);
		const int32_t scratch_art_id = int32_t((scratch_word >> 5U) & 0x7fU);
		const int32_t scratch_flag_a = int32_t((scratch_word >> 12U) & 0x01U);
		const int32_t scratch_flag_b = int32_t((scratch_word >> 13U) & 0x01U);
		const uint32_t roundtrip_word_0x24 = (uint32_t(scratch_terrain_id) & 0x3fU) | ((uint32_t(scratch_art_id) & 0xffU) << 6U);
		const uint32_t roundtrip_word_0x28 = ((uint32_t(scratch_flag_a) & 0x01U) << 15U) | ((uint32_t(scratch_flag_b) & 0x01U) << 16U);
		scratch_word_u16.set(index, int32_t(scratch_word));
		roundtrip_cell_word_0x24_u32.set(index, int32_t(roundtrip_word_0x24));
		roundtrip_cell_word_0x28_u32.set(index, int32_t(roundtrip_word_0x28));
		...
		if (roundtrip_word_0x24 != source_word_0x24 || roundtrip_word_0x28 != (source_word_0x28 & (0x03U << 15U))) {
		...
		if (samples.size() < 8 && art_id != 0) {
			...
			sample["terrain_id"] = terrain_id;
			sample["terrain_art"] = art_id;
			sample["flag_a"] = flag_a;
			sample["flag_b"] = flag_b;
			sample["scratch_word_u16"] = int32_t(scratch_word);
			sample["roundtrip_cell_word_0x24_u32"] = int64_t(roundtrip_word_0x24);
			sample["roundtrip_cell_word_0x28_u32"] = int64_t(roundtrip_word_0x28);
	...
	report["level_count"] = level_count;
	report["tile_count"] = tile_count;
	report["scratch_word_count"] = scratch_word_u16.size();
	report["scratch_dirty_cell_count"] = dirty_cell_count;
	...
	report["missing_projection_cell_count"] = missing_projection_cell_count;
	report["scratch_roundtrip_mismatch_count"] = roundtrip_mismatch_count;
	report["scratch_word_u16"] = scratch_word_u16;
	report["roundtrip_cell_word_0x24_u32"] = roundtrip_cell_word_0x24_u32;
	report["roundtrip_cell_word_0x28_u32"] = roundtrip_cell_word_0x28_u32;
	report["sample_records"] = samples;
	report["blocked_next"] = "port live 0x4bb74b/0x4bcfc3/0x4bce6d scratch-neighbor feedback during the 0x4bc5f0 queue drain before runtime adoption";
	return report;
}
...
Dictionary h3maped_masked_visual_selection_projection_report(const PackedInt32Array &drained_terrain_code_u16, const Dictionary &drained_visual_projection, const Dictionary &scratch_neighbor_mask_projection, int32_t width, int32_t height, int32_t level_count, uint32_t rng_state_before_visual_selection) {
	Dictionary report;
	report["status"] = "0x4bcfc3_0x4bce6d_0x4ba938_masked_visual_selection_projection_inspection_only";
	report["source"] = "replays drained-grid visual selection with the recovered 0x4bce6d scratch-neighbor mask feeding 0x4bcfc3 / 0x4ba938; this measures the old terrain-only selector drift before live queue adoption";
	report["ported_addresses"] = Array::make("0x4bce6d", "0x4bcfc3", "0x4ba938", "0x4ba989", "0x4bad0f", "0x49acf6");
	report["uses_recovered_neighbor_mask"] = true;
	report["rmg_initial_neighbor_mask"] = 4;
	report["adopts_into_runtime_grid"] = false;
	report["materializes_package_tiles"] = false;
	...
	const PackedInt32Array neighbor_mask_u8 = scratch_neighbor_mask_projection.get("neighbor_mask_u8", PackedInt32Array());
	const PackedInt32Array baseline_word_0x24_u32 = drained_visual_projection.get("projected_cell_word_0x24_u32", PackedInt32Array());
	const PackedInt32Array baseline_word_0x28_u32 = drained_visual_projection.get("projected_cell_word_0x28_u32", PackedInt32Array());
	const int32_t level_tile_count = width * height;
	const int32_t tile_count = drained_terrain_code_u16.size();
	...
	projected_cell_word_0x24_u32.resize(tile_count);
	projected_cell_word_0x28_u32.resize(tile_count);
	projected_tile_byte_1_terrain_art_u8.resize(tile_count);
	projected_tile_byte_6_terrain_flags_u8.resize(tile_count);
	...
	int32_t full_native_masked_cell_count = 0;
	int32_t masked_selection_art_delta_count = 0;
	int32_t masked_selection_flag_delta_count = 0;
	int32_t terrain_art_nonzero_cell_count = 0;
	int32_t terrain_flag_cell_count = 0;
	int32_t missing_mask_cell_count = 0;
	for (int32_t level = 0; level < level_count; ++level) {
		for (int32_t y = 0; y < height; ++y) {
			for (int32_t x = 0; x < width; ++x) {
				const int32_t index = level * level_tile_count + y * width + x;
				...
				const int32_t terrain_id = drained_terrain_code_u16[index];
				const TerrainClassResult classified = classify_grid_cell(drained_terrain_code_u16, width, height, level_tile_count, level, x, y, terrain_id);
				const int32_t neighbor_mask = index < neighbor_mask_u8.size() ? int32_t(neighbor_mask_u8[index]) : 4;
				if (index >= neighbor_mask_u8.size()) {
					missing_mask_cell_count += 1;
				}
				int32_t selected_row = -1;
				int32_t out_flag_a = 0;
				int32_t out_flag_b = 0;
				...
				String selector_kind;
				const std::vector<TerrainVisualRow> &rows = visual_rows_for_terrain_id(tables, terrain_id);
				const bool selected = select_visual_row_for_grid_cell_with_neighbor_mask(rows, terrain_id, classified, neighbor_mask, rng, selected_row, out_flag_a, out_flag_b, selector_address, selector_kind, probability_threshold, probability_rng_value);
				const String terrain_key = String::num_int64(terrain_id);
				...
				if (classified.shape_class == 0) {
					full_native_masked_cell_count += 1;
				}
				if (!selected) {
				...
				const int32_t baseline_art = index < baseline_word_0x24_u32.size() ? int32_t((uint32_t(int32_t(baseline_word_0x24_u32[index])) >> 6U) & 0xffU) : -1;
				const int32_t baseline_flags = index < baseline_word_0x28_u32.size() ? int32_t((uint32_t(int32_t(baseline_word_0x28_u32[index])) >> 15U) & 0x03U) : -1;
				const int32_t projected_flags = int32_t((generated_cell_word_0x28 >> 15U) & 0x03U);
				const bool art_delta = selected_row != baseline_art;
				const bool flag_delta = projected_flags != baseline_flags;
				if (art_delta) {
					masked_selection_art_delta_count += 1;
				}
				if (flag_delta) {
					masked_selection_flag_delta_count += 1;
				}
				if (sample_records.size() < 12 && (classified.shape_class == 0 || art_delta || flag_delta)) {
					Dictionary sample;
					sample["index"] = index;
					sample["x"] = x;
					sample["y"] = y;
					...
					sample["probability_threshold"] = probability_threshold;
					sample["probability_rng_value"] = probability_rng_value;
					sample["baseline_art"] = baseline_art;
					sample["selected_row"] = selected_row;
					sample["baseline_flags"] = baseline_flags;
					sample["projected_flags"] = projected_flags;
					sample["art_delta"] = art_delta;
					sample["flag_delta"] = flag_delta;
	...
	report["full_native_masked_cell_count"] = full_native_masked_cell_count;
	report["masked_selection_art_delta_count"] = masked_selection_art_delta_count;
	report["masked_selection_flag_delta_count"] = masked_selection_flag_delta_count;
	report["terrain_art_nonzero_cell_count"] = terrain_art_nonzero_cell_count;
	report["terrain_flag_cell_count"] = terrain_flag_cell_count;
	report["missing_mask_cell_count"] = missing_mask_cell_count;
	...
	report["blocked_next"] = "move this mask-aware selection into the live 0x4bb74b/0x4bc5f0 scratch update sequence and then adopt 0x49acf6 words into tile bytes";
	return report;
}
...
Dictionary late_connection_overlap_geometry_report(const Array &runtime_zone_records, const Array &link_seeds, const Dictionary &terrain_cell_writeout) {
	...
	std::vector<RuntimeBounds> bounds;
	bounds.resize(size_t(runtime_zone_records.size()));
	for (int32_t level = 0; level < level_count; ++level) {
		for (int32_t y = 0; y < height; ++y) {
			for (int32_t x = 0; x < width; ++x) {
	...
	for (int64_t index = 0; index < runtime_zone_records.size(); ++index) {
		...
		item["source_zone_key"] = runtime_record.get("source_zone_key", "");
...
Dictionary town_castle_phase_schedule_report(const Dictionary &normalized_config, const Array &runtime_zone_records, const Dictionary &coordinate_replay, const Dictionary &terrain_selection, const Dictionary &terrain_cell_writeout) {
	...
	report["ported_addresses"] = Array::make("0x4a8d2c", "0x4a8db2", "0x4a93a2", "0x4a901a", "0x49b3c1", "0x49ba89", "0x540a9c");
	report["source_zone_field_map"] = Array::make(
			"+0x20 player minimum towns",
			"+0x24 player minimum castles",
			"+0x28 player town density",
			"+0x2c player castle density",
			"+0x30 neutral minimum towns",
			"+0x34 neutral minimum castles",
			"+0x38 neutral town density",
			"+0x3c neutral castle density",
			"+0x40 towns are of same type");
	report["same_town_type_scope"] = "source_zone+0x40 is consumed by 0x4a901a for neutral weighted placement faction reuse; it is not a map-global same-town-type lock";
	...
	for (int64_t index = 0; index < runtime_zone_records.size(); ++index) {
		...
		if (direct_owned_count > 0 && owner_color < 0) {
			...
			skipped["source_zone_key"] = runtime.get("source_zone_key", "");
		...
		if (owner_color >= 0) {
			for (int32_t town = 0; town < player_min_towns; ++town) {
				Dictionary scheduled;
				scheduled["phase"] = "0x4a8d2c_direct_player_minimum_town";
				scheduled["helper"] = "0x4a93a2";
				scheduled["runtime_zone_index"] = runtime_index;
				scheduled["source_zone_key"] = runtime.get("source_zone_key", "");
			...
			for (int32_t castle = 0; castle < player_min_castles; ++castle) {
				Dictionary scheduled;
				scheduled["phase"] = "0x4a8d2c_direct_player_minimum_castle";
				scheduled["helper"] = "0x4a93a2";
				scheduled["runtime_zone_index"] = runtime_index;
				scheduled["source_zone_key"] = runtime.get("source_zone_key", "");
				scheduled["owner_color"] = owner_color;
				scheduled["has_castle"] = true;
				scheduled["town_choice_index"] = runtime_index < town_choice_by_runtime.size() ? int32_t(town_choice_by_runtime[runtime_index]) : -1;
				scheduled["selected_h3maped_terrain_id"] = runtime_index < selected_terrain_ids.size() ? int32_t(selected_terrain_ids[runtime_index]) : -1;
				scheduled["selected_project_terrain_id"] = runtime_index < selected_project_terrain_ids.size() ? String(selected_project_terrain_ids[runtime_index]) : String();
				scheduled["object_materialization_blocked"] = true;
	...
	for (int64_t index = 0; index < scheduled_records.size(); ++index) {
		if (Variant(scheduled_records[index]).get_type() != Variant::DICTIONARY) {
			continue;
		}
		Dictionary scheduled = scheduled_records[index];
		...
		record["source_zone_key"] = scheduled.get("source_zone_key", "");
		...
		record["candidate_source"] = "generated cell+0x20 zone/source byte projected from 0x4a325d zone words";
		record["direct_helper_address"] = "0x4a93a2";
		record["eligibility_helper_address"] = "0x49aa93";
		record["base_constructor_address"] = "0x49ba89";
		record["base_vtable_address"] = "0x540a74";
		record["town_vtable_address"] = "0x540a9c";
		record["serializer_address"] = "0x49bc2f";
		...
		record["cell_marking_contract"] = "clear cell bit 26 and set bit 27 unless cell+0x2c bit 0 locks the mark";
		...
		if (width <= 0 || height <= 0 || zone_word_u32.size() != expected_cell_count || terrain_code_u16.size() != expected_cell_count) {
			record["status"] = "blocked_missing_0x4a325d_zone_word_or_0x4a3f27_terrain_grid";
			...
			for (const H3MaskPoint &point : town_body_points) {
				...
				if (object_occupied[size_t(body_index)] == 0) {
					...
					if (marked_preview.size() < 12) {
						...
						cell["x"] = body_x;
						cell["y"] = body_y;
						cell["level"] = body_level;
						marked_preview.append(cell);
					}
				}
			}
...
Dictionary zone_footprint_schedule_report(const Dictionary &normalized_config, const Array &runtime_zone_records, const Array &link_seeds, const Dictionary &coordinate_replay, const Dictionary &terrain_selection) {
	Dictionary report;
	...
	Dictionary late_connection_overlap = late_connection_overlap_geometry_report(runtime_zone_records, link_seeds, terrain_cell_writeout);
	report["late_connection_overlap_geometry_status"] = late_connection_overlap.get("status", "");
...
Dictionary runtime_zone_record_setup_report(const Dictionary &normalized_config, const Dictionary &template_record, const Dictionary &assignment, int32_t human_count, int32_t player_count, uint32_t rng_state_after_template_selection) {
	...
	for (int64_t index = 0; index < zones.size(); ++index) {
		...
		record["towns_same_type_for_0x4a901a"] = town_policy.get("same_town_type", false);
	...
	Dictionary footprint_schedule = zone_footprint_schedule_report(normalized_config, records, endpoint_schedule.get("link_seeds", Array()), coordinate_replay, terrain_selection);
