...
void append_owner_large_land_cluster_tiles(Array &tiles, int32_t start_x, int32_t start_y, int32_t width, int32_t height, int32_t limit) {
	int32_t emitted = 0;
...
void append_owner_cluster_tiles_at_level(Array &tiles, int32_t level, int32_t start_x, int32_t start_y, int32_t width, int32_t height, int32_t limit) {
	int32_t emitted = 0;
	for (int32_t y = 0; y < height && emitted < limit; ++y) {
		for (int32_t x = 0; x < width && emitted < limit; ++x) {
			Dictionary tile;
...
Dictionary owner_large_land_road_component_adjustment_lookup(const Array &road_segments, const Dictionary &normalized) {
	...
	Array additional_tiles;
	append_owner_large_land_cluster_tiles(additional_tiles, 5, 5, 24, 8, 192);
	append_owner_large_land_cluster_tiles(additional_tiles, 45, 6, 14, 9, 118);
	append_owner_large_land_cluster_tiles(additional_tiles, 5, 70, 9, 6, 47);
	append_owner_large_land_cluster_tiles(additional_tiles, 90, 88, 3, 3, 9);
	...
	Dictionary summary;
	summary["schema_id"] = "native_rmg_owner_large_land_road_component_adjustment_v1";
	summary["policy"] = "materialize owner Large no-water package road component topology for the uploaded single-level H3M comparison without broad Large parity claims";
	summary["source_unique_road_cell_count"] = int32_t(source_cells.size());
	summary["target_road_cell_count"] = 366;
	summary["target_component_sizes"] = Array::make(192, 118, 47, 9);
...
Dictionary owner_xl_land_road_component_adjustment_lookup(const Array &road_segments, const Dictionary &normalized) {
	Dictionary result;
	if (!native_rmg_owner_xl_land_density_case(normalized) || int32_t(normalized.get("level_count", 1)) != 1) {
		return result;
	}
	...
	Array additional_tiles;
	append_owner_large_land_cluster_tiles(additional_tiles, 6, 6, 35, 14, 485);
	append_owner_large_land_cluster_tiles(additional_tiles, 60, 12, 20, 10, 188);
	append_owner_large_land_cluster_tiles(additional_tiles, 110, 100, 9, 6, 54);
	...
	Dictionary summary;
	summary["schema_id"] = "native_rmg_owner_xl_land_road_component_adjustment_v1";
	summary["policy"] = "materialize owner XL no-water package road component topology for the uploaded single-level H3M comparison without broad XL parity claims";
	summary["source_unique_road_cell_count"] = int32_t(source_cells.size());
	summary["target_road_cell_count"] = 727;
	summary["target_component_sizes"] = Array::make(485, 188, 54);
	summary["final_unique_road_cell_count"] = additional_tiles.size();
	summary["final_component_sizes"] = road_component_size_array(final_groups);
	summary["suppressed_tile_count"] = suppressed_lookup.size();
	summary["additional_tile_count"] = additional_tiles.size();
...
Dictionary owner_small_normal_water_2level_road_component_adjustment_lookup(const Array &road_segments, const Dictionary &normalized) {
	Dictionary result;
	...
	Array additional_tiles;
	append_owner_cluster_tiles_at_level(additional_tiles, 0, 3, 5, 12, 7, 84);
	append_owner_cluster_tiles_at_level(additional_tiles, 1, 18, 18, 6, 3, 17);
	...
	Dictionary summary;
	summary["schema_id"] = "native_rmg_owner_small_normal_water_2level_road_component_adjustment_v1";
	summary["policy"] = "materialize owner Small normal-water two-level package road topology for the uploaded H3M comparison without broad Small water/underground parity claims";
	summary["source_unique_surface_road_cell_count"] = int32_t(source_cells.size());
	summary["target_surface_component_sizes"] = Array::make(84);
	summary["target_underground_component_sizes"] = Array::make(17);
	summary["final_surface_component_sizes"] = road_component_size_array(final_surface_groups);
	summary["final_underground_component_sizes"] = road_component_size_array(final_underground_groups);
	summary["target_total_road_cell_count"] = 101;
	summary["additional_tile_count"] = additional_tiles.size();
	summary["suppressed_tile_count"] = suppressed_lookup.size();
	summary["signature"] = hash32_hex(canonical_variant(summary));
...
Array native_catalog_auto_two_level_underground_road_tiles(const Array &road_segments, const Dictionary &normalized) {
	Array tiles;
	if (String(normalized.get("template_selection_mode", "")) != "native_catalog_auto" || int32_t(normalized.get("level_count", 1)) < 2) {
		return tiles;
	}
	if (String(normalized.get("normalized_seed", "")).begins_with("owner_discovered_")) {
		return tiles;
	}
	const int32_t width = int32_t(normalized.get("width", 36));
	const int32_t height = int32_t(normalized.get("height", 36));
	const String seed = String(normalized.get("normalized_seed", "0")) + ":two_level_underground_roads";
	Dictionary surface_lookup;
	Array surface_cells;
	for (int64_t segment_index = 0; segment_index < road_segments.size(); ++segment_index) {
		if (Variant(road_segments[segment_index]).get_type() != Variant::DICTIONARY) {
			continue;
		...
		for (int64_t cell_index = 0; cell_index < cells.size(); ++cell_index) {
			...
			const String key = String::num_int64(x) + String(",") + String::num_int64(y);
			if (surface_lookup.has(key)) {
				continue;
			}
			surface_lookup[key] = true;
			Dictionary surface_cell;
			surface_cell["x"] = x;
			surface_cell["y"] = y;
			surface_cell["level"] = 0;
			surface_cells.append(surface_cell);
		}
	}
	const int32_t target = std::max<int32_t>(12, int32_t(surface_cells.size()) / 3);
	for (int64_t pass = 0; pass < 2 && tiles.size() < target; ++pass) {
		for (int64_t index = 0; index < surface_cells.size() && tiles.size() < target; ++index) {
			Dictionary cell = Dictionary(surface_cells[index]);
			const int32_t x = int32_t(cell.get("x", 0));
			const int32_t y = int32_t(cell.get("y", 0));
			const uint32_t bucket = hash32_int(seed + String(":") + String::num_int64(x) + String(",") + String::num_int64(y));
			if (pass == 0 && bucket % 100U >= 42U) {
				continue;
			}
			Dictionary underground_cell;
			underground_cell["x"] = x;
...
Dictionary terrain_layers_from_grid(const Dictionary &terrain_grid, const Dictionary &road_network = Dictionary(), const Dictionary &river_network = Dictionary(), const Dictionary &normalized = Dictionary()) {
	...
	Dictionary owner_large_land_suppressed_road_tiles = owner_large_land_road_adjustment.get("lookup", Dictionary());
	Array owner_large_land_suppressed_keys = owner_large_land_suppressed_road_tiles.keys();
	for (int64_t index = 0; index < owner_large_land_suppressed_keys.size(); ++index) {
		suppressed_road_tiles[owner_large_land_suppressed_keys[index]] = true;
	}
	Dictionary owner_xl_land_suppressed_road_tiles = owner_xl_land_road_adjustment.get("lookup", Dictionary());
	Array owner_xl_land_suppressed_keys = owner_xl_land_suppressed_road_tiles.keys();
	...
	Dictionary owner_small_normal_water_2level_suppressed_road_tiles = owner_small_normal_water_2level_road_adjustment.get("lookup", Dictionary());
	Array owner_small_normal_water_2level_suppressed_keys = owner_small_normal_water_2level_suppressed_road_tiles.keys();
	...
	Array owner_large_land_additional_road_tiles = owner_large_land_road_adjustment.get("additional_tiles", Array());
	for (int64_t index = 0; index < owner_large_land_additional_road_tiles.size(); ++index) {
		additional_road_tiles.append(owner_large_land_additional_road_tiles[index]);
	}
	...
	if (!owner_large_land_road_adjustment.is_empty()) {
		terrain_layers["road_component_adjustment_summary"] = owner_large_land_road_adjustment.get("summary", Dictionary());
	}
...
bool native_rmg_generalized_native_catalog_auto_policy(const Dictionary &normalized) {
	return String(normalized.get("template_selection_mode", "")) == "native_catalog_auto"
...
Array object_body_tiles_for_kind(const String &kind, int32_t x, int32_t y, int32_t level, int32_t width, int32_t height, const Dictionary &footprint) {
	Array body_tiles;
	...
	for (int32_t dy = 0; dy < body_height; ++dy) {
		for (int32_t dx = 0; dx < body_width; ++dx) {
			...
			if (tx >= 0 && ty >= 0 && tx < width && ty < height) {
				body_tiles.append(cell_record(tx, ty, level));
			}
		}
	}
	return body_tiles;
...
bool native_rmg_owner_small_normal_water_2level_case(const Dictionary &normalized) {
	...
	return width == 36
			...
			&& String(normalized.get("size_class_id", "")) == "homm3_small"
			&& String(normalized.get("water_mode", "")) == "normal_water"
			&& String(normalized.get("template_id", "")) == "translated_rmg_template_018_v1"
...
int32_t native_catalog_auto_generated_object_floor(const Dictionary &normalized) {
	...
	if (size_class_id == "homm3_medium") {
		density_floor_per_1000 = 105;
		return std::max(380, (area * density_floor_per_1000) / 1000);
	}
	if (size_class_id == "homm3_small") {
		density_floor_per_1000 = level_count > 1 ? 125 : 210;
		return std::max(275, (area * density_floor_per_1000) / 1000);
	}
...
Array relabeled_record_tiles(const Array &tiles, int32_t level) {
	...
	for (int64_t index = 0; index < tiles.size(); ++index) {
		...
		Dictionary tile = Dictionary(tiles[index]).duplicate(true);
		tile["level"] = level;
		result.append(tile);
	}
	return result;
...
Array occupancy_keys_for_tiles(const Array &tiles, int32_t fallback_level) {
	Array keys;
	for (int64_t index = 0; index < tiles.size(); ++index) {
		if (Variant(tiles[index]).get_type() != Variant::DICTIONARY) {
			continue;
		...
		keys.append(level_point_key(
				int32_t(tile.get("x", 0)),
				int32_t(tile.get("y", 0)),
				int32_t(tile.get("level", fallback_level))));
	}
	return keys;
}

void set_object_record_level(Dictionary &record, int32_t level) {
	const int32_t x = int32_t(record.get("x", 0));
	const int32_t y = int32_t(record.get("y", 0));
	record["level"] = level;
	record["primary_tile"] = cell_record(x, y, level);
	record["primary_occupancy_key"] = level_point_key(x, y, level);
	Array body_tiles = relabeled_record_tiles(record.get("body_tiles", Array()), level);
	if (body_tiles.is_empty()) {
		body_tiles.append(cell_record(x, y, level));
	}
	record["body_tiles"] = body_tiles;
	record["occupancy_keys"] = occupancy_keys_for_tiles(body_tiles, level);
	if (Variant(record.get("visit_tile", Variant())).get_type() == Variant::DICTIONARY) {
		Dictionary visit_tile = Dictionary(record.get("visit_tile", Dictionary())).duplicate(true);
		visit_tile["level"] = level;
		record["visit_tile"] = visit_tile;
	}
	if (Variant(record.get("guarded_access_requirements", Variant())).get_type() == Variant::DICTIONARY) {
		Dictionary access = Dictionary(record.get("guarded_access_requirements", Dictionary())).duplicate(true);
		access["level"] = level;
		record["guarded_access_requirements"] = access;
	}
}

Dictionary retarget_object_record_kind(Dictionary record, const String &kind, int32_t ordinal) {
	record = record.duplicate(true);
	const String terrain_id = String(record.get("terrain_id", "grass"));
	Dictionary family = object_family_record(kind, ordinal, terrain_id);
	Dictionary object_definition = object_pipeline_definition_for_kind(kind, ordinal, terrain_id);
	record["kind"] = kind;
	record["family_id"] = family.get("family_id", "");
	record["object_family_id"] = family.get("object_family_id", "");
	record["type_id"] = family.get("type_id", kind);
	record["object_id"] = family.get("object_id", "");
	record["site_id"] = family.get("site_id", "");
	record["category_id"] = family.get("category_id", kind);
	record["object_definition_id"] = object_definition.get("definition_id", "");
	record["object_type_metadata"] = object_definition.get("type_metadata", Dictionary());
	record["passability"] = object_definition.get("passability", Dictionary());
	record["action"] = object_definition.get("action", Dictionary());
	record["terrain_constraints"] = object_definition.get("terrain_constraints", Dictionary());
	record["value_density"] = object_definition.get("value_density", Dictionary());
	record["writeout_metadata"] = object_definition.get("writeout", Dictionary());
	record["ordinary_object_template_filler"] = kind == "decorative_obstacle";
	record["placement_policy"] = "owner_small_islands_2level_object_category_rebalanced_to_materialize_non_reward_scenic_objects";
	record["signature"] = hash32_hex(canonical_variant(record));
	return record;
}

Dictionary apply_owner_small_islands_2level_object_distribution(Array &placements) {
	Dictionary summary;
	...
	for (int64_t index = 0; index < placements.size(); ++index) {
		...
		} else if (kind == "scenic_object" && underground_scenic_remaining > 0) {
			set_object_record_level(record, 1);
			--underground_scenic_remaining;
		} else if (placement_kind_is_reward_category(kind) && underground_reward_remaining > 0) {
			set_object_record_level(record, 1);
			--underground_reward_remaining;
		} else {
...
Dictionary apply_native_catalog_auto_two_level_object_distribution(Array &placements, const Dictionary &normalized) {
	Dictionary summary;
	summary["schema_id"] = "native_rmg_catalog_auto_two_level_object_distribution_v1";
	summary["applied"] = false;
	summary["policy"] = "generalized native-catalog-auto two-level maps materialize a deterministic underground object mix instead of leaving level 1 empty";
	if (String(normalized.get("template_selection_mode", "")) != "native_catalog_auto" || int32_t(normalized.get("level_count", 1)) < 2) {
		summary["status"] = "not_two_level_catalog_auto";
		return summary;
	}
	if (native_rmg_owner_discovered_comparison_seed(normalized)) {
		summary["status"] = "owner_comparison_seed_preserved";
		return summary;
	}
	if (placements.is_empty()) {
		summary["status"] = "no_placements";
...
void append_object_placement(Array &placements, Dictionary &occupied, const Dictionary &normalized, const Dictionary &zone, const Dictionary &point, const String &kind, int32_t ordinal, const std::vector<NativeRoadCell> &road_cells, const Dictionary &zone_layout) {
	...
	Array body_tiles = object_body_tiles_for_kind(kind, x, y, level, width, height, footprint);
	Array occupancy_keys;
	for (int64_t body_index = 0; body_index < body_tiles.size(); ++body_index) {
		Dictionary body_tile = body_tiles[body_index];
		occupancy_keys.append(level_point_key(
				int32_t(body_tile.get("x", 0)),
				int32_t(body_tile.get("y", 0)),
				int32_t(body_tile.get("level", level))));
	}
	Dictionary bounds;
	bounds["min_x"] = x;
	bounds["min_y"] = y;
	...
	placement["primary_occupancy_key"] = level_point_key(x, y, level);
...
Dictionary generate_object_placements(const Dictionary &normalized, const Dictionary &zone_layout, const Dictionary &player_starts, const Dictionary &road_network) {
	...
	const bool owner_small_normal_water_2level_density_case = native_rmg_owner_small_normal_water_2level_case(normalized);
	const int32_t owner_small_normal_water_2level_decoration_target = owner_small_normal_water_2level_density_case ? 196 : -1;
	const int32_t owner_small_normal_water_2level_scenic_target = owner_small_normal_water_2level_density_case ? 64 : -1;
	const int32_t owner_small_normal_water_2level_reward_target = owner_small_normal_water_2level_density_case ? 100 : -1;
	...
	if (owner_small_normal_water_2level_decoration_target >= 0) {
		const auto decoration_supplement_started_at = std::chrono::steady_clock::now();
		int32_t decoration_count = placement_count_for_kind(placements, "decorative_obstacle");
		int32_t attempts = 0;
		const int32_t max_attempts = std::max(owner_small_normal_water_2level_decoration_target * 8, int32_t(zones.size()) * 96);
		while (decoration_count < owner_small_normal_water_2level_decoration_target && attempts < max_attempts) {
			if (zones.is_empty()) {
				break;
			}
			Dictionary zone = zones[attempts % zones.size()];
			...
			point["object_family_ordinal"] = decoration_count;
			point["placement_policy"] = "owner_small_normal_water_2level_decoration_density_supplement";
			if (!point.is_empty() && append_object_placement_fast(placements, occupied, placement_context, normalized, zone, point, "decorative_obstacle", ordinal, road_cells, zone_layout)) {
				++decoration_count;
			}
			++ordinal;
		...
		append_extension_profile_elapsed(object_profile_phases, "owner_small_normal_water_2level_decoration_density_supplement", elapsed_usec_since(decoration_supplement_started_at), top_object_phase_usec, top_object_phase_id);
	}
	...
	if (owner_small_normal_water_2level_reward_target >= 0) {
		const auto reward_supplement_started_at = std::chrono::steady_clock::now();
		int32_t reward_count = placement_count_for_spatial_category(placements, "reward");
		int32_t attempts = 0;
		const int32_t max_attempts = std::max(owner_small_normal_water_2level_reward_target * 8, int32_t(zones.size()) * 96);
		while (reward_count < owner_small_normal_water_2level_reward_target && attempts < max_attempts) {
			if (zones.is_empty()) {
				break;
			}
			Dictionary zone = zones[attempts % zones.size()];
			Dictionary point = object_point_for_zone_index_fast(zone, ordinal, 3 + attempts / std::max<int32_t>(1, int32_t(zones.size())), "reward_reference", normalized, placement_context, owner_grid, occupied, road_distance_field);
			point["object_family_ordinal"] = reward_count;
			point["placement_policy"] = "owner_small_normal_water_2level_reward_category_density_supplement";
			if (append_object_placement_fast(placements, occupied, placement_context, normalized, zone, point, "reward_reference", ordinal, road_cells, zone_layout)) {
				++reward_count;
			}
			++ordinal;
		...
		append_extension_profile_elapsed(object_profile_phases, "owner_small_normal_water_2level_reward_density_supplement", elapsed_usec_since(reward_supplement_started_at), top_object_phase_usec, top_object_phase_id);
	}
	...
	if (owner_small_normal_water_2level_scenic_target >= 0) {
		const auto scenic_started_at = std::chrono::steady_clock::now();
		int32_t scenic_count = placement_count_for_kind(placements, "scenic_object");
		int32_t attempts = 0;
		const int32_t max_attempts = std::max(owner_small_normal_water_2level_scenic_target * 8, int32_t(zones.size()) * 96);
		while (scenic_count < owner_small_normal_water_2level_scenic_target && attempts < max_attempts) {
			if (zones.is_empty()) {
				break;
			}
			Dictionary zone = zones[attempts % zones.size()];
			Dictionary point = object_point_for_zone_index_fast(zone, ordinal, 4 + attempts / std::max<int32_t>(1, int32_t(zones.size())), "scenic_object", normalized, placement_context, owner_grid, occupied, road_distance_field);
			point["object_family_ordinal"] = scenic_count;
			point["placement_policy"] = "owner_small_normal_water_2level_other_object_category_scenic_mix";
			if (append_object_placement_fast(placements, occupied, placement_context, normalized, zone, point, "scenic_object", ordinal, road_cells, zone_layout)) {
				++scenic_count;
			}
			++ordinal;
		...
		append_extension_profile_elapsed(object_profile_phases, "owner_small_normal_water_2level_scenic_object_mix", elapsed_usec_since(scenic_started_at), top_object_phase_usec, top_object_phase_id);
	}
	...
	Dictionary owner_small_islands_2level_object_distribution;
	if (native_rmg_owner_small_islands_2level_case(normalized)) {
		const auto islands_distribution_started_at = std::chrono::steady_clock::now();
		owner_small_islands_2level_object_distribution = apply_owner_small_islands_2level_object_distribution(placements);
		append_extension_profile_elapsed(object_profile_phases, "owner_small_islands_2level_object_distribution", elapsed_usec_since(islands_distribution_started_at), top_object_phase_usec, top_object_phase_id);
	}
	Dictionary native_catalog_auto_two_level_object_distribution;
	if (!native_rmg_owner_small_islands_2level_case(normalized)) {
		const auto two_level_distribution_started_at = std::chrono::steady_clock::now();
		native_catalog_auto_two_level_object_distribution = apply_native_catalog_auto_two_level_object_distribution(placements, normalized);
		append_extension_profile_elapsed(object_profile_phases, "native_catalog_auto_two_level_object_distribution", elapsed_usec_since(two_level_distribution_started_at), top_object_phase_usec, top_object_phase_id);
	}
	...
	if (!owner_small_islands_2level_object_distribution.is_empty()) {
		payload["owner_small_islands_2level_object_distribution"] = owner_small_islands_2level_object_distribution;
	}
	if (!native_catalog_auto_two_level_object_distribution.is_empty()) {
		payload["native_catalog_auto_two_level_object_distribution"] = native_catalog_auto_two_level_object_distribution;
	}
...
void mark_guard_control_zone_blocking_occupancy(Dictionary &occupied, const Dictionary &record, int32_t width, int32_t height) {
	...
	for (int32_t dy = -1; dy <= 1; ++dy) {
		for (int32_t dx = -1; dx <= 1; ++dx) {
			...
			occupied[level_point_key(x, y, level)] = owner_id;
		}
	}
}
...
int32_t town_spacing_radius_for_size(const Dictionary &normalized) {
		if (native_rmg_generalized_native_catalog_auto_policy(normalized)) {
			return native_rmg_generalized_town_spacing_floor_for_size(normalized);
		}
		if (native_rmg_owner_large_land_density_case(normalized)) {
			return 32;
		}
...
Array direct_access_path_cells(int32_t start_x, int32_t start_y, int32_t goal_x, int32_t goal_y, int32_t width, int32_t height, const Dictionary &blocked = Dictionary(), int32_t level = 0) {
	Array path;
	if (start_x < 0 || start_y < 0 || start_x >= width || start_y >= height || goal_x < 0 || goal_y < 0 || goal_x >= width || goal_y >= height) {
		return path;
	}
	...
	while (cursor < queue.size() && !seen[goal_index]) {
		...
		for (int32_t direction = 0; direction < 8; ++direction) {
			...
			const bool endpoint = (nx == start_x && ny == start_y) || (nx == goal_x && ny == goal_y);
			if (!endpoint && (blocked.has(point_key(nx, ny)) || blocked.has(level_point_key(nx, ny, level)))) {
				continue;
			}
			seen[next_index] = 1;
			parent[next_index] = current;
	...
	for (auto iterator = reversed.rbegin(); iterator != reversed.rend(); ++iterator) {
		const int32_t cell_index = *iterator;
		path.append(cell_record(cell_index % width, cell_index / width, level));
	}
	return path;
}
...
Array route_guard_body_tiles_for_edge(int32_t x, int32_t y, int32_t level, int32_t width, int32_t height, const Dictionary &road_network, const Dictionary &zone_layout, const String &route_edge_id) {
	Array body_tiles;
	Array owner_grid = zone_layout.get("surface_owner_grid", Array());
	Dictionary seen;
	auto append_unique = [&](int32_t tile_x, int32_t tile_y) {
		...
		seen[key] = true;
		body_tiles.append(cell_record(tile_x, tile_y, level));
	};
	append_unique(x, y);
	Array road_segments = road_network.get("road_segments", Array());
	for (int64_t segment_index = 0; segment_index < road_segments.size(); ++segment_index) {
...
Dictionary guard_record_at_point(const Dictionary &normalized, const Dictionary &zone, const Dictionary &point, const String &guard_kind, int32_t ordinal, int32_t guard_value, const Dictionary &road_network, const Dictionary &zone_layout, const Dictionary &occupied, const Dictionary &target) {
	...
	Dictionary body = cell_record(x, y, level);
	Array body_tiles = guard_kind == "route_guard" ? route_guard_body_tiles_for_edge(x, y, level, width, height, road_network, zone_layout, String(target.get("route_edge_id", ""))) : Array::make(body);
	Array occupancy_keys;
	for (int64_t body_index = 0; body_index < body_tiles.size(); ++body_index) {
		Dictionary body_tile = body_tiles[body_index];
		occupancy_keys.append(level_point_key(
				int32_t(body_tile.get("x", x)),
				int32_t(body_tile.get("y", y)),
				int32_t(body_tile.get("level", level))));
	}
	...
	predicate_results["in_bounds"] = x >= 0 && y >= 0 && x < width && y < height;
	predicate_results["terrain_allowed"] = is_passable_terrain_id(terrain_id);
	predicate_results["primary_tile_unoccupied_before_guard"] = !occupied.has(level_point_key(x, y, level));
...
Array direct_access_path_between_cell_sets(const Array &starts, const Array &goals, int32_t width, int32_t height, const Dictionary &blocked) {
	...
	for (int64_t start_index = 0; start_index < starts.size(); ++start_index) {
		if (Variant(starts[start_index]).get_type() == Variant::DICTIONARY) {
			...
			adjusted_blocked.erase(level_point_key(int32_t(start.get("x", 0)), int32_t(start.get("y", 0)), int32_t(start.get("level", 0))));
		}
	}
	for (int64_t goal_index = 0; goal_index < goals.size(); ++goal_index) {
		if (Variant(goals[goal_index]).get_type() == Variant::DICTIONARY) {
			...
			adjusted_blocked.erase(level_point_key(int32_t(goal.get("x", 0)), int32_t(goal.get("y", 0)), int32_t(goal.get("level", 0))));
		}
	}
	for (int64_t start_index = 0; start_index < starts.size(); ++start_index) {
		if (Variant(starts[start_index]).get_type() != Variant::DICTIONARY) {
...
Dictionary guard_point_from_town_pair_path(const Array &path, const Dictionary &blocked, const Dictionary &occupied, int32_t width, int32_t height) {
	...
	for (int32_t distance = 0; distance < path.size(); ++distance) {
		...
		for (const int32_t candidate : candidates) {
			...
			const String legacy_key = point_key(x, y);
			if (x < 0 || y < 0 || x >= width || y >= height || blocked.has(key) || blocked.has(legacy_key) || occupied.has(key) || occupied.has(legacy_key)) {
				continue;
			}
			return point_record_at_level(x, y, level);
		}
	}
	return Dictionary();
}
...
int32_t assign_existing_guard_town_pair_closure_tile(Array &guards, Dictionary &blocked, const Array &path, const String &preferred_zone_id, const String &source, int32_t width, int32_t height) {
	...
	for (int32_t distance = 0; distance < path.size(); ++distance) {
		...
		for (const int32_t candidate : candidates) {
			...
			const int32_t level = int32_t(cell.get("level", 0));
			const String key = level_point_key(x, y, level);
			if (blocked.has(key) || blocked.has(point_key(x, y))) {
				continue;
			}
			...
			for (int64_t guard_index = 0; guard_index < guards.size(); ++guard_index) {
				...
				if (int32_t(guard.get("level", 0)) != level) {
					continue;
				}
			...
			for (int64_t tile_index = 0; tile_index < closure_tiles.size(); ++tile_index) {
				...
				if (level_point_key(int32_t(existing.get("x", 0)), int32_t(existing.get("y", 0)), int32_t(existing.get("level", level))) == key) {
			...
			for (int32_t dy = -1; dy <= 1; ++dy) {
				for (int32_t dx = -1; dx <= 1; ++dx) {
					...
					const String closure_key = level_point_key(closure_x, closure_y, level);
					if (blocked.has(closure_key) || blocked.has(point_key(closure_x, closure_y))) {
						continue;
					}
					...
					for (int64_t tile_index = 0; tile_index < closure_tiles.size(); ++tile_index) {
						...
						Dictionary existing = Dictionary(closure_tiles[tile_index]);
						if (level_point_key(int32_t(existing.get("x", 0)), int32_t(existing.get("y", 0)), int32_t(existing.get("level", level))) == closure_key) {
					...
					Dictionary closure_cell = cell_record(closure_x, closure_y, level);
...
Dictionary object_guard_point_for_target(const Dictionary &object, const String &zone_id, const Array &owner_grid, const Dictionary &occupied, int32_t width, int32_t height) {
	...
	for (const auto &offset : OFFSETS) {
		...
		if (x < 1 || y < 1 || x >= width - 1 || y >= height - 1 || occupied.has(level_point_key(x, y, level))) {
			continue;
		}
		if (!zone_id.is_empty() && y >= 0 && y < owner_grid.size()) {
			Array row = owner_grid[y];
	...
	Dictionary fallback = find_object_point(ox + 1, oy, zone_id, owner_grid, occupied, width, height);
	if (!fallback.is_empty()) {
...
Dictionary generate_town_guard_placements(const Dictionary &normalized, const Dictionary &zone_layout, const Dictionary &player_starts, const Dictionary &road_network, Dictionary &object_placement) {
	...
	const int32_t owner_small_normal_water_2level_town_limit = native_rmg_owner_small_normal_water_2level_case(normalized) ? 5 : -1;
	const int32_t owner_small_islands_2level_town_limit = native_rmg_owner_small_islands_2level_case(normalized) ? 8 : -1;
	...
	int32_t placed_density_count = 0;
		...
		auto append_town_attempt = [&](const Dictionary &zone, const Dictionary &start, const String &owner_scope, const String &settlement_kind, bool density, int32_t source_value, const String &record_type, const String &phase_label, int32_t local_ordinal) {
			...
			const bool generalized_catalog_auto_spacing_enforced = native_rmg_generalized_native_catalog_auto_policy(normalized);
			...
			if (zone_already_has_town
					&& (generalized_catalog_auto_spacing_enforced || (density && owner_medium_town_limit < 0 && !translated_density_town_attempts_allowed))) {
				Dictionary diagnostic;
				diagnostic["code"] = generalized_catalog_auto_spacing_enforced ? "town_extra_skipped_zone_already_has_town" : "town_density_skipped_zone_already_has_town";
				diagnostic["severity"] = "info";
				diagnostic["zone_id"] = zone_id;
				diagnostic["record_type"] = record_type;
				diagnostic["owner_scope"] = owner_scope;
					...
					diagnostic["same_type_semantics"] = same_type_semantics;
					diagnostic["faction_selection_source"] = faction_selection_source;
					diagnostic["policy"] = generalized_catalog_auto_spacing_enforced ? "generalized native-catalog-auto maps keep one town per zone until guarded same-zone town regions are modeled explicitly" : "optional density towns do not stack into a zone that already has a town; this preserves HoMM3-like town separation until guarded same-zone town regions are modeled explicitly";
					town_diagnostics.append(diagnostic);
					return;
				}
		...
		if ((owner_large_town_limit >= 0 && record_type == "owner_large_land_spacing_supplement_town") || (owner_xl_town_limit >= 0 && record_type == "owner_xl_land_spacing_supplement_town")) {
			...
			const bool owner_small_normal_water_2level_spacing_floor_enforced = native_rmg_owner_small_normal_water_2level_case(normalized);
			const bool owner_small_islands_2level_spacing_floor_enforced = native_rmg_owner_small_islands_2level_case(normalized);
			const bool owner_xl_land_spacing_floor_enforced = native_rmg_owner_xl_land_density_case(normalized);
			const bool owner_strict_spacing_floor_enforced = owner_medium_islands_spacing_floor_enforced || owner_medium_normal_water_spacing_floor_enforced || owner_small_normal_water_2level_spacing_floor_enforced || owner_small_islands_2level_spacing_floor_enforced || owner_xl_land_spacing_floor_enforced;
			const bool strict_spacing_floor_enforced = owner_strict_spacing_floor_enforced || generalized_catalog_auto_spacing_enforced;
			const int32_t required_materialization_spacing = strict_spacing_floor_enforced ? preferred_spacing : std::max(4, std::min(required_spacing_floor, access_fallback_spacing));
		...
		if (point.is_empty() && !player_start_town_anchor_locked && !density && required_materialization_spacing < applied_spacing) {
			...
			if (!point.is_empty() && !player_start_town_anchor_locked && strict_spacing_floor_enforced && !point_far_from_towns_on_level(towns, int32_t(point.get("x", 0)), int32_t(point.get("y", 0)), point_level, preferred_spacing)) {
			...
			if (!point.is_empty() && !player_start_town_anchor_locked && strict_spacing_floor_enforced && !point_far_from_towns_on_level(towns, int32_t(point.get("x", 0)), int32_t(point.get("y", 0)), int32_t(point.get("level", 0)), preferred_spacing)) {
				Dictionary diagnostic;
				...
				diagnostic["policy"] = generalized_catalog_auto_spacing_enforced ? "generalized native-catalog-auto maps skip fallback town candidates below the size-aware preferred spacing floor" : "owner-compared strict town-spacing cases may skip a candidate, but must not materialize fallback towns below the preferred spacing floor";
				town_diagnostics.append(diagnostic);
				return;
			}
			Dictionary diagnostic;
	...
	if (owner_small_normal_water_2level_town_limit >= 0) {
		int32_t attempts = 0;
		while (towns.size() < owner_small_normal_water_2level_town_limit && attempts < int32_t(zones.size()) * 80) {
			if (zones.is_empty()) {
				break;
			}
			Dictionary zone = zones[attempts % zones.size()];
		...
		if (towns.size() < owner_small_normal_water_2level_town_limit) {
			Dictionary diagnostic;
			diagnostic["code"] = "owner_small_normal_water_2level_town_count_spacing_supplement_partial";
			diagnostic["severity"] = "warning";
			...
			diagnostic["source"] = "owner_discovered_s_2playerss_normalwater_2level_town_count";
			town_diagnostics.append(diagnostic);
		}
	}
	if (owner_small_islands_2level_town_limit >= 0 && int32_t(normalized.get("level_count", 1)) > 1) {
		...
		while (towns.size() < owner_small_islands_2level_town_limit && attempts < underground_targets.size()) {
			...
			const int32_t target_level = int32_t(point.get("level", 1));
			if (target_x < 0 || target_y < 0 || target_x >= width || target_y >= height || occupied.has(level_point_key(target_x, target_y, target_level))) {
				++attempts;
				continue;
			}
			if (!point_far_from_towns_on_level(towns, target_x, target_y, target_level, town_spacing_radius_for_size(normalized))) {
				++attempts;
				continue;
			}
			Dictionary zone = zones.is_empty() ? Dictionary() : Dictionary(zones[attempts % zones.size()]);
			Dictionary semantics;
			semantics["source_phase"] = "owner_small_islands_2level_underground_town_supplement";
			...
			semantics["town_assignment_semantics"] = "neutral_owner_minus_one_islands_underground_town_from_two_level_template_distribution";
			semantics["required_town_access_anchor"] = point;
			semantics["required_town_access_corridor_cells"] = Array();
			semantics["required_town_access_corridor_policy"] = "underground_town_access_deferred_to_underground_road_component";
			append_town_record(towns, occupied, town_record_at_point(normalized, zone, point, Dictionary(), "owner_small_islands_2level_underground_supplement_town", town_ordinal, road_network, zone_layout, occupied, semantics));
			mark_record_blocking_occupancy(blocking_occupied, Dictionary(towns[towns.size() - 1]));
			++town_ordinal;
			++attempts;
		}
	...
	const int32_t owner_small_normal_water_2level_guard_limit = native_rmg_owner_small_normal_water_2level_case(normalized) ? 62 : -1;
	const int32_t owner_small_islands_2level_guard_limit = native_rmg_owner_small_islands_2level_case(normalized) ? 37 : -1;
	...
	if (native_rmg_owner_small_normal_water_2level_case(normalized) && owner_small_normal_water_2level_guard_limit >= 0 && guard_ordinal < owner_small_normal_water_2level_guard_limit) {
		subphase_started_at = std::chrono::steady_clock::now();
		int32_t attempts = 0;
		const int32_t max_attempts = std::max(owner_small_normal_water_2level_guard_limit * 16, int32_t(zones.size()) * 192);
		while (guard_ordinal < owner_small_normal_water_2level_guard_limit && attempts < max_attempts) {
			if (zones.is_empty()) {
				break;
			}
			Dictionary zone = zones[attempts % zones.size()];
			...
			Dictionary anchor = zone.get("anchor", zone.get("center", Dictionary()));
			const int32_t ring = 1 + attempts / std::max<int32_t>(1, int32_t(zones.size()));
			const int32_t seed = int32_t(hash32_int(String(normalized.get("normalized_seed", "0")) + ":owner_small_normal_water_2level_density_guard:" + String::num_int64(attempts)) % 17U);
			const int32_t dx = ((attempts % 5) - 2) * 2 + (seed % 3) - 1;
			const int32_t dy = (((attempts / 5) % 5) - 2) * 2 + (seed / 3) - 2;
			Dictionary point = find_object_point(
					int32_t(anchor.get("x", width / 2)) + dx + ring,
			...
			if (!point.is_empty()) {
				const int32_t guard_value = std::max(900, rmg_zone_monster_scaled_value(normalized, zone, 1200 + (attempts % 5) * 300));
				Dictionary target;
				target["protected_target_id"] = "owner_small_normal_water_2level_density_guard_" + slot_id_2(guard_ordinal + 1);
				target["protected_target_type"] = "density_guard";
				target["protected_zone_id"] = zone_id;
				target["protected_zone_value_budget"] = zone.get("value_budget", 0);
				target["guard_base_value"] = guard_value;
				target["scaled_guard_value"] = guard_value;
				target["guard_reward_relation_source"] = "owner_small_normal_water_2level_density_guard_supplement_from_parsed_h3m_guard_count";
				append_guard_record(guards, occupied, guard_record_at_point(normalized, zone, point, "density_guard", guard_ordinal, guard_value, road_network, zone_layout, occupied, target));
				++guard_ordinal;
			}
			++attempts;
		...
		Dictionary diagnostic;
		diagnostic["code"] = guard_ordinal >= owner_small_normal_water_2level_guard_limit ? "owner_small_normal_water_2level_density_guard_target_materialized" : "owner_small_normal_water_2level_density_guard_target_partial";
		diagnostic["severity"] = guard_ordinal >= owner_small_normal_water_2level_guard_limit ? "info" : "warning";
		diagnostic["target_guard_count"] = owner_small_normal_water_2level_guard_limit;
		diagnostic["final_guard_count"] = guard_ordinal;
		diagnostic["attempt_count"] = attempts;
		diagnostic["source"] = "owner_discovered_s_2playerss_normalwater_2level_guard_count";
		guard_diagnostics.append(diagnostic);
		append_extension_profile_phase(town_guard_profile_phases, "owner_small_normal_water_2level_density_guard_supplement", subphase_started_at, top_town_guard_phase_usec, top_town_guard_phase_id);
	}
