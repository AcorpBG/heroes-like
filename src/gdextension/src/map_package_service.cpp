...
Dictionary runtime_start_tile_for_slot(
		const aurelion::h3maped_rmg_core::FinalHeaderPlayerSlot4ac857 &slot,
		const aurelion::h3maped_rmg_core::RuntimeMapPayloadProjection &projection) {
	// Product start policy: use the recovered main-town entrance exactly. The
	// editor's town/body records are not a license to move the hero off-road.
	// Town ingress/egress is a live movement rule; no source masks are erased.
	if (!slot.has_main_town || runtime_tile_flat(projection, slot.town_x, slot.town_y, slot.town_level) < 0) {
		return Dictionary();
	}
	for (const auto &object : projection.objects) {
		...
		for (const auto &point : object.action_tiles) {
			if (point.x != slot.town_x || point.y != slot.town_y || point.level != slot.town_level) {
				continue;
			}
			Dictionary selected = runtime_tile_point(point);
			selected["selection_radius_from_town_coordinate"] = 0;
			selected["selection_removed_removable_start_block_mask"] = false;
			selected["selection_source"] = "h3maped_main_town_entrance_runtime_start";
			return selected;
		}
	}
	return Dictionary();
	...
	for (const auto &slot : projection.player_slots) {
		...
		if (slot.has_main_town) {
			...
			if (runtime_start_tile.is_empty()) {
				...
				blocked["ok"] = false;
				blocked["status"] = "blocked";
				blocked["error_code"] = "native_rmg_main_town_entrance_missing";
				blocked["message"] = "Native main-town coordinates do not resolve to a projected town entrance.";
