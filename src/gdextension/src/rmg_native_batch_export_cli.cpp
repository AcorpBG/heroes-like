...
std::string manifest_json(const Options &options, const std::filesystem::path &absolute_output_dir, const std::vector<CaseReport> &case_reports, int skipped_count) {
	...
	const std::string blocked_reason = status == "complete"
		...
		const bool same_run_authority_profile_mismatch =
				blocked_reason == "same_run_payload_authority_profile_is_hc1_computer_random_not_fixed_native_2p";
		...
		const std::string generation_core_stage = setup_stack_authority_missing
				...
				: (same_run_authority_profile_mismatch
						? "native_h3maped_workflow_reaches_ordered_final_payload_assembly_but_same_run_payload_authority_profile_mismatches_fixed_native_2p"
				: (final_payload_compare_reached
		...
		const std::string required_next_slice = setup_stack_authority_missing
				...
				: (same_run_authority_profile_mismatch
						? "recover_or_select_same_profile_h3maped_payload_authority_for_fixed_native_2p_before_final_payload_compare"
		...
		const std::string message = setup_stack_authority_missing
				...
				: (same_run_authority_profile_mismatch
						? "This executable is the no-Godot boundary for the single native H3MapEd workflow. It executes ordered phases through final payload assembly but refuses tile/object byte comparison because the available stitched H3MapEd payload authority was captured from the HC/random profile, not the fixed native 2p controlled profile."
				: (type98_source_pair_feed_missing
