...
Array fresh_phase_backlog() {
	...
	backlog.append(phase_record("terrain_and_terrainplacement", "0x49b53d, 0x4a3f27, 0x4bcff5, 0x4bb74b, 0x4bc5f0, 0x49b2b6", "terrainplacement_visual_tables_active_internal_state"));
...
Array current_gap_summary() {
	Array gaps;
	gaps.append("active code contains h3maped binary verification, small scope gate, template RNG selection, player-slot assignment, runtime-zone records, link-seed setup, coordinate replay, private zone footprint/cell ownership buffers, private terrain cell writeout, and decoded TerrainPlacement visual tables");
	gaps.append("live TerrainPlacement feedback, towns, roads, blockers, guards, mines, rewards, and writeout are not implemented in the fresh active module");
...
Dictionary active_generation_state(const Dictionary &normalized_config) {
	...
	state["blocked_next"] = completed.size() >= 8 ? String("live_TerrainPlacement_0x4bb74b_0x4bc5f0_scratch_feedback") : String(completed.size() >= 7 ? "terrainplacement_visual_tables_0x4bcff5" : String(completed.size() >= 6 ? "terrain_cell_writeout_0x4a3f27" : "zone_footprint_source_nodes_0x4a3a03_0x4cc788"));
	return state;
}
